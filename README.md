...
| flame vs PyTorch (matched-dtype, RTX 5070, `F-BENCH-1`) | **FP64: flame TIES/WINS** (torch ÷ flame 0.83–1.10×, flame wins B=4/B=8); **TF32: torch 2–8×** (3.03× @B=1 → 7.88× @B=8); **FP32: torch 2.2–6.6×** | the fair, compiled-step, matched-dtype gap is **single-digit**. The old "~1656× / ~2207×" (#2912) compared flame FP64 vs torch TF32 on the **interpreted** full trainer — re-contextualized to single-digit by BENCH-1. flame value = byte-exact · device-resident · no-LLVM |
...
**Matched-dtype, the flame-vs-PyTorch speed gap is SINGLE-DIGIT — and at FP64 flame ties or wins.** The fair, compiled-step, batch-swept `F-BENCH-1` (RTX 5070) measures: **FP64 — flame TIES at B=2 and WINS at B=4/B=8** (torch ÷ flame = 0.83–1.10×; flame has the highest FP64 throughput, 826 vs 745 samp/s @B=8 — torch FP64 has no tensor-core path); **TF32 — torch 2–8× faster** (3.03× @B=1 → 7.88× @B=8) and **FP32 — torch 2.2–6.6×**, its cuBLAS/inductor GEMM beating flame's naive tiled GEMM. That torch TF32/FP32 win is real and fine — it is just **2–8×, not 1656×**. (The old "~1656× / ~2207×" #2912 figure compared flame **FP64** against torch **TF32** on the **interpreted** full trainer at batch=1 — a precision-mismatched, interpreter-glue, 2-point-extrapolated number; `F-BENCH-1` re-contextualizes it to single-digit.) The hexa-owned own-GEMM reaches **parity (1.08× @D=2048, bit-exact), never a beat** — cuBLAS-TF32 is the roofline (`F-GPU-ROUTEA-KEEPBAND-MEASURE`); its @D=4096 ~1.5× sub-parity is a **fixable scheduling stall, not a hard roofline** (`F-OP45GPU-OCCUPANCY-SWEEP`). The flame batch-fill (≥1.3× @B=2 → 2.95× @B=32) and TF32 fast-mode (4.2× @B=1) above are flame **self**-speedups, not PyTorch beats, capped ~3× by the interpreted per-step glue.
...
>   ─────────────────────────────────────────────────────────────────────────────────────────────
>   cuBLAS-TF32 = ~431 TFLOP/s roofline — OG17 CROSSES PARITY (≤1.3×) @ S=2048, bit-exact (NOT beaten, no superiority)
