...
# sandbox with a fixture watchlist where last_event_ts is in 2020 (→ elapsed >= 72h).
# Asserts: (a) tick emits dormancy_wake_fire, (b) writes growth_bus.jsonl,
# (c) row state flips to "fired_executed".
#
# dormancy_wake_tick.hexa was absorbed; if not present yet, SKIP (rc=77).

set -u
HEXA_ROOT_SRC="${HEXA_ROOT:-/Users/ghost/core/hexa-lang}"
SCRIPT="$HEXA_ROOT_SRC/tool/dormancy_wake_tick.hexa"
if [ ! -f "$SCRIPT" ]; then
    echo "SKIP: $SCRIPT missing (dormancy_wake_tick not yet ported)"
    exit 77
fi

SANDBOX="$(mktemp -d -t hxa_dormancy_XXXXXX)"
trap 'rm -rf "$SANDBOX"; rm -f /tmp/hexa_dormancy_wake.pid' EXIT

mkdir -p "$SANDBOX/state"
cp "$FIXTURES/mini_trigger_watchlist.jsonl" "$SANDBOX/state/meta_trigger_watchlist.jsonl"
: > "$SANDBOX/growth_bus.jsonl"

# HEXA_SANDBOX env var is read by the script (line 13).
out="$(HEXA_SANDBOX="$SANDBOX" "$HEXA_BIN" "$SCRIPT" 2>&1)"
rc=$?
if [ $rc -ne 0 ]; then
