...
| `grammar.hexa` | ★**M1 of OUROBOROS-novel** (ARCHITECTURE `discovery-engine-novel-design-roadmap`) — BLOWUP term enumerator (identity frame · size 1·2 products over the 18-fn basis · deterministic a≤b order) + **FP_k fingerprint table** (rolling hash mod 2⁶¹-1) making "a primitive the previous cycle could not express" a DECIDABLE predicate = size-≤k BOUNDED inexpressibility over [2,N] (c2: k·N named, not absolute). Self-contained (`_gram_af` mirrors identity_engine::af — sidesteps the flattener-trap of importing the unguarded `fn main` selftest; M2 consolidates into identity_engine). No new builtin |
