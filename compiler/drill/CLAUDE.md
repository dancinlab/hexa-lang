...
| `emerge.hexa` | ★[B] **open-well Emerge generator** (README:75 Emerge stage) — `emerge(v0,seed,cycles,min_growth)` exactly ports README E4 (V_0={2,3}·seed=6·300 cycles·`combine(pick,pick)=a+b`→sorted-dedup vocab), `emerge_step` wired into drill_run (each round widens the accumulated open well as a candidate value-signature). Deterministic LCG (Numerical Recipes 1664525/1013904223 `& 0xffffffff` — same constants as qrng.hexa · no new builtin). in-memory only (no atlas write). **M4 emerge2** (additive · E4 fns byte-identical): 5-generator fn-symbol synthesis (G1 sol-indicator · G2 composition-symbolization · G3 Dirichlet-convolution[spf divisor-enum net-new] · G4 iterate/orbit · G5 congruence-witness) + bounded-inexpressibility gate (fp not in FP_k) then `kc_classify` alpha/beta/gamma · new pub `ie_vec_fingerprint` (../atlas/identity_engine · frozen-kernel reuse) · `emerge2_step`/`emerge2_audit_emit`(DRILL_EMERGE2) · drill.hexa diff=0 (wiring=M5). Measured aiden v0.577.0: 19 cands alpha=2/beta=2/gamma=15 · emerge_test +18 gates T1-T18 PASS. gamma=catalog-relative bounded[2,64] not absolute |
...
| `nullmodel_test.hexa` | ★**M6 scramble-oracle null-model** (OUROBOROS-novel · design §3) — scramble `af`→`af_rand` (scale-profiled NR-LCG seed-hash) → inline exact-int verify → KNOWN contrast → inline Bonferroni tribunal (bonferroni/verdict inlined, NOT `texas_sharpshooter` import = cold-compile avoidance). Null γ-rate gate = tribunal-promoted==0 (falsification bar). Measured aiden v0.577.0 (fresh worktree): **21/21 gates PASS · 4.13s · 368MB**. sweep K'=C(32,2)=496×NV=200 (D12 rescale; full K=14535×NV=2000 ≈ 5.8e7 BigInt verify iters = impractical selftest runtime ~290× the 4.13s rescaled run · the earlier "20GB/>2h" was an unrelated stale-checkout runaway, NOT this test — no clean large-scale run was measured). byteeq-neutral companion · Reading-B (γ=verify-gated). GOTCHA: run in a **fresh worktree of the branch** — a stale pool checkout missing `known_catalog.gen.hexa` fails module-not-found |
