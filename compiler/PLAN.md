...
### 2026-05-20 — inbox/patches/ bulk-triage cycle (dup-race precheck)

Bulk triage of `inbox/patches/` per @D g_inbox_processing_loop step-4 variant (no fix cycles; SSOT grep only). Reviewed ~60 inbox markdowns, excluded 4 already-processed-today items (pt-ckpt-cross-substrate-residual-readout · flame-stdp-pair-gpu-kernel · flame-spiking-substrate-primitives · yosys-rtlil note). Per `feedback_inbox_dup_race_precheck` memory, prior closure marker often already present from sub-agent worktree resolves; this cycle adds explicit cross-verified `VERIFIED-CLOSED 2026-05-20` lines so future scans can skip these without re-grepping.

**Mass-flipped: 35 patches** — each carries a one-line `> **VERIFIED-CLOSED 2026-05-20**` marker quoting the specific SSOT path + line number (or commit hash) that proves the fix is live. Patches: enum-variant · format-n-pure · hxc-v2-lib · json-uXXXX · phanes-aws-sigv4 · phanes-linux-self-host · phanes-sigv4-uriencode · print-err · runtime-h-incomplete · self-tui-render-wide-cont · stdlib-9p · stdlib-os-{mount,namespace,pty} · stdlib-ssh-client · stdlib-subsecond-sleep · struct-fn-field-wrap · text-visual-width · thread-channel · wilson-bytes-to-str-raw · wilson-lexer-tokenize-pub · wilson-needs-hexa-build-out-of-tree · wilson-needs-hexa-real-promotion · wilson-shift-tab-csi-Z · self-stdlib-term-unicode-u8-decode · tui-input-paste-buf-quadratic · tui-input-paste-double-encodes-bytes · wilson-mcp-needs-bidi-stdio · wilson-needs-time-now-ms · wilson-channel-symbol-collision (dup of builtin-vs-stdlib) · wilson-fn-arena-escapes-on-push · wilson-shift-enter-newline · stdlib-arr-shift · stdlib-sort · hexa-run-interp-hangs-on-mac (DISSOLVED by @D g_interp_deprecated).

**Genuinely open: 14 patches** — markdown left untouched, listed here with 1-line SSOT-recommendation: absorbed-verb-qrng-qmirror-superseded-by-stdlib (delete legacy `_absorbed_script` qrng/qmirror entries L1043/L1045, low priority — dispatch already routes to stdlib) · flame-path-a-dual-head-and-multiterm-grad (RFC 059 anima §71 multi-term grad — multi-cycle Shape B) · g7-hexa-ld-dlopen (spec/RFC initiative — dlopen substrate, non-blocking ★ priority) · hexa-atlas-pr-only-policy (retire `tool/atlas_cli.hexa::promote` arm + L94 cmd_help text; PR-only via `register`/`pr` only) · hexa-cli-help-stale-after-absorption (audit `cmd_help` for SPEC §16 `pwd→cwd`/`ls→list_dir` intrinsic mentions — STDLIB CLI section already covers RFC 044/045/046) · import-alias-runtime-resolve (wire `__mod_<alias>__<fn>` symbol resolution in codegen alias-import path; `import "x" as ip` → `ip.apply()` runtime undefined) · kick-engine-overlay-dump-mode (add `--dump-overlay` flag to `hexa kick`/`drill`/`omega` round runner — emit overlay-content lines to stdout for downstream consumption) · rfc020-enum-payload-variants (multi-field enum payload — `compiler/` side still pending per patch in_progress, single-field works) · self-tui-render-wide-to-narrow-ghost (extend `render_flush` to consult `_front` cell width when ≠ `_back` width — clear ghost continuation cell) · string-concat-in-unbounded-loop-quadratic-rss (language-level proposal — runtime / codegen recognize `out = out + chunk` loop pattern and route through strbuf) · toplevel-mut-array-push-mismutation-from-fn (module-level `let mut [array]` mutation from fn — distinct from fn-arena fix per repro, root cause un-isolated) · tui-input-needs-decset-2004-bracketed-paste (add `term_write(ESC[?2004h)` to `input_init` + restore on `input_close`) · wilson-needs-streaming-http-sse (stdlib SSE module — pending_external, scope decision needed: stdlib/http vs runtime primitive) · wilson-pi-port-6-gap-prereq (spec meta-gate G7/G8 — covers g7-hexa-ld-dlopen + G8 incremental link).

**Ambiguous: 2 patches** — stdlib-for-cpu-port (meta gate; all 5 children verified-closed, meta itself purely informational — no action required) · wilson-build-broken-pool-openai-compat (hexa-lang side `float()` coercion landed at b14d01cb; residual `pool_invoke_propose` symbol is wilson-tree code, hexa-lang-side closed).

**Next-cycle dispatch priority (gas-bang ratio)**:
1. **import-alias-runtime-resolve** — codegen 1-symbol seam; small Shape A surgical fix; unblocks anima VOICE modular split (currently forced single-file).
2. **tui-input-needs-decset-2004-bracketed-paste** — 1-line `input_init` addition; unblocks every hexa TUI's paste UX; trivial verification.
3. **hexa-atlas-pr-only-policy** — Shape A cmd_help text + tool/atlas_cli.hexa promote-arm deletion; aligns with @D g_atlas_binary_builtin "PR-only" invariant (direct fold-to-live forbidden); 2026-05-19 policy update.
...
### 2026-05-20 — `self/main.hexa` + `tool/atlas_cli.hexa` 3-in-1 inbox cleanup (PR-only · absorbed-verb · cmd_help stale)

Single-cycle 3-patch cleanup (region-disjoint, single agent serial-safe) — landed three open inbox/patches/ markdowns against the CLI dispatch + help surface without any behavior regression for the supported flows.

- `inbox/patches/hexa-atlas-pr-only-policy.md` (wilson, 2026-05-17) — RESOLVED-SSOT. `tool/atlas_cli.hexa` retires the `promote` arm: `cmd_promote()` body deleted, `_slice_args` allow-list entry removed, dispatcher `else if sub == "promote"` removed, `cmd_help()` `promote` row removed, header doc shows the PR-only landing flow (`register → pr` only — direct fold-to-live forbidden, @D g_atlas_binary_builtin symmetry). The `promote_to_atlas()` primitive is still imported and used internally by `cmd_pr` (folds onto a branch then opens the PR). `self/main.hexa::cmd_help` L94 replaced the stale `atlas promote --staging <f.n6>          Write to live (* register/pr = stub)` row with three real rows advertising `atlas register --from-verify` / `atlas register --from-drill` / `atlas pr` and the @D g_atlas_binary_builtin policy footer.

- `inbox/patches/absorbed-verb-qrng-qmirror-superseded-by-stdlib.md` (wilson, 2026-05-17) — RESOLVED-SSOT. `self/main.hexa::_absorbed_script` Phase-4 HW-probe block now contains only `akida`. The `qmirror` / `qrng` registry entries (which silently shadow-claimed the verb names on partial-merge branches and routed `hexa qmirror` to a misleading `8/8 PASS (deterministic stub)`) are replaced with a comment block citing RFC 044 (qrng → stdlib/qrng/) + RFC 045 (qmirror → stdlib/quantum/) + the absorbed-verb-qrng patch. The verb count went `(19)` → `(17)`. Behavior on `main` unchanged (the explicit `else if sub == "qrng"/"qmirror"` stdlib branches in `main()` still fire first); behavior on partial-merge branches now honestly returns `error: unknown subcommand` instead of the misleading stub.

- `inbox/patches/hexa-cli-help-stale-after-absorption.md` (wilson, 2026-05-12) — RESOLVED-SSOT. `cmd_help` now ships an `INTRINSIC SURFACE  (SPEC §16 — absorbed shell-equivalents, 638→752 sites)` block advertising the 10 most common absorbed intrinsics with their shell-equivalents (`cwd ⇆ pwd`, `list_dir ⇆ ls`, `mkdir_p ⇆ mkdir -p`, `getenv ⇆ $VAR`, `path_exists ⇆ test -e`, `path_is_dir ⇆ test -d`, `rm_file ⇆ rm`, `rm_rf ⇆ rm -rf`, `host_target ⇆ uname -sm`, `now_ns ⇆ date +%s%N`) plus an SPEC §18 firmware-absorption pointer at `stdlib/{core,alloc,hal,embedded,mcu}` + `firmware/boards/*`. The stale `HW probes: qmirror akida qrng` row is now `HW probes: akida           (qmirror/qrng → STDLIB CLI via RFC 044/045)`. The dedicated `hexa intrinsics [list|--help]` subcommand and the planned HX9xxx fork-storm lint surfacing on `hexa lint --help` are DEFERRED to a separate cycle (Shape-A scope limited to the surfaced-text fix).
