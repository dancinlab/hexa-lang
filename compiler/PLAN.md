...
### 2026-05-17 — interp-retirement #18: index_set dropped store-back FIXED + 2nd self-host bug isolated

**Branch** `subagent-selfhost-arrayret` (from origin/main `b1dc9b30`).

**원래 진단 재검증**: task 가 지목한 victim-spill `next_slot + 8`→`+16`
(arm64_darwin.hexa `_arm64_build_regmap`) 는 **이미 main 에 landed**
(atlas-SIGSEGV fix 가 동일 사이트 — sibling 아니라 same site). 0 `+ 8` 잔존.
따라서 잔존 crash 는 별개 root-cause. lldb (`b hexa_array_get` auto-continue)
+ gen1.s asm 정밀 분석으로 진짜 원인 격리.

**Root cause — `index_set` codegen 이 `hexa_index_set` 반환값 DROP**:
`compiler/codegen/arm64_darwin.hexa` STMT_ASSIGN `op=="index_set"`
(`arr[i]=v` / `map[k]=v`) lowering 이 `hexa_index_set(container,key,value)`
를 호출하고 **반환 HexaVal 을 container lvalue 에 store-back 안 함**.
주석은 "the store is the side effect" 라 단정했으나 오류 — proven C 경로
(codegen_c2) 는 `container = hexa_index_set(container,key,value)` 로 emit.
arm64 가 drop 하면 `let mut a = [] + push` 로 만든 container (hxc_loader.hexa
`dense`) 가 STALE array header 를 슬롯에 유지 → 런타임 arena dirty-slot /
heapify 가 후속 할당에서 backing 을 relocate → `_build_node(dense)` 가
dangling/garbage array 수령 → 비결정 `container is not an array
(tag=<garbage>)`. self-host ap2 가 atlas hxc sidecar load 시 즉사 (= #18 마지막
하드 블로커, `_normalize_argv`/atlas-loader).

**FIX** (`compiler/codegen/arm64_darwin.hexa`, correctness-preserving):
`bl hexa_index_set` 직후 x0:x1 을 container Operand 가 storable
`local`/`global` 일 때 store-back (`_hv_store` mirror, `hexa_array_push`
패턴과 동일). 단일 분기 추가.

**검증**:
- 회귀: index_set 미사용 프로그램 (arr/args/sh_smoke repro) **byte-identical**
  asm (ap1 vs ap1fix). index_set 사용 (bn_repro) 정확히 2 line 차이
  (store-back 2 instr) + 결과 deterministic correct.
- 스케일: 16,000-row dense-build+index_set+pass (i64 및 nested-struct return,
  `_build_node`/`load_atlas_hxc` 패턴 정밀 미러) → fix 후 deterministic correct,
  `hexa run` 과 exit 일치.
- self-host: ap1fix → flat.hexa → ap2fix. **atlas-loader 하드 crash 제거** —
  ap2fix 가 atlas load·lex·parse·bind 까지 진행 (직전: atlas load 에서 즉사,
  main loop 진입 불가). no-hxc 환경(`atlas: loaded 0 nodes from source`)에서
  sh_smoke 헤더까지 emit.
- build_aprime.sh smoke `exit(6*7)==42` PASS.

**잔존 — 별개 2번째 self-host codegen 버그 (본 task scope 밖)**:
real `dist/atlas.hxc` (6.4 MB, ~16k row) 로 ap2fix 실행 시 동일 시그니처
crash 가 `_build_node_bb0+16 ← load_atlas_hxc_bb9` 에서 재현 (x0=garbage
ptr-range, 비결정). 그러나 fix 적용 asm 에서 `dense[ci]=cells[j]` 의
store-back 은 정상 emit 확인 (`stp x0,x1,[x15] ; hv store L105`), 16k-scale
minimal repro (struct-return 포함) 전부 PASS. → `dense` 자체는 정상 유지되나
real `_build_node` (16-arg dense · nested AtlasNode{GradeInfo,EdgeInfo} ·
`_parse_json_str_arr` · 6.4 MB 실데이터) 경로에서 caller frame slot [sp+1680]
이 어떤 callee 에 의해 corrupt 되는 별도 클래스. minimal repro 비격리 — 다음
cycle 에서 real-data-dependent 격리 필요 (struct-return ABI 또는
`_split_pipes`/`_parse_json_str_arr` 의 frame 상호작용 의심).

**Net**: documented array-return/store-back 블로커 종결 (byte-clean,
scale-correct). self-host fixpoint 는 2번째 버그로 미달 — ap2fix 가 ap1fix 와
byte-identical flat.hexa 산출 못 함 (real atlas 경로에서 abort). 정직 보고:
1차 블로커 fix 는 확정, fixpoint 는 추가 cycle 필요.
