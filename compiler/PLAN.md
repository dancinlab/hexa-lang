...
### 진행 로그 — gate ① atlas-verifier 3-MISMATCH 클러스터 CLOSED (#30 index_of + #31 bool-literal, cycle ac6ea98a)

브랜치 `agent-ac6ea98a-atlas-verifier` (worktree-격리). origin/main `21da8b45` 기점.

**클러스터**: `atlas_doctrine_smoke` · `atlas_tecsl_verify_smoke` · `atlas_wave3_smoke`
3종이 tier-1 aprime_cc vs tier-2 `hexa build` oracle 에서 verdict 불일치 — predicate
wrong-value codegen (crash 아님). triage 가설 "node-scan loop 내부 boolean 평가
오류" 적중. 단, 단일 root 가 아니라 **두 독립 버그**가 같은 클러스터를 만든 것:

**#30 — `index_of` string-method 가 `hexa_array_index_of` 로 miscompile.**
`compiler/codegen/arm64_darwin.hexa` `_builtin_runtime_sym` 이 `index_of` →
`hexa_array_index_of` 로 무조건 매핑. `hexa_array_index_of` 는 비-array receiver
에 `-1` 반환 → 모든 `string.index_of(...)` 가 aprime 컴파일 시 항상 -1. audit
스캔의 `raw.index_of(token) >= 0` 가 전부 false → `external_entity_audit` /
`falsifier_wellformed_audit` 등 verdict flip. fix: arg-count-aware 전용 STMT_CALL
분기 — 2-arg `hexa_str_index_of_from` / 1-arg `hexa_str_index_of`, int-box.
`codegen_c2.hexa gen2_method_builtin` 과 동일 의미. bad 매핑 제거.
→ `atlas_doctrine` + `atlas_wave3` flip. commit `3d350d05`.

**#31 — bool 리터럴이 TAG_BOOL 아닌 TAG_INT 으로 lowering.**
`compiler/lower/hir_to_mir.hexa` `literal_bool` 이 `true`/`false` 를
`_const_int_op` (TAG_INT) 로 lower. 비교 연산자(`==` `!=` `<` …)는 `hexa_bool`
경유 TAG_BOOL 반환. `hexa_eq` 는 tag 불일치 시 false → 리터럴 bool 이 계산된
bool 과 절대 같지 않음 → `bool_a == true` / `cond_a != cond_b` predicate 가
조용히 오답. `s8_mu_eq_sigma_squarefree_tecsl` 의 `eq != squarefree` 가 해당.
fix: `_const_bool_op` 생성자(kind=`const_bool`) 신설 + arm64 `_hv_load` 에
`const_bool` 분기(TAG_BOOL=2 pair) + raw-int resolver 2곳. `literal_char` →
TAG_STR 선례·`codegen_c2.hexa` `true`→`hexa_bool(1)` 와 동일 패턴.
→ `atlas_tecsl_verify` flip. commit `8e5d8d4e`.

**검증 배터리:**
- build_aprime smoke `exit(6*7)==42` PASS.
- gate-1 sweep 44-file (tier-1 aprime vs `hexa build` oracle, HEXA_MAC_BUILD_OK=1):
  **before 26/44 → after 36/44** (target ≥36 충족). 3 atlas-verifier 전부
  MISMATCH→MATCH. 추가로 동일 construct 사용하던 7종(`sigma_phi_tau_uniqueness`·
  `static_index_lazy_poc`·`symmetric_group_s6`·`texas_sharpshooter`·
  `three_root_theorem`·`verify_cli`·`vortex_math_verifier`)도 동반 flip.
  **regression 0** (before-MATCH ⊃ after-MATCH 차집합 공집합 확인).
- ap1(pre-fix) vs ap1f(post-fix) asm: index_of/bool-literal 미사용 프로그램
  byte-identical(`pascal_perfect_smoke`); 사용 프로그램의 diff 라인은 전부
  fix-attributable(old `const_int TAG_INT` bool → new `const_bool TAG_BOOL`,
  index_of 재라우팅) — collateral asm 변경 0.
- self-host fixpoint 보존: `/tmp/fx_flat.hexa` 38-file closure fresh flatten
  (22296 lines, +68 = 추가 코드분). ap1f → flat emit → ap2f → re-emit,
  **ap1f.s == ap2f.s byte-identical** (231824 lines / 9267479 B / md5
  `852282973d8d08392aa3d44bd3689e0e` 일치). bit-stable self-host fixpoint 유지.

**상태**: gate ① atlas-verifier 클러스터 3/3 CLOSED. gate-1 36/44.
잔여 MISMATCH 2종(`repo_taxonomy_audit`·`t34_net_listen`, ap_rc=137/138 crash —
별도 root) + APFAIL 6종(aprime emit/asm 실패 — 별도 root). 이들은 본 클러스터와
무관, 다음 cycle 포인터.
