...
### 2026-05-18 — cycle h20: try/catch/throw tier-1 codegen LANDED — 마지막 진짜 tier-1 gap 종결

**작업**: `try { … } catch e { … }` + `throw <expr>` 를 tier-1 (aprime_cc direct-asm)
파이프라인에 구현. tier-2 (`self/codegen_c2.hexa` TryCatch/ThrowStmt) 가 이미 setjmp/
longjmp + `__hexa_try_*` 로 lowering 하는 것을 tier-1 이 따라잡는 catch-up 작업 (신규
설계 아님). 5-commit 슬라이스:

1. **lex + AST scaffolding** (`36f99562`): `KwTry`/`KwCatch`/`KwThrow` 토큰 + lexer
   keyword map; `ExprKind::Try`/`ExprKind::Throw` AST variant. stage0 enum 은 match
   exhaustive 강제 → 10개 파일 (`ast.hexa`·`check/{bind,types,units,equational,
   annotations,citation,resolve}.hexa`·`lower/ast_to_hir.hexa`·`discover/discover.hexa`)
   의 모든 ExprKind match arm 에 Try/Throw 추가.
2. **parser + bind** (`f7225445`): `parse_try_expr`/`parse_throw_expr` (parse_primary
   dispatch). `catch e` / `catch(e)` 양 surface 수용 (self/hexa_full.hexa 미러).
   bind: `_bind_is_try_kind` probe + catch_var 를 catch-body 전용 scope 에 define.
3. **hir_to_mir** (`b9747feb`): 4개 신규 MIR stmt — `STMT_TRY_SETJMP`/`STMT_TRY_POP`/
   `STMT_TRY_LAST_ERR`/`STMT_THROW`. try-lowering 이 explicit CFG 생성 (entry→
   try_body→catch→after). 'err_local' → 'catch_err_local' rename (clang
   `<mach/error.h>` 매크로 충돌 회피).
4. **arm64 codegen** (`fb72dab7`): 신규 stmt asm 발행. per-fn jmp_buf region —
   `STMT_TRY_SETJMP` 당 192-byte slot (Darwin arm64 jmp_buf sizeof), spill region
   위에 배치. setjmp: `add x0,sp,#off; bl _setjmp; cbnz w0,catch; bl ___hexa_try_push`.
   throw: `bl _hexa_throw`. Mach-O 심볼 mangling — `_fmt_label` 이 darwin `_` 무조건
   prepend → `setjmp`→`_setjmp`, `__hexa_try_push`→`___hexa_try_push`.

**검증 배터리** (전수 PASS):
- `build_aprime.sh` smoke `exit(6*7)==42` PASS.
- minimal repro `try { throw "boom" } catch e { println(e) }` — tier-1 stdout/rc
  == tier-2 oracle byte-identical (`boom` / rc=0).
- t38-style repro (early-return-in-try `try { return 42 } catch e {…}` +
  throw-catch `try { throw "boom" } catch e { return e }`) — tier-1 `pass=2 fail=0`
  == tier-2 oracle byte-identical.
- gate-1 44-file sweep: **37/44 MATCH** (변동 없음). t38_nanbox try/catch HX2001
  소멸 확인 — 잔여 APFAIL 원인은 **line 94 의 inline closure `fn(x){…}`** (별개의
  pre-existing tier-1 gap, try/catch 무관).
- regression: 44/44 smoke 파일 ap1(prefix `33667fc9`) vs ap1f(post) **asm
  byte-identical** — try/catch 는 순수 additive, 기존 경로 0 변경.
- **self-host fixpoint 보존**: flat 38-file compiler closure → ap1f emit → ap2f
  빌드 → ap2f emit = ap3f.s. **ap1f.s == ap3f.s byte-identical** (9,667,769 B).
  emit-determinism 도 확인 (동일 입력 2회 emit byte-identical).

**결산**: try/catch = goal-2 가 지목한 "마지막 진짜 tier-1 codegen-correctness gap"
종결. t38_nanbox 가 gate-1 MATCH 까지 가려면 추가로 inline closure-expression
(`fn(x){…}`) tier-1 지원 필요 — TAG_FN value + 환경 capture + indirect-call lowering
= 별도 RFC-sized 작업 (tier-1 에 closure 인프라 전무). try/catch cycle 범위 밖.

**남은 gate-1 비-MATCH 7건 분류** (변동 없음):
- t38_nanbox APFAIL — 이제 try/catch 아닌 **closure-expr** (신규 분리 gap)
- atlas_verify MISMATCH — tier-2 codegen 잔여 발산 (tier-1 결함 아님)
- t35/t36/t37 — tier-2 oracle FFI/clang 실패
- repo_taxonomy_audit/t34_net_listen — env (OOM/socket)
