...
/* P1B-a'' RACEFIX knob. Zero freshly cudaMalloc'd OUTPUT buffers so an
 * uninitialized-tail read (the device-glue ~7e-6 run-to-run non-determinism)
 * becomes a deterministic 0.0. Default ON (the fix); HEXA_DEVGLUE_DETERMINIZE=0
 * restores the legacy garbage-tail behavior for the A/B determinism probe. */
static int g_devglue_determinize = -1;
static int _devglue_determinize_on(void) {
    if (g_devglue_determinize < 0) {
        const char* v = getenv("HEXA_DEVGLUE_DETERMINIZE");
        g_devglue_determinize = (v && v[0] == '0') ? 0 : 1;  /* default ON */
    }
    return g_devglue_determinize;
}
...
int _hx_cuda_farr_matmul_gpu(int64_t a_id, int64_t M, int64_t K,
    ...
    if (!cs->d_buf || cs->len != ce->len) {
        ...
        cs->len = ce->len;
        /* P1B-a'' RACEFIX: zero the GEMM-C tail (ce->len may exceed M·N). */
        if (_devglue_determinize_on())
            cudaMemset(cs->d_buf, 0, (size_t)ce->len * sizeof(double));
    }
...
int _hx_cuda_farr_matmul_batched_gpu(int64_t a_id, int64_t M, int64_t K,
    ...
    if (!cs->d_buf || cs->len != ce->len) {
        ...
        cs->len = ce->len;
        /* P1B-a'' RACEFIX: zero the batched-GEMM-C tail. */
        if (_devglue_determinize_on())
            cudaMemset(cs->d_buf, 0, (size_t)ce->len * sizeof(double));
    }
    double* A_dev = g_slots[a_id].d_buf;
 ...
 * (x · cdf). Device erf() is the IEEE-correct libm erf (same contract as the
 * host erf builtin) → bit-exact to the host scalar loop (max|Δ|=0). Fuses the
 * gelu ×3 host glue in clm_prod_fwd (hg0/ex0/ex1 — the ③ fwd-only 0% floor)
 * and keeps OUT FARR_DEVICE so the next conv GEMM H2D-skips it. */
__global__ void _hx_k_gelu(const double* __restrict__ IN,
    ...
    for (int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
         i < n; i += stride) {
        double x = IN[i];
        double cdf = 0.5 * (1.0 + erf(x * inv_sqrt2));
        OUT[i] = x * cdf;
    }
}
...
__global__ void _hx_k_gelu2(const double* __restrict__ G0,
    ...
    for (int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
         i < n; i += stride) {
        double x0 = G0[i];
        double cdf0 = 0.5 * (1.0 + erf(x0 * inv_sqrt2));
        A0[i] = x0 * cdf0;
        double x1 = G1[i];
        double cdf1 = 0.5 * (1.0 + erf(x1 * inv_sqrt2));
        A1[i] = x1 * cdf1;
    }
}
...
__device__ static double _hx_gelu_dev(double x) {
    /* byte-identical to _hx_k_gelu / gn_lib.hexa _gn_gelu. */
    const double inv_sqrt2 = 0.70710678118654752440;
    double cdf = 0.5 * (1.0 + erf(x * inv_sqrt2));
    return x * cdf;
}
...
__global__ void _hx_k_moe_block2(const double* __restrict__ EO0,
    ...
    for (int64_t t = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
        ...
        for (int64_t c = 0; c < C; c++) {
            int64_t j = t * C + c;
            double x0 = EO0[j];
            double g0 = x0 * 0.5 * (1.0 + erf(x0 * inv_sqrt2));
            EX0[j] = g0; EX_OUT[j] = g0;
            double x1 = EO1[j];
            double g1 = x1 * 0.5 * (1.0 + erf(x1 * inv_sqrt2));
...
__global__ void _hx_k_gelu_bwd(const double* __restrict__ G,
    ...
    for (int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
        ...
        double x   = G[i];
        double cdf = 0.5 * (1.0 + erf(x * inv_sqrt2));
        double pdf = inv_sqrt2pi * exp(0.0 - 0.5 * x * x);
 ...
 * Device exp() is the IEEE libm contract (same as the host `exp`), and the
 * 1/T = 1.0/(double)T scaling matches `1.0 / to_float(T)` exactly → bit-exact
 * (max|Δ|=0). Fuses the clm_ce_grad host glue at the head of the full-step
 * driver (before clm_prod_bwd). Keeps DLOGITS FARR_DEVICE so the first
 * conv-bwd GEMM H2D-skips it. */
__global__ void _hx_k_ce_grad(const double* __restrict__ LOGITS,
    ...
    for (int64_t t = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
        ...
        for (int64_t i2 = 0; i2 < V; i2++)
            sm += exp(LOGITS[base + i2] - mx);
        for (int64_t v3 = 0; v3 < V; v3++) {
            double p = exp(LOGITS[base + v3] - mx) / sm;
...
static int _ensure_dev_alloc_out(int64_t out_id, int64_t need_len) {
    ...
    if (!s->d_buf || s->len != e->len) {
        ...
        s->len = e->len;
        /* P1B-a'' RACEFIX (device-glue non-determinism): a freshly cudaMalloc'd
         * output buffer holds DRIVER GARBAGE (varies process-to-process). Glue
         * kernels write only the live `need_len` region; if any downstream op /
         * D2H copies the full `e->len` (the host length, ≥ need_len) the
         * uninitialized TAIL leaks into the reduction → ~7e-6 RUN-TO-RUN jitter
         * on a fully-synchronized single stream (the P1B-a' wall). Zeroing the
         * buffer on fresh alloc makes the tail a DETERMINISTIC 0.0 — the live
         * region is fully overwritten by the kernel, so CE is byte-unchanged
         * for any correct kernel, and run-to-run becomes reproducible. */
        if (_devglue_determinize_on()) {
            cudaError_t ze = cudaMemset(s->d_buf, 0,
                                        (size_t)e->len * sizeof(double));
            if (ze != cudaSuccess) {
                fprintf(stderr, "[cuda] cudaMemset out(%lld) failed: %s\n",
                        (long long)e->len, cudaGetErrorString(ze));
                return -1;
            }
        }
    }
