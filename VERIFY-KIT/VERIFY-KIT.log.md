...
## 2026-05-26 — V8 (P4a) `--verifier-cmd` pluggable external-verifier arm (phanes tenant-verifier 연결)

### 무엇
- `hexa verify --verifier-cmd "<sh-cmd>" --claim "<text>" [--expect "<str>"]` 신설. tenant 가 **자기 검사기를 가져오면(bring-your-own-verifier)** hexa 가 그 검사기를 위임 실행하고 verdict 를 tier 로 매핑한다.
- phanes 모델 = "테넌트가 objective + verifier 를 가져오고, phanes 가 OUROBOROS 루프를 구동". V8 이 그 verifier 슬롯을 `hexa verify` 에 연결 — 외부 cmd 가 곧 verifier(delegation).

### delegation 경로
- `tool/verify_cli.hexa::cmd_verifier` 가 기존 `exec_capture(<sh-cmd>)` shell-out 을 재사용 → `[stdout, stderr, exit_code]` 반환 (`tool/hexa_ld.hexa` 와 동일 호출 규약, codegen-wired 확인됨).
- verdict 는 **verbatim 기록** (g5 — LLM self-judge 없음, paraphrase 없음). 외부 verifier 는 반드시 deterministic.

### tier 매핑
- exit 0, `--expect` 없음 → 🟢 SUPPORTED-NUMERICAL
- exit 0, `--expect` stdout 일치 → 🟢 / 불일치 → 🔴 FALSIFIED (closed negative, NOT laundered)
- exit 1..126 (nonzero) → 🔴 FALSIFIED
- exit 127 / exec-fail / cmd 미공급 → 🟠 INSUFFICIENT

### 빌드 (transpiler #1170 — clean, bootstrap-free)
- `bin/hexa-verify` 를 HEAD 소스에서 clean 빌드 (모듈 로더 먼저 빌드 → `HEXA_MODULE_LOADER` env 로 6-`use` flatten). bessel `_Generic` blocker 없음. **codegen 무변경** (verify_cli.hexa 편집만).

### 매트릭스 (verbatim → `.verdicts/verify-kit-verifier-plugin/v8.txt`)
- `test 4 -eq 4` → **🟢** (pass)
- `test 4 -eq 5` → **🔴** (fail, NOT laundered)
- 부재-cmd (rc=127) → **🟠** (no checker path)
- `echo PASS --expect PASS` → **🟢** (stdout match)
- `echo NOPE --expect PASS` → **🔴** (stdout mismatch)
- empty `--verifier-cmd` → **🟠** (nothing to delegate)
- 회귀 `--expr sigma 6 12` → **🔵** (내장 경로 intact)

## 2026-05-26 — V7 flatten-drop 해소: `bin/hexa-verify` 가 nth_prime/catalan/… 포함하여 clean link, V10 빌드 unblocked

### 결론 (root-cause)
- V9 INBOX 노트(`verify_cli_congruence_engine_flatten_drop_2026_05_26.md`)가 별개의 "module-flatten layer" 버그로 진단했으나, 실제는 **#1170 이 이미 고친 stale-transpiler** 와 동일한 뿌리였다. `pub`-missing 아님 · flatten-list 누락 아님 · int-vs-float 분기 아님.
- 6개 V7 int fn(`is_prime_int`/`nth_prime`/`factorial_int`/`catalan`/`bell`/`partition`)은 `congruence_chain_engine.hexa`에서 `sigma_k`/`euler_phi`와 **동일 시그너처**(`pub fn … -> int`, 같은 모듈·같은 `use`). sigma_k 가 flatten 되면 int 블록도 flatten 된다.
- stale transpiler(05-25 15:34)가 이들을 미선언 indirect `hexa_call1(is_prime_int, n)` 로 emit → 12 undeclared. 이는 #1170 verdict 가 `hexa_call1(bessel_j0, x)` 로 기록한 것과 **같은 indirect-emission family**.

### 타임라인
- 02:08 #1150 V7 int fn 소스 랜딩
- 04:36 V9 가 **stale transpiler** 로 `bin/hexa-verify` 빌드 → V7 fn 🟠 INSUFFICIENT (배포본)
- 04:42 #1170 Mac transpiler regen (indirect-emission family 일괄 해소)
- 05:05 #1175 V9 가 INBOX 노트 커밋 (#1170 regen 픽업 전 환경)

### 검증 (HEAD #1170 transpiler + fresh module_loader, verbatim → `.verdicts/verify-cli-flatten-fix/`)
- clang undeclared 12 → **0**. emitted C 에 fwd-decl(103-108) + body(3957-4056) + direct call(5676) 존재.
- `--expr nth_prime 5 11` → **🔵** · `catalan 4 14` → **🔵** · `partition 10 42` → **🔵** · `is_prime 7 1` → **🔵**
- 회귀 `--expr sigma 6 12` → **🔵** · `gamma 5 24 --tol 1e-6` → **🟢** (V4 intact)

### 조치
- 소스 편집 없음 (블로커는 #1170 transpiler regen 으로 이미 해소). V7/V10 + 향후 모든 verify_cli 빌드 **unblocked at HEAD**. 배포된 04:36 `bin/hexa-verify` 는 HEAD-transpiler 빌드로 재설치해야 V7 fn 노출 (별도 g27 ship-cycle).

## 2026-05-26 — V10 (P5) bignum 슬라이스 랜딩: σ(P)=2·P 를 int64 넘는 완전수에 대해 bignum 으로 검증

### 결론
- VERIFY-KIT 마지막 마일스톤 V10 의 **최소 슬라이스(bignum only · g0/g33)** 랜딩. PSLQ/연분수/symbolic/ODE 는 손대지 않음(별도 엔진 필요 = above-$0-lane frontier, depletion_not_terminal).
- 동기: F6 가 완전수 P_k 에서 σ-product 가 int64 를 넘쳐 `bc` 를 썼다. V10 = `bc` 없이 hexa-native bignum 으로 σ(P_k)=2·P_k 를 검증.

### 접근 (string/limb-decimal compare)
- 신규 fn `mersenne_perfect_sigma(p)` (`self/runtime/bignum_mini_pure.hexa`): Mersenne 지수 p 를 받아 짝수 완전수 P=2^(p-1)·(2^p−1) 의 Euclid–Euler 항등식 σ(P)=2P 를 **전부 radix-10 bignum** 으로 검증. 사용 primitive = 기존 `bignum_mul_int_pure(_,2)`(doubling) · `bignum_add_pure` · `bignum_sub_pure` · `bignum_cmp_pure` + 신규 `bignum_pow2_pure`/`bignum_geosum_pow2_pure`. **bc·libgmp 0**.
- 비-tautological 핵심: 독립 기하급수 합 Σ_{i<p}2^i == 2^p−1 (Euclid 항등식) 을 σ(P)=2P 와 별개 경로로 확인.
- CLI 배선: `tool/verify_cli.hexa::_recompute` 에 `mersenne_perfect_sigma` 1-arg 추가 (입력 p 는 작은 int, 내부 산술만 bignum → 기존 `is_perfect` 0/1 패턴 그대로 미러). codegen 무변경.

### 빌드 unblock (선존 blocker)
- `bignum_mini_pure.hexa` 가 **사상 최초로 바이너리에 flatten** 됨(테스트는 body 를 inline 복사, verify_cli 는 지금까지 미사용). 그 결과 모듈의 `return nil`/`== nil` 관용구가 transpiler 의 `'nil' is not a value` 에 막힘 — 기존 shipped 3개 fn(`bignum_from_int_pure`/`bignum_sub_pure`/`bignum_mul_int_pure`)도 동일. 전부 **empty-array `[]` 센티넬**(canonical zero `[0]` 와 구별)로 전환 → flatten clean.
- `bin/hexa-verify` 를 HEAD #1180 transpiler + fresh `build/hexa_module_loader` 로 clean 재빌드 (V7 flatten fix #1178 + 본 nil→[] 수정). V9 가 BLOCKED 로 남겼던 빌드가 이제 통과.

### 검증 (verbatim → `.verdicts/verify-kit-bignum/v10.txt`, ASCII)
- p=17 (P_6=8589869056) → **🔵** · p=19 (P_7=137438691328) → **🔵**
- p=61 (P_9=2658455991569831744654692615953842176 ≫ int64 9.2e18) → **🔵** · p=89 (P_10≈1.9e53) → **🔵** — int64 는 P_9/P_10 을 표현조차 못 함.
- wrong-value 컨트롤(p=61 에 claim 0) → **🔴 FALSIFIED** · p=0 (완전수 아님, claim 1) → **🔴**
- compute-mode(`mersenne_perfect_sigma 61 --compute`) → **🔵** (self-verified deterministic)
- 회귀 `--expr sigma 6 12` → **🔵** (V7 정수 경로 intact)

### V1–V10 COMPLETE
- VERIFY-KIT 로드맵 P0–P5 의 랜딩 가능 슬라이스 모두 닫힘. P5 잔여(PSLQ·연분수·symbolic·수치적분/ODE) = 본 도메인 gap 아님, @D depletion_not_terminal 의 상위 레인(별도 엔진 필요).
