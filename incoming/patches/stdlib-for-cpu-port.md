...
|---|---|---|
| 1 | `incoming/patches/stdlib-os-mount-linux.md` | `mount(2)` / `umount(2)` + `MS_*` flag constants. macOS stub. |
| 2 | `incoming/patches/stdlib-os-namespace-linux.md` | `unshare(2)` / `setns(2)` / `pivot_root(2)` + `CLONE_NEW*` flags. macOS stub. |
| 3 | `incoming/patches/stdlib-os-pty.md` | `openpty` / `forkpty` + termios raw/cooked toggle. macOS + Linux. |
| 4 | `incoming/patches/stdlib-9p-codec.md` | Plan 9 9P2000.L T/R message codec + `Attacher` interface. |
| 5 | `incoming/patches/stdlib-ssh-client.md` | SSH client (dial / exec / port-fwd / key auth). Largest; can also be deferred behind `exec("ssh")` fallback. |
