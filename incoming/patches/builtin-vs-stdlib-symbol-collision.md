...
> **trees**: `self/native/thread.c` (rename globals — DONE) · `self/hexa_full.hexa` (interp's own `fn channel_send/recv/close` for TAG_CHANNEL Val store — unchanged, no collision now) · `incoming/patches/thread-channel-primitive.md` (API rename — TODO)
...
- **RFC 문서 업데이트** — `incoming/patches/thread-channel-primitive.md` 의 API 예시 (`channel_new()` → `thread_channel_new()` 등) 도 같이 수정.
