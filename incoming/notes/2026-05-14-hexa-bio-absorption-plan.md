...
| LATTICE_POLICY deployment trail (Wave K → all dancinlab repos 2026-05-12) | `LATTICE_POLICY.md` header + `AGENTS.md` "Origin: dancinlab Wave K, 2026-05-12" | metadata note in `incoming/notes/2026-05-14-hexa-bio-absorption-plan.md` (this file) | DONE-via-this-plan |
...
| `_absorption_bridge/` 9 protein-AI adapters | `alphafold3/af3_smoke.py`, etc. | Offline-replay fixture + `--selftest` SKIP-on-missing-dep | **Note pattern** in `incoming/patches/` if hexa-lang ever wants a similar adapter shape for ML model bridges; **do not port the adapters themselves** (bio-domain consumer surface) |
