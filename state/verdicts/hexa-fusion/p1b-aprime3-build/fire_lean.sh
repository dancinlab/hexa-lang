#!/usr/bin/env bash
# fire_lean.sh — lean ASYNC-OFF capstone: eager x2 + mega x2 (each a FRESH process to
# avoid the cross-run cudaFree/Malloc allocator fragmentation that slowed the x3 ladder).
# Gate 1 = run-to-run Δ0 within each arm (the 2 reps). Gate 2 = max|Δ| eager-vs-mega.
set -uo pipefail
cd ~/work
...
run_one(){ local lbl=$1; shift
 ...
 env CUDA_VISIBLE_DEVICES=0 CLM_PROD_DEVRESIDENT=1 CLM_PROD_DEVFEED=1 CLM_PROD_BATCHED=1 \
     ...
     CLM_PROD_CORPUS=~/work/corpus.txt "$@" timeout 1200 ./clm_prod_gpu >"$t" 2>&1
...
echo "############ ASYNC=0 lean capstone (eager x2, mega x2) ############"
