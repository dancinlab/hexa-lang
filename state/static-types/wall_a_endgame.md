...
**Default-ON substrate the ladder rides on** (these fire without any flag): HX3003 call-arg check via `_types_equal` (types.hexa:2827–2846), HX3004 return check (types.hexa:2221 statement-return; :3921 tail-expr; fn return lowered at :3897 via `_types_lower_type_ref`), HX3005 match-arm body consistency (`_check_match`, types.hexa:2853–2894). `_types_equal` on main is kind-string-only (types.hexa:1705–1709); `_types_assignable` adds int/float-literal coercion + HexaVal wildcard (types.hexa:1767–1778).
