...
**Segment(b) rungs on top of the value-flow closure**: HX3005 match-join unit-permissive guard (#4606 Rung A, if/else-join parity); Rung B actual-`unit` permissive guard on the return/assign/module-let Error sites (unannotated-return call resolving to `unit` no longer false-fires HX3004/HX3001; bare `return` still fires E0069); **HX3025 index-on-scalar REJECT (Rust E0608)** — indexing a KNOWN scalar (int/float/bool/char is a guaranteed `hexa_array_get` abort; **`string` now included as of round4 R1** — `s[i]` is a guaranteed runtime error, `hexa_throw "string[…]: use .chars()[i] or substring"`) surfaced to S3, `_types_strict_for` banded; **HX3026 field-on-scalar REJECT (Rust E0610)** — `n.foo` on a KNOWN scalar (round3 R1), real struct-field / builtin-selector / callee-position all exempt; **round4 R2 — binop result-type propagation SYMMETRY (Rust E0308)** — the arith numeric arm's `return lt` tail returned the LHS type, so `a_i32 + 1` already yielded i32 but `1 + a_i32` yielded i64 (lt = the literal's universal i64); R2 returns the TYPED side when the LHS is an untyped int/float literal and both operands share the numeric kind, erasing 2 order-asymmetry false-positives (`take32(1 + a_i32)` HX3003, `x = 1 + x` HX3011) while surfacing the previously-masked `take64(1 + a_i32)` HX3003 faithful fire (result i32 ≠ i64). This is a diagnostic-only pass (types erase at MIR; `lower` restarts from raw AST at main.hexa:742) so byteeq is preserved structurally — corpus-clean PR CI is the proof. catalog highest is now HX3026 (round3 R1); corpus census greps `HX30(1[167]|2[4-6])`.
