...
**아키텍처 이미 존재**(2026-06-16 확인): runtime.c 의 `HEXA_RT_SELFEMIT` 모드 = 런타임 심볼 EXTERNALLY LINKABLE(`hexa_array_new`/`hexa_str` non-static) → self-emitted native 런타임이 제공 가능. native 컴파일러는 typed/untyped/`extern fn *Void` 전부 컴파일(실측). **pickup-ready 다세션 플랜**: `.verdicts/zeroc-breakthrough/Z-ROADMAP-literal-zeroc.txt` (Z2a runtime_hi_gen.c 제거 → Z3 arena → Z2b tag → Z4 setjmp → Z2c core 954fn → Z5 byte-eq 졸업). 각 단계 pod-backed byte-eq, 독립 착지.
