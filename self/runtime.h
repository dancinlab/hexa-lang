...
HexaVal rt_read_f32_array_at(HexaVal path, HexaVal byte_off, HexaVal n_floats); /* runtime_core.c — native-FP32 ranged f32→farr32 reader (4 B/elem, RSS≈raw; 1x vs read_f32_at's 2x f64 farr) */
...
/* HEXA-FUSION L3 (LLM-path ATTN FWD glue half) — device-resident GQA attention
 * core forward. forge_dispatch_attn_core_fwd(Q, K, V, P_out, ctx_out, T, nh,
 * nkv, hd) -> int rc (0 ok / -1 host). ALL 9 args are farr-ids/ints (NO scalar-
 * by-value — scale is recomputed from hd inside). Reproduces nn_lib.hexa
 * nn_attn_core_fwd EXACTLY: per (hh,i) with L=i+1 and kvh=hh/(nh/nkv),
 * scale=1/sqrt(hd) [libm sqrt = bare `sqrt` builtin, correctly-rounded IEEE-754
 * → device sqrt(double) is bit-identical], scores[j]=Σ_c Q·K·scale (SEQUENTIAL),
 * causal softmax over scores[0..L] (subtract max, e=dt_exp(s−m), renormalise by
 * Σe), then ctx[i,hh,c]=Σ_j P·V. The softmax exp = dt_exp on BOTH host and
 * device: the host _nn_softmax_row migrated libm exp → dt_exp (F-OP19), and the
 * device kernel routes that same 12-term scaled-Taylor / range-halving through
 * _hx_dt_exp_dev — so exp is byte-identical on all arch/OS (libm exp diverged
 * 1 ULP arm64-macos vs x86-linux, which no device exp could match). Every
 * reduction accumulates SEQUENTIALLY in the host (hh→i→j→c) order under ONE
 * thread (NO tree re-assoc, NO atomics, NO warp partials); every mul feeding an
 * add uses __dmul_rn/__dadd_rn to defeat FMA contraction (host FP_CONTRACT OFF)
 * → bit-exact (max|Δ|=0) via _hx_cuda_farr_attn_core_fwd_gpu. P_out/ctx_out kept
 * DEVICE-RESIDENT. Gated behind HEXA_DEVRESIDENT_NN+CLM_PROD_DEVRESIDENT; no-CUDA
 * → -1 → host nn_attn_core_fwd (byte-eq). The frozen F-RFC043-LAYER-EQ-ATTN-FWD
 * max|Δ|=0 anchor stays green: its in-test reference softmax migrated to dt_exp
 * in lockstep. */
HexaVal hexa_forge_dispatch_attn_core_fwd(HexaVal Q_v, HexaVal K_v, HexaVal V_v,
                             HexaVal P_out_v, HexaVal ctx_out_v, HexaVal T_v,
                             HexaVal nh_v, HexaVal nkv_v, HexaVal hd_v);      /* runtime.c — fusion L3 attn fwd glue */
HexaVal forge_dispatch_attn_core_fwd(HexaVal Q_v, HexaVal K_v, HexaVal V_v,
                             HexaVal P_out_v, HexaVal ctx_out_v, HexaVal T_v,
                             HexaVal nh_v, HexaVal nkv_v, HexaVal hd_v);      /* runtime.c — fusion L3 attn fwd glue seam */
 ...
 * -1 → host nn_attn_core_bwd (byte-eq). (The FWD seam is now WIRED too — see
 * forge_dispatch_attn_core_fwd below; its host softmax migrated libm exp →
 * dt_exp so a bit-exact device _hx_dt_exp_dev twin exists.) */
