#!/bin/bash
# dispatch_c.sh — forge Phase R / C (Stage 1 diagnostic) fire dispatch (vast.ai H100/H200).
#
# Source: self/cuda/experiments/c_linear_autograd.cu
# Output: state/forge_phaseR_c_2026_05_17/result.json + logs
# Stage 1 measures: linear-layer fwd+bwd separate vs CUDA Graphs · HBM redundancy
# ratio · per-kernel breakdown · achieved BW utilization. Stage 2 (custom co-emitted
# kernel) gated on Stage 1 hint.
#
...
PHASE_ID="forge_phaseR_c_2026_05_17"
LOCAL_DIR="/Users/ghost/core/hexa-lang/state/forge_phaseR_c_2026_05_17"
SRC_DIR="/Users/ghost/core/hexa-lang/self/cuda/experiments"
PHASE_LABEL="forge-phaseR-C-linear-autograd"
REMOTE_WORK="/workspace/forge_phaseR_c"
...
[ -f "$SRC_DIR/c_linear_autograd.cu" ] || { echo "ERROR: c_linear_autograd.cu missing"; exit 1; }

mkdir -p "$LOCAL_DIR"
cd "$LOCAL_DIR"
echo "=== ${PHASE_ID} vast.ai dispatch (Phase R / C Stage 1, 2026-05-17) ==="
date -u
...
# ── 5) Upload c_linear_autograd.cu ────────────────────────────────────
echo "[5/9] Upload c_linear_autograd.cu ..."
$SCP_CMD "$SRC_DIR/c_linear_autograd.cu" "root@$SSH_HOST:$REMOTE_WORK/"

# ── 6) Build + run ────────────────────────────────────────────────────
echo "[6/9] Build + run c_linear_autograd (separate vs CUDA Graphs · HBM redundancy) ..."
$SSH_CMD "cd $REMOTE_WORK && \
    nvcc -O3 -std=c++14 -gencode arch=compute_90,code=sm_90 \
        c_linear_autograd.cu -lcublas -lcudart -lm -lrt \
        -o c_linear_autograd 2>&1 | tee build.log ; \
    BUILD_RC=\${PIPESTATUS[0]} ; \
    echo \"BUILD_RC=\$BUILD_RC\" ; \
    nvidia-smi --query-gpu=name,memory.used,utilization.gpu --format=csv,noheader > nvidia_smi_pre.csv ; \
    if [ \"\$BUILD_RC\" = '0' ]; then \
      ./c_linear_autograd 2>&1 | tee fire.log ; \
...
s = d.get('summary',{})
print('max_redundancy_ratio: %.3fx' % s.get('max_redundancy_ratio',0.0))
print('max_graph_speedup_pct: %+.2f%%' % s.get('max_graph_speedup_pct',0.0))
print('max_bw_util_pct: %.2f%%' % s.get('max_bw_util_pct',0.0))
print('decision_hint: ' + s.get('decision_hint','?'))
print('Per-shape:')
for sh in d.get('shapes', []):
    print('  M=%d Din=%d Dout=%d  sep=%.3f ms  graph=%.3f ms  speedup=%+.2f%%  redundancy=%.3fx  bw_util=%.1f%%  bit_eq Y/dW/dX=%d/%d/%d' % (
        sh['M'], sh['Din'], sh['Dout'], sh['t_separate_ms'], sh['t_graph_ms'], sh['graph_speedup_pct'],
        sh['bytes_redundancy_ratio'], sh['bw_util_pct_separate'],
        sh['bit_equal_Y'], sh['bit_equal_dW'], sh['bit_equal_dX']))
"
fi
echo "DONE"
