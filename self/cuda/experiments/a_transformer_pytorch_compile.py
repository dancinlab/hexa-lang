"""
a_transformer_pytorch_compile.py — forge Phase R / A Stage 2 Phase 2:
PyTorch torch.compile baseline for the Llama-style transformer block.
...
CLI:
  python a_transformer_pytorch_compile.py [preset] [mode]
    preset ∈ {all, small, medium, large}; default = all
    mode   ∈ {default, reduce-overhead}; default = default

Output: ./pytorch_compile_<mode>_result.json in CWD.

Note: torch.compile gives Inductor a chance to fuse the matmul→softmax→matmul
attention chain, the SwiGLU pointwise chain, and RMSNorm into single kernels.
This is the SOTA path. AOT vs this comparison is the strong baseline.
"""
...
class TransformerBlock(nn.Module):
    """Single Llama-style transformer block. Identical to a_transformer_pytorch.py
    but suitable for torch.compile wrapping. We use the unfused matmul→softmax→matmul
    attention path so that Inductor has the same architectural surface as our AOT
    impl (architectural fairness — both go through general matmul + softmax)."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Attention path (unfused — same as AOT for fair comparison)
        h = self.norm1(x)
        q = self.wq(h).view(B, L, nh, hd).transpose(1, 2)
...
def main():
    ...
    all_configs = [
        # Extended warmup: torch.compile JIT cost is paid in iter #1, can recompile on iter #2 if dynamic.
        dict(label='small',  B=1, L=64,  D=512,  nh=8,  Df=2048,  n_warm=8, n_iter=30),
        dict(label='medium', B=1, L=128, D=2048, nh=16, Df=5632,  n_warm=8, n_iter=20),
        dict(label='large',  B=1, L=512, D=4096, nh=32, Df=11008, n_warm=5, n_iter=10),
    ]
    configs = [c for c in all_configs if preset == 'all' or c['label'] == preset]
    print(f"[PyT-CT] preset={preset} · mode={mode} · selected {len(configs)} configs · torch={torch.__version__}")

    results = []
    compile_kwargs = {} if mode == 'default' else {'mode': mode}

    for c in configs:
        print(f"\n[PyT-CT] === config: {c['label']} B={c['B']} L={c['L']} D={c['D']} nh={c['nh']} hd={c['D']//c['nh']} Df={c['Df']} mode={mode} ===")

        model = TransformerBlock(c['D'], c['nh'], c['Df']).to(device, dtype=torch.float64)
        compiled = torch.compile(model, **compile_kwargs)
        opt = torch.optim.AdamW(compiled.parameters(), lr=1e-4, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-2)
        ...
        # Initial loss (compile triggers on this call)
        t_compile_start = time.perf_counter()
        opt.zero_grad(set_to_none=True)
        y0 = compiled(X)
        initial_loss = float(F.mse_loss(y0, target).item())
        torch.cuda.synchronize()
        t_first_iter_ms = (time.perf_counter() - t_compile_start) * 1000.0
        print(f"[PyT-CT]   first-iter (incl compile) = {t_first_iter_ms:.2f} ms")

        # Warmup (post-compile, reach steady state)
        ...
        print(f"[PyT-CT]   median={med:.4f} ms · min={mn:.4f} · max={mx:.4f} · mean={mean:.4f} · initial_loss={initial_loss:.6f} · final_loss={final_loss:.6f}")
    ...
    out = dict(
        experiment='forge_phaseR_a_transformer_pytorch_compile',
        date='2026-05-17',
    ...
    print(f"\n[PyT-CT] DONE — {len(results)} configs (mode={mode}) · output → ./{out_path}")
