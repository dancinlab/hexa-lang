#!/bin/bash
# dispatch_b.sh — forge Phase R / B (Stage 1 diagnostic) fire dispatch (vast.ai H100).
#
# Source: self/cuda/experiments/b_ffn_baseline.cu
# Output: state/forge_phaseR_b_2026_05_17/result.json + logs
# Stage 1 measures: cuBLAS FFN chain wall time, CUDA Graphs speedup, achieved
# HBM bandwidth utilization. Stage 1 PASS gates Stage 2 (custom DSM kernel).
#
...
PHASE_ID="forge_phaseR_b_2026_05_17"
LOCAL_DIR="/Users/ghost/core/hexa-lang/state/forge_phaseR_b_2026_05_17"
SRC_DIR="/Users/ghost/core/hexa-lang/self/cuda/experiments"
PHASE_LABEL="forge-phaseR-B-ffn-baseline"
REMOTE_WORK="/workspace/forge_phaseR_b"
...
[ -f "$SRC_DIR/b_ffn_baseline.cu" ] || { echo "ERROR: b_ffn_baseline.cu missing"; exit 1; }

mkdir -p "$LOCAL_DIR"
cd "$LOCAL_DIR"
echo "=== ${PHASE_ID} vast.ai dispatch (Phase R / B Stage 1, 2026-05-17) ==="
date -u

# ── 1) Search H100 offers ─────────────────────────────────────────────
echo "[1/9] Searching H100 offers under \$10/hr ..."
...
# ── 5) Upload b_ffn_baseline.cu ───────────────────────────────────────
echo "[5/9] Upload b_ffn_baseline.cu ..."
$SCP_CMD "$SRC_DIR/b_ffn_baseline.cu" "root@$SSH_HOST:$REMOTE_WORK/"

# ── 6) Build + run on H100 ────────────────────────────────────────────
echo "[6/9] Build + run b_ffn_baseline (separate vs CUDA Graphs · BW utilization) ..."
$SSH_CMD "cd $REMOTE_WORK && \
    nvcc -O3 -std=c++14 -gencode arch=compute_90,code=sm_90 \
        b_ffn_baseline.cu -lcublas -lcudart -lm -lrt \
        -o b_ffn_baseline 2>&1 | tee build.log ; \
    BUILD_RC=\${PIPESTATUS[0]} ; \
    echo \"BUILD_RC=\$BUILD_RC\" ; \
    nvidia-smi --query-gpu=name,memory.used,utilization.gpu --format=csv,noheader > nvidia_smi_pre.csv ; \
    if [ \"\$BUILD_RC\" = '0' ]; then \
      ./b_ffn_baseline 2>&1 | tee fire.log ; \
...
print('Activation: ' + d.get('activation',''))
s = d.get('summary',{})
print('graph_speedup_pct: [%.2f, %.2f]' % (s.get('min_graph_speedup_pct',0.0), s.get('max_graph_speedup_pct',0.0)))
print('bw_util_pct (separate): [%.2f, %.2f]' % (s.get('min_bw_util_pct',0.0), s.get('max_bw_util_pct',0.0)))
print('decision_hint: ' + s.get('decision_hint','?'))
print('Per-shape:')
for sh in d.get('shapes', []):
    print('  M=%d D=%d FD=%d  sep=%.3f ms  graph=%.3f ms  speedup=%+.2f%%  d1=%.4f/silu=%.4f/d2=%.4f  bw=%.0f/%.0f GB/s util=%.1f%%  bit_eq=%d max|Δ|=%.2e' % (
        sh['M'], sh['D'], sh['FD'], sh['t_separate_ms'], sh['t_graph_ms'], sh['graph_speedup_pct'],
        sh['t_dgemm1_ms'], sh['t_silu_ms'], sh['t_dgemm2_ms'],
        sh['achieved_bw_separate_GBs'], sh['achieved_bw_graph_GBs'],
        sh['bw_util_pct_separate'], sh['bit_equal'], sh['max_abs_delta']))
"
fi
echo "DONE"
