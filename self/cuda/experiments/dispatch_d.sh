#!/bin/bash
# dispatch_d.sh — forge Phase R / D fire dispatch (vast.ai H100).
#
# Adapted from anima/state/hexad_gpu_fire_2026_05_16/dispatch_h100.sh
# (anima 패턴 차용, anima 복제 X — forge 산출은 hexa-lang state/ 에 저장).
#
# Source: self/cuda/experiments/d_determinism.cu
# Output: state/forge_phaseR_d_2026_05_17/result.json + logs
# Falsifier: D paradigm default-on if max cost_pct ≤ 15% across shapes.
#
...
PHASE_ID="forge_phaseR_d_2026_05_17"
LOCAL_DIR="/Users/ghost/core/hexa-lang/state/forge_phaseR_d_2026_05_17"
SRC_DIR="/Users/ghost/core/hexa-lang/self/cuda/experiments"
PHASE_LABEL="forge-phaseR-D-determinism"
REMOTE_WORK="/workspace/forge_phaseR_d"
...
[ -f "$SRC_DIR/d_determinism.cu" ] || { echo "ERROR: d_determinism.cu missing"; exit 1; }

mkdir -p "$LOCAL_DIR"
cd "$LOCAL_DIR"
echo "=== ${PHASE_ID} vast.ai dispatch (Phase R / D, 2026-05-17) ==="
date -u

# ── 1) Search H100 offers ─────────────────────────────────────────────
echo "[1/9] Searching H100 offers under \$10/hr (DSM-capable Hopper; \$5 cap had 0 offers 2026-05-17) ..."
...
[ -z "$OFFER_ID" ] && { echo "ERROR: no H100 offer parsed"; exit 1; }
echo "  Selected: id=$OFFER_ID dph=\$$OFFER_DPH gpu=$OFFER_GPU cuda=$OFFER_CUDA"
echo "$OFFER_ID" > offer_id.txt

# ── 2) Rent instance with CUDA devel image ────────────────────────────
echo "[2/9] Renting instance with nvidia/cuda:12.4.1-cudnn-devel-ubuntu22.04 ..."
...
# Cleanup trap — SAVE_POD=1 retains on partial; explicit destroy after good pull
cleanup() {
    local rc=$?
...
# ── 5) Upload d_determinism.cu ────────────────────────────────────────
echo "[5/9] Upload d_determinism.cu ..."
$SCP_CMD "$SRC_DIR/d_determinism.cu" "root@$SSH_HOST:$REMOTE_WORK/"

# ── 6) Build + run on H100 ────────────────────────────────────────────
echo "[6/9] Build + run d_determinism (PEDANTIC vs DEFAULT, FP64 Dgemm sweep) ..."
$SSH_CMD "cd $REMOTE_WORK && \
    nvcc -O3 -std=c++14 -gencode arch=compute_90,code=sm_90 \
        d_determinism.cu -lcublas -lcudart -lm -lrt \
        -o d_determinism 2>&1 | tee build.log ; \
    BUILD_RC=\${PIPESTATUS[0]} ; \
    echo \"BUILD_RC=\$BUILD_RC\" ; \
    nvidia-smi --query-gpu=name,memory.used,utilization.gpu --format=csv,noheader > nvidia_smi_pre.csv ; \
    if [ \"\$BUILD_RC\" = '0' ]; then \
      ./d_determinism 2>&1 | tee fire.log ; \
...
SAVED=$($SSH_CMD "test -f $REMOTE_WORK/result.json && echo SAVED" 2>/dev/null || true)
...
if [ "$SAVED" = "SAVED" ]; then
    ...
    echo "  WARNING: result.json missing on remote — SAVE_POD=1 retained for inspection"
    PULL_OK=0
fi
...
if [ $PULL_OK -eq 1 ]; then
    ...
    $VASTAI destroy instance "$INSTANCE_ID" 2>&1 | head -3 || true
    SAVE_POD=0  # trap will skip destroy (already destroyed)
else
...
print('Device: cc=%s mem=%dMB cublas=%s' % (d.get('device_cc','?'), d.get('device_mem_mb',0), d.get('cublas_version','?')))
print('Hypothesis: ' + d.get('hypothesis',''))
s = d.get('summary',{})
print('max_cost_pct: %+.4f%%' % s.get('max_cost_pct', 0.0))
print('falsifier_verdict: ' + s.get('falsifier_verdict','?'))
print('Per-shape:')
for sh in d.get('shapes', []):
    print('  M=%d K=%d N=%d  t_def=%.3f ms  t_ped=%.3f ms  cost=%+.2f%%  bit_eq=def:%d/ped:%d  cross:%d  max|Δ|=%.2e' % (
        sh['M'], sh['K'], sh['N'], sh['t_default_ms'], sh['t_pedantic_ms'], sh['cost_pct'],
        sh['default_bit_equal_within'], sh['pedantic_bit_equal_within'],
        sh['cross_mode_bit_equal'], sh['cross_max_abs']))
"
fi
echo "DONE"
