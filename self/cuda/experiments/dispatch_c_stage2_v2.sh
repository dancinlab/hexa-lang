#!/bin/bash
# dispatch_c_stage2.sh — forge Phase R / C Stage 2 fire (custom fused fwd+bwd kernel).
#
# Source: self/cuda/experiments/c_fused_linear_stage2_v2.cu (single-block SMEM-resident fusion)
# Output: state/forge_phaseR_c_stage2_v2_v2_2026_05_17/{result.json, fire.log}
#
# Falsifier (RFC 044 §"Falsifier battery — C' tier"):
#   F-FORGE-C-STAGE2-FUSED-CEILING — HBM traffic ≤ 0.75 × separate (analytic: 0.667)
#   F-FORGE-C-STAGE2-DET-PRESERVE  — Y/dW/dX equivalence to separate at TOL_OP ≤ 1e-9
#
# Hardware: any CUDA GPU with sm_70+ (no DSM/cluster needed for single-block fused).
# Shape limited by SMEM: M=K=N=64 max single-block (96 KB SMEM).

set -uo pipefail

PHASE_ID="forge_phaseR_c_stage2_v2_v2_2026_05_17"
LOCAL_DIR="/Users/ghost/core/hexa-lang/state/forge_phaseR_c_stage2_v2_v2_2026_05_17"
SRC_DIR="/Users/ghost/core/hexa-lang/self/cuda/experiments"
PHASE_LABEL="forge-phaseR-C-stage2-v2-multiblock"
REMOTE_WORK="/workspace/forge_phaseR_c_stage2_v2"
...
[ -f "$SRC_DIR/c_fused_linear_stage2_v2.cu" ] || { echo "ERROR: c_fused_linear_stage2_v2.cu missing"; exit 1; }

mkdir -p "$LOCAL_DIR"
cd "$LOCAL_DIR"
echo "=== ${PHASE_ID} vast.ai dispatch (Phase R / C Stage 2, 2026-05-17) ==="
date -u
...
# ── 5) Upload ─────────────────────────────────────────────────────────
echo "[5/9] Upload c_fused_linear_stage2_v2.cu ..."
$SCP_CMD "$SRC_DIR/c_fused_linear_stage2_v2.cu" "root@$SSH_HOST:$REMOTE_WORK/"

# ── 6) Build + run ────────────────────────────────────────────────────
# Compile for whatever arch the GPU is — use sm_70+ flag for compatibility
echo "[6/9] Build + run c_fused_linear_stage2_v2 ..."
$SSH_CMD "cd $REMOTE_WORK && \
    nvcc -O3 -std=c++14 -arch=native \
        c_fused_linear_stage2_v2.cu -lcublas -lcudart -lm -lrt \
        -o c_fused_linear_stage2_v2 2>&1 | tee build.log ; \
    BUILD_RC=\${PIPESTATUS[0]} ; \
    echo \"BUILD_RC=\$BUILD_RC\" ; \
    nvidia-smi --query-gpu=name,memory.used --format=csv,noheader > nvidia_smi_pre.csv ; \
    if [ \"\$BUILD_RC\" = '0' ]; then \
      ./c_fused_linear_stage2_v2 2>&1 | tee fire.log ; \
...
s = d.get('summary',{})
print('Verdict: ' + s.get('falsifier_verdict','?'))
print('  traffic_pass=%d det_pass=%d (TOL_OP=%g)' % (s.get('all_traffic_pass',0), s.get('all_det_pass',0), s.get('tol_op',0)))
print('Per-shape:')
for sh in d.get('shapes', []):
    print('  M=%d K=%d N=%d  sep=%.4f ms  fused=%.4f ms  ratio_wall=%.3f  bytes_ratio=%.4f  max|Δ|max=%.2e  traffic=%d det=%d' % (
        sh['M'], sh['K'], sh['N'], sh['t_separate_ms'], sh['t_fused_ms'], sh['fused_over_separate'],
        sh['bytes_ratio_analytic'], max(sh['max_abs_Y'], sh['max_abs_dW'], sh['max_abs_dX']),
        sh['falsifier_traffic_pass'], sh['falsifier_det_pass']))
"
fi
echo "DONE"
