#!/bin/bash
# dispatch_a_stage2.sh — forge Phase R / A Stage 2 fire (large compute regime).
#
# Source: self/cuda/experiments/{a_aot_trainer.cu, a_pytorch_baseline.py}
#         with preset="stage2" (3 large configs only)
# Output: state/forge_phaseR_a_stage2_2026_05_17/{result.json, pytorch_result.json}
#
# Hypothesis (F-FORGE-A-STAGE2-LARGE): even in compute-dominated regime
# (>10ms wall), AOT single-binary >= 1.1× PyTorch eager.
#
# Configs (large compute spectrum):
#   large_b128  : D=8192   B=128   ~3-4ms wall  (overhead ~1.5ms → ~1.4×)
#   large_b512  : D=8192   B=512   ~10ms wall   (~1.15-1.2×)
#   xlarge_b128 : D=16384  B=128   ~10-20ms wall (~1.07-1.15×)

set -uo pipefail

PHASE_ID="forge_phaseR_a_stage2_2026_05_17"
LOCAL_DIR="/Users/ghost/core/hexa-lang/state/forge_phaseR_a_stage2_2026_05_17"
SRC_DIR="/Users/ghost/core/hexa-lang/self/cuda/experiments"
PHASE_LABEL="forge-phaseR-A-stage2-large"
REMOTE_WORK="/workspace/forge_phaseR_a_stage2"
PRESET="stage2"
...
mkdir -p "$LOCAL_DIR"
cd "$LOCAL_DIR"
echo "=== ${PHASE_ID} vast.ai dispatch (Phase R / A Stage 2 large, 2026-05-17) ==="
date -u

# ── 1) Search offers ─────────────────────────────────────────────────
echo "[1/9] Searching H100/H200/A100 offers under \$15/hr (Stage 2 A 무관 of DSM, A100 OK) ..."
...
# ── 6) Build + run CUDA AOT (Stage 2 preset) ─────────────────────────
echo "[6/9] Build + run a_aot_trainer (preset=$PRESET) ..."
$SSH_CMD "cd $REMOTE_WORK && \
    ...
    if [ \"\$BUILD_RC\" = '0' ]; then \
      ./a_aot_trainer $PRESET 2>&1 | tee fire_aot.log ; \
...
echo "[6.5/9] Run a_pytorch_baseline.py (preset=$PRESET) ..."
$SSH_CMD "cd $REMOTE_WORK && \
    python a_pytorch_baseline.py $PRESET 2>&1 | tee fire_pytorch.log ; \
...
for lbl in sorted(aot_cfgs.keys()):
    ...
    print('%-18s  %4d  %5d×%5d×%5d    %8.3f  %8.3f   %5.3f×        %s' % (
