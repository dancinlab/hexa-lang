...
#
# Source: self/cuda/experiments/c_fused_linear_stage2.cu (single-block SMEM-resident fusion)
# Output: state/forge_phaseR_c_stage2_2026_05_17/{result.json, fire.log}
#
...
PHASE_ID="forge_phaseR_c_stage2_2026_05_17"
LOCAL_DIR="/Users/ghost/core/hexa-lang/state/forge_phaseR_c_stage2_2026_05_17"
SRC_DIR="/Users/ghost/core/hexa-lang/self/cuda/experiments"
PHASE_LABEL="forge-phaseR-C-stage2-fused-linear"
REMOTE_WORK="/workspace/forge_phaseR_c_stage2"
...
[ -f "$SRC_DIR/c_fused_linear_stage2.cu" ] || { echo "ERROR: c_fused_linear_stage2.cu missing"; exit 1; }
...
# ── 5) Upload ─────────────────────────────────────────────────────────
echo "[5/9] Upload c_fused_linear_stage2.cu ..."
$SCP_CMD "$SRC_DIR/c_fused_linear_stage2.cu" "root@$SSH_HOST:$REMOTE_WORK/"

# ── 6) Build + run ────────────────────────────────────────────────────
# Compile for whatever arch the GPU is — use sm_70+ flag for compatibility
echo "[6/9] Build + run c_fused_linear_stage2 ..."
$SSH_CMD "cd $REMOTE_WORK && \
    nvcc -O3 -std=c++14 -arch=native \
        c_fused_linear_stage2.cu -lcublas -lcudart -lm -lrt \
        -o c_fused_linear_stage2 2>&1 | tee build.log ; \
    BUILD_RC=\${PIPESTATUS[0]} ; \
    echo \"BUILD_RC=\$BUILD_RC\" ; \
    nvidia-smi --query-gpu=name,memory.used --format=csv,noheader > nvidia_smi_pre.csv ; \
    if [ \"\$BUILD_RC\" = '0' ]; then \
      ./c_fused_linear_stage2 2>&1 | tee fire.log ; \
