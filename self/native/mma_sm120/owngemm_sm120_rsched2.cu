...
extern "C" __global__ void gemm_sm120(const float* __restrict__ A,
    ...
    for(int k=0; k<nk; k++){
        ...
        // ROUND-A r-sched2 (HEXA TF32 fastmode): software-pipeline the smem->reg
        // fragment LDS at the k8 sub-step granularity. We hoist the load of k8
        // sub-step `ks` into register fragments BEFORE the MMA loop, and inside
        // the MMA loop we PREFETCH the next sub-step's fragments into a second
        // register set while the current sub-step's MMAs execute. The LDS for k8
        // step t+1 thus overlaps the MMA pipe of step t (latency hide), instead
        // of being issued just-in-time before each MMA. Accumulation order and
        // operand values are IDENTICAL -> bit-for-bit baseline result.
        auto ld_frags=[&](int ks, unsigned af[WM_FRAG][4], unsigned bf[WN_FRAG][2]){
            #pragma unroll
        ...
        unsigned af0[WM_FRAG][4], bf0[WN_FRAG][2];
        unsigned af1[WM_FRAG][4], bf1[WN_FRAG][2];
        ld_frags(0, af0, bf0);                 // prime sub-step 0
        #pragma unroll
        for(int ks=0; ks<BK; ks+=8){
            bool cur0 = ((ks/8)&1)==0;
            unsigned (*afc)[4] = cur0?af0:af1; unsigned (*bfc)[2] = cur0?bf0:bf1;
            // prefetch next sub-step's fragments into the OTHER register set.
            if(ks+8<BK){ if(cur0) ld_frags(ks+8, af1, bf1); else ld_frags(ks+8, af0, bf0); }
            #pragma unroll
            for(int fmi=0; fmi<WM_FRAG; fmi++){
                #pragma unroll
                for(int fni=0; fni<WN_FRAG; fni++)
                    mma_m16n8k8(acc[fmi][fni], afc[fmi], bfc[fni]);
            }
        }
        __syncthreads();
    }
