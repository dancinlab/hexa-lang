...
// ======================================================================
// MODE 6 — W13 DEEP-ASYNC-RING GEMM. Resumes from the W10 gemm_w10 (70.7 TFLOP/s, 2 CTA/SM,
//   bit-exact). W12 (#2851) root-caused the wall: NOT occupancy but the decode<->MMA OVERLAP.
//   In gemm_w10 the gmma decode scratch is a SINGLE shared buffer; the per-slab sequence is
//       mbar_wait -> decode(shared->shared composed index) -> __syncthreads ->
//       wgmma.fence -> 4x wgmma -> wgmma.wait_group 0 -> __syncthreads -> next slab
//   The wgmma.wait_group 0 + the shared decode buffer SERIALIZE decode(N+1) behind wgmma(N):
//   the next decode cannot start writing the (single) gmma band until wgmma(N) has fully
//   drained it. W13 keeps the W10 decode-copy (W12 proved REMOVING it kills the kernel) but
//   RINGS the gmma decode scratch NSTG-deep so decode(N+1) writes a DIFFERENT gmma band while
//   wgmma(N) still reads band N -> the decode latency is HIDDEN behind the prior wgmma.
//
//   THE OVERLAP MECHANICS (software pipelining the decode<->MMA dependency W12 identified):
//     - gmma scratch is NSTG independent bands (As0/As1/B0/B1 per band), ring-indexed ki%NSTG.
//     - PROLOGUE: decode the first (NSTG-1) slabs into bands 0..NSTG-2 (no wgmma yet).
//     - STEADY: for slab ki, FIRST decode slab ki+(NSTG-1) into band (ki+NSTG-1)%NSTG (the
//       PRODUCER half), THEN issue the 4 wgmma for slab ki reading band ki%NSTG and
//       wgmma.wait_group(0) (the CONSUMER half). Because the decode of the FUTURE slab is
//       issued BEFORE the wgmma of the CURRENT slab, and they touch DIFFERENT gmma bands, the
//       scheduler overlaps the (memory-bound) decode of ki+NSTG-1 with the (tensor-core) wgmma
//       of ki. This is the classic producer-ahead software pipeline, now on the gmma band.
//   SMEM COST: each added gmma band = GMMA bytes (32KB at 128x128). NST swizzled ring is kept
//   at the W10 value; NSTG is the NEW knob. The whole W13 question is whether NSTG>=2 fits the
//   2-CTA/SM smem budget (228KB/SM on H100 -> <=114KB/CTA for 2 CTA) AND lifts 70.7.
//   bit-exact GATE (rel_rms<=3e-3) is the arbiter BEFORE any perf number (g5).
// ======================================================================
extern "C" __global__ void gemm_w13(const __grid_constant__ CUtensorMap tmapA,
                                     const __grid_constant__ CUtensorMap tmapB,
    ...
    extern __shared__ __align__(128) float sm[];
    const int ASW=TM*TKSW, BSW=TN*TKSW;          // swizzled landings (staged NST-deep)
    const int ABND=64*TKSW, BB=TKSW*64;          // gmma-laid bands (now staged NSTG-deep)
    const int SWBUF=ASW+BSW;                      // swizzled tile ring stride
    const int GMMA=2*ABND+2*BB;                   // one gmma-band group (As0+As1+B0+B1)
    // layout: [NST*SWBUF swizzled ring][NSTG*GMMA gmma decode ring][NST full mbar]
    float* gmma=sm + (size_t)NST*SWBUF;           // base of the gmma decode RING
    uint64_t* full =(uint64_t*)(gmma + (size_t)NSTG*GMMA);
    int tid=threadIdx.x; int wg=tid>>7; int band=wg; int lt=tid&127;
    int nks=K/TKSW;
    const int NATOM=TN/TKSW;
    const uint32_t bytesA=ASW*4, bytesB=BSW*4;
    ...
    uint32_t fph=0;
    int stages=NST<nks?NST:nks;
    // prime the swizzled-tile TMA ring (same as W10).
    if(tid==0){
        for(int st=0;st<stages;++st){
            float* base=sm+(size_t)st*SWBUF; float* Asw=base; float* Bsw=base+ASW;
            mbar_expect_tx(&full[st], bytesA+bytesB);
    ...
    // gmma band g: As0[g]/As1[g]/B0[g]/B1[g] inside the NSTG-deep ring.
    #define GAs0(g) (gmma + (size_t)(g)*GMMA)
    #define GAs1(g) (GAs0(g)+ABND)
    #define GB0(g)  (GAs1(g)+ABND)
    #define GB1(g)  (GB0(g)+BB)

    // ---- DECODE one K-slab `ki` from its swizzled-tile ring slot into gmma band `g`. ----
    // (the W10 composed-index decode, factored so it can be issued for a FUTURE slab ahead
    //  of the current slab's wgmma — the producer-ahead half of the W13 pipeline.)
    auto decode_slab=[&](int ki,int g){
        int st=ki%NST;
        // wait the swizzled tile for THIS ki to have landed (TMA producer fills full[st]).
        mbar_wait(&full[st], (uint32_t)((ki/NST)&1));
        float* base=sm+(size_t)st*SWBUF; float* Asw=base; float* Bsw=base+ASW;
        float* As0=GAs0(g); float* As1=GAs1(g); float* B0=GB0(g); float* B1=GB1(g);
        for(int i=tid;i<TM*TKSW;i+=256){
            int m=i/TKSW, k=i%TKSW;
            int a=m>>3, r=m&7;
            int sw = a*256 + r*32 + (((k>>2)^(r&7))<<2) + (k&3);
    ...
    // ---- after a decode, re-issue the TMA for the swizzled-tile slot just consumed. ----
    auto reload_swizzle=[&](int ki){
        if(tid==0){
            int load_ki=ki+stages;
            if(load_ki<nks){
                int lst=load_ki%NST;
    ...
    // ---- the 4 wgmma k8 sub-steps for slab `ki` reading gmma band `g` (consumer half). ----
    auto mma_slab=[&](int g){
        float* As=(band==0)?GAs0(g):GAs1(g);
        float* B0=GB0(g); float* B1=GB1(g);
        uint32_t aAb=(uint32_t)__cvta_generic_to_shared(As);
        uint32_t a0b=(uint32_t)__cvta_generic_to_shared(B0), a1b=(uint32_t)__cvta_generic_to_shared(B1);
        asm volatile("wgmma.fence.sync.aligned;\n":::"memory");
        #pragma unroll
    ...
    int depth=NSTG<nks?NSTG:nks;
    // PROLOGUE: producer-decode the first depth-1 slabs into bands 0..depth-2 (no wgmma yet).
    for(int g=0; g<depth-1; ++g){
        decode_slab(g,g);
        reload_swizzle(g);
        __syncthreads();                 // band g fully written before any wgmma can read it
    }
    // STEADY STATE: for slab ki, FIRST decode the FUTURE slab ki+(depth-1) into its band
    // (producer ahead), THEN run wgmma for slab ki (consumer). Different gmma bands -> overlap.
    for(int ki=0; ki<nks; ++ki){
        int fk=ki+depth-1;
        if(fk<nks){
            decode_slab(fk, fk%NSTG);     // PRODUCER: decode the slab (depth-1) ahead
            reload_swizzle(fk);
        }
        __syncthreads();                  // future band written + current band safe to read
        mma_slab(ki%NSTG);                // CONSUMER: wgmma for the current slab
        __syncthreads();                  // current band drained before it is re-decoded
    }
    int rbase=bm+band*64;
    int w=lt>>5,l=lt&31,rb=w*16+(l>>2),cb=(l&3)*2;
    #pragma unroll
...
int main(int argc,char**argv){
    ...
    if(MODE==6){
        // ---- W13 DEEP-ASYNC-RING GEMM: gmma decode ring NSTG-deep, bit-exact gate then perf ----
        int NST =argc>3?atoi(argv[3]):2;
        int NSTG=argc>4?atoi(argv[4]):2;        // NEW: gmma decode ring depth (the W13 knob)
        int Mx=S,Nx=S,Kx=S;
        ...
        size_t SWBUF=(size_t)(TM*TKSW + TN*TKSW);
        size_t GMMA=(size_t)(2*64*TKSW + 2*TKSW*64);
        size_t smsz=(size_t)NST*SWBUF*4 + (size_t)NSTG*GMMA*4 + (size_t)NST*8;
        dim3 grid(Nx/128,(Mx+TM-1)/TM); int blk=256;
        CK(cudaFuncSetAttribute(gemm_w13,cudaFuncAttributeMaxDynamicSharedMemorySize,(int)smsz));
        auto launch=[&](){ gemm_w13<<<grid,blk,smsz>>>(tmapA,tmapB,dD,Mx,Nx,Kx,NST,NSTG); };
        { int occ=0; cudaOccupancyMaxActiveBlocksPerMultiprocessor(&occ,(const void*)gemm_w13,blk,smsz);
          printf("OCCUPANCY MODE=6 NST=%d NSTG=%d blk=%d dynsmem=%zuB (%.1fKB/CTA) -> %d CTA/SM (%d compute-thr/SM)\n",
                 NST,NSTG,blk,smsz,smsz/1024.0,occ,occ*blk); }
        CK(cudaMemset(dD,0,szD*4));
        launch();
        cudaError_t e=cudaGetLastError(); if(e==cudaSuccess)e=cudaDeviceSynchronize();
        if(e!=cudaSuccess){printf("MODE6 OWN-FAULT NST=%d NSTG=%d %s\n",NST,NSTG,cudaGetErrorString(e));return 4;}
        CK(cudaMemcpy(hD,dD,szD*4,cudaMemcpyDeviceToHost));
        double se=0,sr=0;for(size_t i=0;i<szD;++i){double dd=(double)hD[i]-hR[i];se+=dd*dd;sr+=(double)hR[i]*hR[i];}
        double rr=sqrt(se/fmax(1e-30,sr));
        if(rr>3e-3){printf("W13 S=%d MODE=6 NST=%d NSTG=%d rel_rms=%.3e FAIL — no perf (g5)\n",S,NST,NSTG,rr);return 2;}
        cudaEvent_t s0,s1;CK(cudaEventCreate(&s0));CK(cudaEventCreate(&s1));int it=20;
        launch();CK(cudaDeviceSynchronize());
        CK(cudaEventRecord(s0));for(int i=0;i<it;++i)launch();
        CK(cudaEventRecord(s1));CK(cudaEventSynchronize(s1));
        ...
        float mc;CK(cudaEventElapsedTime(&mc,s0,s1));mc/=it;
        double tfc=fl/(mc*1e-3)/1e12,ratio=tfc/tfo;
        printf("W13 S=%d MODE=6 NST=%d NSTG=%d own=%.1f TFLOP/s cuBLAS-TF32=%.1f ratio(cuBLAS/own)=%.2fx rel_rms=%.3e PARITY=%s\n",
               S,NST,NSTG,tfo,tfc,ratio,rr,ratio<=1.3?"YES":"NO");
        return 0;
    }
