 ...
 * with A transposed on the left.
 *
 * _hx_metal_farr32_matmul_NT_a_gpu(A, M, K, B, K2, N, C) —
 *   C[M, N] = A[K, M]^T · B[K, N]   (A as passed is K×M row-major;
 *                                    MPS transposes internally;
 *                                    K2 must equal K — both name the
 *                                    contracting/inner dimension)
 *   C[i, j] = sum_{k=0..K-1} A[k, i] * B[k, j]
 *
 *   This is the bwd-weight matmul shape for ag_linear:
 *     dW = X^T · dY
 *   where X is M_batch × K_in row-major (= A here with spec's K=M_batch,
 *   spec's M=K_in), dY is M_batch × N_out row-major (= B here with
 *   spec's K=M_batch, spec's N=N_out), and dW is K_in × N_out (= C
 *   with spec's M × N). Together with _hx_metal_farr32_matmul_NT_b_gpu
 *   (the dX = dY · W^T shape) this closes the full transposed-matmul
 *   family needed for flame ag_linear backward.
 *
 *   Apple docs reference:
 *     MPSMatrixMultiplication.init(device:transposeLeft:transposeRight:
 *                                  resultRows:resultColumns:
 *                                  interiorColumns:alpha:beta:)
 *   When transposeLeft=YES, MPS interprets the left matrix as
 *   logically (resultRows × interiorColumns) = (M × K) but PHYSICALLY
 *   laid out as K rows × M columns row-major — exactly matching A's
 *   K×M row-major storage in _hx_farr32_table.
 *
 *   Memcpy strategy: same as the no-T and NT_b variants — straight
 *   memcpy host float buf → MTLBuffer.contents → MPS → memcpy back.
 *   No transpose on the host; MPS handles it via its tile scheduler.
 *
 *   Returns 0 ok / -1 err. Any -1 → runtime.c falls through to the
 *   CPU kij-NT_a fallback (safe).
 * ═══════════════════════════════════════════════════════════════════ */
int _hx_metal_farr32_matmul_NT_a_gpu(int64_t a_id, int64_t M, int64_t K,
                                     int64_t b_id, int64_t K2, int64_t N,
                                     int64_t c_id) {
    if (_metal_ensure_init() != 0) return -1;
    if (a_id < 0 || b_id < 0 || c_id < 0) {
        fprintf(stderr, "[metal] farr32_matmul_NT_a: bad ids %lld %lld %lld\n",
                (long long)a_id, (long long)b_id, (long long)c_id);
        return -1;
    }
    if (a_id >= _hx_farr32_count || b_id >= _hx_farr32_count ||
        c_id >= _hx_farr32_count) {
        fprintf(stderr, "[metal] farr32_matmul_NT_a: ids out of range "
                        "(a=%lld b=%lld c=%lld count=%lld)\n",
                (long long)a_id, (long long)b_id, (long long)c_id,
                (long long)_hx_farr32_count);
        return -1;
    ...
    if (M <= 0 || K <= 0 || N <= 0) {
        fprintf(stderr, "[metal] farr32_matmul_NT_a: bad shape "
                        "M=%lld K=%lld N=%lld\n",
                (long long)M, (long long)K, (long long)N);
        return -1;
    }
    if (K2 != K) {
        fprintf(stderr, "[metal] farr32_matmul_NT_a: K mismatch (K=%lld K2=%lld)\n",
    ...
    HexaFarr32Entry* ce = &_hx_farr32_table[c_id];
    if (!ae->buf || !be->buf || !ce->buf) {
        fprintf(stderr, "[metal] farr32_matmul_NT_a: NULL host buf\n");
        return -1;
    }
    /* A is K×M row-major (transposed view of M×K original). Validate
     * against K*M and K*N. */
    if (ae->len < K * M || be->len < K * N || ce->len < M * N) {
        fprintf(stderr, "[metal] farr32_matmul_NT_a: host len mismatch "
                        "(a=%lld<%lld b=%lld<%lld c=%lld<%lld)\n",
                (long long)ae->len, (long long)(K*M),
                (long long)be->len, (long long)(K*N),
                (long long)ce->len, (long long)(M*N));
        return -1;
    }
    ...
    @autoreleasepool {
        NSUInteger a_bytes = (NSUInteger)(K * M) * sizeof(float);
        NSUInteger b_bytes = (NSUInteger)(K * N) * sizeof(float);
        NSUInteger c_bytes = (NSUInteger)(M * N) * sizeof(float);
        ...
        id<MTLBuffer> c_buf = [g_metal_device newBufferWithLength:c_bytes
                                                          options:MTLResourceStorageModeShared];
        if (!a_buf || !b_buf || !c_buf) {
            fprintf(stderr, "[metal] farr32_matmul_NT_a: newBufferWithLength "
                            "OOM (a=%llu b=%llu c=%llu bytes)\n",
                    (unsigned long long)a_bytes,
                    (unsigned long long)b_bytes,
                    (unsigned long long)c_bytes);
        ...
        /* Descriptors:
         *   A: K rows × M cols row-major, rowBytes = M*4
         *      (transposeLeft=YES tells MPS to interpret as M×K)
         *   B: K rows × N cols row-major, rowBytes = N*4
         *   C: M rows × N cols row-major, rowBytes = N*4 */
        MPSMatrixDescriptor* a_desc =
            [MPSMatrixDescriptor matrixDescriptorWithRows:(NSUInteger)K
                                                  columns:(NSUInteger)M
                                                 rowBytes:(NSUInteger)(M * sizeof(float))
                                                 dataType:MPSDataTypeFloat32];
        MPSMatrixDescriptor* b_desc =
            [MPSMatrixDescriptor matrixDescriptorWithRows:(NSUInteger)K
                                                  columns:(NSUInteger)N
        ...
        MPSMatrix* c_mat = [[MPSMatrix alloc] initWithBuffer:c_buf descriptor:c_desc];
        if (!a_mat || !b_mat || !c_mat) {
            fprintf(stderr, "[metal] farr32_matmul_NT_a: MPSMatrix init failed\n");
            return -1;
        }

        /* The transpose-left magic: MPS computes A^T · B natively.
         * transposeLeft:YES + transposeRight:NO. resultRows=M,
         * resultColumns=N, interiorColumns=K — matches the math
         * contract C[i,j] = sum_k A[k,i] * B[k,j] verbatim. */
        MPSMatrixMultiplication* mm =
            [[MPSMatrixMultiplication alloc] initWithDevice:g_metal_device
                                              transposeLeft:YES
                                             transposeRight:NO
                                            ...
                                            interiorColumns:(NSUInteger)K
                                                      alpha:1.0
                                                       beta:0.0];
        if (!mm) {
            fprintf(stderr, "[metal] farr32_matmul_NT_a: "
                            "MPSMatrixMultiplication init failed\n");
            return -1;
        }
        ...
        [cmd waitUntilCompleted];
        if (cmd.status != MTLCommandBufferStatusCompleted) {
            fprintf(stderr, "[metal] farr32_matmul_NT_a: command buffer "
                            "status=%ld (err=%s)\n",
                    (long)cmd.status,
                    cmd.error ? [[cmd.error description] UTF8String]
                              : "(nil)");
