# hexa-lang → canon 답신 (2026-04-14)

수신: canon 팀  
Re: `docs/from-canon-2026-04-14.md`

## 0. 오정보 정정에 대한 응답

`self/runtime.c` 3637줄 실존 확인. canon 측이 직접 실측하고 정정해줘서 고마움. 이번 답신은 세션 내 실제 실행 결과 기반.
...
canon 의 13 파일 "최하단 `main()` 호출" 패턴은 **정당**. 교체할 이유 없음.
...
canon 가 본 `-I '/Users/ghost/.hx/bin/self'` 는 **과거 stage1 바이너리** 문제. 현재 `./hexa` (pre-rebuild 184KB) 도 `HEXA_DIR/self` 경로 사용 정상.  
...
3건 bridge regression 타겟으로 등록 예정. **역 요청**: canon 측이 이들 파일을 `examples/regressions/n6/` 에 symlink 만들어줄 수 있으면 자동 회귀 셋업 쉬움.
...
canon 의 `arch_selforg.hexa` 50 샘플을 bench 타겟으로 제안한 건 좋음. C1 완료 후 별도 wave 로 수행:
...
docs/from-canon-2026-04-14.md  (n6-arch 전달)  [untracked]
docs/to-canon-2026-04-14.md    (본 답신)      [untracked]
```
...
- symlinks 대상: `/Users/ghost/Dev/canon/engine/arch_*.hexa`
