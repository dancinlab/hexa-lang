...
| `mc_slab_demo.hexa` / `.py` | **D80 g_hexa_only pilot (2026-05-20)** — clean-room hexa-native 1-D slab neutron MC. Single energy group, monoenergetic source, isotropic scattering, vacuum boundaries. Runs WITHOUT OpenMC / Geant4 / ENDF — pattern proof for the hexa-native port, with bit-identical hexa-vs-python parity at the same LCG seed and ~1e-4 relative parity vs the Beer-Lambert analytic oracle at N=10^6. Companion `mc_slab_demo.py` is the Python oracle. **`absorbed=false`** — illustrative cross-sections, not measured nuclide data. See `inbox/notes/hexa-native-port-pattern-pilot-mc-transport.md` in the demiurge repo for the port pattern. |
| `mc_slab_demo_test.hexa` | Parity test — 8 assertions across pure-absorber + scattering + analytic-oracle cases. `hexa run` from the repo root. |
