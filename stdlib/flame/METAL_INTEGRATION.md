...
1. **HX_FARR32** (FP32 farr table) — without this any Metal route is
   FP64→FP32→FP64 round-trip per call (~6MB per d768·d768 W double-buffered).
   Decide: (a) parallel `_hx_farr32_table` or (b) per-call conversion shim
   at the dispatch boundary. (a) is consistent with the design but doubles
   the farr machinery; (b) is one O(n) memcpy per matmul.
2. **runtime.c dim-gate** mirroring lines 6157-6175 with `HEXA_METAL`
   guard. Inside: convert inputs FP64→FP32 (per choice above), call
   either (short-term) `MPSMatrixMultiplication` or (long-term) a hexa-
   native `_hx_metal_farr_matmul_gpu` symbol that maps to the MSL kernel
   compiled at build-time. Mac-only — `HEXA_CUDA`-no-MPS coexist fine.
...
- **F-RFC075-METAL-MATMUL-NUMERIC-EQ (PASS, this cycle)** — hand-emitted
...
- **F-RFC075-METAL-AG-LINEAR-EQ (OPEN)** — flame `ag_linear` forward+
  backward routed through the Metal dim-gate matches CPU `nn_linear_bwd`
  byte-eq under a `HEXA_METAL=1` env, on a small d128/2L decoder shape.
  This is the actual "flame on Apple GPU" north-star.
