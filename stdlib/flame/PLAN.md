...
## 2026-05-20 — RFC 059 inbox patch resolved-ssot status flip (anima Path-A dual-head / multi-term grad / PureFieldFFN)

Inbox patch `inbox/patches/flame-path-a-dual-head-and-multiterm-grad.md`
(anima §71, filed 2026-05-19) status flipped `rfc-drafted` →
`resolved-ssot` (2026-05-20). dup-race precheck (g_inbox_processing_loop
step 1) confirms RFC promote is already on main:

- `f69ae5bf` / `344fee3a` — RFC 059 draft (3-cycle phasing §3.1/3.2/3.3
  + falsifier battery §6 + open design decisions §10) +
  cycle-1 scaffold (RFC-only anchor comment markers at 5 call sites,
  zero behavior change).
- PLAN entry already registered at L595–646 ("RFC 059 drafted (anima
  Path-A dual-head + multi-term grad + PureFieldFFN, multi-cycle
  scoping)").
- Anchor markers live verbatim in `stdlib/flame/decoder_lib.hexa`
  (L44 head_g M-offset · L62 logits_g Mc-offset · L214 grad_with_aux
  wrapper) + `nn_lib.hexa` + `decoder_block_lib.hexa`.

g3 honest scope (Shape B over-claim guard, re-asserted):
**implementation NOT done**. Patch grep `pub fn (nn_decoder_fwd_dual
| nn_decoder_grad_dual | nn_decoder_grad_with_aux | m_off_head_g |
mc_off_logits_g | m_total_dual | mc_total_dual)` = 0 hits as of this
flip. Inbox routing is what is closed: RFC drafted, scaffold landed,
this PLAN entry registered, downstream filer (anima) has a
machine-locatable RFC + cycle plan to track. Cycle 1/2/3 source emit
remains separately user-gated per RFC 059 §10 (aux-seed interface
granularity · nil sentinel convention · GELU exact-vs-tanh · per-layer
block-mode mechanism). No source change this cycle; only inbox status
flip + this single PLAN entry per g_plan_consolidation domain-exception
for stdlib/flame.

cross-link: inbox patch L8-10 (new status) · RFC 059 §1 + §10 · PLAN
L595-646 (2026-05-19 cycle-1 scaffold entry) · g_inbox_processing_loop
Shape B scope_split · g_atlas_binary_builtin (cycle 1 implementation
will land via PR per §3 byte-eq invariant).
