...
| **4-C-2b** | extend `tool/flame_phase4b3_a2_build.sh` (or fork to `flame_phase4c_build.sh`) with the fused primitive concat step; verify build PASS + wall measurement on `flame_block_test.hexa` (single-block scope) | YES | 1 cycle | F-RFC048-FUSED-WALL-IMPROVED single-block (expected ≥1.3× over paired-call baseline at block level) | MID — single-block wall variance high; need ≥5-run avg per PERF.md convention |
