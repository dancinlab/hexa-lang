...
// see J. Dular et al., IEEE TASC, 2020.

Group {
    // ------- PROBLEM DEFINITION -------
    // Test name - for output files
    name = "pancakes_ref/";
    DefineConstant [testname = StrCat["hphi_ref_", Sprintf["%g", num_pancakes], "_pancakes_", Sprintf["%g", num_tapes], "_tapes"]]; // Test name - for output files

    nbConductors = num_tapes * num_pancakes;
    nbConductors_per_pancake = num_tapes;
    ...
    Cond = Region[{}];
    BndOmegaC = Region[{}];
    THICK_CUT = SURF_OUT+2*num_tapes*num_pancakes+1;
    THICK_CUT = THICK_CUT+num_tapes*num_pancakes;
    For i In {1:nbConductors}
        Cond += Region[ (TAPE + TAPE_OFFSET*(i-1)) ];
        BndOmegaC += Region[ (TAPE_BND + TAPE_OFFSET*(i-1)) ];
        Tape~{i} = Region[ (TAPE + TAPE_OFFSET*(i-1)) ];
...
Function{
    formulation = 0; // h-phi ref = 0, full-h ref = 1
    If(full_h_form)
        formulation = 1;
    EndIf
    fill_factor = 1.0;
...
Constraint {
    ...
    // h-phi form: one constraint per conductor
    For i In {1:num_pancakes} 
    ...
    { Name Current ; Type Assign; // for full-h form
        Case {
            For i In {1:nbConductors}
                { Region Tape~{i}; Value 1.0; TimeFunction I[]; }
            EndFor
        }
    }
    ...
    { Name h_space; Type Form1;
        BasisFunction {
            { Name gradpsin; NameOfCoef phin; Function BF_GradNode;
                Support Omega_h_AndBnd; Entity NodesOf[OmegaCC]; }
...
PostOperation {
    ...
    { Name detailedPower ; LastTimeStepOnly 1 ;
        ...
        Operation{
            For i In {1:nbConductors}
                Print[ dissPower[Tape~{i}], OnGlobal, Format TimeTable, File outputPowerCond~{i} ];
            EndFor
        }
        ...
        Operation {
            For i In {1:num_pancakes}
                For j In {1:num_tapes}
                    Print[ jz, OnElementsOf Tape~{nbConductors_per_pancake*(i-1)+j}, Depth 0, File Sprintf["test_j/j_%g_%g.txt",i,j], Format TimeTable ];
                EndFor
            EndFor
        }
    }
