  ...
  For j In {1:NumFilaments~{i}}
    ...
    If(FilamentShape==0)
        ...
        Else
            p0 = newp; Point(p0) = {xr, yr, 0, LcFilament*mm};
            p1 = newp; Point(p1) = {xr+FilamentRadius*mm*Cos[theta], yr+FilamentRadius*mm*Sin[theta], 0, LcFilament*mm};
            p2 = newp; Point(p2) = {xr+FilamentRadius*mm*Cos[theta+2*Pi/3], yr+FilamentRadius*mm*Sin[theta+2*Pi/3], 0, LcFilament*mm};
            p3 = newp; Point(p3) = {xr+FilamentRadius*mm*Cos[theta+4*Pi/3], yr+FilamentRadius*mm*Sin[theta+4*Pi/3], 0, LcFilament*mm};

            l1 = newl; Circle(l1) = {p1, p0, p2};
            l2 = newl; Circle(l2) = {p2, p0, p3};
            l3 = newl; Circle(l3) = {p3, p0, p1};

            l4 = newl; Line(l4) = {p0, p1};
            l5 = newl; Line(l5) = {p0, p2};
            l6 = newl; Line(l6) = {p0, p3};

            nr = 2*(FilamentRadius / LcFilament) + 1;
            nc_tmp = Floor(FilamentRadius*2*Pi/3 / LcFilament);
            nc = nc_tmp + nc_tmp%2 + 1; // Will be even
            Transfinite Line{l4,l5,l6} = nr Using Progression 1/1.05;
            Transfinite Line{l1, l2, l3} = nc;
        EndIf
    ...
    EndIf
    If(Flag_TA == 0)
        If(StructuredFilaments && FilamentShape==0)
            ll1 = newll; Line Loop(ll1) = {l1, -l5, l4};
            s1 = news; Plane Surface(s1) = {ll1};
            ll2 = newll; Line Loop(ll2) = {l2, -l6, l5};
            s2 = news; Plane Surface(s2) = {ll2};
            ll3 = newll; Line Loop(ll3) = {l3, -l4, l6};
            s3 = news; Plane Surface(s3) = {ll3};
            Transfinite Surface{s1} = {p0, p1, p2};
            Transfinite Surface{s2} = {p0, p2, p3};
            Transfinite Surface{s3} = {p0, p3, p1};
        Else
    ...
    If(FilamentShape == 0 && StructuredFilaments == 1)


    ElseIf(FilamentShape==1 && FilamentMeshTransfinite)
    ...
    ElseIf(FilamentShape==3) // Structured filaments


    EndIf
    If(Flag_TA == 0)
        If(StructuredFilaments == 1)
            llf_tmp = newll; Line Loop(llf_tmp) = CombinedBoundary{ Surface{s1}; Surface{s2}; Surface{s3}; };
            llf_0[] += llf_tmp[];
        Else
            llf_0[] += ll1;
        EndIf
    ...
    If(ThreeD && TwistFraction && Flag_TA == 0)
        If(StructuredFilaments==0 || FilamentShape!=0)
        ...
        Else
            Physical Surface(Sprintf("Filament bottom boundary (%g in layer %g)", j, i),
              BND_FILAMENT + 1000 * i + j) = {s1, s2, s3}; // bottom
            Physical Line(Sprintf("Filament bottom boundary loop (%g in layer %g)", j, i),
              BND_FILAMENT + 1300 * i + j) = {l1, l2, l3};
          sb~{i}~{j} = {s1, s2, s3};
        EndIf
      ...
      For k In {1:splits}
        ...
        Else
            ...
            tmp2[] = Extrude {{0,0,h}, {0,0,1}, {0,0,0}, PitchInGeo*2*Pi / splits * TwistFraction} {
              Surface{ tmp2[0] }; Layers{N}; //Recombine;
            };
            tmp3[] = Extrude {{0,0,h}, {0,0,1}, {0,0,0}, PitchInGeo*2*Pi / splits * TwistFraction} {
              Surface{ tmp3[0] }; Layers{N}; //Recombine;
            };
            v[] += tmp[1];
            s[] += tmp[{2:4}];
            v2[] += tmp2[1];
      ...
      Else
          Physical Surface(Sprintf("Filament top boundary (%g in layer %g)", j, i),
            BND_FILAMENT + 1100 * i + j) = {tmp[0], tmp2[0], tmp3[0]}; // top
          st~{i}~{j} = {tmp[0], tmp2[0], tmp3[0]};
          phys_fil_top += BND_FILAMENT + 1100 * i + j;

          s_lat = CombinedBoundary{Volume{v[]}; Volume{v2[]}; Volume{v3[]};};
          s_lat_full = Boundary{Volume{v[]}; Volume{v2[]}; Volume{v3[]};};
          s_lat_full() -= s_lat();
          s() -= s_lat_full();
          s_2() -= s_lat_full();
          s_3() -= s_lat_full();
          ...
          //sbottom() = Surface In BoundingBox {xr-FilamentRadius-1e-5, yr-FilamentRadius-1e-5, -1e-5, xr+FilamentRadius+1e-5, yr+FilamentRadius+1e-5, 1e-5};
          //s_lat() -= s1;//sb~{i}~{j}();

          Physical Surface(Sprintf("Filament lateral boundary (%g in layer %g)", j, i),
            BND_FILAMENT + 1200 * i + j) = {s(), s_2(), s_3()}; // sides
          Physical Volume(Sprintf("Filament volume (%g in layer %g)", j, i),
            FILAMENT + 1000 * i + j) = {v[], v2[], v3[]};

        phys_fil += FILAMENT + 1000 * i + j;
        sf[] += {s(), s_2(), s_3()};
        ll2 = newll; Line Loop(ll2) = CombinedBoundary{ Surface{tmp[0]}; Surface{tmp2[0]}; Surface{tmp3[0]}; };
        llf_1[] += ll2();
      EndIf
    ElseIf(Flag_TA == 1)
...
EndFor
/*
s1_1_extruded[] = Extrude {0,0,h} {
  Surface{ s1_0[] }; //Layers{N}; //Recombine;
};
*/
//s1_1_extruded = news; s1_1_extruded() = Translate {0, 0, 2*TwistPitch*mm * TwistFraction} {Duplicata{Surface{s1_0};}};
...
//Mesh 3;
If(ThreeD)
    Periodic Surface{s1_1} = {s1_0} Translate{0, 0, TwistPitch*mm * TwistFraction};
    Periodic Surface{s11_1} = {s11_0} Translate{0, 0, TwistPitch*mm * TwistFraction};
    Periodic Surface{s111_1} = {s111_0} Translate{0, 0, TwistPitch*mm * TwistFraction};
    ...
    // This file does not handle the non-conducting matrix case
    /*
