#!/usr/bin/env python3
"""
quantum_smallmol_alt_vqe_v7.py — Phase β #3 (v7 follow-up 2026-05-11).

3 small-mol (hd6/clc1/mfn2) 의 orig + alt SMILES → 2e/2o frontier
active-space VQE (EfficientSU2 depth=1, COBYLA 400 iter). orig vs alt
정량 비교용.

geom JSON 소비: tests/smallmol_alt_geoms.json (smallmol_alt_audit_v7.py 결과).
"""
...
def main() -> int:
    with open("tests/smallmol_alt_geoms.json") as f:
        geoms = json.load(f)

    print("# §12.2.c 3 small-mol alt 2e/2o frontier VQE\n")
    results = []
    for name, geom in geoms.items():
        ...
        print(f"  VQE={vqe['e_vqe_total_ha']:+.6f}  delta={delta_mha:.4f} mHa "
              f"({delta_uha:.2f} µHa)  "
