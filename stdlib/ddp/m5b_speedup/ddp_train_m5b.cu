// ═══════════════════════════════════════════════════════════════════════════
//  ddp_train_m5b.cu — HEXA-DDP DDP-M5b: 4-GPU end-to-end TRAINING speedup.
//
...
int main(int argc,char** argv){
    ...
    if (ndev<4){ printf("FATAL: DDP-M5b needs >= 4 GPUs on one node (found %d)\n",ndev); return 3; }
    ...
    if (crossover_H>=0) printf("crossover: 4-GPU speedup > 1.0x FIRST at H=%d (compute>comm there)\n",crossover_H);
    ...
    printf("transport: %s\n", transport);
    printf("=== DDP-M5b: %s ===\n", overall_green? "GREEN - 4-GPU DDP train byte-eq + speedup measured" : "RED");
