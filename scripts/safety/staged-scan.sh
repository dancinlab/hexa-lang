...
if [ -n "${HIVE_SAFETY_ALLOW:-}" ]; then
  ...
  # logged to state/safety_bypass_audit/audit.jsonl per OS-level7 schema so
...
# no-hardcode strengthening 2026-04-30 personal-path-leak-ban-iter4: block staged
...
# forms + state/* audit ledgers (per OS-level7 operational record).
...
printf '%s\n' "$staged" | while IFS= read -r f; do
  ...
  # no-hardcode strengthening 2026-04-30 personal-path-leak-ban-iter6: filename
  ...
  # (placeholder for operator-name-token catalog when needed).
  if ! printf '%s' "$f" | grep -Eq "$PERSONAL_PATH_EXEMPT_RE"; then
    if printf '%s' "$f" | grep -Eq "$PERSONAL_CONTENT_RE"; then
      printf '  PERSONAL_NAME  %s   (no-hardcode personal-path-leak-ban-iter6 filename match; HIVE_SAFETY_ALLOW=intent reason to bypass)\n' "$f" >> "$TMP"
    fi
  fi
  ...
  if [ -f "$f" ]; then
    ...
    # no-hardcode strengthening 2026-04-30 personal-path-leak-ban-iter4 enforcement.
    if ! printf '%s' "$f" | grep -Eq "$PERSONAL_PATH_EXEMPT_RE"; then
      if git show ":$f" 2>/dev/null \
           | grep -Ev "$PERSONAL_PLACEHOLDER_RE" \
           | grep -Eq "$PERSONAL_CONTENT_RE"; then
        printf '  PERSONAL_PATH  %s   (no-hardcode personal-path-leak-ban; HIVE_SAFETY_ALLOW=intent reason to bypass)\n' "$f" >> "$TMP"
      fi
    fi
  fi
done
