...
> One-file roadmap + cycle ledger for `stdlib/cloud` (RunPod · Vast.ai ·
> plain SSH). Domain SSOT per `AGENTS.tape` `@D g_plan_consolidation`:
> cycle-by-cycle progression appended to `compiler/PLAN.md`; this file is
> the *forward-looking checklist* + *brainstorm-to-exhaustion roadmap*.
