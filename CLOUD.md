...
- [ ] M2 — pod TTL deadman: `cloud rent --ttl 30m` starts a background watchdog that force-terminates the pod when wall exceeds · agent-crash-safe
...
- [ ] M8 — 자원 카탈로그 SSOT `~/.hx/cloud/providers.json` 신설 · demiurge `PROVIDERS.json` 4-tier 패턴 흡수 (vast_alternatives_cpu · hpc_tier_walltime_killers · gpu_accelerated_dft · academic_free) + walltime_optimizations + hexa_cloud_integration_status · 항목 메타 cost_usd_per_hr · walltime_speedup · fit_score (1-3) · highlight · notes · campaign-specific recommendation 은 per-project manifest 측으로 분리
- [ ] M9 — `cloud providers [list|fit|recommend]` verb (read-only initial) · list = 전체 카탈로그 · fit = 현 job 요구(cores/gpu/walltime)에 맞는 provider 점수순 · recommend = 캠페인 단위 추천 (maintenance vs walltime-crunch vs free-tier)
- [ ] M10 — sidecar hook 정합: commons `_pods_snapshot()` 가 `~/.hx/cloud/active-pods.json` 를 읽도록 (현 cwd `./pods.json` 기반에서 전환) · pods-route `--register` 가 같은 SSOT 등록 확인 · commons `_providers_snapshot()` 신설 (M8 providers.json 한 줄 inject) · sidecar commons/pods-route 별도 PR
- [ ] M11 — demiurge 정리: `pods.temp.json` 은 글로벌 manifest 흡수 후 archive · demiurge `PROVIDERS.json` 은 `~/.hx/cloud/providers.json` 의 캠페인 특화 superset 으로 정합 (RTSC 특화 추천표 유지, 공용 카탈로그는 글로벌 SSOT 참조)
