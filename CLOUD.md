...
- [x] M11 — demiurge 정리: `PROVIDERS.json` + `pods.temp.json` 폐기 (글로벌 M8/M5 SSOT 가 canonical)  ✅ demiurge PR #500 (superset note) → #501 (완전폐기): PROVIDERS.json `git rm` · PROVIDERS.easy.md 참조 → 글로벌 `hexa cloud providers` 포인터 · RTSC 캠페인 픽은 `domains/rtsc.compute_services.md` 에 무손실 캡처 · stale pods.temp.json orphan rm (origin 은 #491 로 이미 pods.json rename 완료) · 사용자 미커밋 작업 보존
