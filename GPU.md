...
- [x] **F-RFC071-HIR-MATMUL-AUTO-SYNTH (N143, `a3abaa7c`)** 🛸 — Triple-nested for-loop matmul pattern auto-detected at HIR→MIR layer + synthesizes STMT_BINOP("matmul") MIR shape (no explicit `gpu_matmul(...)` builtin call required). Closes N100 generality gap — source code with natural matmul triple-loop now auto-lowers through N86 codegen → N128 silicon-fire path. **Conservative exact-template gate**: matches ONLY canonical desugared-for HIR shape produced by `ast_to_hir.hexa::_desugar_for`. Triple-predicate gate at `_lower_fn` (BEFORE body recursion): (1) `@gpu_kernel` annotation (2) param names `a`/`b`/`c` at positions 0/1/2 (3) body matches strict `block { outer-desugared-for { let i; middle-desugared-for { let j; let sum=0.0; inner-desugared-for { let k; assign sum=...*...; idx++ }; assign c[idx]=sum; idx++ }; idx++ } }` shape with `_build_for_body` splice lengths exactly {3, 5, 3} and accumulator rhs containing `*` binop of two `index` exprs. Accepts both `sum = sum + a*b` and `sum += a*b` rhs shapes. **Anything outside (transposed inputs, loop-order swap, bounds-check `if`, hoisted accumulator, fused MAC) falls through to normal nested-while lowering with zero behaviour change**. **3 mir_test cases PASS**: case (g) positive 4-stmt skeleton at entry + 1-block CFG no STMT_BR · case (g-no-gpu) negative no `@gpu_kernel` ann → matcher skips · case (g-no-index-store) negative `sum = sum` instead of `c[idx] = sum` → matcher skips. **Downstream PASS counts unchanged**: mir_test (a)(b)(d)(e)(f) PASS (case c FAIL pre-existing on main, unrelated SSA-rename); nvptx_lower_test 31/31; metal_lower_test PASS; rocm_lower_test PASS. **CPU codegen MD5-IDENTICAL** (arm64_darwin 6377b904 + x86_64_linux b50a2a3c, F-RFC055-CPU-CODEGEN-UNTOUCHED preserved). **Deferred**: transposed-input loop variants (b[i*K+k]*a[k*M+j]) — no source-loop pattern discriminates transpose direction unambiguously, still need `gpu_matmul_NT_a`/`gpu_matmul_NT_b` builtin. Loop-order permutation / bounds-check wrapper / hoisted accumulator / fused MAC = separate scoped cycles. Files: hir_to_mir.hexa +382/-3 + mir_test.hexa +482.
