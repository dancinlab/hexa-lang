...
- [x] **F-RFC075-METAL-ELEMENTWISE-ON-N41-BASE (N52, `1f225fb4`)** 🛸 — Metal codegen **17 → 22 shapes** = N41 base (vec-add/mul/sub/div/scale + vec-neg/abs/sqrt + vec-exp/log/sin/cos + reduce-sum + matmul + matmul_NT_a + matmul_NT_b + matmul_bf16) + N52 element-wise (vec-max + vec-min + vec-floor + vec-ceil + vec-round). Closes N39 + N47 stale-base failure pattern (3rd attempt, succeeded via fresh-write strategy). 25-case lower_test all PASS (full hexa build + run). CPU codegen MD5-identical. Emitted bodies verbatim: `c[i] = max(a[i], b[i]);` / `c[i] = min(a[i], b[i]);` / `c[i] = floor(a[i]);` / `c[i] = ceil(a[i]);` / `c[i] = round(a[i]);`. MSL §5.10 builtin function-call form (NOT infix). +595/-28 diff in 2 metal files only. Honest scope: this re-applied N41 17 shapes that were SILENT-WIPED from main HEAD AGAIN (3rd time, per `feedback_runtime_c_deploy_regen_wipe` analog for metal_target.hexa). Worktree based on `0b3be802` (post-N41 base) successfully pulled in via cherry-pick `--theirs`.
- [x] **F-RFC075-FLAME-AG-LINEAR-BWD-CONSUMER-WIRED (N53, `c22e5fc3`)** 🛸🛸🛸 — **METAL_INTEGRATION 5/5 end-to-end COMPLETE** (consumer level). flame `ag_linear` is now end-to-end FP32 Metal capable on BOTH fwd AND bwd when `HEXA_METAL=1` + shape passes 8192 dim-gate. New `_ag_linear_metal_fp32_bwd(x, W, og, dx_out, dW_out, B, D, C)` helper in `stdlib/flame/ag_tape.hexa`: down-casts FP64 → FP32 farr32, calls `farr32_matmul_NT_b` for `dx = og·W^T` + `farr32_matmul_NT_a` for `dW = x^T·og`, up-casts back to FP64. `ag_k_linear` bwd handler env-gates `env("HEXA_METAL") == "1" && (B*D > 8192 || D*C > 8192)` → new FP32 path, else falls through to unchanged `matmul_bwd_auto` (preserves F-RFC043-AUTOGRAD-AUTO-MATMUL-BYTE-EQ green). N40's `_ag_linear_metal_fp32_fwd` re-applied alongside (main HEAD didn't have it — recurring silent-wipe pattern). Case 1 (HEXA_METAL unset) PASS by non-interference; Case 2 (HEXA_METAL=1 numeric oracle) DEFERRED per honest scope (N15/N18/N26/N34/N46 runtime symbols not on main HEAD at checkout; build cycle re-applies). `hexa parse` rc=0 both files. Honest scope: env-gate kept narrow to `ag_k_linear` bwd handler only; `matmul_bwd_auto` untouched. Artifact: `inbox/fires/rfc075_flame_ag_linear_bwd_consumer_wired_2026_05_21/` (host_check.hexa + RESULT.md)
- [x] **F-RFC075-ROCM-EMIT-VEC-ADD (N62, `befb3b49`)** 🛸 — ROCm/HIP codegen 0 → 1 shape. `compiler/codegen/rocm_target.hexa` previously P0 stub (PR #232 `a1a2a8fa`) returning `""`; now emits real HIP source for vec-add MIR pattern. Emitted source: `#include <hip/hip_runtime.h>\nextern "C" __global__ void vec_add(const float* a, const float* b, float* c, int n) { uint i = blockIdx.x * blockDim.x + threadIdx.x; if (i < n) c[i] = a[i] + b[i]; }`. 3/3 lower_test cases PASS (P0 empty-module + syntax-fragment + F-RFC075-ROCM-EMIT-VEC-ADD 15-substring battery). CPU codegen MD5-identical. **No silicon fire** — hipcc not available locally + AMD GPU pool empty (N10/N14/N22 4 cycles BLOCKED). Re-fire ready when stock returns. Multi-session follow-on: vec-mul/sub/div/scale + unary family + reduce-sum + matmul on AMD lane.
- [x] **F-RFC071-NVPTX-DIV-RN-F64 (N57, `1ab49261`, codegen-only)** — Closes N56 codegen gap. `_nvptx_binop_mnemonic` now has `div`/`/` case returning `"div.rn.f64"` (FP64-only per PTX ISA §9.7.1.4 — NO `div.approx.f64` exists). Emitted PTX now contains `div.rn.f64 %fd18, %fd15, %fd17;  // binop /` (was `// unsupported binop: /` honest stub at N56). Stub count `grep -c "unsupported binop: /"` = 0 (was 1). Gap closed at PTX emit layer. **Silicon-fire DEFERRED**: ubu-2 (10.142.0.2 LAN + 100.72.76.118 Tailscale + 192.168.50.60:2222) and ubu-1 ALL unreachable — every SSH route timed out. No alt sm_80+ host accessible. Static evidence only. Honest scope (`@D g3`): div_f16/_bf16/_f32 mixed-prec siblings NOT added (hir_to_mir doesn't synthesize div_* yet, no callers). FP32 `div.rn.f32` vs `div.approx.f32` policy deferred. **Independent pre-existing issue observed** (NOT introduced by N57): fresh `/tmp/hexa_n57_driver` P3-Path-B flatten emits broken i32/i64 bank classification on vec_mul + vec_div (`mul.f64 %fd6, %r4, %r5` instead of `mul.lo.s32`, missing `setp.lt.s64`). Healthy `nvptx_p4_vec_mul_test.hexa.ptx` was from earlier driver. Orthogonal to N56 div gap, separate cycle. CPU codegen MD5-identical, F-RFC055-CPU-CODEGEN-UNTOUCHED preserved.
