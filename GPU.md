...
- [x] **F-RFC075-ROCM-VEC-FAMILY (N65, `660db753`)** 🛸 — ROCm codegen **1 → 6 shapes**: vec-add (N62 baseline) + vec-mul + vec-sub + vec-div + vec-scale + **reduce-sum**. HIP infix operators (`a[i] OP b[i]`) for binary; reduce-sum via `__shfl_xor` butterfly across wave32 (RDNA gfx1100+) + `__lane_id() == 0u` lane-0 gate writes per-wavefront output `c[wave_id]`. Ternary `(i < n) ? a[i] : 0.0f` preserves warp-uniformity for shuffle. CDNA wave64 deferred to follow-on. 8/8 lower_test PASS (3 baseline + 5 new). hipcc smoke SKIPPED (not installed locally). CPU codegen MD5-identical (x86_64_linux + arm64_darwin + thumbv7em_eabihf + metal_target + nvptx_target byte-identical). **CRITICAL FINDING**: N62 `befb3b49` was SILENT-WIPED from main by `43c3b27e` (same pattern as N57 wipe per N67). Restored from `befb3b49` before extending; commit includes both restored baseline + P1+ extension. Negative-guard subtlety documented: HIP's `blockIdx.x * blockDim.x + threadIdx.x` thread-idx expression lives in EVERY kernel body (Metal pushes into signature attr), so bare ` + ` / ` * ` negative guards false-positive — used specific `a[i] OP b[i]` patterns. Artifact: worktree (codegen-only, no silicon-fire — AMD GPU pool empty per N10/N14/N22 4 cycles BLOCKED).
