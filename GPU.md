...
- [ ] **TIMED SILICON WALL — DEFERRED-to-serial** — `host_fusion_launch.c fused_chain.ptx baseline_chain.ptx <n> <reps>` on ubu-2 RTX 5070 records `fused_med_ms` / `baseline_med_ms` / `pct_faster` (≥30% gate). ubu-2 is shared; parallel timed fires contend → this is a serial follow-up. **§10 ">=30% whole-program fusion advantage" box stays `[ ]`** until the timed wall confirms the projection. Both PTX + host are ptxas-clean and ready.
