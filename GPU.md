...
- [x] **F-RFC071-NVPTX-MATMUL-CODEGEN (N86, `6b2b10c0`)** 🛸 — NVPTX codegen 3 matmul shapes added (mirrors Metal N41 + ROCm N72). 3 shape recognizers `_nvptx_mfunc_is_matmul_shape` + `_nvptx_mfunc_is_matmul_NT_a_shape` + `_nvptx_mfunc_is_matmul_NT_b_shape` (STMT_LOAD+STMT_LOAD+STMT_BINOP(<op>)+STMT_STORE skeletons, BINOP op string discriminator). 3 emit functions producing `.visible .entry <name>` PTX blocks with 6-param `.u64` bank + `%ctaid`-based tile compose + f32 accumulator zero-init + K-loop step-16 with `setp.lt.s32`/bra + WMMA load/mma/store with per-shape layout qualifiers. **WMMA family chosen**: `m16n16k16.f16.f16.f32` (canonical `.row.col`, NT_a `.col.col`, NT_b `.row.row`) — matches existing `_nvptx_wmma_mnemonic` table + HGEMM hand-emit baseline N76. Dispatcher `_nvptx_module_matmul_op_or_empty` + assembler `_nvptx_emit_matmul_module` + 3 helpers (`_nvptx_emit_matmul_param_bank` + `_nvptx_emit_matmul_reg_decls` + `_nvptx_emit_matmul_kernel_prologue/epilogue`). `codegen_emit_ptx_for_sm` peeks first MFunc → matmul-family routes to matmul module path; else passes through unchanged (byte-identical for 28 existing fixtures, none of which build STMT_BINOP matmul* MFunc). Cases 29/30/31 lower_test: header/entry/param/tile-compose/K-loop/WMMA positives + 4 negative guards each (wrong A-layout, wrong B-layout, wrong mma variant, FP64 fma residue). CPU codegen MD5-IDENTICAL (arm64_darwin + x86_64_linux byte-identical pre/post). Honest scope: build-runtime verification BLOCKED at sub-agent's base commit by pre-existing PTX_OP_SHFL_SYNC_BFLY_B32 undeclared issue (fixed on main HEAD via `842d6565`). Substring contracts verified by manual emit-text trace. Real-silicon fire = multi-cycle follow-on (RFC 071 P5++ HIR→MIR matmul pattern recognition + source-to-silicon fixture).
