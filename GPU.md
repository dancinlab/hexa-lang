...
- [x] **F-RFC075-METAL-REDUCE-SUM-NUMERIC-EQ** 🛸 — Apple M3 silicon-fire for the FIRST non-element-wise Metal kernel (reduce-sum via `simd_sum` intrinsic). N=1024 input filled with `1.0`s, threadgroup=32 → 32 per-SIMD-group outputs, all **exactly 32.0** (byte-eq, max|Δ|=0.0, byte_mismatch=0/32). Hand-emit `reduce_sum.metal` uses Apple's `simd_sum` + lane-0 gated per-group write. Artifact: `inbox/fires/rfc075_metal_reduce_2026_05_21/` (reduce_sum.metal + .air + .metallib + host_reduce.swift + fire.log + result.json). **N5 codegen integration DEFERRED** to follow-up cycle (sub-agent worktree `worktree-agent-a1508e23ed0bf1b12` commit `063cc728` adds `codegen_emit_metal_msl` recogniser for reduce-sum shape, but cherry-pick conflicts with N2's parallel vec-sub/div edits in 6 blocks across metal_target.hexa + metal_lower_test.hexa — clean 3-way merge requires dedicated cycle). Honest scope: silicon-fire proves Apple M3 `simd_sum` works for the reduce pattern; the codegen-side claim ("hexa emits matching MSL") relies on cross-reference to N5's worktree-committed emit helpers (md5 of hand-emit `reduce_sum.metal` matches traced codegen output per N5 report), not on a main-branch codegen path
