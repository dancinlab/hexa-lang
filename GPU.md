...
- [x] **F-RFC075-METAL-SIMDGROUP-MATMUL-FP16** (N25 fire, Apple M3 FP16 measured) — Hand-emit FP16 `simdgroup_matrix<half, 8, 8>` MMA kernels (3 variants: 8x8 + 16x16 + 32x32_tg) silicon-fired on Apple M3. **Peak: 789 GFLOPS (32x32_tg @ 1024³)**. **FP16 peak = 0.87× FP32 peak (789 vs 911) — NOT 2× as hypothesized**. Apple M3's `simdgroup_matrix<half,8,8>` runs at roughly FP32 throughput; **Apple M3 GPU does NOT have a dedicated 2× FP16 MMA boost** (unlike NVIDIA tensor cores' 2× FP16 path). FP16 wins at largest shapes (1024³ FP16 789 vs FP32 506 = 1.56×) from memory-bandwidth savings (half the buffer size), not from compute density. Numeric correctness: all 15 rows ≤4.5e-3 rel_err vs FP16-accumulator CPU ref (numerically sane); vs FP32 ref, K≥768 trips just over 1e-2 threshold from expected FP16 accumulator precision loss in K-loop. **MPS FP32 (1.7 TFLOPS) still 2.16× faster** than best hexa-emit FP16. Recommendation: mixed-precision (FP16 inputs + FP32 accumulator, MSL §6.7 supports) would restore precision while keeping bandwidth wins — most useful for ML. Artifact: `inbox/fires/rfc075_metal_simdgroup_matmul_fp16_2026_05_21/` (3 FP16 kernels + .air + .metallib + Swift harness + result.json with 15-row comparison vs FP32 baseline from N16)
- [x] **F-RFC071-NVPTX-PARAM-TYPE-PROPAGATION (ce4abc11)** — N28: Pass 0 in `_nvptx_classify_locals` reads `Local.type_id` -> register bank. Case 29 PASS: `setp.lt.s64 %p2,%rd1,%rd3` all-U64 vs pre-N28 mixed `%fd3`. 29/29 cases. Closes N17 honest gap. Does NOT alone unblock N23 E2E silicon-fire (wider classifier work remains).
- [x] **F-RFC075-METAL-FARR32-NUMERIC-EQ (bf545c41, flame Metal step 3/5)** — N26: HX_FARR32 native FP32 path. self/runtime.c +291 lines (6 builtins + dim-gate to MPS), self/metal/runtime_metal.m +179 lines (native FP32 MPS, zero down-cast). 3 smoke cases PASS max_rel=0.0. ABI byte-static (nm verify). METAL_INTEGRATION 3/5 done.
- [x] **F-RFC075-METAL-MATMUL-CODEGEN-NUMERIC-EQ (713c0a07, Apple WMMA codegen)** — N24: matmul shape recogniser + emit (verbatim N16 32x32_tg simdgroup_matrix port) in compiler/codegen/metal_target.hexa. Codegen ext +matmul (first MMA shape). Synthetic MIR `STMT_BINOP op=matmul` (no parser/HIR lowering today). 16-case lower_test all PASS via hexa build + run. Apple M3 silicon-fire 3/3 PASS (d=128/256/512), rel_err 1.79e-7/2.56e-7/3.29e-7 matches N16 hand-emit numerics to last decimal. Single-shot GFLOPS up to 550 (steady-state N16 baseline 638 @ 512^3). Artifact: inbox/fires/rfc075_metal_matmul_codegen_2026_05_21/
