...
- [x] **F-RFC071-NVPTX-DIV-INTEGER (N71-C, `9652fd5e`)** 🛸 — Integer `/` binop wired to NVPTX codegen. `_nvptx_binop_mnemonic_for_kind` (kind-aware, L826): `div` → `div.s32` (U32 bank) + `div.s64` (U64 bank). `_nvptx_binop_mnemonic` (FP64-default): `div_f32` → `div.rn.f32` (IEEE rn, sibling of N57's `div.rn.f64`). Signed default (HIR has no signedness distinction at this seam; matches existing `add.s32`/`mul.lo.s32` family). New lower_test case 29 (2-MFunc module: U64-dst via `to_i64`-cast + U32-dst via thread-id intrinsic): emitted PTX shows `div.s64 %rd4, %rd1, %rd3` and `div.s32 %r2, %r0, %r1` exactly + no `unsupported binop: /` markers. **29/29 PASS**. `nvptx_emit_test` PASS (FP64 `mul.f64`/`add.f64` byte-identical → no regression). Honest scope: N70 PTX re-emit harness not exercised this cycle (proxy verification via lower_test only); future u32-strict needs new RKIND or signedness flag. **Combined with N71-A (if landed concurrently)**: N70 multi-thread warp-reduce should fully PASS silicon-fire.
