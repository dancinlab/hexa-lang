...
- [x] **F-RFC075-ROCM-UNARY-FAMILY (N92, `c6ee61da`)** 🛸 — ROCm codegen **9 → 16 shapes**: + vec_neg + vec_abs + vec_sqrt + vec_exp + vec_log + vec_sin + vec_cos. 7 HIP scalar builtin constants (`ROCM_OP_NEG = "-"` prefix + `fabsf`/`sqrtf`/`expf`/`logf`/`sinf`/`cosf` f-suffix FP32 variants from `<math.h>`). 7 `_rocm_emit_vec_<op>_body` helpers using shared `_rocm_emit_unary_kernel_signature` (2-buffer + `int n`; reused from N65 vec-scale/reduce-sum tier). Shared `_rocm_mfunc_is_unary_shape_with_op` recogniser + 7 thin per-shape wrappers (mirrors Metal pattern). Dispatch order: most-discerning last (unary fires after matmul_NT_b). 18/18 lower_test PASS (15-positive substring battery + 5 negative guards each: no `b[i]`, no rocWMMA header, no sibling builtin, no `a[i] + b[i]`, no `a[i] * b[i]`). CPU codegen MD5-identical. hipcc smoke SKIPPED (not installed locally). **Codegen scoreboard (all vendors after N92)**: NVPTX matmul (N86 3 shapes), Metal 22 shapes (N52), ROCm 16 shapes (N62 + N65 + N72 + N92). Honest scope: codegen-only, no AMD silicon fire (pool empty N10/N14/N22), single-precision f32 only — fabs/sqrt/etc no-suffix FP64 + remaining vec-floor/ceil/round are follow-on. Artifact: worktree.
