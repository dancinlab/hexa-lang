...
- [x] **🛸🛸🛸 F-RFC071-E2E-WARP-REDUCE-SUM-NUMERIC-EQ CLOSED (N99, `b15c9399`)** — **N70 chain finally PASSed on silicon!** FP64 `gpu_warp_shuffle_xor` composition wired in `nvptx_target.hexa` via hi/lo u32 decompose + 2× shfl.bfly.b32 + mov.b64 reassemble. Per PTX ISA §9.7.4.3 + §9.7.13.4. **Silicon-fire ubu-2 RTX 5070 PASS**: `expected=512.5, got=512.5, abs_err=0, ulp_err=0`. Sample 32 per-warp outputs: `warp[0]=0.515625, warp[1]=1.515625, ..., warp[31]=31.515625` (each warp accumulates 32 lanes × `(i+1)/N` strided values via butterfly reduction). PTX pure ASCII; ptxas accepted. Emit pattern (warp_reduce_sum L77-83):
```
cvt.u32.u64 %r_shfl_mask_18, %rd16;
mov.b64 {%r_shfl_lo_18, %r_shfl_hi_18}, %fd9;
shfl.sync.bfly.b32 %r_shfl_dlo_18, %r_shfl_lo_18, %r_shfl_mask_18, 0x1f, 0xffffffff;
shfl.sync.bfly.b32 %r_shfl_dhi_18, %r_shfl_hi_18, %r_shfl_mask_18, 0x1f, 0xffffffff;
mov.b64 %fd18, {%r_shfl_dlo_18, %r_shfl_dhi_18};
add.f64 %fd19, %fd9, %fd18;
```
Surfaced bonus gap mid-cycle: mask was U64 (`mask: i32 = 16` widened by MIR layer to `%rd16`); original `_nvptx_reg_u32(mask.local_id)` rendered undeclared `%r16` → added `cvt.u32.u64` mask-narrow. 4 dst-id-scoped scratch regs (lo/hi/dlo/dhi) + 1 mask-narrow scratch declared in Pass 3.5 (mirrors N45 scratch pattern). **NEW const**: `PTX_OP_MOV_B64`. **lower_test 29/29 PASS** (28/28 baseline + Case 32 new asserts 4-instr FP64 composition pattern). **N70 closure chain (5 cycles)**: N64 (5 codegen gaps) + N71-A (shfl.bfly 32-bit) + N71-C (integer div) + N81 (const restore) + **N99 (FP64 composition)**. Multi-thread warp-reduce class CLOSED.
- [ ] **F-RFC067-HEXA-SGEMM-PERSISTENT (N94, HONEST NEGATIVE)** — Persistent kernel on N77 baseline. V1 (persistent only) chosen; V2 (split-K) deferred. Bit-exact PASS all 6 shapes (max_abs=0). **Peak 35.92 TFLOPS @ M=1536 (ratio 0.540 vs cuBLAS 66.58) = -0.39% vs N77 36.06 (PARITY within noise) / -3.10% vs N89 37.07**. Target "ratio toward 0.7+" NOT MET. **Sweep PV_PERSISTENT_FACTOR ∈ {1,2,4,8,16}**: factor=1 catastrophic regression M≥512 (no oversubscription, M=1536 → 24.99 TFLOPS, -31%); factor=8/16 parity with N77 (35.91-35.92 ± 0.4%); factor=16 with 576 tiles at M=1536 → 1 tile/CTA degenerates to N77's exact launch shape. **Why no gain (`@D g3`)**: per-tile work (~10s of µs at M=1536 with 96 K-step iterations) dwarfs grid-scheduler dispatch overhead. CUDA scheduler on Hopper/Blackwell efficiently dispatches 576 CTAs; persistent only helps when launch overhead dominates compute (very small tile work or >>1000 CTA grids). Square M=N=K shapes also don't benefit from K-axis split since M×N already saturates SMs. V2 (split-K) deferred — V1 no gain to compound, atomic-add cost would dominate at K≤1536. K>>M shapes would be natural follow-up. Artifact: `inbox/fires/rfc067_pV_hexa_sgemm_persistent_2026_05_21/`
- [x] **F-RFC071-NVPTX-MATMUL-SOURCE-TO-SILICON (N100, `fcb72487`)** 🛸 — HIR→MIR synthesis layer PASS; silicon-fire DEFERRED. **`gpu_matmul`/`gpu_matmul_NT_a`/`gpu_matmul_NT_b` builtins**: call-branch intercept in `compiler/lower/hir_to_mir.hexa` synthesises STMT_LOAD + STMT_LOAD + STMT_BINOP("matmul" / "matmul_NT_a" / "matmul_NT_b") + STMT_STORE shape; strips `gpu_` prefix to derive the binop op the codegen recogniser keys on. 3 names registered in `_bind_builtin_names`. mir_test case (f) verifies 4-stmt entry-block shape + negative STMT_CALL guard — all 3 variants PASS under `HEXA_MAC_BUILD_OK=1 hexa build + run`. New `@phase("parse_only")` fixture `compiler/codegen/nvptx_p10_matmul_test.hexa`: `@gpu_kernel fn matmul_kernel(a: [f16], b: [f16], c: [f32], M: i32, N: i32, K: i32) { gpu_matmul(c, a, b, M, N, K) }`. CPU codegen MD5-IDENTICAL. **N86 WIPE RECOVERED**: N100 sub-agent surfaced that N86 codegen (`6b2b10c0`, +424 lines: 3 shape recognisers + emit fns + dispatcher + lower_test cases 29/30/31) was SILENT-WIPED from `nvptx_target.hexa` by N88 (`e9c89904`, exactly -424 lines on same file — `feedback_worktree_merge_silent_filedrop` pattern). Restored as `cff2ebc9` this cycle. Silicon-fire path: source-derived PTX emit chain now WORKS at MIR layer; deployed driver advance to P3 + ubu-2 silicon-fire @ M=N=K=64 with ≤4 ULP CPU FP32 ref = follow-on cycle.
