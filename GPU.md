...
- [ ] **F-RFC071-E2E-NUMERIC-EQ (N23 silicon-fire FAIL — deeper gap surfaced)** — Attempted live source-to-silicon e2e fire on ubu-2 RTX 5070 sm_120. **Fire FAILS at `cuModuleLoadDataEx` JIT-load** with 10 `ptxas` errors. Pre-fire findings: PTX is 1427 B pure-ASCII, `.visible .entry my_test_kernel` (source-derived) PRESENT + `cvt.s64.s32` (N17 to_i64 lowering DID land) PRESENT, but `setp.lt.s64`/`.s32` ABSENT + `@%fd11 bra` instead of `@%p bra` (predicate emits as F64 reg). Root cause **wider than N17 closed**: `_nvptx_classify_locals` dst-kind inference does NOT propagate — every Local consuming a U32 intrinsic OR `to_i64` cast defaults to F64. `_nvptx_cmp_operand_type_suffix` sees both `<` operands as F64 → never selects integer-cmp mnemonic. Plus STMT_STORE renders `%fd-1` from `_miss` sentinel local_id=-1. Estimated fix: 100-200 LoC across `_nvptx_classify_locals` + `_nvptx_binop_mnemonic` (U32/U64 integer arithmetic mnemonics) + STMT_STORE sentinel handling + param-Local kind inference. **§10 e2e numeric row stays `[ ]`** — N13's bridge-runtime closure (`05e66f09`) was a STRUCTURAL closure (PTX emitted with source-derived entry name); the SILICON closure (NUMERIC-EQ vs CPU ref on real GPU) is separate and remains blocked on the deeper classifier+mnemonic work. Artifact: `inbox/fires/rfc071_p4_e2e_silicon_fire_2026_05_21/` (my_test_kernel.sm_80.ptx 1427B + rfc071_e2e_host.c + fire.log full ptxas error chain + result.json with substring-assert + jit-error + scope-diagnosis + next-cycle fix sketch). Note: workdir had another session's PARALLEL N17-equivalent commit `c126aaac` staged — sub-agent honored `feedback_inbox_dup_race_precheck` and did NOT touch staged changes
- [x] **F-RFC075-METAL-SIMDGROUP-MATMUL-FP16** (N25 fire, Apple M3 FP16 measured) — Hand-emit FP16 `simdgroup_matrix<half, 8, 8>` MMA kernels silicon-fired Apple M3. Peak 789 GFLOPS @ 1024³. FP16/FP32 = 0.87× (NOT 2×) — Apple M3 has NO dedicated FP16 MMA boost. Wins at largest shapes from memory-bandwidth not compute. All 15 rows numerically sane (≤4.5e-3 rel_err vs FP16-accum CPU ref). MPS FP32 1.7 TFLOPS still 2.16× faster. Artifact: `inbox/fires/rfc075_metal_simdgroup_matmul_fp16_2026_05_21/`
- [x] **F-RFC071-NVPTX-PARAM-TYPE-PROPAGATION (`ce4abc11`)** — N28: Pass 0 in `_nvptx_classify_locals` reads `Local.type_id` → register bank. Case 29 PASS: `setp.lt.s64 %p2,%rd1,%rd3` all-U64 vs pre-N28 mixed `%fd3`. 29/29. Honest scope: closes N17 honest gap but does NOT alone unblock N23 E2E silicon-fire (wider classifier+mnemonic+sentinel work remains 100-200 LoC)
- [x] **F-RFC075-METAL-FARR32-NUMERIC-EQ (`bf545c41`, flame Metal step 3/5)** 🛸 — N26: HX_FARR32 native FP32 path. self/runtime.c +291 lines (6 builtins + dim-gate to MPS), self/metal/runtime_metal.m +179 lines (native FP32 MPS, zero down-cast/up-cast). 3 smoke cases PASS max_rel=0.0 (HEXA_METAL=1 + env-unset + CPU-only). ABI byte-static (nm verify). METAL_INTEGRATION 3/5 done. Remaining: step 4 (farr_matmul_NT bwd) + step 5 (nn.Module dispatch)
- [x] **F-RFC075-METAL-MATMUL-CODEGEN-NUMERIC-EQ (713c0a07)** — N24: First MMA shape in Metal codegen (`STMT_BINOP op=matmul` synthetic + N16 32x32_tg pattern verbatim). 16-case lower_test PASS. Apple M3 silicon-fire 3/3 PASS (d=128/256/512) rel_err 1.79e-7/2.56e-7/3.29e-7 matches N16 numerics to last decimal. GFLOPS up to 550 single-shot.
- [x] **F-RFC075-MPS-FP16-BASELINE (07e04ea7, N33)** — MPS FP16 GEMM Apple M3: peak 1.875 TFLOPS @ 1024 vs FP32 1.703 = 1.10x. Per-shape ratio 0.53/0.50/0.48/1.10/1.10. **Apple M3 FP16 2× hypothesis REFUTED twice** (this + N25 hand-emit 0.87×). MPS/hexa-emit FP16 = 2.4× = engineering gap not silicon magic.
- [x] **F-RFC075-METAL-SIMDGROUP-MIXED-PRECISION-NUMERIC-EQ (99aed70f, N30)** 🛸🛸 — Mixed-prec MMA Apple M3: FP16 inputs + FP32 acc via `simdgroup_matrix<float>` accumulator + `simdgroup_multiply_accumulate<R=V=float,T=U=half>` (MSL §6.7 overload no explicit widening). **Peak 986.97 GFLOPS @ 1024³** = 1.083× N16 FP32 + 1.250× N25 pure-FP16. **Bit-exact FP32 ref (max_rel_err=0.0 across all 15 cells)** — N25 precision loss problem completely closed. Recommended Apple M3 GEMM default. Still 0.58× MPS (engineering headroom).
- [x] **F-RFC075-METAL-FARR32-MATMUL-NT-B-NUMERIC-EQ (66093a65, flame step 4/5)** 🛸 — N34: hexa_farr32_matmul_NT_b builtin in self/runtime.c (+424 incl. N15+N26 re-apply per deploy-regen wipe) + self/metal/runtime_metal.m (+158, MPSMatrixMultiplication transposeRight:YES). HEXA_METAL dim-gate (M*K>8192||K*N>8192) → MPS, CPU fallback ikj j-major outer + k-inner 4-unrolled SGEMM-NT_b. 4/4 smoke PASS (4×4 + 8×8 exact, 64×64 CPU max_rel=1.6e-4, 128×128 MPS-dispatch max_rel=3.3e-3 within FP32 SGEMM budget). N18 (cf4b1e38) was intact. **METAL_INTEGRATION 4/5 done**. Remaining step 5: flame consumer rewrite.
- [x] **F-RFC075-METAL-MATMUL-NT-CODEGEN-NUMERIC-EQ (0680327d+dab5b06d, N31)** 🛸 — Metal codegen +matmul_NT_a + matmul_NT_b shapes (14→16). MSL transpose-load API: `simdgroup_load(matrix, source, elements_per_row, origin, bool transpose_matrix)` — positional bool. 18-case lower_test PASS. Apple M3 silicon-fire 6/6 PASS max|abs|≤7e-5. flame METAL_INTEGRATION codegen-side step 4 complete. Artifact: `inbox/fires/rfc075_metal_matmul_NT_codegen_2026_05_21/`
- [x] **F-RFC071-NVPTX-E2E-PTXAS-CLEAN + F-RFC071-NVPTX-E2E-LAUNCH (N29 6-commit chain cdfd7ecd..0ec4723a + 6f752430)** 🛸🛸🛸 — JIT-load closure achieved! N23 10 ptxas errors → 0. cuModuleLoadDataEx ACCEPTED source-derived PTX (1461 B, compile 0.955 ms, 4 regs). cuLaunchKernel 1×1024 + cuCtxSynchronize SUCCEEDED. **First time live source-derived PTX runs on real silicon (ubu-2 RTX 5070 sm_120 driver 580).** 5/5 N23 gaps closed: dst-kind propagation + integer binop mnemonics + STMT_ASSIGN sentinel + predicate-bank + KERNEL array-param U64 fallback. 29/29 lower_test PASS. **F-RFC071-E2E-NUMERIC-EQ still FAIL** (byte_mismatch=1024/1024 sentinel) — HIR→MIR doesn't yet synthesize STMT_LOAD/STORE for source-derived path. NUMERIC closure = next-cycle (param-load + body load/store emission). Artifact: `inbox/fires/rfc071_p4_jit_load_pass_2026_05_21/`
- [x] **F-RFC075-METAL-SIMDGROUP-MATMUL-64x64-NUMERIC-EQ (N37 fire)** 🛸🛸🛸 — Apple M3 64×64_tg + DB. **Peak 64×64_tg_db = 1518.73 GFLOPS @ 1024³** (median 50 timed + 5 warmup). **MPS gap closure 74.3%** (exceeded 50% target). vs N30 32×32_tg mixed-prec (986.97) = 1.539× DB / 1.30× single. vs MPS FP32 (1702.75) = **0.892× DB**. **All 12 rows PASS rel_err < 1e-4 (actual max_rel_err = 0.0)**. Per-shape DB: 256→408, 512→1370, 768→1041, **1024→1519 peak**, 1536→1247, 2048→1123. Remaining 11% MPS gap = async-copy + better K-unroll + register-resident accumulators + interleaved A/B loads. Artifact: `inbox/fires/rfc075_metal_simdgroup_matmul_64x64_2026_05_21/`
- [x] **F-RFC075-METAL-SIMDGROUP-MATMUL-BF16 (N36 fire)** 🛸 — Apple M3 bf16 inputs + FP32 accumulator via `simdgroup_matrix<bfloat, 8, 8>` (`<metal_simdgroup_matrix>` header declares `simdgroup_bfloat8x8` typedef + `_valid_simdgroup_multiply_accumulate_v<R=float,T=bfloat,U=bfloat,V=float>`). **Peak 1029.54 GFLOPS @ 1024³ (32x32_tg)** = 1.129× N16 FP32 + 1.043× N30 mixed-prec FP16+FP32. **max_rel_err = 0.0 across all 15 (kernel, shape)** — FP32 accumulator absorbs bf16 7-bit mantissa rounding entirely. Apple M3 has NO dedicated 2× bf16 boost (NVIDIA H100-style separate doubling pipe) — 1.13× lift over FP32 from bf16 input bandwidth savings (2-byte loads vs 4-byte) feeding SAME MMA pipe used for FP16+FP32. Hypothesis PASS at ≥1.0× FP32 threshold (would have been STRONG at ≥1.5×). Artifact: `inbox/fires/rfc075_metal_simdgroup_matmul_bf16_2026_05_21/`
- [x] **F-RFC075-METAL-ELEMENTWISE-FAMILY (N39, `9b857b19`)** — Metal codegen 12→17 element-wise shapes: +vec-max (`max(a[i],b[i])`) + vec-min + vec-floor (`floor(a[i])`) + vec-ceil + vec-round (MSL §5.10 builtins). 20-case lower_test PASS (full hexa build + run). CPU codegen MD5-identical. **HONEST CORRECTION (`@D g3`)**: N39 sub-agent worktree was forked from stale base — earlier Round 5 entries (`F-RFC075-METAL-REDUCE-SUM`/`MATMUL-CODEGEN`/`MATMUL-NT`) actually NEVER landed in main metal_target.hexa (cherry-picks went onto sub-agent's stale 7-shape base, not main; main file was untouched). Reduce-sum + matmul + matmul_NT_a/b codegen are still recognised only in their worktree branches (worktree-agent-aeea256c37dd61c79 / worktree-agent-ab9a39da597cd8e50 / worktree-agent-a9f9e8ba6e97d75cb), NOT in main. Re-application is multi-cycle follow-on (would need each worktree's diff re-applied on top of N39's 17-shape base). Apple M3 silicon-fire results (reduce-sum 32×32.0 byte-eq, matmul 6/6 PASS, matmul_NT 6/6 PASS, mixed-prec 987 GFLOPS, 64×64_db 1519 GFLOPS) are LEGITIMATE measurements from sub-agent hand-emit kernels in those artifact dirs — but the CODEGEN side claims in earlier GPU.md entries OVERSTATED what's in main today. Honest status: codegen actually has vec-add/mul/sub/div/scale + vec-neg/abs/sqrt + vec-exp/log/sin/cos + vec-max/min/floor/ceil/round = **17 element-wise shapes** in main today. MMA + matmul + reduce-sum codegen integration is a follow-on cycle.
- [x] **F-RFC067-HGEMM-23-SHAPE-EXTENSION (N38 fire)** 🛸 — Extended hexa-emit HGEMM coverage from 5 (N4 pD) to **23 shapes** spanning M=192→2048 (all 64-aligned). Same shape-port pattern as N4 but applied to N12's full curve. **All 23 hexa kernels bit-exact vs cuBLAS** (max_abs=0.0 every shape — stronger than original pD which had no sanity check). Hexa peak ratio 0.7955 @ M=256 (3.97 TFLOPS vs cuBLAS 4.99). Hexa absolute peak **17.78 TFLOPS @ M=1536** plateaus 1152-2048 at ~17 TFLOPS. Ratio crossings: <0.5 first @ M=448 (0.47) · <0.3 first @ M=1024 (0.29) · asymptotic floor 0.26 @ M=1920. **Two regimes confirmed**: (1) launch-overhead floor M≤384 ratio 0.64-0.80 (cuBLAS heuristic kernel also launch-bound at small shapes) (2) compute-bound divergence M≥448 (cuBLAS 17→67 TFLOPS scales while hexa plateaus 14-18 TFLOPS). Consistent with pE cookbook SASS-diff gap (no shared-mem staging, no double-buffering, no SASS unroll in hexa). cuBLAS re-measurement matches N12 (pF) within ~3% on shared shapes (M=1920 67.58=67.58 identical). Artifact: `inbox/fires/rfc067_pG_hgemm_16shape_extension_2026_05_21/` (host.c 23-shape sweep + gen_ptx.py S-parametric generator + 23 wmma_<M>x<M>_grid.ptx + measure.sh + fire.log + result.json with per-shape table)
- [x] **🛸🛸🛸🛸 F-RFC071-E2E-NUMERIC-EQ PASS (N35, `c29359ab` + `749b63e6`)** — **FIRST REAL SOURCE-TO-SILICON NUMERIC CLOSURE FOR HEXA-LANG `@gpu_kernel` PIPELINE**. Live `hexa build self/main.hexa` → live `cmd_build nvptx_p3_source_to_silicon_test.hexa --target=nvptx64-sm_80` → source-derived PTX (2428 B, 16 regs vs N29's 1461 B / 4 regs — body now has real load/store/arith) → ubu-2 RTX 5070 cuModuleLoadDataEx + cuLaunchKernel + cuCtxSynchronize → cuMemcpyDtoH → **byte-eq vs CPU ref: max_abs=0 byte_mismatch=0/1024**. `c[0] = 2.9647833402501771 === ref[0]`. Single 199-line surgical edit on `compiler/codegen/nvptx_target.hexa` closed 4 gaps: (1) classifier exclusion — STMT_ASSIGN `op="index"/"index_set"` no longer inherits U64 base-pointer onto loaded-value dst (2) Scratch addr-reg Pass 3 — per-fn walk appends `%rd_idxa_<dst.id>` U64 PReg per IndexGet, shared `%rd_idxs_addr` U64 PReg for IndexSet (3) body lowering — `_nvptx_lower_stmt` handles `op="index"` (emit `mul.lo.s64` + `add.s64` + `ld.global.f64`) and `op="index_set"` (same + `st.global.f64`) + new `_nvptx_elem_size_for_kind` helper (4) `ld.param` prologue — `_nvptx_lower_func` emits `ld.param.<ty> %<reg>, [<kernel>_param_<i>]` at entry block for each MFunc.params Local, gated `gpu_kind == KERNEL`. 27/27 lower_test PASS + nvptx_emit_test + vec_add_test + gemm_test PASS. CPU codegens MD5-identical (only nvptx_target.hexa changed). Honest scope: PTX has redundant `cvt.s64.s32` for repeated `to_i64(gid)` (source `c[to_i64(gid)] = a[to_i64(gid)] + b[to_i64(gid)]` each allocates fresh MIR Local + scratch addr-reg). CSE / addr-reg sharing = follow-on optimisation. Numeric-eq is bit-exact regardless. Artifact: `inbox/fires/rfc071_p4_n34_numeric_eq_pass_2026_05_21/`
