...
- [x] **F-RFC071-HIR-BF16-SYNTH (N68, `aabd4c40`)** — `_op_with_precision` in `compiler/lower/hir_to_mir.hexa` now synthesizes bf16/f16/f32-suffixed opcodes (`add_bf16`, `sub_bf16`, `mul_bf16`, `div_bf16`, etc.) when HIR carries low-precision type tags. **NEW**: `div_*` synthesis (N57 gap closed — was missing for all precisions). Plus `/` → `div` canonicalisation so HIR-frontend `/` reaches the suffix synth. 13 lower_test assertions added (12 mixed-precision pairs + 1 f64/i64 byte-eq guard). Parser-side bf16 keyword ALREADY recognised in `compiler/check/types.hexa` (line 191/906/1426/1443) — no parser blocker. CPU codegen MD5-IDENTICAL vs origin/main (`_precision_from_typ_kind` returns `""` for f64/i64 → existing CPU paths byte-equivalent). F-RFC055-CPU-CODEGEN-UNTOUCHED preserved. Downstream caveats: `_nvptx_binop_mnemonic` has no `div.<prec>` row yet (N57 added `div.rn.f64` but not f32/f16/bf16 siblings); `_metal_mfunc_is_div_vec_shape` checks `op == "div"` only (div_bf16 shape-match is future work). Codegen consumers already silicon-validated for the matmul_bf16 family (N41 + N51). Unblocks flame bf16 path future cycles.
