...
- [x] **F-RFC071-NVPTX-PARAM-TYPE-PROPAGATION (`ce4abc11`, closes N17 honest scope gap)** — N28 cherry-pick adds Pass 0 in `_nvptx_classify_locals` that walks `mfn.params` BEFORE the existing statement-walk passes. `_nvptx_param_kind_for_type_id(type_id) → RKIND` helper reads `Local.type_id` (1=i64→U64, 2=i32→U32, 3=f64→F64, 4=f32→F32, 5=bool→PRED). Unmapped (0=synthetic/aggregate, 6/7/8=string/char/unit) returns "" — Pass 0 skips, preserving exact existing behavior for 28 prior lower_test cases. New Case 29 (`_test_param_type_propagation`): asserts `setp.lt.s64 %p2, %rd1, %rd3` (all-U64) instead of pre-N28 mixed `setp.lt.s64 %p2, %rd1, %fd3`. 29/29 cases PASS. CPU codegen MD5-identical. Honest scope: Pass 0 only stamps the register-bank classification — wiring `ld.param.u64` so kernel body sees the actual param value at runtime is a separate follow-on. **Does NOT alone unblock N23's E2E silicon-fire FAIL** (the wider classifier+mnemonic+sentinel work surfaced by N23 is a separate 100-200 LoC cycle)
- [x] **F-RFC075-METAL-FARR32-NUMERIC-EQ (`bf545c41`, flame Metal step 3 of 5)** 🛸 — N26 cherry-pick lands HX_FARR32 native FP32 storage path. `self/runtime.c` (+291 lines): `HexaFarr32Entry` struct, `_hx_farr32_table` parallel state, 6 new C builtins (`hexa_farr32_zeros/_get/_set/_len/_free/_matmul`), all registered in `_hexa_init_fn_shims`. `hexa_farr32_matmul` dim-gates to `_hx_metal_farr32_matmul_gpu` on `M*K > 8192 || K*N > 8192`. `self/metal/runtime_metal.m` (+179 lines): `_hx_metal_farr32_matmul_gpu` native FP32 MPS path — **zero down-cast/up-cast**, direct memcpy host→MTLBuffer→MPS→memcpy back. Reuses N18's lazy-init device/queue. Fixed latent N15/N18 link bug: widened FP64 farr table export gate from `#ifdef HEXA_CUDA` to `#if defined(HEXA_CUDA) || defined(HEXA_METAL)`. **Smoke test 3 cases all PASS (max_rel=0.0)**: HEXA_METAL=1 (MPS dispatch fires) + env-unset (CPU fallback) + CPU-only build. ABI byte-static verified via `nm` (default Mac build unchanged). **METAL_INTEGRATION progress: 3/5 done** (gap #1 FP32 storage + gap #2 HEXA_METAL hook + gap #3 native FP32 path). Remaining: step 4 (`farr_matmul_NT` backward) + step 5 (nn.Module Metal dispatch rewrite)
