...
- [x] **F-RFC075-METAL-FARR32-MATMUL-NT-B-NUMERIC-EQ (66093a65, flame step 4/5)** 🛸 — N34: hexa_farr32_matmul_NT_b builtin in self/runtime.c (+424 incl. N15+N26 re-apply per deploy-regen wipe) + self/metal/runtime_metal.m (+158, MPSMatrixMultiplication transposeRight:YES). 4/4 smoke PASS (CPU + MPS dispatch). **METAL_INTEGRATION 4/5**. Remaining step 5: flame consumer-side rewrite.
- [x] **F-RFC075-METAL-MATMUL-NT-CODEGEN-NUMERIC-EQ (0680327d+dab5b06d, N31)** 🛸 — Metal codegen +matmul_NT_a + matmul_NT_b shapes (14→16). MSL transpose-load API: `simdgroup_load(matrix, source, elements_per_row, origin, bool transpose_matrix)` — positional bool. Both NT variants share N24 matmul preamble + 6-arg signature + tgid/lid/sgid/slid attrs; only slab cooperative-load indexing for transposed operand + bool flag change. 18-case lower_test PASS (full hexa build + run). Apple M3 silicon-fire 6/6 PASS (2 kernels × d=128/256/512) max|abs|≤7e-5 (FP32 dot-product noise bound). flame METAL_INTEGRATION codegen-side step 4 complete. Artifact: `inbox/fires/rfc075_metal_matmul_NT_codegen_2026_05_21/`
- [x] **F-RFC071-NVPTX-E2E-PTXAS-CLEAN + F-RFC071-NVPTX-E2E-LAUNCH (N29 6-commit chain)** 🛸🛸🛸 — JIT-load closure achieved! N23 10 ptxas errors → 0. cuModuleLoadDataEx ACCEPTED source-derived PTX (1461 B, compile 0.955 ms, 4 regs). cuLaunchKernel 1×1024 + cuCtxSynchronize SUCCEEDED. **First time live source-derived PTX actually runs on real silicon (ubu-2 RTX 5070 sm_120 driver 580).** 5/5 N23 gaps closed: (1) dst-kind propagation via `_nvptx_infer_binop_dst_kind` (2) integer binop mnemonics `add.s32/.s64`, `sub.s32/.s64`, `mul.lo.s32/.s64` (3) STMT_ASSIGN _miss sentinel dropped (no more `%fd-1`) (4) predicate-bank emission for symbol-form `<` aliases + `mov.u32/.u64/.pred` per dst kind (5) KERNEL array-param U64 fallback. 29/29 lower_test PASS + nvptx_emit_test + vec_add_test + gemm_test PASS. CPU codegen MD5-stable. **F-RFC071-E2E-NUMERIC-EQ still FAIL** (byte_mismatch=1024/1024, all zeros sentinel) — kernel body has no `ld.global.f64`/`st.global.f64` because HIR→MIR source-to-silicon path doesn't yet synthesize STMT_LOAD/STORE (lower_test fixtures have them but source-fixture path is separate). NUMERIC closure = next-cycle (param-load + body load/store emission). Artifact: `inbox/fires/rfc071_p4_jit_load_pass_2026_05_21/`
