...
- [ ] **F-RFC067-HEXA-WGMMA-SCAFFOLD (N195, STRUCTURAL FINDING — IMPOSSIBLE on RTX 5070)** — First hexa wgmma.async (warpgroup async MMA, sm_90+ Hopper-class) fire attempt on ubu-2 RTX 5070 sm_120. **VERDICT: ARCHITECTURALLY IMPOSSIBLE on consumer Blackwell, not a code bug.** ptxas accepts on sm_90a target (cleanly assembles m64n16k16.f32.f16.f16 — 34 reg / 2560 B smem / 0 spills / cubin 3776 B). **cuModuleLoadDataEx on sm_120 REJECTED** (CUDA_ERROR_INVALID_PTX, driver 580.159.03 API 13000) with explicit errors for all 4 wgmma families: `wgmma.fence` / `wgmma.mma_async with floating point types` / `wgmma.commit_group` / `wgmma.wait_group` all "cannot be compiled for architecture 'sm_120a'". **Architectural conclusion (`@D g3`)**: wgmma.async = Hopper sm_90a ONLY. Blackwell consumer (GB202 / sm_120a) does NOT inherit wgmma + also lacks datacenter Blackwell tcgen05 path (sm_100a / B200). Consumer Blackwell falls back to warp-level mma.sync. **Matches N104 SASS-diff**: cuBLAS on RTX 5070 picks `cutlass_80_tensorop_s16816gemm_f16` — Ampere mma.sync kernel. **cuBLAS itself does NOT use wgmma here**. **The 16% gap (ratio 0.84) CANNOT be closed by instruction-class promotion to wgmma on this silicon.** Scaffold built + ready for Hopper fire if H100/H200 pool appears (PTX descriptor encoding per PTX ISA §9.7.13.5.5, all-ones inputs → expected sum 16384.0). **Implications**: N199 (wgmma+Hilbert combined) will fail same way; "easy" 0.84→1.0 path killed; closure must come from mma.sync scheduling/tiling/occupancy or any sm_120-available TMA-class (N196 probes). Numeric/GFLOPS N/A (kernel never executed). Artifact: `inbox/fires/rfc067_pwgmma_scaffold_2026_05_22/`
- [x] **🛸🛸🛸 F-RFC067-HEXA-TMA-PROBE PASS (N196, TMA AVAILABLE on consumer Blackwell sm_120)** — `cp.async.bulk.tensor.2d` 64×64 fp16 tile load **bit-exact** on RTX 5070 sm_120 (mismatch=0/4096 halves, byte_mismatch=0). Driver JIT (CUDA 13 driver 580.159.04, ptxas 12.9) accepts + kernel runs to completion. **ptxas accept matrix**: `.target sm_120a` + PTX `.version 8.7` REQUIRED (`.target sm_90a` rejected by driver JIT for sm_120 device: "cannot be compiled to future architecture" — forward-compat does NOT work for TMA, PTX must be authored at sm_120a). **TMA descriptor encoding**: `cuTensorMapEncodeTiled` succeeds on sm_120 (standard 2D, FLOAT16, globalDim={W,H}, globalStride[1]={W*2 bytes}, boxDim={64,64}, elemStride={1,1}, no interleave/swizzle); passed as 128-byte `__grid_constant__` value param. **3 PTX bring-up pitfalls** (recorded in notes.md): (1) descriptor address through `cvta.param.u64` from `mov.b64 ..., tmap_param;` else CUDA_ERROR_ILLEGAL_ADDRESS (700) (2) dst-space qualifier `shared::cluster` NOT `shared::cta` even for non-cluster launches (3) canonical sequence `mbarrier.init → cp.async.bulk.tensor → mbarrier.arrive.expect_tx.release.cta.shared::cta.b64` (combined not separate expect_tx); wait via `mbarrier.try_wait.parity.shared::cta.b64 %p, [bar], 0`. **🔑 CRITICAL: TMA UNBLOCKS warp-specialization at mma.sync level**: `mbarrier::complete_tx::bytes` lets DMA engine drive mbarrier completion directly, replacing full-CTA `bar.sync` with per-tile mbarriers. Solves N127's structural blocker (producer warps issue TMA loads against tile-local mbarriers, consumer warps wait per-tile, no producer-consumer thread-side sync). Hexa codegen change required: emit `.target sm_120a` + PTX `.version 8.7` for TMA-bearing kernels (follow-up cycle). **Combined with N195** (wgmma impossible on sm_120): cuBLAS catch-up path on consumer Blackwell = mma.sync + TMA mbarriers (NOT wgmma). Different recipe than Hopper. Single-descriptor smoke only — not tested: double-buffer / multiple in-flight / larger box / swizzled / cluster-scope / perf vs cp.async (multi-cycle follow-on). Artifact: `inbox/fires/rfc067_ptma_probe_2026_05_22/`
- [ ] **F-RFC067-HEXA-PERSIST-SPLITK-HILBERT (N198, CATASTROPHIC NEGATIVE — confirms N94 at large M)** — Persistent CTA (P=48 = num_SMs) + Split-K (G=4) + Hilbert combined on ubu-1 RTX 5070. Bit-exact PASS all shapes (maxabs=maxrel=ulp_relative=0.0; test data 1/16, 1/8 accumulates exactly in fp32 mantissa, atomic-add ordering non-deterministic but data exact). **CATASTROPHIC REGRESSION** at every shape, worsens with M: M=4096 56.99→**35.37 TFLOPS (-38.0%, ratio 0.821→0.510)** · M=6144 58.49→**22.53 (-61.9%, ratio 0.834→0.321)** · M=8192 59.48→**9.51 (-84.1%, ratio 0.847→0.136)**. **3 compounding root causes**: (1) **Lost SM occupancy** 192 CTAs / 48 SMs = 4 CTAs/SM vs N149 8 CTAs/SM = 2× less latency-hiding pool (2) **Split-K atomic traffic** × G=4, M=8192: 4 K-groups × 16384 tiles × 32 atomic-add = **~2.1M atomic ops per call** serialised per 32-B cache line (3) **Hilbert locality INVERTS**: 4 CTAs/SM from different K-groups touch disjoint A/B regions = 4× balloon L2 working set (defeats Hilbert). Memset-vs-kernel-only diff tiny (0.33% @ M=8192) — collapse is atomic+L2+occupancy, not memset. **Conclusion**: confirms N94 "scheduler already efficient" generalizes to LARGE M. PPSH does NOT push past 0.85; pushes BACK to 0.14. **N149 PHILB Hilbert-only is local optimum on this substrate**. Combined with N195 (wgmma impossible) + N196 (TMA available): the catch-up path is NOT persistent+split-K, it's TMA mbarriers driving warp-spec. Artifact: `inbox/fires/rfc067_ppersist_splitk_hilbert_2026_05_22/`
- [ ] **F-RFC067-HEXA-WARP-SPEC-NAMED-BAR (N197, NEGATIVE — named bars NECESSARY but NOT SUFFICIENT)** — 4P+4C warp specialization with named barriers (bar.arrive + bar.sync register barno) on ubu-2 RTX 5070 sm_120. Bit-exact PASS all shapes (max_abs=0). **Substantially SLOWER than both N127 (predecessor) and N149 (current best)**. **Peak per-shape**: M=1024 24.56 / 0.462 (-34.1% vs N127's 37.28) · M=2048 28.38 / 0.424 · M=4096 20.70 / **0.296** · M=8192 9.64 / **0.136** (-83.8% vs N149's 59.48 / 0.847). Reg=56 (down from N127's 94, per-consumer mma chain went 16→8). **Did named-bar unlock perf? NO.** **Root cause (4 compounding)**: (1) `cp.async.wait_all` per K-step inside producer KILLS overlap — should be multi-stage `cp.async.wait_group N-1` to race ahead (2) 256 thd/CTA halves CTAs/SM vs N107's 128 thd/CTA = occupancy regression (3) 4 active mma-issuing warps × 8 mma = same chain density as N107 but half occupancy (4) **No swizzle → L2 thrash collapses M=8192 to 0.136 (same cliff regime as N130 pre-Hilbert)**. **Useful negative**: N127's named-bar hypothesis was correct as DECOUPLING primitive, but missing pieces are **multi-stage cp.async pipelining + L2 swizzle preservation** — not just barrier-protocol swap. **Single-session ceiling stays at N149's 0.847 @ M=8192**. Combined with N195 (wgmma impossible) + N196 (TMA available on sm_120!) + N198 (PPSH catastrophic): closing remaining 0.15 to cuBLAS requires **TMA mbarriers driving warp-spec at mma.sync level** (multi-cycle: N196's TMA + N197's named-bar combined, NOT either alone). Wgmma path dead, but TMA path lives. Artifact: `inbox/fires/rfc067_pwspec_named_bar_2026_05_22/`
