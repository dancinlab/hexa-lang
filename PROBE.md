...
| 🔵 inflight (round 14 cluster — surgical lexer/parser) | `${name}` JS 진단 [#478] · `nil`/`null` 진단 [#479] · slice open-range [#480] · hex-float `0x1.8p+1` [#477] · `.codepoints()` alias [#476] · `to_string` nan/inf casing [#475] |
| 🟡 deferred (round 14 — design-level → RFC) | postfix `?` + Result ABI · `?.` optional chaining · panic/try-catch 채널 · Some/None prelude · `[].pop()` Option lane · macro expander Phase 2 · enum `<`/`>` ordering · Range `.start`/`.end` repr · Swift `0...5` inclusive |
