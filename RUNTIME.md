...
### 2026-05-22 — step 4 cycle 96: hexa_to_string scalar branches (잔여 #2 partial discharge)

- ✅ `hexa_to_string` SCALAR branches (TAG_INT outside cache / TAG_FLOAT
  integer-valued / TAG_BOOL / TAG_VOID / TAG_STR) dispatch to
  `rt_to_string_scalar`
- C wrapper retains:
  - Small-int cache (-128..255 fast path via `_cached_small_ints`)
  - Non-integer-valued float `%g` (more precision than %.1f)
  - `TAG_ARRAY` / `TAG_MAP` / `TAG_VALSTRUCT` recursive `_hexa_to_
    string_rec` (avoids self-recursion through ported scalar fn)
- hexa-source `rt_to_string_scalar` uses `type_of` dispatch + cycle-85
  typed-let casts; integer-valued floats route through `rt_format_
  float_f(f, 1)` (cycle 60)
- **잔여 #2 partial discharge**: ~5/8 branches now hexa. Array/map
  recursion + ValStruct minimal-repr remain C (genuine self-recursion
