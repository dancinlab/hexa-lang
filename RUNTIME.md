...
### 2026-05-21 — Step 3 잔여 (genuine remainder: CORE-tier infrastructure work)

After cycles 76-88, the following candidates are NOT-yet-portable
without CORE-tier infrastructure changes. Each entry lists *why* it's
blocked + the infra path that would unblock it.

1. **`hexa_len(v)`** — polymorphic tag dispatch
   - `len()` builtin in hexa source IS `hexa_len` → porting via
     `len(v)` infinite-recurses
   - Unblock: expose `byte_len(s)` / `arr_len(arr)` / `map_len(m)`
     as type-specific hexa-source builtins so a port can pick the
     right one per type_of branch

2. **`hexa_to_string(v)`** — polymorphic + recursive + arena
   - Each branch needs a different formatter (int via cycle 60
     _rt_int_to_dec_str OK; float via cycle 60 rt_format_float_f OK;
     bool/void via literal; **array/map via recursive call into
     `hexa_to_string(child)` would recurse**; string via passthrough)
   - Plus the C body uses arena strings + interning for the result
   - Unblock: expose arena-alloc string-builder builtin OR
     accept arena-disabled fallback (heap-only port)

3. **`hexa_str_concat(a, b)`** — arena_alloc CORE-tier
   - Uses `hexa_arena_alloc` + `_hx_stats_str_concat` counters
   - Hot path in self-host parser (token + token, prefix + name)
   - Unblock: arena allocator exposed to hexa source as a builtin

4. **`hexa_eq(a, b)`** deep equality + intern pointer check
   - Float-coerce path, intern-pointer shortcut, array deep eq
     (recursive call), valstruct pointer eq, map structural eq
   - Multiple sub-paths each with their own blockers (intern pointer
     access, array recursive eq via callback similar to flat_map)
   - Unblock: HX_STR pointer-identity builtin (intern check) + accept
     polymorphic typed fast-paths

5. **Map operations** — `hexa_map_set(m, const char*, val)` ABI
   - C signature uses `const char* key` (bare C pointer, not HexaVal)
   - hexa-source can't pass `const char*` directly
   - Unblock: HexaVal-keyed map_set variant (`hexa_map_set_v`) or
     codegen-side bare-pointer marshalling

6. **`hexa_array_push_nostat(arr, item)`** / **`hexa_array_pop`** —
   heap/arena promotion CORE-tier
   - `hexa_array_pop_nostat` already done in cycle 74 via
     `arr.truncate()` builtin
   - The `_nostat` variants directly manipulate `HX_SET_ARR_CAP`,
     `hexa_array_promote_to_heap` — not exposed
   - Unblock: more array internal builtins, or accept perf cost of
     normal push (already done elsewhere)

7. **`hexa_concat_many(int n, HexaVal* parts)`** — variadic C call
   - Signature takes a C-array pointer; hexa source has no way to
     produce a `HexaVal*` raw buffer
   - Unblock: HexaVal-array-based variant (`hexa_concat_array(arr)`),
     codegen rewires concat-many lowering

These are the **genuine cycles-89+ candidates**, each requiring its
own infra investigation. Step 3 closes here at **88 fns ported** with
4 codegen activations (76/85/87 + the implicit 74 in-place mutation
runtime support). Step 4 (CORE-tier hexa-source ownership) is where
this list reopens.
...
- ✅ `hexa_to_int` polymorphic port via cycle-85 typed-let propagation.
  Float path: `let f: float = v as float; let i: int = f as int` —
  cycle-85 makes `i` a typed-int let, codegen direct-emits both casts
- type_of dispatch for int/float branches; string branch stays C-side
  (uses hexa_str_parse_int — separate cycle if needed)
- Also added `rt_to_float_poly` for symmetry (different from cycle-13
  rt_to_float which is universal coerce)
- aprime_cc smoke exit(42) PASS · 24 externs (baseline preserved) ·
  binary 1,160,840 B

### 2026-05-21 — step 3 cycle 85 ACTIVATED: typed-let annotation propagation 🛸🛸🛸

- ✅ **3rd codegen blocker SOLVED**. `let X: T = ...` annotation now
  registers X as known-int/float, surviving the cur_lets shadow
  bypass that previously blocked it
- hexa_cc.c patches: new globals `_gen2_current_fn_let_names` +
  `_gen2_current_fn_let_types` parallel arrays. Populated at LetStmt
  registration site (line ~17264). `_is_known_int_name` and
  `_is_known_float_name` consult these BEFORE the cur_lets bypass —
  promoting typed-let names to direct-emit
- Verified active:
  - `let g: float = 3.14; g + 1.0` → `hexa_float(HX_FLOAT(g) + HX_FLOAT(...))` (no hexa_add!)
  - `let f: float = v as float; f as int` → `hexa_int((int64_t)HX_FLOAT(f))` (no hexa_to_int!)
- Unblocks `hexa_to_int` / `hexa_to_float` polymorphic ports (previously
  trapped in `as int` → `hexa_to_int(v)` recursion when v is HexaVal
  but coerced through a typed let)
- aprime_cc smoke exit(42) PASS · 24 externs (baseline preserved) ·
  binary 1,160,792 B
