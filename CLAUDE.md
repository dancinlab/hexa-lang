...
├─ atlas/           — `README.md` 단일 원장 = 수학 지도 (거시↔양자 노드맵) + `.n6` 아티팩트
...
- **atlas math-map** — `atlas/` 영역의 사람용 원장은 `atlas/README.md` **단일 SSOT**
  (거시 수학체계 ↔ 양자 수학체계를 잇는 수학 지도). 지도는 README.md 에 **점진적으로
  그려나간다** (one-shot 아님 · 노드/다리를 계속 채움). `.tape` 원장(`MAIN.tape`·
  `VERIFY.tape`·`AGENTS.tape`·`MAIN.log.tape`·`inbox/`)은 retired — verdict atom 의
  기계 SSOT 는 `compiler/atlas/embedded.gen.hexa`, history 는 CHANGELOG + git. **n=6 은
  지도 중심이 아니라 정수론 노드 1개** (lattice-as-tool · 다른 영역 anchor 금지) · 저해상도
  미판독 수식은 날조 금지(c2).
