...
- **Source evidence (g3 — every claim anchored)**:
  ...
  - Phase 4-A PARTIAL LANDED measurement (3-run avg full 80-step corpus, post commits `bbaa4bbf` through `6fa735c7`): **~16s wall** (range 15.70-16.28s) vs Phase 3-J baseline ~17-18s → **~10-15% wall reduction** from 7 outer-product accumulator wire-ins. Granularity floor for `farr_matmul`-routing established at ~32K ops (drin per-t inner-dot below floor, anti-perf). Stage 2/3 IR-level fusion is the remaining ~80%+ wall savings path
...
- **Phase 4-A — Stage 1 epilogue fusion + bwd projection routing (RE-PRIORITIZED 2026-05-17; PARTIALLY LANDED with HONEST RESULT)**. ~10 fused-matmul variants in `self/runtime.c` (or `self/forge/`), restructure nn_lib call sites. ALSO route the `nn_decoder_grad` bwd projection-accumulating loops through `_db_proj_batch_farr` (mirroring the Phase 3-J fwd-side wire-in). Falsifiers: F-RFC046-FUSED-MATMUL-EPILOGUE-EQ + F-RFC046-STEP-EQ on Phase 3-F-3 baseline.
  **LANDED PARTIAL (commits `bbaa4bbf` through `6fa735c7`)**: 7 outer-product accumulators wire-in completed (dWq/dWk/dWv/dWd/dWo/dWg/dWu) via `_db_grad_accum_farr` helper. Real wall improvement at d=32·3L on M-Mac CPU: **~10-15% (3-run avg)** — `~17-18s → ~16s` on the 80-step d_corpus_fire benchmark. NOT the 33% headline earlier reported in commit `e8c78f4e` — that was a measurement outlier (single-shot 12.30s cold-cache or system-load noise; 3-run average is 16.06s ± 0.30s, range 15.70-16.28s). Honest revision adopted; future RFC 046 wall comparisons use 3-run averaging.
