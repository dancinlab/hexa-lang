...
> header comment block. The `*_bf16_gpu` kernel BODIES are now
> **production-wired (2026-05-19)** — each drives the proven cuBLAS
> GemmEx call shape from the MEASURED Stage 1 kernels
> (`r049_bf16_fused_ffn.cu` `gemm_ex_bf16` / `cublas_ffn_chain_bf16`,
> `r049_layercast_linear.cu`), reusing the shared `g_cublas` handle.
> The bodies compile `cc -fsyntax-only` clean as plain C; the actual
> GPU fire-validation is a separate cost-bearing step. Components 3
> (LayerCast JIT cast policy) and 5 (AdamW master-weight integration)
> remain follow-up RFC work.
