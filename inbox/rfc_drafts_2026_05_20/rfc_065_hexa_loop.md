...
- authority:
  ...
  - `@D g7 inbox-patches-pipeline` — loop output is filed to
    `inbox/{atlas,lens}_candidates/` (sibling stream to
    `inbox/patches/`), never merged inline.
...
`inbox/patches/<slug>.md`, opens a PR, regenerates
...
- **G-L0** — `hexa loop` verb registered in `self/main.hexa`; `--help`
  ...
  - Exit fixture: `hexa loop --once --no-fire --dry-run` runs the full
    ...
    a tmpdir (not `inbox/`), and returns rc=0.
...
- **G-L4** — fire gate is opt-in: `--no-fire` (default) emits only
  ...
  - Exit fixture: `--no-fire` cycle with 1 fire-required candidate
    emits it as `STATUS: deferred-fire` in `inbox/atlas_candidates/`;
...
`inbox/rfc_drafts_2026_05_20/design.md` (sibling file) per
...
| 4 | Candidate output trays | `inbox/{atlas,lens}_candidates/` sibling streams |
...
- `--dry-run` — emit candidates to tmpdir not `inbox/`, no
...
1. SCAN   — read compiler/atlas/embedded.gen.hexa + recent N commits +
            inbox/patches/ + compiler/PLAN.md unresolved TODO → corpus
...
3. DEDUP  — drop candidates whose slug is in gap_cooldown (unexpired)
            ...
            existing inbox/{atlas,lens}_candidates/<slug>.md.
...
6. DRAFT  — write inbox/{atlas,lens}_candidates/<slug>.md with full
...
| `@D g7 inbox-patches-pipeline` | output trays are sibling streams (`inbox/{atlas,lens}_candidates/`) |
...
- **Not a replacement for `hexa kick` (RFC-pending in `inbox/notes/`).**
...
| **C** — measurement | seed lens bodies populated, `--no-fire` produces first real candidates, `inbox/atlas_candidates/` first PR | G-L2..G-L5 exit fixtures |
...
- [ ] approved → file moves to `inbox/rfc_landed/` and Phase B starts
