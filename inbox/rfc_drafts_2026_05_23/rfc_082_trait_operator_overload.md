...
**🟢 Decided 2026-05-23: **A**. atlas-bound · strict-lint 8 stage 철학과 일관. self-host corpus는 단일 module group 이라 orphan rule 영향 미미.
...
| indexing | `a[i]` | `Index` / `IndexMut` | NO v1 (D3a decided 2026-05-23) |
...
**🟢 Decided 2026-05-23: D3a = NO v1 (`Index`/`IndexMut` 미포함, custom array/map type 등장 시 add) · D3b = YES (`*Assign` variant 포함, binary op 와 동시 도입).**
...
**🟢 Decided 2026-05-23: **A**. self-host corpus의 enum/struct 다수 `Eq`/`Hash` 필요 — manual impl ceremony 비현실적.
...
**🟢 Decided 2026-05-23: **C** (둘 다) — Rust 사용자 친화. parser 비용은 적음.
...
**🟢 Decided 2026-05-23: **A** — `Eq: PartialEq` · `Ord: PartialOrd + Eq` 표준 형태 따라감.
...
**🟢 Decided 2026-05-23: D7 deferred to RFC 082-follow.** D1=A (static only) 확정으로 vtable 자체가 v1에 없음 — vtable layout · symbol naming 결정은 dyn Trait 도입 시점 RFC 082-follow에서.
