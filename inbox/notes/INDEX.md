...
> Triage pass 2026-05-19 (rfc043-hexa-torch). `inbox/notes/` holds
> escalations / observations / session-logs (distinct from
> `inbox/patches/`, which are actionable downstream bug/feature
> requests). After this pass the directory is split into:
>
> - **2 still-actionable notes** — left in place at `inbox/notes/`,
>   listed below. Each describes an open issue/gap that still needs work.
> - **47 stale-historical notes** — moved to `inbox/notes/archive/`
...
Moved to `inbox/notes/archive/` — see that directory. Breakdown:
...
fixpoint), `inbox/notes/archive/` can be moved wholesale to
`docs/sessions/` or a similar archival location. The 2 actionable
notes graduate to closure (or to `inbox/patches/`) before then.
