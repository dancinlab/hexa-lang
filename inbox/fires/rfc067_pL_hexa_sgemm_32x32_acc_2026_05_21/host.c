/* RFC 067 PL -- hexa-emit SGEMM + 32x32 per-warp accumulator (2026-05-21)
 *
 * Direct comparator to:
 *   - PK (rfc067_pK_hexa_sgemm_multistage_2026_05_21 / N66) -- 2-stage 16x16 per-warp.
 *     Peak 13.35 TFLOPS @ M=1536 ratio 0.406 vs cuBLAS on ubu-2 RTX 5070 sm_120.
 *
 * This fire bumps the per-warp accumulator from 16x16 to 32x32 (4 m16n16
 * fragments arranged 2x2) -- amortising load/setup over 4x more output per
 * warp. CTA tile is now 128x128 (was 64x64), still 16 warps in 4x4 grid.
 * Same 2-stage cp.async.ca SW pipeline as PK.
 *
 ...
 * Run:
 *   ./host sgemm_32x32_256x256_grid.ptx ... sgemm_32x32_1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_32x32_256x256_grid.ptx ... sgemm_32x32_1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-32x32 run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-32x32 SGEMM        median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.6f\n",
            ...
            printf("[M=%4d] hexa-32x32 SGEMM        SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PL-hexa-sgemm-32x32-acc\",\n");
    fprintf(rj, "  \"date_utc\": \"2026-05-21\",\n");
    fprintf(rj, "  \"host\": \"%s\",\n", getenv("PL_HOST") ? getenv("PL_HOST") : "ubu-2");
    fprintf(rj, "  \"device\": \"%s\",\n", dev_name);
    ...
    fprintf(rj, "  \"variant\": \"PL (TF32 wmma m16n16k8 + 32x32 per-warp accumulator (4 fragments) + cp.async.ca 2-stage pipeline)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_32x32_SxS_grid -- 16-warp 4x4 grid 128x128 output tile, TF32 wmma K=8 inner loop, 4 m16n16 fragments per warp (32x32 per-warp), 2-slab _tg_a/_tg_b shared staging (8192 B each), cp.async.ca producer (4 size=4 per thread per K-step) + wmma.load consumer (2 A + 2 B + 4 mma per K-step)\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (TF32 dispatch on sm_80+)\",\n");
    fprintf(rj, "  \"baseline_pK_n66\": \"rfc067_pK_hexa_sgemm_multistage_2026_05_21 (2-stage 16x16 per-warp, peak 13.35 TFLOPS @ M=1536 ratio 0.406 on RTX 5070)\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        fprintf(rj, "    {\n");
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_32x32_tflops\":      %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_32x32_median_ms\":   %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_32x32_mean_ms\":     %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_32x32_std_ms\":      %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_32x32_min_ms\":      %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_32x32_max_ms\":      %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"ratio\":                   %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_32x32_tflops\":      null,\n");
            fprintf(rj, "      \"hexa_32x32_median_ms\":   null,\n");
            fprintf(rj, "      \"hexa_32x32_mean_ms\":     null,\n");
            fprintf(rj, "      \"hexa_32x32_std_ms\":      null,\n");
            fprintf(rj, "      \"hexa_32x32_min_ms\":      null,\n");
            fprintf(rj, "      \"hexa_32x32_max_ms\":      null,\n");
