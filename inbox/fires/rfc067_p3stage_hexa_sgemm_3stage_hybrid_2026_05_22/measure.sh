#!/usr/bin/env bash
# RFC 067 P3STAGE -- HYBRID: N107 (4-warp 64x64 tile) + 3-stage cp.async pipeline.
#
...
ART_DIR_BASENAME="rfc067_p3stage_hexa_sgemm_3stage_hybrid_2026_05_22"

echo "== STEP 0: regen PTX (idempotent) ==" >&2
python3 gen_sgemm_4warp_3stage_ptx.py

echo "== STEP 1: rsync to ubu-2:/tmp/${ART_DIR_BASENAME}/ ==" >&2
ssh ubu-2 "mkdir -p /tmp/${ART_DIR_BASENAME}"
scp -q host.c gen_sgemm_4warp_3stage_ptx.py *.ptx "ubu-2:/tmp/${ART_DIR_BASENAME}/"
...
ssh ubu-2 "cd /tmp/${ART_DIR_BASENAME} && ./host \
    sgemm_4warp_3stage_256x256_grid.ptx \
    sgemm_4warp_3stage_384x384_grid.ptx \
    sgemm_4warp_3stage_512x512_grid.ptx \
    sgemm_4warp_3stage_768x768_grid.ptx \
    sgemm_4warp_3stage_1024x1024_grid.ptx \
    sgemm_4warp_3stage_1536x1536_grid.ptx 2>&1" | tee fire.log
