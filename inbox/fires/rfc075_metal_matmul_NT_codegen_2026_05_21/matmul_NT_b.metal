    ...
    uint slid [[thread_index_in_simdgroup]])
{
    ...
    for (uint kk = 0u; kk < K8; ++kk) {
        if (tid < 256u) {
            ...
            As[r * TG_K + cc] = (a_row < M && a_col < K) ? a[a_row * K + a_col] : 0.0f;
        }
        if (tid < 256u) {
            const uint r = tid / TG_K;
            const uint cc = tid % TG_K;
            const uint b_row = block_col_base + r;
            const uint b_col = kk * 8u + cc;
            Bs[r * TG_K + cc] = (b_row < N && b_col < K) ? b[b_row * K + b_col] : 0.0f;
        }
        ...
        simdgroup_load(Bmat, Bs, ulong(TG_K), ulong2(0ul, ulong(sg_x * 8u)), true);
