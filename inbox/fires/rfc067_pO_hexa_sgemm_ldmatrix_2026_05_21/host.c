/* RFC 067 PO -- hexa-emit HGEMM + 2-stage cp.async pipeline + ldmatrix.x4 (2026-05-21)
 *
 * Comparator chain:
 *   - PK (N66, TF32 + cp.async multistage) -- peak 13.35 TFLOPS @ M=1536, ratio 0.406 vs cuBLAS SGEMM 32.83
 *   - This (PO) PATH-B: switch to FP16 HGEMM so we can use ldmatrix.x4 natively.
 *     Comparator becomes cuBLAS HGEMM, not SGEMM. N38 prior: peak ~67.58 TFLOPS HGEMM at M=1920.
 *
 * Falsifier F-RFC067-HEXA-SGEMM-LDMATRIX:
 *   - Bit-exact-ish: per-element maxabs vs cuBLAS HGEMM within tolerance
 *     scale*K*1e-2 (f16-mul-f32-acc chains diverge from cuBLAS due to tiling)
 *   - Per-shape median TFLOPS recorded over 200 reps (20 warmup)
 *   - Compare to N66 PK baseline AND to cuBLAS HGEMM on same device
 *
 * Build:
 *   nvcc -O2 -arch=sm_90 -o host host.c -lcuda -lcublas -lm
 * Run:
 *   ./host sgemm_ldmatrix_256x256_grid.ptx ... sgemm_ldmatrix_1536x1536_grid.ptx
 */
...
static int run_cublas_hgemm(cublasHandle_t handle,
                            ...
                            double *out_times_ms)
{
    /* cuBLAS HGEMM via cublasGemmEx, half inputs + float compute */
...
static int run_hexa(CUfunction f_hexa,
                    ...
                    double *out_times_ms)
{
    const int K_PER_TILE = 16;  /* FP16 wmma K=16 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_ldmatrix_256x256_grid.ptx ... sgemm_ldmatrix_1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_ldmatrix_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si]) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-ldmatrix PTX shape %d (%s)\n", shapes[si], entry);
        } else {
            load_ok[si] = 0;
            snprintf(load_err[si], sizeof(load_err[si]),
                "PTX load/lookup failed for %s", argv[1 + si]);
            printf("FAILED hexa-ldmatrix PTX shape %d -- skipping\n", shapes[si]);
        }
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        /* Host input: small finite f16-representable values. */
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-ldmatrix run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-ldmatrix HGEMM     median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f\n",
            ...
            printf("[M=%4d] hexa-ldmatrix HGEMM     SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PO-hexa-hgemm-ldmatrix\",\n");
    fprintf(rj, "  \"date_utc\": \"2026-05-21\",\n");
    fprintf(rj, "  \"host\": \"%s\",\n", getenv("PO_HOST") ? getenv("PO_HOST") : "ubu-2");
    fprintf(rj, "  \"device\": \"%s\",\n", dev_name);
    ...
    fprintf(rj, "  \"variant\": \"PO (FP16 wmma m16n16k16 + cp.async.ca.shared 2-stage + ldmatrix.x4 / ldmatrix.x4.trans for shared->frag)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_ldmatrix_SxS_grid -- 16-warp 4x4 grid 64x64 output tile, FP16 wmma K=16 inner loop, 2-slab _tg_a/_tg_b shared staging (4096 B each), cp.async.ca producer + ldmatrix.x4 consumer (a row + b col-trans)\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"path_chosen\": \"B (FP16 HGEMM) -- ldmatrix native b16, TF32 reinterpret skipped (no public PTX precedent)\",\n");
    fprintf(rj, "  \"baseline_pK_n66\": \"rfc067_pK_hexa_sgemm_multistage_2026_05_21 (TF32 cp.async multistage, peak 13.35 TFLOPS @ M=1536 ratio 0.406 on RTX 5070)\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        fprintf(rj, "    {\n");
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_ldmatrix_tflops\":      %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_ldmatrix_median_ms\":   %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_ldmatrix_mean_ms\":     %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_ldmatrix_std_ms\":      %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_ldmatrix_min_ms\":      %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_ldmatrix_max_ms\":      %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"hexa_vs_cublas_maxrel\":   %.6f,\n", res[si].hexa_maxrel);
            fprintf(rj, "      \"ratio\":                   %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_ldmatrix_tflops\":      null,\n");
            fprintf(rj, "      \"hexa_ldmatrix_median_ms\":   null,\n");
            fprintf(rj, "      \"hexa_ldmatrix_mean_ms\":     null,\n");
            fprintf(rj, "      \"hexa_ldmatrix_std_ms\":      null,\n");
            fprintf(rj, "      \"hexa_ldmatrix_min_ms\":      null,\n");
            fprintf(rj, "      \"hexa_ldmatrix_max_ms\":      null,\n");
