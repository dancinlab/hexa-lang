// RFC 071 P4 N50 — F-RFC071-E2E-VEC-MUL-NUMERIC-EQ host launcher
//
// Generalisation cycle for N34 (c29359ab) + N45 (830e44c0): same shape
// as N34's `my_test_kernel` (vec-add) but kernel name = `vec_mul` and
// payload op = `*` instead of `+`. PASS = N35 fix is general across
// kernel shapes, not kernel-specific.
//
// Loads /tmp/rfc071_n50_vec_mul.ptx via cuModuleLoadDataEx (driver-JIT,
// sm_120 forward-compat from sm_80 PTX per `reference_gpu_fire_infra`),
// launches `vec_mul` with 1 block x 1024 threads, FP64 a * b -> c,
// compares to CPU ref bit-exact.
//
// Build: gcc host_vec_mul.c -o /tmp/host_vec_mul -lcuda
// Run:   /tmp/host_vec_mul
// Output: JSON single line with PASS/FAIL + max_abs_diff +
...
#define N 1024
#define PTX_PATH "/tmp/rfc071_n50_vec_mul.ptx"
...
int main(void) {
    ...
    CUfunction fn;
    if (check(cuModuleGetFunction(&fn, mod, "vec_mul"), "cuModuleGetFunction")) {
    ...
    // Deterministic LCG fill — same seed as N34 so we can cross-check
    // the FIRST element if needed (but href payload changes: a*b not a+b).
    ...
    int pass = (bm == 0);
    printf("{\"status\":\"%s\",\"max_abs_diff\":%.17g,\"byte_mismatch\":%d,\"N\":%d,\"first_mis\":%d,\"c[0]\":%.17g,\"ref[0]\":%.17g,\"a[0]\":%.17g,\"b[0]\":%.17g}\n",
           pass ? "PASS" : "FAIL", max_abs, bm, N, firstmis, hc[0], href[0], ha[0], hb[0]);
    return pass ? 0 : 1;
}
