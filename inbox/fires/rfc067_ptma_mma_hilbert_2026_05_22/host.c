...
int main(int argc, char **argv) {
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_tma_mma_hilbert_%dx%d_grid", shapes[si], shapes[si]);
        load_err[si][0] = 0;
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
                            load_info[si], sizeof(load_info[si]),
                            load_err[si], sizeof(load_err[si])) == 0) {
            load_ok[si] = 1;
            printf("loaded N200-full PTX shape %d (%s)\n", shapes[si], entry);
            if (fp_info) {
            ...
            load_ok[si] = 0;
            printf("FAILED N200-full PTX shape %d -- %s\n", shapes[si], load_err[si]);
        }
        if (load_ok[si]) {
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            res[si].hexa_valid = 1;
            printf("[M=%4d] N200-full TMA+mma+Hilbert median=%.6f ms  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  CTAs=%u  regs=%d  shmem=%d\n",
            ...
            printf("[M=%4d] N200-full SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-N200-FULL-tma-mma-hilbert\",\n");
    ...
    fprintf(rj, "  \"variant\": \"N200-full: TMA cp.async.bulk.tensor.2d (2 descriptors, expect_tx 4096/K-iter) + named mbarrier (parity-tracked) + 8 mma.sync.m16n8k16 per warp (4-warp 64x64 tile) + Hilbert d2xy CTA-swizzle (N149 PHILB pattern, byte-identical mma chain). sm_120a + .version 8.7 + ASCII-only PTX.\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_tma_mma_hilbert_SxS_grid\",\n");
    ...
    fprintf(rj, "    \"N200_SMOKE\": \"TMA+mbarrier integration smoke (M=64, write-A-back-to-C) PASS mismatch=0/256\",\n");
    ...
    fprintf(rj, "    \"smem_a_bytes\": 2048,\n");
    fprintf(rj, "    \"smem_b_bytes\": 2048,\n");
    fprintf(rj, "    \"smem_mbar_bytes\": 16,\n");
    fprintf(rj, "    \"smem_total_per_cta\": 4112\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_n200_tflops\":         %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_n200_median_ms\":      %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_n200_std_ms\":         %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_n200_min_ms\":         %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_n200_max_ms\":         %.6f,\n", res[si].hexa.max);
            ...
            fprintf(rj, "      \"hexa_n200_tflops\":         null,\n");
            fprintf(rj, "      \"hexa_n200_median_ms\":      null,\n");
            fprintf(rj, "      \"hexa_n200_std_ms\":         null,\n");
            fprintf(rj, "      \"hexa_n200_min_ms\":         null,\n");
            fprintf(rj, "      \"hexa_n200_max_ms\":         null,\n");
