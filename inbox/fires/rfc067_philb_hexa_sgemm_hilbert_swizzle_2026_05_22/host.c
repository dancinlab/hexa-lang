/* RFC 067 PHILB -- 4-WARP 64x64 + Hilbert-curve CTA-swizzle (Pattern B) cliff sweep (2026-05-22)
 *
 * Goal: follow-on to N134 (Pattern A 2D super-block 4x4). N134 recovered the M>=6144
 *       L2-thrash cliff (M=6144 ratio 0.655, M=8192 0.624). This fire replaces the
 *       super-block swizzle with a Hilbert space-filling curve d2xy mapping: adjacent
 *       linear CTA IDs map to Manhattan-adjacent output tiles, giving a tighter 2D L2
 *       working set than a row-major super-block. Does Hilbert beat super-block at the
 *       cliff?
 *
 * Launch grid: p x p where p = next_pow2(N/64). Kernel computes (sw_x,sw_y)=d2xy(p,d)
 ...
 *         5120 -> grid 80x80  -> p=128 (16384 launched, 6400 real, 9984 padding-return)
 *         6144 -> grid 96x96  -> p=128 (16384 launched, 9216 real, 7168 padding-return)
 *         8192 -> grid 128x128 -> p=128 (no padding)
 *
 * Sweep (same shapes as N134 for direct comparison):
 *   - M = 4096  (N134 super-block 58.03 / ratio 0.828)
 *   - M = 5120  (N134 super-block 50.45 / ratio 0.717)
 *   - M = 6144  (N134 super-block 46.37 / ratio 0.655 -- does Hilbert beat it?)
 *   - M = 8192  (N134 super-block 44.17 / ratio 0.624)
 *
 * Falsifier F-RFC067-HEXA-SGEMM-HILBERT-SWIZZLE:
 ...
 * Run:
 *   ./host sgemm_4warp_hilbert_4096x4096_grid.ptx \
 *          sgemm_4warp_hilbert_5120x5120_grid.ptx \
 *          sgemm_4warp_hilbert_6144x6144_grid.ptx \
 *          sgemm_4warp_hilbert_8192x8192_grid.ptx
 */
...
/* Hilbert launch grid is p x p where p = next_pow2(side). The kernel computes
 * (sw_x,sw_y)=d2xy(p,d) over the p x p square and early-returns padding CTAs. */
                        ...
                        float *out_maxabs, float *out_maxrel)
{
    ...
    unsigned long long k_arg = (unsigned long long)K_TILES_TOTAL;
    void *kargs[4] = { &da, &db, &dc, &k_arg };
    /* Hilbert: launch p x p (next_pow2 of side); kernel d2xy + padding return. */
    unsigned int side = (unsigned int)(N / 64);
    unsigned int p = next_pow2_u(side);
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr,
            "usage: %s sgemm_4warp_swizzle_4096x4096_grid.ptx sgemm_4warp_swizzle_5120x5120_grid.ptx sgemm_4warp_swizzle_6144x6144_grid.ptx sgemm_4warp_swizzle_8192x8192_grid.ptx\n",
            argv[0]);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_4warp_hilbert_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
                            load_info[si], sizeof(load_info[si])) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-PHILB PTX shape %d (%s)\n", shapes[si], entry);
            if (fp_info) {
            ...
            printf("FAILED hexa-PSWZ PTX shape %d -- skipping\n", shapes[si]);
        }
    ...
    /* Free VRAM probe before allocating large buffers. */
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-PHILB run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                snprintf(res[si].note, sizeof(res[si].note),
                ...
                printf("[M=%4d] hexa-PHILB 4warp+Hilbert median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f  CTAs=%d  regs=%d\n",
            ...
            printf("[M=%4d] hexa-PHILB 4warp+Hilbert SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    /* N134 Pattern A super-block (4-warp 64x64 + 4x4 super-block CTA-swizzle) baselines.
       Comparison reference: does Hilbert (Pattern B) beat the super-block?
       Per-shape order matches shapes[] = {4096, 5120, 6144, 8192} -- all measured in N134. */
    double n134_hexa_tflops[N_SHAPES]   = { 58.027160, 50.449294, 46.374741, 44.172591 };
    double n134_cublas_tflops[N_SHAPES] = { 70.047008, 70.389579, 70.833182, 70.817809 };
    double n134_ratio[N_SHAPES]         = {  0.828403,  0.716715,  0.654704,  0.623750 };
    /* N130 pre-swizzle cliff (4-warp 64x64, NO swizzle) for context only. */
    double n130_hexa_tflops[N_SHAPES]   = { 57.330442, 0.0, 16.552417, 13.908097 };
    double n130_ratio[N_SHAPES]         = { 0.818463,  0.0,  0.233724,  0.304167 };

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PHILB-hexa-hgemm-hilbert-swizzle\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PHILB -- N107 PY 4-warp 64x64 + Pattern B Hilbert-curve CTA-swizzle for L2 reuse. Launch grid p x p, p=next_pow2(side); d=ctaid.y*p+ctaid.x; (sw_x,sw_y)=hilbert_d2xy(p,d) unrolled log2(p) rounds (no runtime loop); early-return padding CTAs (sw_x>=gx||sw_y>=gy). Adjacent CTA IDs map to Manhattan-adjacent output tiles. Rest of kernel byte-identical to N107 PY / N134 super-block.\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_4warp_hilbert_SxS_grid -- N107 PY + Hilbert d2xy CTA swizzle\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"swizzle_pattern\": \"Hilbert space-filling curve d2xy (Pattern B). Launch p x p (p=next_pow2(side)), d2xy(p, ctaid.y*p+ctaid.x), drop padding tiles outside gx x gy. Bijective over real grid -> bit-exact. d2xy unrolled at gen time (6 rounds @ p=64, 7 rounds @ p=128).\",\n");
    fprintf(rj, "  \"super_block_size\": null,\n");
    fprintf(rj, "  \"hilbert_p_per_shape\": \"4096->64, 5120->128, 6144->128, 8192->128\",\n");
    fprintf(rj, "  \"l2_size_mb\": 32,\n");
    fprintf(rj, "  \"stack_inputs\": {\n");
    fprintf(rj, "    \"N107_PY\":  \"rfc067_pY_hexa_sgemm_4warp_swizzle_2026_05_21 peak 51.65 TFLOPS @ M=1536 ratio 0.777\",\n");
    fprintf(rj, "    \"N130_Pmax\": \"rfc067_pmax_hexa_sgemm_n107_maxM_2026_05_22 cliff @ M=6144 (16.55 / 0.234), M=8192 (13.91 / 0.304)\",\n");
    fprintf(rj, "    \"N134_PSWZ\": \"rfc067_pswz_hexa_sgemm_cta_swizzle_2026_05_22 Pattern A 4x4 super-block: M=4096 0.828, M=5120 0.717, M=6144 0.655, M=8192 0.624\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"N134 super-block (Pattern A) recovered the L2-thrash cliff by keeping a ~6 MB row-major super-block working set. Pattern B (Hilbert curve) maps adjacent CTA IDs to Manhattan-adjacent output tiles, so the working set of recently-scheduled CTAs is a tight 2D blob rather than a row-strip -- potentially tighter L2 reuse at M>=6144. Headline: does Hilbert beat super-block at the cliff? If equal, super-block was already sufficient for 32 MB L2 (useful negative).\",\n");
    ...
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        /* Headline comparison is vs N134 Pattern A super-block (measured all 4 shapes). */
        double pct_over_n134 = (res[si].hexa_valid)
            ? 100.0 * (res[si].hexa_tflops - n134_hexa_tflops[si]) / n134_hexa_tflops[si]
            : 0.0;
        double ratio_delta_n134 = (res[si].hexa_valid)
            ? (res[si].ratio - n134_ratio[si])
            : 0.0;
        int beats_superblock = (res[si].hexa_valid && res[si].hexa_tflops > n134_hexa_tflops[si]);
        int has_n130 = (n130_hexa_tflops[si] > 0.0);
        ...
        fprintf(rj, "      \"n134_superblock_cublas_tflops\": %.6f,\n", n134_cublas_tflops[si]);
        fprintf(rj, "      \"n134_superblock_hexa_tflops\":   %.6f,\n", n134_hexa_tflops[si]);
        fprintf(rj, "      \"n134_superblock_ratio\":         %.6f,\n", n134_ratio[si]);
        if (has_n130) {
            fprintf(rj, "      \"n130_preswizzle_hexa_tflops\":   %.6f,\n", n130_hexa_tflops[si]);
            fprintf(rj, "      \"n130_preswizzle_ratio\":         %.6f,\n", n130_ratio[si]);
        } else {
            fprintf(rj, "      \"n130_preswizzle_hexa_tflops\":   null,\n");
            fprintf(rj, "      \"n130_preswizzle_ratio\":         null,\n");
        }
        fprintf(rj, "      \"philb_regs_per_thd\":      %d,\n", res[si].regs_per_thd);
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_philb_tflops\":       %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_philb_median_ms\":    %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_philb_mean_ms\":      %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_philb_std_ms\":       %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_philb_min_ms\":       %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_philb_max_ms\":       %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"hexa_vs_cublas_maxrel\":   %.6f,\n", res[si].hexa_maxrel);
            fprintf(rj, "      \"ratio_vs_cublas\":         %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"pct_over_n134_superblock\":   %.4f,\n", pct_over_n134);
            fprintf(rj, "      \"ratio_delta_vs_n134\":        %.6f,\n", ratio_delta_n134);
            fprintf(rj, "      \"hilbert_beats_superblock\":   %s,\n", beats_superblock ? "true" : "false");
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_philb_tflops\":       null,\n");
            fprintf(rj, "      \"hexa_philb_median_ms\":    null,\n");
            fprintf(rj, "      \"hexa_philb_mean_ms\":      null,\n");
            fprintf(rj, "      \"hexa_philb_std_ms\":       null,\n");
            fprintf(rj, "      \"hexa_philb_min_ms\":       null,\n");
            fprintf(rj, "      \"hexa_philb_max_ms\":       null,\n");
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   null,\n");
            fprintf(rj, "      \"hexa_vs_cublas_maxrel\":   null,\n");
            fprintf(rj, "      \"ratio_vs_cublas\":         null,\n");
            fprintf(rj, "      \"pct_over_n134_superblock\":   null,\n");
            fprintf(rj, "      \"ratio_delta_vs_n134\":        null,\n");
            fprintf(rj, "      \"hilbert_beats_superblock\":   null,\n");
            fprintf(rj, "      \"note\":                    \"%s\"\n", res[si].note);
        }
        fprintf(rj, "    }%s\n", (si == N_SHAPES - 1) ? "" : ",");
    }
