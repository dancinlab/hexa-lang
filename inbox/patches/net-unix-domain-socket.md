...
> **VERIFIED-CLOSED 2026-05-19**: SSOT grep cross-verified — `self/native/net.c` carries `AF_UNIX` / `_hexa_net_parse_any` (grep ×12). Already `applied` 2026-05-14; this is a close-only marker (no source change, no fix re-run — fix already live in SSOT, dup-race precheck per @D g_inbox_processing_loop).
