...
- **`self/stdlib/proc.hexa`** landed: `popen_lines(cmd) -> [string]` and
  ...
  (`hexa run self/stdlib/proc.hexa --selftest`). Zero impact on self-host fixpoint
  self-host fixpoint (new file, not referenced from `self/main.hexa`
  bootstrap chain).
- **NOT landed (honest-caveat honest C3)**: the streaming iterator form
...
- Embed their entire render loop inside a helper Python (hexa wrapper becomes a thin invoker — defeats hexa-only-strict spirit)
