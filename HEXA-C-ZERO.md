...
- [ ] C4 — **keystone**: native backend(ir_to_arm64/x86+elf) HEXA_BACKEND default flip → generated C 52개 미생성 · ⏩ **2026-06-10 DEFAULT FLIP REALIZED (shipped-level)**: `tool/promote_selfhost.sh install --default` → gen3(네이티브 기계어 직접 emit) = default `hexa` on ghost · parity gate 5/5 PASS · 가역(`--revert`, backup `hexa.real.pre-selfhost.*`). 검증: 기본 `hexa build verify.hexa` → built → run "selfhost-default-OK" = **기본 컴파일 경로가 C 0개 생성**. fixpoint cc-gen3==cc-gen4 (sha d0792379, 3461680B) + main 랜딩 #3024/#3025/#3020. 잔여(full [x]): in-source `HEXA_BACKEND` default 모든 호스트(현재 promote-shim·ghost) + aprime 부트스트랩-C(warm-seed = C1 축). verdict `.verdicts/hexa-cc-native/F-HEXA-CC-NATIVE-N5-FIXPOINT-ACHIEVED.txt`
