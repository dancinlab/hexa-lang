...
**갭**: `/system` (sidecar) 의 manifest SSOT 계약(SKILL 0.9.0 `commands/system.md` L23·L30-31)은 **cwd `./pods.json` (extended)** — `jobs.<id>` = `{pod · workdir · kind · terminal_marker · stage · pid · watcher · metric_parser · verdict}` + `drive` durable-state 마커 + ledger 동기화까지 전부 이 campaign-local 파일에 산다. 반면 hexa-lang CLOUD 가 소유한 **글로벌 `~/.hx/cloud/active-pods.json` (M5/M8 SSOT)** 의 스키마는 **POD-RENTAL ONLY**: `{id · provider · created_at · rented_by · ssh_host · ssh_port · status · ttl_sec · max_cost_usd · cumulative_cost_usd}`. **`jobs` 섹션이 없고 · verdict/stage/hypothesis/pid/watcher 를 담을 자리가 없다.**

**왜 막히나 (닫힌 인과)**: demiurge RTSC 가 `/system` 으로 35 job (32 + 3 pilot · verdict/stage/hypothesis/pid/watcher/recovery 보유) 를 글로벌 SSOT 한 곳에서 추적하려면, 글로벌 active-pods.json 이 job-level 데이터를 담아야 한다. 현재는 못 담으므로 per-project `pods.json` 을 폐기할 수 없다 (폐기 시 LIVE watcher heartbeat 가 읽을 표면 상실). 즉 "/system → 글로벌 SSOT" 전환은 **이 스키마 확장이 upstream 에 착지해야 do-able**.

**fix 방향 (제안)**: ① 글로벌 active-pods.json 에 optional `jobs` 섹션 추가 — `/system` manifest 의 job 스키마(`pod · stage · verdict · pid · watcher · kind · terminal_marker · metric_parser` + campaign-자유 필드 hypothesis/anchor 등)를 superset 으로 수용. pod-rental 필드와 직교하므로 기존 `/cloud` pods 동작 불변(additive). ② campaign-scoping — 한 글로벌 파일에 다수 캠페인이 공존하므로 `campaigns.<name>.jobs` 네임스페이스 또는 job 에 `campaign` 필드. ③ `/cloud` CLI 가 job 섹션을 보존(read-modify-write 시 drop 금지)하도록 atomic write 계약 확장.

**범위 주의**: 이건 **enhancement 이지 BLOCKER 아님** — demiurge 측은 per-project `pods.json` 을 KEPT-LIVE 로 두고 계속 운영 가능 (현 동작 정상). 이 확장이 착지하면 그때 글로벌 전환을 마저 한다. 또한 `/system` 은 sidecar 소유 command 이므로 — active-pods.json 이 job 섹션을 갖춘 뒤 sidecar `/system` 의 manifest 탐색 경로를 cwd `./pods.json` → 글로벌 fallback 으로 확장하는 별도 sidecar 변경도 동반 필요(이건 hexa-lang 범위 밖, sidecar 측 후속).

**severity**: low-medium (enhancement; 현 campaign 추적은 per-project pods.json 으로 정상 동작 · 글로벌 단일-SSOT 편의만 차단). **ref**: demiurge `domains/rtsc.log.md` 2026-05-29 "pods.json → 글로벌 전환 (M11-b)" entry · 글로벌 파일 `~/.hx/cloud/active-pods.json` (227 B, pod-only 확인) · sidecar `/system` SKILL 0.9.0 manifest 계약.

## 2026-05-29 — ✅ RESOLVED (PR #1992) · 🔴 BLOCKER · `release.yml` Stage-0 `hexa_v2` self-host transpiler 컴파일 3타겟 全 FAILED → `edge` prerelease 2026-05-25 에 stuck → 모든 신규 wired fn `hexa verify --expr` 永 🟠

> **✅ RESOLVED — PR #1992 (`4456294eb`, merged origin/main 2026-05-29)**. **근본 원인 확정**: Stage-0 는 fresh CI 체크아웃에서 `clang -I self self/native/hexa_cc.c self/runtime.c -o build/hexa_v2` 로 **첫 hexa 를 C 로 cold-bootstrap** 함 (아직 hexa 없음). 그런데 `self/runtime.c` 가 `#include "runtime_core.c"` + 16 `native/*.c` + `forge/forge_tier_v1.c` 를, `runtime_core.c` 가 다시 `#include "runtime_hi_gen.c"` 를 텍스트 amalgamate 하는데 — 이 **19 개 `.c` 가 `.c-text` 캠페인(#1866/#1874/#1879/#1884)으로 git-rm + gitignore** 됨. 빌드시 `*_emit.hexa` SSOT 에서 regen 하도록 의도됐으나 그 regen(`build_hexa_cli _regen_runtime_core`)은 **STRICTLY-OPTIONAL** — PATH 에 외부 hexa-run 이 없으면 SKIP. CI Stage-0 는 hexa 가 없으니 `#include` 가 `fatal error: 'runtime_core.c' file not found` 로 깨짐 → 全 release + edge + 5/25 이후 모든 `verify --expr` fn 차단. **fix**: 19 `.c` 를 TRACKED 상태로 복원(`git add -f` · `.gitignore` 편집 無 → sign-gate 不要). pre-rm parent blob 에서 git-restore, 19/19 모두 `*_emit.hexa` emitter 출력과 **byte-identical** 검증(`hexa-run <base>_emit.hexa <out>` + `cmp`) → 기본 build_hexa_cli regen 경로는 clean no-op, CI cold-bootstrap 은 tracked `.c` 직접 사용(문서화된 STRICTLY-OPTIONAL fallback). **로컬 게이트**: 정확한 release.yml Stage-0 명령(`clang -O2 -std=gnu11 -D_GNU_SOURCE -Wno-trigraphs -I self self/native/hexa_cc.c self/runtime.c -o hexa_v2`) → **Mach-O 64-bit executable arm64** (직전: file-not-found fatal). bootstrap 된 hexa_v2 가 `module_loader.hexa → C` (Stage-1) 도 OK. 복원된 19 파일: `runtime_core.c`(#1874) · `runtime_hi_gen.c`(#1879) · `forge/forge_tier_v1.c`(#1884) · `native/{crypto_openssl,crypto_sodium,exec_argv_sha256,exec_pipe,fp_init,mount,namespace,net,persistent_pipe,proc_fork,pty,signal_flock,tensor_kernels,term_ffi,thread,wait}.c`(#1866). (진단서는 17 로 셌으나 iterative 로컬 컴파일로 `runtime_hi_gen.c`+`forge_tier_v1.c` 2 개 추가 surfacing → 총 19.) 비고: `.c=0 완전한 .hexa only` 목표와 기술적으로 상충하나, cold-bootstrap 은 첫 hexa 생성 전 clang 만 가용 → `runtime.c` amalgamation + `hexa_cc.c` 는 본질적으로 C-seed 여서 `.c=0` 불가; 릴리스 파이프라인 복구가 카운트 축소보다 우선(user: "inbox fix first").
...
## 2026-05-29 — 🟡 부분 완화 (PR #1992) · 🔴 BLOCKER · `build/hexat_linux` 가 대형 emit.hexa transpile 시 SEGFAULT (rc=139) — generated C 산출 불가

> **🟡 부분 완화 — PR #1992 (`4456294eb`)이 본 엔트리의 `fix 방향 ②`("generated C 파일을 git-committed 산물로 전환 … fresh clone 만으로 빌드")를 amalgamation member 3 종에 대해 정확히 구현**. 이제 origin/main 에 `self/runtime_core.c` · `self/runtime_hi_gen.c` · `self/native/tensor_kernels.c` 가 TRACKED → **fresh clone 만으로 이 3 파일 확보** = "전부 git 미추적 → emit 한 길뿐 → segfault" 라는 닫힌 사슬(아래 "왜 치명적")이 이 3 파일에 대해 깨짐. **하지만 미해결 2 축**: (1) 본 엔트리가 함께 나열한 `self/cuda/runtime_cuda.c` · `self/cuda/runtime_bf16.c` 는 runtime.c amalgamation 체인 밖(Stage-0 불필요)이라 #1992 범위에 없음 — **여전히 emit-only · untracked** (`git cat-file -e origin/main:self/cuda/runtime_cuda.c` = does not exist). anima 트레이너가 CUDA 경로를 쓰면 이 2 종 emit 이 필요한데 그 emit 이 곧 이 segfault. (2) **transpiler SEGFAULT 근본 버그(rc=139) 자체는 미수정** — 대형 string-literal emit.hexa 처리 시 여전히 죽음. 즉 #1992 는 **빌드 차단을 우회**(committed `.c` 직접 사용)했을 뿐 hexat 의 입력-크기 의존 크래시를 고치지 않음. **잔여 작업**: ① `runtime_cuda.c`/`runtime_bf16.c` 도 동일 패턴으로 git-committed 전환(`cuda/*_emit.hexa` byte-eq 검증 후 `git add -f`) → CUDA-포함 pod 빌드 완전 unblock, 또는 ② hexat 의 대형 string-literal 크래시 근본 fix(stack-overflow 가설 → `ulimit -s unlimited` 1-pod 테스트 가치). 본 엔트리는 ①(cuda 2종) + ② 가 남았으므로 **OPEN 유지**.
...
>
> **출처**: demiurge RTSC 캠페인 첫 측정-grade verdict (YH₁₀ el-ph). `allen_dynes_tc(λ, ω_log, μ*)` = **3-operand** 닫힌형. `hexa verify --expr allen_dynes_tc 2.8197 1300.43 0.10` → 🟢 SUPPORTED-NUMERICAL (227.501, |Δ|=1.3e-4) 정상. 그러나 그 verified 노드를 atlas 에 fold 하려 `atlas register --from-verify allen_dynes_tc 2.8197 1300.43 0.10 227.501` 호출 시 **3-operand 를 mis-parse → false 🔴**.
...
### gap 2 — `cloud exec` 가 unknown `--flag` 를 positional host 로 silent-오파싱 → 혼란스러운 255 (정정됨 2026-05-28 · ✅ RESOLVED 2026-05-28)
> **✅ RESOLVED** — `_checked_host(av[si+1])` 가드를 9개 host-taking verb(run·exec·nohup·fire·poll·tail·watch·copy-to·copy-from) 전부에 적용. host 슬롯 토큰이 `--`로 시작하면 `eprintln` + `exit(2)` (positional `<host> ... -- <argv>` 안내). recommend (b) 구현 = P1. (recommend (a) "모든 unknown flag enumerate-reject" 는 더 큰 작업으로 격하 — `--` host-guard 가 실측 친 함정의 근본.)
