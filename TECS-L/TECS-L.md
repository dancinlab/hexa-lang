...
- [ ] F-NEW-1 — Γ₀(N) sweep N=31..60: 30 candidates closed-form ψ(N) recompute. MF1 [1,30] 확장 · F10 next-seed 2. 각 N 별 `hexa verify --expr "gamma0_index N ψ(N)"` exact 정수산술 🔵 expected.
- [ ] F-NEW-2 — σ(M_p)=2^p Lucas-Lehmer 인접 atom batch p∈{5,7,13,17,19}: 5 candidates. M_5=31·M_7=127·M_13=8191·M_17=131071·M_19=524287 전부 prime → σ(M_p)=M_p+1=2^p (Euclid-Euler, MR3 family · F10 next-seed 4). 각 verify `hexa verify --expr "sigma M_p 2^p"`.
- [ ] F-NEW-3 — jacobi/kronecker (a/p) atlas pad batch: 16 quadratic-reciprocity candidates a∈{2,3,5,7}·p∈{3,5,7,11} (F10 next-seed 5). atlas L-laws pad 확장.
...
- [x] F18 — **NOVEL F17 successor + atlas fold (2026-05-27)**: 5-task batch (s1+s2+s3+s4+s5) completes F17 deferred seeds. **(s1)** ω=9/10 D-sweep extension — 🔴 4/4 CLOSED-NEGATIVE on primorial-extension witnesses (n=223092870 primorial #9, n=6469693230 primorial #10, + k=2 variants n=446185740, n=669278610); 12 hexa-native σ/φ/τ components 🔵; D(n) ∈ {3.05e16, 2.56e19, 1.42e17, 2.98e17} all non-zero — F14/F16/F17 zero-density extends predictably up to ω=10 primorial scale ~6.5e9. ω=10 σφ overflows int64 → bignum bc, components in int64. **(s2)** σ_6/σ_7/σ_8 Euclid-Euler tower — 🔵 8/9 SUPPORTED-FORMAL via F17 general-k closed-form at k∈{6,7,8} for small p (P_1 sigma_6=47450 sigma_7=282252 sigma_8=1686434; P_2 sigma_6=489541650 sigma_7=13599182072 sigma_8=379283617986; P_3 sigma_6=15126187641744322 sigma_7=7443513564413795552; σ_8(P_3)=3.68e21 bignum-only). Tower k∈{1..8} closed at small-p witness. **(s3)** ω=4 Ore non-Mersenne structural — 🔴 closed-negative + 4/4 🔵: {2970, 18620} non-Mersenne Ore exhibit ZERO per-prime Ore (8/8 per-prime σ(p^a)∤p^a(a+1) FAIL); Ore status = cross-prime cancellation NUMERICAL coincidence, NOT structural identity → rules out uniform closed-form for non-Mersenne Ore residual class (sharpens F15 universal Ore-NEG by IDENTIFYING obstruction mechanism). **(s4)** Weight-4 newform level 6 — 🛸 NOVEL n=6 distinction RECOVERY: dim S_4(Γ_0(6))=1 hexa-verified (Eichler-Selberg cross-check: g=0, ν_∞=4 → dim=(3)(-1)+(4)=1); level 6 = MINIMAL level for weight-4 cusp form on Γ_0 (N=1,2,3: dim=0); LMFDB 6.4.a.a anchor (Hecke + L-values 🟡 citation, T_p builtin INBOX). n=6 distinction RECOVERS at weight 4 after F17 weight-2 closed-neg — multilayer non-lift sharpens to weight-2-only. **(s5)** `dedekind_psi` INBOX entry filed to root `INBOX.md`: F17 ψ-identity 🟡 cite cause + F18 weight-4 ψ(N)-cross-check potential; proposed `verify_cli::_recompute` 1-arg arm + `stdlib/core/math.hexa::dedekind_psi(n: i64) -> i64` (calc-gap family #1230 / σ_3-#1281 lineage). Non-blocking. **Atlas fold SUCCESS via manual splice** (4 atoms, `compiler/atlas/embedded.gen.hexa` 1416→1420 F-formulas): `tecs_l_f18_d_omega_9_10_zero_density` (🔴) · `tecs_l_f18_sigma_k_tower_6_7_8_closed_form` (🔵) · `tecs_l_f18_ore_non_mersenne_cross_prime_coincidence` (🔴) · `tecs_l_f18_weight_4_newform_level_6_recovery` (🛸🔵). 24 hexa-native component verifies + 3 closed-negative + 1 NOVEL n=6-recovery atom + 1 INBOX entry. → `.verdicts/tecs-l-f18-novel-mk10/` (5 verdict files) · TECS-L.log.md F18 entry · `INBOX.md` dedekind_psi entry. **다음 round seeds (F19)**: (a) ω=11,12 D-sweep (primorial #11=200560490130, #12=7420738134810 — bignum for ALL components), (b) σ_9/σ_10 tower (closed-form bignum-only beyond σ_8(P_3)), (c) weight-4 newform 6.4.a.a Hecke eigenvalue a_p hexa-native (T_p builtin landing — unblocks ψ + a_p witnesses), (d) weight-6 newform level 6 dim S_6(Γ_0(6))=3 structure exploration (3-dim space), (e) Ore non-Mersenne further entries {2730=2·3·5·7·13, 6048=2⁵·3³·7} structural variants.
- [x] F17 — **NOVEL F16 successor + atlas fold (2026-05-27)**: 5-task batch (s1+s2+s3+s4+s5) completes F16 deferred seeds. **(s1)** ω=6/7/8 D-sweep extension — 🔴 6/6 CLOSED-NEGATIVE on primorial-extension witnesses (n=30030, 39270, 510510, 690690, 9699690, 19399380); 18 hexa-native σ/φ/τ components 🔵; D(n) ∈ {5.5e8, 9.5e8, 1.6e11, 2.9e11, 5.8e13, 2.7e14} all far from 0 — F14/F16 zero-density extends predictably up to ω=8 primorial scale. **(s2)** σ_k Euclid-Euler general-k closed-form — 🔵 7/7 SUPPORTED-FORMAL: `σ_k(2^(p-1)·M_p) = [(2^(kp)−1)/(2^k−1)] · [1+(2^p−1)^k]` ∀ k≥1 verified at k=4 (P_1..P_4: 1394, 655746, 64558805410, 4655469689405474) and k=5 (P_1..P_3: 8052, 17766056, 30988222753952); F16 (k=3) generalizes to all k. σ_5(8128) overflows int64 — closed-form bignum-only. **(s3)** A001599 ω=4 Ore subfamily structure — 🔵 4/4 σ/τ + 🔴 partial-cover closed-negative: F16 Mersenne-extended subfamily covers {1638=2·3²·7·13, 18600=2³·3·5²·31} (M_3 and M_5 fits) but NOT {2970=2·3³·5·11, 18620=2²·5·7²·19} (no bare M_p factor) — Mersenne subfamily is PROPER subset of ω=4 Ore set; residual class admits no uniform closed-form (analog of F15 Ore non-perfect). **(s4)** L(s, Γ_0(6)) conductor=36 probe — 🔴 structural CLOSED-NEGATIVE: dim S_2(Γ_0(6))=0 + gamma0_genus(6)=0 ⇒ J_0(6) trivial ⇒ NO weight-2 newform L(s,f) at level 6; conductor=36=N² invariant vacuous; F7 (geometric) + F15 (Γ(N)) + F16 (Hecke/Galois) + F17 (analytic L) = 4-LAYER multilayer non-lift, n=6 identity is ARITHMETIC-LAYER ONLY across all canonical layers. **(s5)** arxiv mining round 2 (Dedekind ψ identities) — 🔵 6/6 SUPPORTED-FORMAL + 🛸 NOVEL: C1 ψ(n)=σ(n) on square-free locus (verified at 6 sqfree n: 6,10,15,30,105,2310; σ matches hexa-native exactly); C3 NOVEL **n=6 = unique square-free even perfect number** (Euclid-Euler: every even perfect = 2^(p-1)·M_p, sqfree ⟺ p=2 ⟺ n=6 uniquely; corollary ψ(n)=2n ⟺ n=6 among sqfree); C2 ψ·φ = n²·∏(1−1/p²) 🟡 citation (dedekind_psi calc-gap INBOX). **Atlas fold SUCCESS via manual splice** (5 atoms, `compiler/atlas/embedded.gen.hexa` 1411→1416 F-formulas): `tecs_l_f17_d_omega_6_7_8_zero_density` (🔴) · `tecs_l_f17_sigma_k_euclid_euler_general_closed_form` (🔵) · `tecs_l_f17_ore_omega_4_subfamily_partial_cover` (🔴) · `tecs_l_f17_L_function_gamma_0_6_structural_neg` (🔴) · `tecs_l_f17_n6_unique_squarefree_perfect` (🔵). 41 hexa-native component verifies + 10 closed-negative findings + 5 novel atoms. → `.verdicts/tecs-l-f17-novel-mk10/` (7 main verdict + 36 component .txt) · TECS-L.log.md F17 entry. **다음 round seeds (F18)**: (a) ω=9,10 D-sweep (primorial #7 = 223092870), (b) σ_6/σ_7/σ_8 Euclid-Euler tower (int64-safe small p), (c) ω=4 Ore non-Mersenne residual {2970, 18620} structural characterization, (d) L(f, s) algebraicity at weight-4 newform level 6 LMFDB, (e) `dedekind_psi` calc-path INBOX → unblock ψ·φ identity hexa-native.
...
- [x] F19 — Clay-RH/BSD methodology-transfer **candidate-spec ONLY**: 2 partial-angle lanes (RH Mertens M(n) 1..20 + BSD CN integer rational-point witnesses at n=5,6) with TECS-L primitives mapped 1:1. **0 novel atoms · 0 atlas fold · 0 paper draft** — paper_significance gate FAIL on both (no pre-registered falsifier with Δ-finding). RH lane: 20 μ(k) 🔵 + cumulative M(n) sweep 🟢 in `.verdicts/tecs-l-f19-rh-mertens/` (descriptive; |M(n)|≤√n holds in [1,20] but Mertens cnj **disproved** at ~10⁶⁴ Odlyzko-te Riele 1985). BSD lane: 2 EXACT integer witnesses (n=5: (−4,6) and n=6: (−3,9) on y²=x³−n²x, both ✓ integer-exact) + 5 σ/τ/φ/μ anchor verdicts 🔵 → UNCONDITIONAL rank(E_5)≥1 and rank(E_6)≥1 (only uses rational-point ⇒ rank-positive, NOT BSD). spec docs `TECS-L/millennium/f19/{README.md,spec-rh-mertens-witness-lane.md,spec-bsd-congruent-number-witness.md}`. **honest framework-analogy, NOT real Clay progress** — extreme-honesty mandate per F19 task brief
