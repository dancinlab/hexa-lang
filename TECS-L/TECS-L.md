...
- [x] F20 — Clay retry **verify-infra extension only** (g5 mode): F19 두 lane 의 width-extension + 3 INBOX entries (calc-fn gaps) + 1 verify-infra land. **(s1) INBOX 3 entries** at `INBOX.log.md` 2026-05-27T00:30Z: (a) `elliptic_witness(x,y,n)` P1 4-op BSD · (b) `tunnell_count_{odd,even}(n)` P2 ternary-form BSD · (c) `mertens(n)` P0 single-arg RH. **(s2) Source-land** `pub fn mertens` in `compiler/atlas/symbolic/congruence_chain_engine.hexa` + `_recompute` arm in `tool/verify_cli.hexa` (both `hexa parse` 🔵). **Binary activation DEFERRED** — `hexa cc --regen` 가 pool-refused + local-bound heavy-gate signed-token 요구, live verify 가 여전 🟠 "no path for 'mertens'". **(s3) RH lane M(n) extension** n=21..30 (10 μ(k) 🔵 + sweep table 🟢; F19 의 n=1..20 → n=1..30 누적). **BSD lane CN witness extension** n∈{7,14,15,20,21,28,30} 7 integer rational points exhibited on E_n (F19 의 n=5,6 → 9 총 witnesses) + 7 σ(n) anchors 🔵. **0 novel atoms · 0 atlas fold · 0 paper** (paper_significance + paper_negative_ok 둘 다 미충족; scale-extension 만으로는 Δ-finding 아님). verdicts `.verdicts/tecs-l-f20-rh-mertens/` + `.verdicts/tecs-l-f20-bsd-witnesses/`. README `TECS-L/millennium/f20/README.md` — **honest negative: scale-extension does NOT change asymptotic Clay question**
