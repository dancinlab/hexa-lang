<!-- @canonical-origin: canon@a86ca143:papers/n6-topology-paper.md (moved 2026-05-10) -->
<!-- gold-standard: shared/harness/sample.md -->
---
domain: topology
requires: []
---
# [CANONICAL v2] Ultimate topology (HEXA-TOPOLOGY) — n=6 arithmetic coordinate mapping

> **Author**: Minwoo Park (canon)
> **Category**: topology — n=6 arithmetic seed paper
> **Version**: v2 (2026-04-14 canonical)
> **Prior BT**: BT-9, BT-91, BT-92, BT-109, BT-232
> **Linked atlas node**: `topology` 10/10 EXACT [10*]

---

## 0. Abstract

This paper demonstrates, as a candidate framing, that the core parameters of the topology domain
...
values of topology. atlas.n6 records 10/10 items as EXACT.

This paper does not claim a new topology; rather, it is a seed paper that overlays an **n=6
...
topology is re-read within the n=6 arithmetic system. The perfect number n=6 simultaneously
satisfies the number-theoretic constant group σ(6)=12, τ(6)=4, φ=2, sopfr(6)=5, and this is
structurally aligned with the core parameters of the topology domain. **This paper overlays an
n=6 arithmetic coordinate system onto the existing knowledge of topology.**

| Effect | Baseline | After HEXA-TOPOLOGY | Experienced change |
|--------|----------|------------------------|--------------------|
...
pattern that meshes necessarily with the basic numerical values of topology.

### What the n=6 coordinate mapping changes

```
  Before: "Why is this topology value this number?" → experience / convention
  HEXA:   "This topology value = σ(6) or τ(6) or sopfr(6)" → number-theoretic necessity
       ↓
...
## §2 COMPARE (baseline topology vs n=6) — performance comparison (ASCII)
...
### Performance comparison ASCII bars (baseline topology method vs HEXA-TOPOLOGY)
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                    HEXA-TOPOLOGY    system structure     │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
4. **Domain overlap**: the σ=12 axis is a common parameter across dozens of domains beyond topology.
...
Stage-wise maturation roadmap for HEXA-TOPOLOGY — verification density increases per Mk:

<details open>
<summary><b>Mk.V — 2045+ integration target</b></summary>

Target: integrate the entire topology domain into n=6 arithmetic. Cross-reference with 295
...
Map core topology parameters onto σ/τ/φ/sopfr/J₂.
...
#### TP-TOPOLOGY-7: scaling exponent τ=4
- **Verify**: measure the log-log slope of topology scaling laws
- **Predict**: slope ≈ 4.0 ± 0.3
- **Tier**: 2

#### TP-TOPOLOGY-8: ±10% convex optimum
- **Verify**: ±10% sensitivity around n=6
- **Predict**: f(5.4), f(6.6) both worse than f(6) (convex extremum)
- **Tier**: 1

#### TP-TOPOLOGY-9: χ² p-value > 0.05
- **Verify**: compute atlas 10/10 EXACT under H₀ (chance)
- **Predict**: p > 0.05 → "chance" rejectable (n=6 structure significant)
- **Tier**: 1

#### TP-TOPOLOGY-10: OEIS triple registration
...
Log-log regress the main scaling laws of topology to check whether they follow the τ(6)=4 or
sopfr(6)=5 exponent.
...
Domain-specific physical upper bounds (Carnot/Shannon/Bekenstein, etc.) checked separately for topology.

### §7.6 CHI2 — H₀ (n=6 chance hypothesis) p-value
Compute 10/10 EXACT under H₀ (random matching) → p-value.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-TOPOLOGY n=6 honesty verification (stdlib only, topology domain)
#
...
FALSIFIERS = [
    "If main topology parameter n=6 alignment < 70%, retract the core claim of this paper",
    "If sigma(n)*phi(n) = n*tau(n) is observed at any n other than n=6, retract the uniqueness result",
    "If atlas 10/10 EXACT re-measurement falls below 70%, downgrade from Mk.I",
