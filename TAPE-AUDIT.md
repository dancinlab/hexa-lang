...
Empty `AGENTS.md` (zero bytes) + symlinked `CLAUDE.md`. Identity carried implicitly via root domain files (`HEXA-NATIVE-ONLY.md`, `FLOW.md`, `LATTICE_POLICY.md`, `LIMIT_BREAKTHROUGH.md`, `ROADMAP.md`, `SPEC.md`). A `hexa-lang/identity.tape` would carry compiler-self-id + atlas-pin-sha + active-RFC state — a legitimate slot, but lower-priority than D.

## C. Domain.md files

Several root `*.md` files including domain-shaped: `FLOW.md`, `HEXA-NATIVE-ONLY.md`, `LATTICE_POLICY.md`, `LIMIT_BREAKTHROUGH.md`, `ROADMAP.md`, `SPEC.md`. Strong convention — sibling `<DOMAIN>.tape` files (`ROADMAP.tape`, `LATTICE_POLICY.tape`) would capture decision/transition events while leaving the `.md` head intact; the spec/log split is the active discipline (`<DOMAIN>.md` = current spec, `<DOMAIN>.log.md` = chronological history, e.g. `COMPILER.log.md` · `ROADMAP.log.md`).
