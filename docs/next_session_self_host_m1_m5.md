...
(interp regression 추적 · baseline dispatch_tag main 병합 ceremony).
...
### 6.3 baseline dispatch_tag worktree commit 병합

Worktree commit `4a5dc42c` 의 baseline dispatch_tag 변경사항을 main 으로 cherry-pick / ceremony 병합. 내용 검토 후 회귀 probe 있으면 skip 가능 — 단순 ceremony.
