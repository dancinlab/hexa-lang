...
`tool/resolver_bypass_marker_audit.hexa` (darwin-resolver-bypass 강화 lint, hive 2026-04-28) 이 32 markers 스캔 → **21 violations** 검출. 그 중 **16x** 이 `darwin-native byte-canonical pipeline` 동일 reason 으로 HXC 압축 family (`hxc_a9_tokenizer.hexa`, `hxc_a11..a22`, `hxc_base94_codec.hexa`, `hxc_d1_canary_watcher.hexa`) 에 일괄 부착됨.

darwin-resolver-bypass 정의는 **darwin-native syscall** (chflags / launchctl / sandbox-exec / DYLD / SBPL / plutil / codesign) 에 한정. HXC 압축 알고리즘은 어느 syscall 도 쓰지 않음 → marker 오용.
...
`@memory-cap-bypass(reason="<why>", min-mem-mb=<N>)` 신규 marker 도입. darwin-resolver-bypass 의 darwin-native scope 와 분리.
...
2. `@resolver-bypass(reason=...)`: 기존 darwin-resolver-bypass 동작 유지

### lint 변경 (`resolver_bypass_marker_audit.hexa`)

기존 lint:
- Tier-1: darwin-resolver-bypass syscall 키워드
...
4. darwin-resolver-bypass entry 강화 — scope 에서 "memory-headroom" 명시적 제외 + `@memory-cap-bypass` 별도 raw 등록 검토
5. cron `dev.hexa-lang.hexa-runtime-watcher-audit` 의 verdict 에 marker 분류 통계 추가

## raw 등록 후보

`@memory-cap-bypass` 가 `@resolver-bypass` 와 동급 contract 라면 별도 raw 필요. darwin-resolver-bypass 가 darwin-native scope 만 다루므로, 신규 raw (예: follow-up `execution-memory-headroom-bypass`) 등록 + 트라이어드 (cli-lint + os-level + hive-agent) 적용 권장. 단, 사용자 directive 받은 후 진행.

## 폐쇄 루프 매핑 (closed-loop)
...
- step 4 emit-ledger: marker 변경 시 audit row
...
3. follow-up 등록 여부 (yes 라면 closed-loop step 6 lint 추가)

## 참조

- darwin-resolver-bypass (execution-darwin-resolver-bypass)
- closed-loop (agent-closed-loop-problem-solving-methodology)
