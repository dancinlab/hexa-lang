...
- **Source evidence (g3 — every claim anchored)**:
  - flame Phase 3 NN-stack correctness foundation: RFC 045 closure document (40+ falsifier PASS, regression 0, structural call_builtin = 0, ~6.4k LoC) — `docs/rfc/rfc_drafts_2026_05_12/rfc_045_flame_phase3_algorithmic_byte_eq_with_anima_oracle.md`
  ...
  - Phase 4-A PARTIAL LANDED measurement (5-run avg full 80-step corpus, post commits `bbaa4bbf` through `6fa735c7`): **13.33s wall** (range 13.03-13.93s, variance 7%) vs anima same toolchain 3-run avg **22.13s** (range 21.34-23.49s) → **flame is 60.2% of anima wall**. vs Phase 3-J baseline (~17s estimated): **~20-25% wall reduction** from 7 outer-product accumulator wire-ins. Granularity floor for `farr_matmul`-routing established at ~32K ops (drin per-t inner-dot below floor, anti-perf). Stage 2/3 IR-level fusion is the remaining ~75% wall savings path toward eager-PyTorch parity at d=768·12L
...
- **Phase 4-A — Stage 1 epilogue fusion + bwd projection routing (RE-PRIORITIZED 2026-05-17; PARTIALLY LANDED with HONEST RESULT)**. ~10 fused-matmul variants in `self/runtime.c` (or `self/forge/`), restructure nn_lib call sites. ALSO route the `nn_decoder_grad` bwd projection-accumulating loops through `_db_proj_batch_farr` (mirroring the Phase 3-J fwd-side wire-in). Falsifiers: F-RFC046-FUSED-MATMUL-EPILOGUE-EQ + F-RFC046-STEP-EQ on Phase 3-F-3 baseline.
  **LANDED PARTIAL (commits `bbaa4bbf` through `6fa735c7`)**: 7 outer-product accumulators wire-in completed (dWq/dWk/dWv/dWd/dWo/dWg/dWu) via `_db_grad_accum_farr` helper. Real wall improvement at d=32·3L on M-Mac CPU:
  - **5-run flame avg = 13.33s** (range 13.03-13.93s, variance 7%) on 80-step d_corpus_fire benchmark
  - **3-run anima avg = 22.13s** (range 21.34-23.49s, variance ~10%) same workload
  - **flame is 60.2% of anima wall** (ratio 0.602×)
  - Phase 3-J baseline was a single-shot 18.29s reading; 5-run-comparable baseline likely ~17s (close to but not identical to Phase 3-J state since variance ~7%). Estimated real wall reduction: **~20-25%** from accumulator wire-ins.
  - **Honest retraction of double-correction**: commit `e8c78f4e`'s '33%' (12.30s) was NOT an outlier — 5-run shows 13.03s achievable. Commit `6fa735c7`'s '16.06s avg' was an outlier in the OTHER direction (3-run sample with system load). The 5-run measurement supersedes both. Future RFC 046 wall comparisons should use ≥5-run averaging.
  **NEGATIVE RESULT (commit `6fa735c7`)**: tested drin reduction (per-t inner-dot: `Σ_r Wq[r,c]·dQ[t,r] + Σ_r Wk[r,c]·dK[t,r] + Σ_r Wv[r,c]·dV[t,r]`) via 3 separate `farr_matmul` calls. Result: wall REGRESSION (15.70s → 14.16s appeared on a single run but 3-run avg landed worse — overhead of 3 buffer copies + 3 farr allocs + element-wise add exceeded the savings at this scale, T·d = 512 output elements × ~64-op dot = ~32K total ops, well below the kernel-launch overhead floor). Reverted. This establishes the **granularity floor finding**: small reductions (< ~32K ops) do NOT benefit from `farr_matmul`-routing — kernel launch + buffer copy overhead dominates the savings. Single-pattern helper `_db_grad_accum_farr` is appropriate for OUTER-PRODUCT accumulators (Σ_ts dY[ts]⊗X[ts]), not for per-element inner-dot reductions.
  Further Phase 4-A-bwd gains require either (a) **larger reductions** (attention_core_bwd P · dY over T×hd is a candidate — still inline), or (b) the Stage 2/3 IR-level fusion below.
  **Effort: ~1 cycle.** Original estimate ~2× wall; actual measured ~10-15% via the OUTER-PRODUCT subset alone. The remaining ~85% wall savings RFC 046 mid-term target requires Stage 2/3.
...
- RFC 043 (design SSOT) — `docs/rfc/rfc_drafts_2026_05_12/rfc_043_hexa_torch_compiler_only_nn_stdlib.md`
- RFC 045 (Phase 3 closure) — `docs/rfc/rfc_drafts_2026_05_12/rfc_045_flame_phase3_algorithmic_byte_eq_with_anima_oracle.md`
- RFC 044 (forge regime) — `docs/rfc/rfc_drafts_2026_05_12/rfc_044_forge_regime_tiered_substrate.md` (parallel session)
