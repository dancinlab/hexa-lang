...
|---|---|---|---|
| G1 | **Real-corpus end-to-end training run** (not the fixed-seed micro-step) | GPU-build-gated — the `clm_prod_gpu` `-DHEXA_CUDA` build is turnkey (`F-OP24C`) but the run is env-gated; reviewers may ask "does it train a real model, not just a step?" | **high** — the headline says "trains a real NN"; the byte-eq is proven on the *step*, end-to-end real-corpus is build-deferred |
