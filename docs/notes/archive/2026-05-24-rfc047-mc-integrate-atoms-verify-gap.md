# [RESOLVED 2026-05-24] RFC 047 mc-integrate atom 등록 BLOCKED — verify 계산기 float-path 부재 (2026-05-24)

## 해결 (2026-05-24, 후속 PR)
- PR #587 (`feat(verify): float recompute path — welch_t_crit + wilson_hilferty`) — verify float-NUMERICAL path 추가.
- PR #593 (`feat(atlas): allow 🟢 NUMERICAL tier in register_from_event`) — register 게이트 🟢 허용.
- 이번 PR (atlas/rfc047-046-atoms-register-2026-05-24) — atlas 실제 등록:
  - `@F verified-welch_t_crit-num = welch_t_crit(1.0) = 12.706 :: formula` 🟢 SUPPORTED-NUMERICAL
  - `@F verified-wilson_hilferty_p-num = wilson_hilferty_p(0.0,10.0) = 1.0 :: formula` 🟢 SUPPORTED-NUMERICAL
- 게이트 verdict (g5 VERBATIM):
  - `hexa verify --expr welch_t_crit 1 12.706` → `tier   = 🟢 SUPPORTED-NUMERICAL  (hexa-native libm-class recompute, TECS-L n6-rep Tier2)`
  - `hexa verify --expr wilson_hilferty_p 0 10 1.0` → `tier   = 🟢 SUPPORTED-NUMERICAL  (hexa-native libm-class recompute, TECS-L n6-rep Tier2)`
