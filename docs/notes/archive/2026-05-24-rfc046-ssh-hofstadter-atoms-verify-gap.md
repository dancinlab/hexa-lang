# [RESOLVED 2026-05-24] RFC 046 sim-universe ssh/hofstadter 정수-identity atom 등록 BLOCKED — verify int-path 미지원 (2026-05-24)

## 해결 (2026-05-24, 후속 PR)
- PR #592 (`feat(verify): ssh_winding + tknn_chern integer recompute`) — verify int-path 확장.
- PR #593 (`feat(atlas): allow 🟢 NUMERICAL tier in register_from_event`) — register 게이트 🟢 허용.
- 이번 PR (atlas/rfc047-046-atoms-register-2026-05-24) — atlas register 의 mirror table 동기화 (PR #592 surface gap fix) + 4 atoms 실제 등록:
  - `@F verified-ssh_winding-1-2 = ssh_winding(1,2) = 1 :: formula` 🔵 SUPPORTED-FORMAL
  - `@F verified-tknn_chern-2-5-1 = tknn_chern(2,5,1) = 3 :: formula` 🔵 SUPPORTED-FORMAL
- 게이트 verdict (g5 VERBATIM):
  - `hexa verify --expr ssh_winding 1 2 1` → `tier   = 🔵 SUPPORTED-FORMAL  (hexa-native closed-form, g_self_verify · TECS-L Tier1)`
  - `hexa verify --expr tknn_chern 2 5 1 3` → `tier   = 🔵 SUPPORTED-FORMAL  (hexa-native closed-form, g_self_verify · TECS-L Tier1)`
