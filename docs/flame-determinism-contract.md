...
1. **Three distinct `exp` implementations are each load-bearing.** The step uses
   three *different* `exp` functions, picked per phase, and they are NOT
   interchangeable. A "unify the exp" refactor would silently break byte-eq.

   | exp impl | where | source |
   |---|---|---|
   | libm `exp` builtin | CE **backward** grad (`clm_ce_grad`) | F-OP11 |
   | `dt_exp` (hand-rolled scaled-Taylor, `flame_math.hexa`) | CE **forward** loss (`nn_ce_loss_allpos`) | F-OP11 |
   | `_moe_exp` (hand-rolled scaled-Taylor, range-reduce + 14-term) | MoE router softmax | F-OP8 |

   These are three separate code paths. Swapping any one for another (libm ↔
   Taylor ↔ `_moe_exp`) changes the bits. (The hand-rolled `sqrt` impls are
...
| **LOSS bwd** — CE + softmax fused gradient | F-OP11-CE-SOFTMAX-ORACLE | `dL/dlogits[t,v] = (softmax[v] − [v==tgt])/T`; exp = **libm `exp`**; per-row max-sub ON; denom **v-ascending** sequential; write `p·invT` for all v, **THEN** `dlogits[tgt] −= invT` (scale-then-subtract; do NOT refold to `(p−1)·invT`) | tree-reduce denom, drop max-sub, swap exp impl, or refold the target subtraction (the `(p−1)·invT` fold is float-different — ~1.39e-17) |
...
```
  ids ──[INPUT embed gather]──> X
   ...
   [LOSS]  fwd: mean NLL  (dt_exp/dt_ln, t-ASC, clamp 1e-6)  F-OP11 (fwd)
           bwd: (softmax−onehot)/T  (libm exp, v-ASC denom,  F-OP11 (bwd)
...
Three exp impls live in this diagram: **`_moe_exp`** (MoE), **`dt_exp`** (loss
fwd), **libm `exp`** (loss bwd). Each is load-bearing.

## what breaks the contract

A non-exhaustive checklist — any of these silently breaks `max|Δ| = 0`:

- **Unifying the exp.** Three impls (libm / `dt_exp` / `_moe_exp`) by design.
...
- **Hand-rolling `erf`/`sqrt`** where a libm builtin (`erf`) or the specific
  Newton seed (`_gn_sqrt`, `_adamw_sqrt`) is locked.
