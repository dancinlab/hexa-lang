...
- **A lossy or incomplete CHECKPOINT.** Serializing training state through TEXT
  ...
  2.027e-6), through an **fp32** store (the `.clm` int4/fp32 INFERENCE export is
  NOT a training checkpoint — an fp32-truncated resume diverges, MEASURED 1.77e-8
  within 2 steps), or **without the AdamW m/v + applied step t** (a reset t
  restarts bias-correction — MEASURED 0.042 divergence). The canonical primitive
  is `ckpt_lib` (`"FCK\x01"`: binary fp64-LE bit-pattern reinterpret, fixed field
