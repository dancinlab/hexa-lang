...
### 구조적 메커니즘 — "왜 6·28인가" (2026-06-16, `scripts/scratch/anchor_proof.py`)
brute 가 아닌 **곱셈성(multiplicativity)** 으로 앵커의 정체를 규명. σ·τ·φ·id 가 전부 곱셈적이므로
비(比) `g(n) = σ(n)τ(n)/(n·φ(n))` 도 곱셈적 → **`στ=nφ` ⟺ `∏_p g(p^a) = 1`**.

```
28 = 2²·7 :  g(2²)=21/8  ×  g(7)=8/21  = 1   ← 정확한 역수쌍(reciprocal pair)
 6 = 2·3  :  h(2)=3/4    ×  h(3)=4/3   = 1   ← 6-법칙(h=σφ/nτ)도 동일 역수쌍 메커니즘
```

**핵심 구조**: `g(p^a) > 1` 인 prime-power 는 **단 5개** {2¹=3, 2²=21/8, 2³=15/8, 2⁴=155/128,
3¹=4/3} 뿐 — 나머지 모든 소수거듭제곱은 `g(p^a) < 1`. 따라서 `∏ g = 1` 을 만들려면 이 유한한 '키'
중 하나에 **정확히 약분되는 파트너**가 필요 → 해가 극도로 희소. 소수 ≤200·지수 ≤4 의 2·3-소수 곱
전수 탐색에서 **유일하게 28** 출현.

**상태(c9 정직)**: 이는 *완전 증명이 아니라 메커니즘 특성화 + 부분 논증*이다. `g>1` 집합이 유한·소형임을
보여 유일성의 **이유**(역수쌍 희소성)를 규명했으나, 모든 지수·임의 소수 개수에 대한 곱=1 의 비존재를
엄밀히 닫진 못했다(σφτ Diophantine 일반난해). 단, 6-법칙(`σφ/τ=6`)의 유일성은 기지 정리.
재현: `python3 scripts/scratch/anchor_proof.py`.
