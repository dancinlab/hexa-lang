...
- 비유: 지하철 노선도 — 역(개념)을 선(검증된 관계)으로 잇고, 공사 중 구간은 점선으로 표시
...
원본 노드맵의 4색을 atlas verdict tier 로 정규화한다.
...
> ⚠️ **정직 단서** — 원본 노드맵에는 `G = D∘P/I`, `골든 MoE`, `표준모형(미완)`,
> `홀로그래피(미완)` 처럼 **미검증 모델/가설** 노드가 섞여 있다. 이들은 🟧 로
> 명시 표기하며 atlas verdict 를 부여하지 않는다 (honesty — `../CLAUDE.md` 거버넌스).
> 외부 주체·외부 컴파일러에 격자-맞춤 주장도 금지.

---

## 🌐 거시 ↔ 양자 (노드맵 ①)

지도의 **척추** = 거시(macro) 와 양자(quantum) 두 체계, 그리고 그 사이의 **다리**.
각 가지 끝 `→ §N` 은 아래 〈9 도메인 매핑〉의 번호다.

```
        거시 수학체계                    │                양자 수학체계
   ───────────────────────             │           ───────────────────────
   🟦 정수론 (number theory)            │           🟦 힐베르트 공간 |ψ⟩∈ℍ
   ├─ 🟦 소수 (p=2,3) ........ → §2     │           ├─ 🟦 스펙트럼 이론 ...... → §3
   ├─ 🟦 리만 제타 ζ(s) ...... → §2     │           ├─ 🟦 리 군/대수 ......... → §8
   ├─ 🟦 해석학 (Basel π²/6) . → §2     │           ├─ 🟦 게이지 보존 ........ → §3
   ├─ 🟦 오일러 곱 ........... → §2     │           ├─ 🟦 스핀 기하·디랙 ..... → §3
   └─ 🟦 완전수·약수 항등식 ... → §1     │           └─ 🟦 비가환 기하 ........ → §8
                                        │
   🟩 동역학계 (Wick t→iτ) ... → §3     │           🟩 경로적분 ∫Dφ e^{iS/ħ} . → §3
   🟧 조화급수 H₋₁=5/6 ⭐상한 . → §2     │           🟩 재규격화 RG 흐름 ...... → §3
   🟩 군론 SU(3)×SU(2)×U(1) .. → §3     │           🟧 표준모형 (미완) ....... → §3
                                        │           🟧 홀로그래피 (미완) ..... → §6
                  ╲                     │                     ╱
                   ╲─────────── 다리 (bridges) ──────────────╱
                                ▼   § BRIDGES (§10)   ▼
        스펙트럼 ⇄ 제타영점 (Hilbert–Pólya)  ·  RG 흐름 ⇄ 동역학계
        S_VN = −∑ ln(p)  ·  Chern–Simons  ·  Wilson RG
```

> 두 체계를 **잇는 다리** 가 지도의 핵심 관심사다. 원본 노드맵의 illegible 한
> 구체 수식(특정 ζ-값·137 조합 등)은 **지어내지 않는다**. 검증된 atom 은
> `../compiler/atlas/embedded.gen.hexa` 에 entry id 로 박제돼 있다.

---

## 🟢 연결 현황 (노드맵 ②)

"순수 산술 — 미연결 포함". 각 영역이 다른 개념과 **다리로 연결(검증)됐는지**
표시한다. 🟢 = 닫힘 · 🟧 = 미연결/구조화 대기.

```
§A 약수·항등식  🟢  완전수·약수 합 항등식 — 닫힘 ........................ → §1
§B 미해결       🟧  137·e·ζ 조합 — 산술 일치 관찰, 대응 UNVERIFIED ...... 미연결 → §10
§C 로그·초월    🟢  Basel ζ(2)=π²/6 · libm log/exp Newton ............. 닫힘 → §2
§D 해석학       🟧  CMB α≈1/137 — literature anchor (🟡 citation) ...... 부분 → §6
§E 항등식 모음  🟧  골든비·exp 조합 — C3 0% 구조화 대기 ................ 미연결
§F 모듈러·격자  🟢  Leech/Golay · Moonshine χ→Monster 8-step .......... 닫힘 → §8
```

- **닫힘(🟢)** = hexa-native verifier 가 PASS 한 연결 (Stage 1 symbolic / Stage 2 numerical)
- **미연결(🟧)** = 산술적 일치는 관찰됐으나 verifier 미작성 또는 외부 대응 미검증
...
지도의 각 가지가 어느 verdict 도메인에 속하는지.
...
verdict tier 의 default 는 🟠 INSUFFICIENT (honesty-by-default — 검증 전 silent 승급 금지).
verdict-bearing atom 은 real-limit anchor (Shannon·Bekenstein·Carnot·c·ℏ·k 등) 1개 이상 +
falsifier 를 동반한다. lattice-tautology (σ·φ=n·τ at n=6) 단독 검증은 불충분.
...
> full-suite 는 atlas embed 가 커서 로컬 4GB memcap 을 넘기므로 `HEXA_MEM_UNLIMITED=1` 필수.
...
| 🌐 `../CLAUDE.md` | 거버넌스 SSOT (atlas-area governance 포함) |
...
- 지도의 🟧 노드(미검증 모델·미완 이론)는 verdict 미부여 — 정직하게 "아직 안 됐다" 로 표기.
