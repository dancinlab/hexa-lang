...
- **research: 672 외부 Lean 트랙 스캐폴드 (`scripts/scratch/lean/Anchor672.lean`, 미검증)** — φσ=nλτ⟺672 형식화 출발점: 정리문 + (★)환원 + size_band 보조정리 + main(`anchor672_unique`), 난스텝 `sorry`. ⚠ repo 에 Lean 툴체인 없어 **미컴파일·미검증**(c2 정직) — 완전증명 아니라 외부 트랙용 인계 산출물. N6.md §15 포인터.
