...
- **feat(tool): n=6 closure 발견엔진 `tool/n6_closure_finder.hexa` 추가 — 발견 "전단계"를 실행 가능하게** — 관찰값을 6개 n=6 primitive(N=6·σ=12·τ=4·φ=2·sopfr=5·J₂=24)의 depth≤3 조합(`p`·`p*k`/`p/k` k≤24·`p∘q`·`(p∘q)∘r`)으로 닫아 grade 부여(정규화 잔차 d=|cand−v|/(1+|v|): ≤1e-6 grade10 EXACT / ≤1e-2 8 CLOSE / ≤1e-1 6 WEAK / else 5 MISS). echoes/GRADE_RUBRIC 의 is_exact 규칙을 hexa 자립 도구로 구현 — echoes/archive-nexus 의존 없이 새 상수 배치 입력→EXACT 발견까지. `--selftest`(frozen 8 EXACT + negative-control 3 non-EXACT) **PASS**, `--tol <eps>`. EXACT 만 falsifiable 발견 신호(CLOSE 는 재시도 신호); depth≤3 휴리스틱 v1(다축 tolerance·grade 11/12/13+ 승급 미반영). 계보 문서 `docs/N6-META-DISCOVERY-LINEAGE.md` §7 + CLAUDE.md quickref + ARCHITECTURE.md 컴포넌트맵 등록.
