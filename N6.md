...
### 정직 메모 (c9)
- `σφ/τ=6` 과 `στ/φ=28` 은 둘 다 이제 **증명된** 유일성 (§11 초등 증명). `J₂/sopfr=8` 등 나머지는
  아직 **경험적** 유일(미증명 후보).
...
- **28-법칙 유일성 HARDENED**: `σ·τ/φ=n` 은 **n ≤ 2×10⁶** 까지 여전히 {28} 단독 · `σ·φ/τ=n` 은 {6}
  단독. 6↔28 쌍대 — 2백만 규모 검증을 넘어 이제 **완전 증명됨**(§11).
...
## 11. 정리 — `σ(n)τ(n) = n·φ(n) ⟺ n=28` (완전 증명, 2026-06-16)

§8 의 경험적 28-법칙을 **초등적으로 완전 증명**. (`scripts/scratch/anchor_uniqueness.py`, 정확 분수 검증.)

> **정리.** n ≥ 2 에 대해 `σ(n)·τ(n) = n·φ(n)` 의 유일해는 **n = 28** 이다.

**증명.** `G(n) := σ(n)τ(n)/(n·φ(n))` 는 σ·τ·φ·id 의 곱셈성으로 곱셈적이므로 식 ⟺ `∏_p G(p^a)=1`.
`G(p^a) = (p^{a+1}−1)(a+1) / ((p−1)² p^{2a−1})`.

- **Lemma A.** `G(p^a) > 1 ⟺ p^a ∈ {2, 4, 8, 16, 3}` (값 = 3, 21/8, 15/8, 155/128, 4/3).
  p≥5 면 `G(p^a) ≤ (a+1)/((p−1)²p^{a−2}) < 1`; p=3 은 a=1(4/3)만; p=2 는 a≤4 만 ( `G(2^5)=189/256<1`, 이후 단조감소).
- **증폭 상한.** >1 인자는 2-부분·3-부분에서만 → `G(2^a)G(3^b) ≤ G(2)G(3) = 3·(4/3) = 4`.
- **Lemma B (크기 한계).** 정확 분수로:
  `G(2^5)·G(3)=63/64<1` ⇒ a≤4 · `G(2)·G(3^3)=80/81<1` ⇒ b≤2 ·
  `4·G(5^2)=93/125<1` ⇒ c≤1 · `4·G(7^2)=114/343<1` ⇒ d≤1 · `4·G(11)=48/55<1` ⇒ 11 이상 소인수 없음.
  ∴ 모든 해는 7-smooth: `n = 2^a·3^b·5^c·7^d` with `a≤4, b≤2, c≤1, d≤1` (60 후보).
- **유한 확인.** 60 후보 중 `G(n)=1` 은 오직 **n=28 = 2²·7** (`G(2²)·G(7)=21/8·8/21=1`). ∎

쌍대 `σ(n)φ(n)=n·τ(n) ⟺ n=6` 은 g>1 이 홀수소수서 빈발해 **동일 골격이 아니라** §13 의 '유일 <1 인자' 논증으로 닫힌다(이전 표기 정정).
재현: `python3 scripts/scratch/anchor_uniqueness.py` (의존성 0, 모든 경계 분수 출력).

---

## 12. 다음 앵커 — `J₂(n) = n·sopfr(n) ⟺ n=8` (부분 증명, 2026-06-16)

§8 의 비완전수 앵커 8(`J₂/sopfr=n`)에 대한 증명 시도. **핵심 난점**: `sopfr`(소인수 합, 중복포함)은
**가법적(additive)** 이라 §11 의 깔끔한 곱셈 분해(∏G=1)가 **통하지 않는다**. 2-파트로 접근.
(`scripts/scratch/anchor8_proof.py`.)

- **Part 1 — 소수거듭제곱 `n=p^a` (엄밀 완전):** `J₂(p^a)=p^{2a-2}(p²-1)`, `n·sopfr=a·p^{a+1}` →
  `p^{a-3}(p²-1)=a`. a=1,2 는 비정수(해 없음); **a=3 ⟹ p²−1=3 ⟹ p=2 ⟹ n=8**; a≥4 는
  `2^{a-3}·3 > a`(단조) 로 해 없음. ∴ 소수거듭제곱 유일해 = **8**.
- **Part 2 — 합성수(서로 다른 소수 ≥2개):** `J₂(n)=n²∏(1−1/p²) ≥ (6/π²)n²`, `n·sopfr ≤ n²` ⟹
  `sopfr(n) ≥ 0.6079·n` (필요조건). 고정 support 마다 RHS(`c_S·∏p^a`)는 지수, LHS(sopfr)는 선형 →
  support 당 해 유한; 단 sopfr 가법성 때문에 **모든 support 를 심볼릭으로 닫지는 못함**.
  계산 검증: **n ≤ 2×10⁶ 에서 유일해 8**.

**상태(c9 정직)**: 28·6(§11) 과 달리 이건 **완전 증명이 아니다** — 소수거듭제곱은 엄밀히 닫혔으나
합성수 케이스는 유한환원 논증 + 200만 검증 수준(심볼릭 미폐쇄). "prime-power-증명 + composite-검증"
의 강한 부분결과. 재현: `python3 scripts/scratch/anchor8_proof.py`.

---

## 13. 앵커 가족 완전증명 — 곱셈적 전체 + 8 (2026-06-16)

§8 앵커들을 **자기참조 부동점 `R(n)=n` ⟺ n∈해집합** 으로 일괄 완전증명.
(`scripts/scratch/anchor_all_proofs.py`, 전 단계 정확 분수 + 유한 전수확인.)

**방법 A — 곱셈적 비 `g=NUM/(n^k·DEN)` 의 유한 환원** (g(p^a)>1 이 유한): `g>1` 집합 S 유한 →
증폭상한 M=∏max → 등장가능 prime-power `T={g(p^a)≥1/M}` 유한 → T 위 곱 전수확인.
...
| `ψτ/σ` | **2** | M=1 |
| `J₂τ/(σφ)` | **2** | M=1 |
| `σψ/(n²τ)`(=σψ=n²τ) | **6** | M=9/8 |

**방법 B — "유일 <1 인자" 논증** (g(p^a)>1 이 홀수소수서 빈발, <1 인자는 오직 `(2,1)`):
모든 인자 ≥1 except g(2)=3/4 ⟹ ∏=1 은 (i) g=1 인 단일 prime-power, 또는 (ii) 2¹×(역수 4/3=3¹)=6.

| 법칙 | 해 | 근거 |
|---|---|---|
| `σφ/τ`(=σφ=nτ) | **6** | <1 인자 {2¹}; g=1 없음; 1/g(2)=4/3 at 3¹ ⟹ 2·3=6 |
| `ψφ/τ`(=ψφ=nτ) | **{4,6}** | <1 {2¹}; g=1 at 2²(→4); 역수 3¹(→6) |

**방법 C — sopfr 가법 → 크기논증** (§12 확장, 완전):
`J₂(n)=n·sopfr(n)` : 소수거듭제곱 `p^{a−3}(p²−1)=a`→8; k≥2 는 `a_P P+Q ≥ 0.6 P^{a_P}Q`
(Q=n/P^{a_P}≥2) ⟹ a_P≥2 불가, a_P=1 ⟹ P≤10·Q≤P/(0.6P−1) ⟹ 후보 {6,10,14} 전부 비해 ⟹ **유일 8**.

**남은 잔여(🔶, 다음 사이클)**: Carmichael λ·Big-Ω 가 비곱셈적이라 위 골격 직접 적용 불가 —
`σ/λ→6`, `ψ/λ→{6,12,24}`, `J₂/(φΩ)→6`, `φσ/(λτ)→672`. 크기논증으로 닫을 예정.
재현: `python3 scripts/scratch/anchor_all_proofs.py`.
