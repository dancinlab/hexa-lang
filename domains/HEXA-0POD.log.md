...
## 2026-06-13 — OP-145 DONE: "array-store-in-fn HANGS (RC=143)" → 🟢 STALE-BINARY-ONLY ARTIFACT (current source runs it), trigger MISDIAGNOSED (immutable-`let` loop-counter reassign, not arrays)
- TASK (LANE-1): root-cause the RC=143 hang multiple campaign agents hit running an array index-STORE inside a function body on the deployed Jun-7 hexa; decide (a) real current-source codegen bug vs (b) stale-deployed-binary artifact (re-promote-closeable, like OP-124/126/132).
- REPRO + BISECTION (deployed Jun-7 `hexa run`, timeouts): array store was a RED HERRING. `arr[0]=v` single store RC=0; `let mut i` + index-store loop RC=0; but the minimal `fn main(){ let i=0; while i<3 { i=i+1 } }` (NO array) HANGS, and `let mut i=0` of the same RUNS. Real axis = reassigning a plain immutable `let` binding inside a `while`. stdlib `sort([3,1,2])` and `map_arr(...)` hang for the same reason (both use `let i` counters).
- ROOT CAUSE (gen-C witness): codegen const-folds `let x=<lit>`→literal and inlines reads; fold invalidated on plain-Ident reassign but the AssignStmt is emitted AFTER the `while` condition → condition folded with stale `0` → `i<3` becomes `0<3` (always true) → infinite loop. hexa `let` is REASSIGNABLE (162× `let i=0;…i=i+1` in shipped stdlib) so the idiom MUST work — not a missing-`mut` lint.
- FIX ALREADY ON MAIN: self/codegen.hexa WhileStmt "MISCOMPILE-ZERO" arm pre-invalidates the fold for body-reassigned names BEFORE the condition (#2650 3b550c6af 2026-06-04 + #2660 if-body sibling). Frozen bootstrap seed (151c52c8, 2026-05-29) predates #2650 → that's why frozen-seed hexat also hangs (seed ≠ main; the stale-oracle trap).
- AUTHORITATIVE FRESH-SOURCE: `tool/build_selfhost.sh -j` → aprime_cc (current self/*.hexa, has #2650); --emit=obj + hexa_ld + rt.o ran every repro + stdlib sort + the regression test → ALL RC=0 correct ("3"/"102"/"1 2 3"/PASS 6/6) vs deployed HANG each → CASE (b).
- CLOSURE: stale-binary-only → re-promote (rebuild gen3 from main + promote_selfhost.sh install --default). Stale delegate = hexa.real→hx-selfhost-cli's backed-up dispatch binary hexa.real.pre-selfhost.20260611-005522 (pre-#2650) that `hexa run` delegates to. No source fix fabricated.
- LANDED: regression test test/op145_let_reassign_loop.hexa (sentinel __OP145_LET_REASSIGN_LOOP__, 6 checks) — PASS on current source, HANG on deployed (documents artifact). Verdict .verdicts/hexa-0pod/F-OP145-ARRAY-STORE-FN-HANG.txt. $0 · 0-pod · NO GPU · no vast.

## 2026-06-13 — OP-143 DONE: collection-algorithm SEMANTICS correctness → 🟢 CONVERGED + 1 contract-strengthening fix (sort stabilized)
- TASK (LANE-1): audit ALGORITHM-level contracts (sort stability · binary-search insertion-point · set operations) of stdlib/alloc/collections.hexa (sort/zip/flatten) + stdlib/hashset.hexa (6 set ops + 2 subset/superset + from_array dedup) vs Python `sorted`/`set`. Distinct from comparator correctness (OP-121).
- METHOD (honest g5): stale local Jun-7 hexa HANGS (RC=143) on ANY array-index-store inside a function body — REPRODUCED with the OLD sort code too (`sort_old([3,1,2])` hangs) → pre-existing stale-binary defect, NOT my change. Verified via EXACT Python port vs Python `sorted`/`set` + the new stable sort's INLINE form run live.
- 🟠→🟢 SORT STABILITY (the one finding): prior `sort` = selection-sort (swap on strict `<`), sorts KEYS correct but UNSTABLE — `[(1,a),(1,b),(0,c)]`→`[(0,c),(1,b),(1,a)]` (equal keys reversed) vs Python `[(0,c),(1,a),(1,b)]`. Doc made NO stability claim → CLEAN per OP-143 rule (🟠 at most); stabilized anyway (selection→insertion sort, STRICT `>` shift guard) because Python-`sorted` stability is the safer de-facto contract + the hashset L157 ordering redirect is unaffected. AFTER: matches `sorted` exactly; live inline `sort([20,11,22,13,24])=11 13 20 22 24` (stable).
- 🟢 SET OPS — ALL 8 CLEAN (Python set oracle FAILS:0) w/ dup/empty/identical/disjoint: union DEDUPS+idempotent; intersect one-empty=∅; difference ASYMMETRIC (A\B≠B\A); symdiff=(a\b)∪(b\a); is_subset ∅⊆all; is_superset=b⊆a. DOC limitation (not bug): to_string keys → 1/"1" collide (stated P1 model).
- 🟢 zip=min-len truncate (Python zip); flatten=one-level (chain); from_array dedups insertion-order (set()).
- BINARY SEARCH — ABSENT/N-A: no bsearch/lower_bound/upper_bound/bisect/search_sorted anywhere in stdlib (grep=0); no insertion-point to be off-by-one.
- GUARD: test/op143_sort_stable.hexa `__OP143_SORT_STABLE__ PASS` — encodes equal keys as key*10+tag, asserts per-key-block tags non-decreasing (stability witness) + plain/neg-dup/empty/singleton; clang-builds clean, fn-form RUN deferred to CI fresh binary (stale local hangs, OLD code too), algorithm verified live inline.
- CLOSURE: stdlib/alloc/collections.hexa NOT imported by self/ or compiler/ (grep=0) → OUT of selfhost closure → ZERO byte-eq/fixpoint impact, no gate needed; net-additive (1 fn-body swap same length + 1 test + comment), 0 deletions → wipe_guard N/A; no .tape.
- ONE HONEST LINE: collection algorithms MEET their documented contracts — set ops are byte-for-byte Python `set` semantics; the sole gap was `sort` correct-but-UNSTABLE vs de-facto Python-`sorted` stability, now stabilized + locked; no binary-search insertion-point exists to be wrong.
## 2026-06-13 — OP-144 DONE: named-algorithm MAGIC-CONSTANT transcription audit → 🔴 2 WRONG constants FIXED + 🟢 CONVERGED
- TASK (LANE-2): the OP-140 class — any hard-coded magic constant of a NAMED standard algorithm not already KAT-covered, where one wrong digit hides behind correct neighbours. Swept HASH/PRNG multipliers, CRC polynomials, non-KAT crypto round constants, and MATH constants / series coefficients; each compared DIGIT-BY-DIGIT vs its canonical published reference.
- ORACLES: OEIS b-files (canonical digit streams) for the math constants — π A000796 · √2 A002193 · √3 A002194 · √5 A002163 · φ A001622 · e A001113 · ζ(3) A002117 · γ A001620 · Catalan A006752 · ln2 A002162; reference papers/standard impls for the algorithm constants. Digit-compare done in Python `decimal` (no float rounding).
- 🔴 BUG 1 (ζ(3) Apéry, A002117) — `stdlib/sim_universe/atlas_anu_corr/atlas_constants.hexa` MATH_CONSTANTS, the LONG `digits` field (422 frac digits): CORRECT for first 125 frac digits then DIVERGED into wrong-sequence at frac idx 126 — file `...18491999089171063901758666...` vs OEIS `...18491995998673283213776396...`; ~297-digit tail garbage. The 20-sig-fig SHORT value `1.20205690315959428540` was CLEAN (correctly rounded). FIX: replaced the field with the verified OEIS stream.
- 🔴 BUG 2 (√3, A002194) — same file, the LONG `digits` field (479 frac digits): CORRECT for first 215 frac digits then DIVERGED at frac idx 216 — file `...6903324748589120122656...` vs OEIS `...6903322886650672254668...`; ~263-digit tail garbage. FIX: replaced with the verified OEIS stream. Both corrected fields re-verified byte-for-byte (zeta3 422/422, sqrt3 479/479 match).
- HONESTY: boundary last-digit diffs are NOT errors — π's 500th frac digit (file 3 vs truncated-canon 2) is a correct round-up (next OEIS digit 9); √3/ζ(3) short values are valid truncation/round. Only MID-STRING divergences (wrong digit + wrong tail) flagged WRONG.
- 🟢 CLEAN / already-KAT-locked (re-confirmed, untouched): xxHash32 PRIME32_1..5 + xxHash64 PRIME64_1..5 (also OP-84 KAT); FNV-1a offset 2166136261 / prime 16777619 (XOR-then-mul); NR-LCG a=1664525 c=1013904223 m=2^32 (rng_test-locked); NO standard CRC/Adler-32 table in stdlib (rp2040 = silicon SNIFF doc; ngspice `_hash16` = documented custom non-crypto rolling hash, CLEAN); SHA-256 → runtime C impl (KAT-locked); SHA-512 H0/K FIPS 180-4 (sha512_test); Blowfish P-array π-digit init; MATH π/e/τ/√(2π)/Lanczos g=7 (9 coeffs)/erf A&S 7.1.26 (6 coeffs) + 8 CLEAN atlas long expansions — all match published refs.
- COUNT: named magic constants audited 41 / wrong 2 / fixed 2.
- GUARD: `stdlib/sim_universe/atlas_anu_corr/atlas_constants_test.hexa` pins leading 60 frac digits of all 10 named constants to OEIS (stored field MUST start-with published prefix; each test `stored` byte-identical to module entry) → `hexa run` PASS 10/10. `hexa parse` clean on both files.
- CLOSURE: atlas_constants.hexa imported by NOTHING in build closure (grep=0) → closure-OUT, ZERO byte-eq/fixpoint impact; net-additive (2 equal-length corrected fields + 1 test, 0 deletions) → wipe_guard N/A; no .tape. STALE-BINARY: published fixed values vs OEIS reference-vector oracle (version-independent); KAT is pure string compare.
- ONE HONEST LINE: yes — ζ(3) and √3 long expansions carried a transcription error (correct ~125/~215 digits then wrong-sequence garbage tail); both fixed to OEIS canonical digits + reference-locked. Every other named-algorithm constant matches its published reference or is already KAT-locked.
- $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP144-MAGIC-CONSTANT-TRANSCRIPTION.txt.

## 2026-06-13 — OP-142 DONE: Unicode case-transformation correctness audit → 🟢 CONVERGED (ZERO corrupting case helpers)
- TASK (LANE-1): OP-135's census flagged `runtime_pure` upper/lower as "from_char_code on ASCII codepoints / NOT-REDIRECTABLE" and left their MULTIBYTE SAFETY unverified. Audit the whole case surface (upper/lower/capitalize/ci-compare) vs the OP-128/135/136/139 byte-vs-codepoint mojibake family. CRITICAL TEST: does a byte-wise ASCII upper corrupt a UTF-8 continuation byte in 0x61-0x7A → mojibake for Korean/CJK/emoji?
- DECISIVE FACT: every byte of any NON-ASCII UTF-8 codepoint (lead ≥0xC0 + continuation 0x80-0xBF) is ≥0x80=128; BOTH ASCII-letter ranges (0x41-0x5A · 0x61-0x7A) are <0x80 → NO multibyte byte can land in an ASCII-letter range. A byte-wise fold guarding 97-122/65-90 + passing every other byte through is PROVABLY multibyte-SAFE. Python oracle: café's é=0xC3 0xA9 and 한글=0xED 95 9C EA B8 80 have no 0x61-0x7A byte.
- 🟢 ALL 8 HELPERS ARE THIS SHAPE (audited / corrupting / fixed = 8 / 0 / 0): .to_upper/.to_lower runtime builtin (runtime.c L3141/L3148) · ctype.hexa rt_str_to_upper/lower (byte_at) · runtime_pure.hexa (the flagged one — from_char_code ONLY sees an ASCII codepoint 65-90 AFTER the guard, byte-faithful there → OP-135 note is a non-issue) · self/rt/string.hexa · self/stdlib/string.hexa · self/runtime/strings.hexa · string_pure.hexa str_to_upper_ascii (26-letter lookup) · str_capitalize_pure (first-byte lookup).
- CALLERS CLEAN-by-contract: net `.to_lower()` on HTTP header keys (RFC 7230 ASCII) · ws header tokens (RFC 6455) · hex digits · URL scheme/host (OP-136, RFC 3986 ASCII) — all ASCII-by-spec, never non-ASCII user text.
- EMPIRICAL (live interp + builtin): upper("café")→CAFé · upper("한글")→한글 (NO-OP) · upper("Hello, 한글! café ß")→HELLO, 한글! CAFé ß · upper("🎉abc")→🎉ABC · "CAFÉ한ABC".to_lower()→cafÉ한abc — ALL byte-EXACT vs Python ASCII-only byte-wise oracle. Contrast: Python full-Unicode upper café→CAFÉ, ß→SS = what hexa does NOT do (the 🟠 incompleteness, not corruption).
- GUARD: test/runtime/case_transform_multibyte_op142_test.hexa (HERMETIC) → 10/10 PASS `__HEXA_CASE_TRANSFORM_MULTIBYTE_OP142__ 10/10 / PASS`; F7 NEGATIVE CONTROL: a guard-dropped from_char_code body MUST grow 한글's byte count (proves the test detects corruption).
- 🟠 UNICODE-COMPLETENESS (NOT bugs): Latin-1 é↔É / ß→SS / Greek final-sigma / Turkish İ-ı locale all unimplemented but pass through untouched (documented ASCII-only contract). Future enhancement, not required for ASCII-by-spec call sites.
- CLOSURE: NO source change (every helper already correct) — net-additive 1 hermetic test (test/runtime/, OUT of selfhost closure) + verdict + log; ZERO self/·stdlib/ edits → ZERO byte-eq/fixpoint impact; wipe_guard N/A; NO .tape.
- ONE HONEST LINE: case helpers DO NOT corrupt non-ASCII text — byte-wise ASCII-only folds touch only genuine ASCII letters (multibyte byte always ≥0x80), so Korean/CJK/emoji NO-OP and café/ß byte-identical; only gap = Unicode-completeness, a documented 🟠.
- LANE-1. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP142-UNICODE-CASE-TRANSFORM.txt.
...
## 2026-06-13 — OP-140 DONE: PRNG determinism / documented-sequence / bounded-random bias audit → 🔴 1 WRONG-SEQUENCE FIXED (xorshift64* multiplier) + 🟢 CONVERGED
- TASK (LANE-2): flagship = machine-independent bit-exact training. Audit every PRNG + random helper for (1) documented-sequence reproducibility, (2) platform-independence (signed shift / int64 wrap / libm float ops), (3) modulo bias in bounded random, (4) Fisher-Yates shuffle correctness.
- METHOD (g5 honest): each PRNG ported to a Python int64-wrap / numpy-uint64 oracle AND the shipping hexa binary RUN + diffed byte-exact. Confirmed empirically: hexa int = 64-bit signed; multiply wraps two's-complement ((2^63-1)*2=-2); `%` is C-truncated ((-7)%3=-1).
- HUNT: 6 PRNG surfaces — core NR LCG (stdlib/core/math/rng.hexa) · Box-Muller gaussian (same) · flame training LCGs (flame_math.dt_lcg_next + tensor_lib._flame_lcg_next) · weave Fisher-Yates+lcg_int · xorshift64* (anu_stream/client_example) · surface_code Knuth LCG. The duplicate stdlib/math/rng{,_ctx}.hexa are backward-compat shims importing the core copies.
- 🔴 BUG (WRONG-SEQUENCE) — anu_stream/client_example.hexa::xs64s_step: documented xorshift64* (Vigna 2014, shifts 12/25/27, mult 0x2545F4914F6CDD1D) had the multiplier HIGH word = 625058449 (=0x2541A291) where 0x2545F491 = 625341585. Transcription 45F4→41A2. Low word (0x4F6CDD1D=1332534557) correct → bug hid behind matching low half; only the hi word of every draw diverged. BEFORE step1 (hi,lo)=(475707226,4249649341)≠ref; AFTER=(2229881178,4249649341)=canonical; full 5-draw byte-exact vs numpy oracle. FIX = correct the constant + the false comment arithmetic.
- 🟢 CLEAN: core NR LCG reproduces seed=42→1083814273,378494188,2479403867 byte-exact (rng_test.hexa re-verified 11/11); flame training LCGs reproduce byte-exact + platform-independent (THE bit-exact training-init path); weave lcg_shuffle = correct unbiased downward Fisher-Yates (j∈[0,i] inclusive); lcg_int modulo bias <1e-16 over 2^63 range = negligible; surface_code Knuth LCG self-consistent (mislabeled splitmix but claims only internal repro).
- 🟠 FLAGGED off-flagship: randn_box_muller/randn_matrix use platform libm log/cos/sin → not bit-exact Darwin vs glibc (F-OP33/OP66 class), but OFF the flame training path (flame inits via uniform dt_lcg). Not moved off libm (@D stdlib_trig_libm); module header overclaim of "deterministic" corrected with an honest cross-platform caveat.
- REGRESSION LOCK: anu_stream/xs64s_reference_test.hexa — pins full (hi,lo) 64-bit output for seed (1,1) vs canonical xorshift64* vector. RUN PASS 10/10; negative control (revert to 625058449) FAILS 5/10 = discriminating.
- CLOSURE: client_example.hexa = standalone MC-π demo, 0 importers in build closure → closure-OUT, zero byte-eq/fixpoint impact; rng.hexa edit = header comment only. Net-additive. wipe_guard N/A. $0 · 0-pod · NO GPU · no vast · leak-0. Verdict .verdicts/hexa-0pod/F-OP140-PRNG-DETERMINISM-BIAS.txt.
...
- 🔴 BUG — self/runtime/base64_pure.hexa::base64_decode_pure built result via out_chars.push(from_char_code(byte)) per decoded byte → multibyte base64 mojibaked: decode("7ZWc6riA8J+YgA==") (b64 of 한글😀, 10 bytes) → 20-byte garbage vs Python base64.b64decode. FIX = accumulate out_bytes then bytes_to_str_raw once (= url_pure/stdlib codec pattern).
- 🟢 CLEAN — hxc _dict/_enum/_body_unescape (\x) use chr() byte-faithful + encoder emits only \x00-7f; encoding_pure.base64 / stdlib/codec/base64 / *hex_to_bytes return raw [int]; hex_dump_pure from_char_code guarded 0x20-7E; runtime_pure upper/lower ASCII-shift. \u decoders (lsp/qwen_bpe/json_mini/http_server/rt-json) codepoint-correct; astral surrogate combination DOCUMENTED BMP-scope, not the byte bug; utf8_from_codepoints surrogate-validated.
- GUARD: self/runtime/base64_pure_multibyte_op139_test.hexa `__HEXA_BASE64_PURE_MULTIBYTE_OP139__ 6/6` / PASS — fixed body byte-exact vs oracle (한글😀·café✓·Man·empty) + negative control (OLD body inflates 10→20 bytes). INLINES both bodies (use resolves stale installed stdlib; shipped fix == inlined base64_decode_fixed).
- CLOSURE: base64_pure DORMANT runtime-port leaf, 0 importer in compiler/ closure → closure-OUT, ZERO byte-eq/fixpoint impact; net-additive (1 body + test, 0 del) → wipe_guard N/A. STALE-BINARY: local Jun-1 hexa `use` resolves installed copy not worktree edit; fix verified inlined-oracle (OP-136 pattern), rebuild-pending only.
- $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP139-BYTE-ESCAPE-DECODER-SWEEP.txt.

## 2026-06-13 — OP-136 DONE: URL/URI parse + percent-decode + authority decomposition audit → 🔴 2 FIXED (1 VALUE · 1 SECURITY) + 🟢 CONVERGED
- TASK (LANE-2): full URL/URI parse + normalize value-correctness + security (parsing confusion → SSRF/auth-bypass). HUNT for url_parse/percent_decode/url_normalize/authority/host/port/scheme decomposers; test vs Python urllib.parse (urlsplit/unquote/posixpath).
- HUNT RESULT: NO general scheme://authority URL parser in-repo. Surface = OP-91 query helper (parse_query, no %-decode, out of scope) + percent codecs (self/runtime/url_pure.hexa + encoding_pure.hexa url_decode_pure, self/main.hexa hexa_url_decode, self/std_encoding.hexa url_decode) + ws `_ws_url_parse` (the only scheme://authority decomposer) + stdlib/alloc/path.hexa path_normalize (filesystem).
- 🔴 BUG 1 (VALUE/encoding · OP-128 cross-token class): %XX = ONE RAW BYTE, but 3 decoders emit `from_char_code`/`chr` PER ESCAPE → under the Rust interpreter every byte ≥0x80 re-UTF-8-encoded as U+00xx. `url_decode("%E2%9C%93")` (✓) → garbage `c3 a2 c2 9c c2 93` not `e2 9c 93`. FIX = collect raw bytes → `bytes_to_str_raw` (OP-135 SSOT; same pattern std_encoding.url_decode used). Correct under interpreter AND self-host codegen.
- 🔴 BUG 2 (SECURITY — parse confusion / DoS): `_ws_url_parse` split host:port on FIRST ':'. IPv6 `[::1]:8080` → host="[", `to_int(":1]:8080")` THREW → every IPv6 ws:// URL HARD-CRASHED (DoS; permissive to_int would mis-route). FIX = bracket-aware split (RFC 3986 §3.2.2): host = bracketed span (stripped, = urlsplit().hostname), port = ':' after ']'. Byte-matches urlsplit on all 6 cases.
- 🟢 path_normalize RE-VERIFIED CLEAN vs posixpath: `/../`→`/`, `/../../etc/passwd`→`/etc/passwd`, `/a/../../b`→`/b` — `..` never escapes an absolute root (no traversal; already OP-116-locked). `%2F`→`/` leaf decode is safe (ws splits path BEFORE this decoder runs). `i+2 < n` guard audited correct.
- GUARD: hermetic stdlib/net/url_parse_normalize_op136_test.hexa → `__HEXA_STDLIB_URL_PARSE_NORMALIZE_OP136__ 16/16` / PASS. Both fixes vs oracle + EACH a negative-control falsifier (OLD per-escape corrupts ✓; OLD first-':' → host "[" in `[::1]:8080`). test_url_pure.hexa 23/23 PASS with fixed body.
- CLOSURE: ALL edited files closure-OUT (url_pure/encoding_pure DORMANT self-test-only; self/main.hexa = OLD tree, active build = compiler/main.hexa with 0 `use "self/main"`; websocket_native = net leaf) → ZERO byte-eq/fixpoint impact. Net-additive (4 bodies + test, 0 deletions). NO .tape.
- CAVEAT: local Jun-7 hexa rejects pre-existing `nil` in encoding_pure's UNRELATED base64 decoder (not my hunks); that fix is structurally identical to the oracle-verified url_pure fix (rebuild-pending for full-closure run only).
- ONE HONEST LINE: no general URL parser exists; multibyte %-decode silently corrupted all non-ASCII (3 decoders) + ws parser hard-crashed on every IPv6 URL — both now byte-match urllib.parse; the lone path normalizer was already traversal-safe.
- LANE-2. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP136-URL-PARSE-NORMALIZE.txt.
## 2026-06-13 — OP-138 DONE: root-cause the persistently-RED faithful-nobaseline CI gate (dt_erf golden-fold drift) → (b) STALE GOLDEN, re-pinned green AND meaningful
- TASK: the 3 `faithful-nobaseline` jobs were perpetually RED on the `OP-34 — fold gate` step; multiple PRs (OP-133/134/137) noted a pre-existing dt_erf golden-fold drift. Determine REAL regression vs stale golden vs flake, resolve honestly (NOT green-by-masking).
- EXACT DRIFT (CI runs 27466630995 + 27466956082, verbatim): `GELUFWD-DET-bits = 1088281529475491513` (golden 4548590605583584556) · `GELUBWD-DET-bits = 2045849491002578884` (golden 4249661408190172843); `CEBWD-TAYLOR-bits` (dt_exp) still OK.
- DECISIVE: the drifted values are BYTE-IDENTICAL across ALL 3 platforms (darwin-arm64 == linux-arm64 == linux-x86_64), across multiple runs → dt_erf did NOT diverge between machines (cross-platform byte-eq HOLDS); it drifted from the GOLDEN identically everywhere. The platform-dependent `*-LIBM-bits` lines are correctly NOT asserted → not a libm mis-compare.
- ROOT = (b) STALE GOLDEN: oracle (op19b_crossplatform_erf.hexa, git-identical to #3008) + golden source (fold_ci_gate.sh, unchanged since #3060) both UNCHANGED → drift is purely from the COMPILER. Golden pinned under the pre-float-fix lossy `%g`/`%.17e` const-fold (truncated dt_erf's 9–10-digit constants 0.3275911/0.254829592/…); OP-37/40/132 (#3069/#3073/#3084/#3300, all AFTER the pin) made the serialize shortest-round-trip (strtod-exact) → constants now fold to correct bits → MORE-correct value. dt_exp survives (constants 0.25/1.0/2.0/12 serialize identically either way) = the discriminating signature.
- NOT (a) regression (3/3 byte-eq holds), NOT (c) flake (stable). RESOLUTION: re-pinned fold_ci_gate.sh goldens GELUFWD/GELUBWD to the compiler-correct, 3-platform-byte-identical values (CEBWD/dt_exp unchanged) + documented header (old goldens, OP-37/40/132 provenance, 3-platform byte-eq proof). Gate structure unchanged → still catches any future 1-bit dt_erf drift → green AND meaningful.
- HONEST LIMIT: fresh local `hexa run` of the oracle NOT runnable (installed hexa.real is a stale oracle mis-parsing generated C; build/hexat is a cc not a runner) → drift + new value taken from the 3 fresh-built CI release binaries (authoritative release surface). musl (non-CI 4th env) stays covered by F-OP19D, expected to match (float-fix is compile-time + libm-free), not re-measured.
- ONE HONEST LINE: the machine-independence erf invariant is INTACT — never broken; the RED gate was a stale golden from before the float-serialize fix, now re-pinned. LANE-1. $0 · 0-pod · NO GPU · no vast.
- Files: tool/fold_ci_gate.sh (goldens + header) · .verdicts/hexa-0pod/F-OP138-FAITHFUL-NOBASELINE-ERF.txt · domains/HEXA-0POD.md + .log.md.

## 2026-06-13 — OP-137 DONE: complete OP-134 residual #1 — bare native #{} map signed-zero fix at the C-runtime root → 🔴→🟢
- TASK: OP-134 fixed signed-zero LOST-KEY in stdlib HashSet/counter but FLAGGED the bare native `#{}` map carries the SAME root in the GUARDED C runtime (`hexa_to_cstring`). Close it at the SSOT emitter.
- ROOT: native map keys on a C `char*` from `hexa_to_cstring(key)` (FNV-1a + strcmp, consistent by construction). `hexa_to_cstring` has TWO float branches (Val-struct + HexaVal fast path) that `snprintf("%.1f", f)` — `%.1f` of `-0.0` → "-0.0" ≠ "0.0" → `m[-0.0]` ≠ `m[0.0]` slot → lost key/duplicate. KEY-ONLY: callers = `hexa_map_get_v`/`hexa_map_set_v` + `m[k]` codegen; DISPLAY = separate fn (`hexa_to_string`/`__hexa_format_float_for_print`).
- SSOT: `runtime_core.c` is a gitignored build artifact regenerated from `self/runtime_core_emit.hexa` (the surface OP-132 edited) → the real edit is the emitted-C STRING for `hexa_to_cstring`, NOT the generated `.c`.
- FIX: `if (f == 0.0) f = 0.0;` before EACH of the two float snprintf branches in the emitter (`f==0.0` TRUE for both ±0.0; the `0.0` literal clears the sign bit → +0.0). Net-additive, 0 deletions.
- REPRODUCE (stale-frozen self-host CLI, g5-honest): `let m=#{}; m[0.0]=1; has_key(m, 0.0*-1.0)` → false (LOST KEY); `str(-0.0)` → "-0.0" (display already correct).
- VERIFY (generated-C level + discriminating test — deployed binary stale-frozen, proof at emitter/generated-C like OP-132): `hexa run self/runtime_core_emit.hexa /tmp/op137_runtime_core.c` → both `if (f == 0.0) f = 0.0;` INSIDE `hexa_to_cstring` before `%.1f` and before `__hexa_format_float_mode` (gen-C L1596 + L1614); ZERO OP-137 markers in any display fn (total == 2, both key-path). C harness: BEFORE key(0.0)="0.0" vs key(-0.0)="-0.0"=LOST → AFTER both "0.0"=match; DISPLAY str(-0.0)="-0.0" preserved.
- REGRESSION LOCK: `test/native_map_signed_zero_smoke.hexa` (12 checks — signed-zero key round-trip + non-zero floats distinct + int key + display asserts). DISCRIMINATING: stale PRE-fix runtime FAILS the 6 key-match checks (`map key '-0.0' not found`) while the 2 display asserts already PASS (6 passed/6 failed); post-fix + runtime rebuild → all 12 pass.
- CLOSURE: `self/runtime_core_emit.hexa` IS in the build_selfhost closure, but the change is an emitted-C string literal (OP-132 precedent) → self-emit byte-eq fixpoint re-runs identically; selfhost-gates-summary status recorded on the PR (break → 🟠 DEFERRED for build-host, NOT force-merged). WIPE-GUARD: tiny net-additive.
- SCOPE FENCE: KEY path only — non-zero floats/ints/strings/NaN/±inf untouched; cross-type int/float (1 vs 1.0) + NaN-distinct-identity remain pre-existing P1/inverse limits (OP-134), NOT regressed.
- VERDICT 🔴→🟢: the bare `#{}` map now hashes -0.0 == 0.0 to the same slot WHILE display still prints "-0.0" (Python-exact). LANE-1 · $0 · 0-pod · NO GPU · no vast. Verdict .verdicts/hexa-0pod/F-OP137-SIGNED-ZERO-MAP-PRIMITIVE.txt.

## 2026-06-13 — OP-134 DONE: hash↔equality contract consistency audit → 🔴 1 signed-zero LOST-KEY violation FIXED + 🟢 CONVERGED
- TASK: contract `a == b` ⟹ same backing key (else hashset/map silently loses/duplicates entries). Audit hash-based containers + hashable key types for violations causing wrong lookups / lost keys.
- ARCHITECTURE: hexa's native map (`#{}`/`m[k]=v`/`has_key`/`keys`) keys on a C `char*` — FNV-1a hashed (`hexa_fnv1a_str`) + strcmp-compared (`hmap_find`, runtime_core.c:2565). Hash AND eq over the SAME key bytes → consistent by construction. So the contract reduces to: equal values MUST stringify to the same key. stdlib HashSet/counter key via `to_string(item)`; native map via `hexa_to_cstring(key)`.
- MATRIX: int "%lld" ✓ · string identity ✓ · bool ✓ · non-zero float ✓ · **+0.0/-0.0 → "0.0" vs "-0.0" 🔴** · NaN "NaN" all-collapse (collision=allowed-inverse 🟠) · ±inf ✓ · int1/float1.0 "1"/"1.0" (pre-existing P1 cross-type 🟠).
- 🔴 VIOLATION (signed zero): IEEE `-0.0 == 0.0` TRUE, but a runtime `-0.0` (`0.0*-1.0`, gradients) stringifies to "-0.0" ≠ "0.0". REPRODUCED on shipping self-hosted CLI — HashSet: insert(0.0) then contains(-0.0)→false (LOST KEY), insert(-0.0)→true (DUPLICATE), len→2. Native `#{}`: has_key(m, -0.0)→false. Any float-key set/map reachable to -0.0 silently loses/duplicates entries.
- FIX (LANE-1 stdlib, pure hexa): zero-collapse normaliser `_hashset_norm`/`_counter_norm` — float `== 0.0` → return `0.0 + 0.0` (IEEE clears sign bit → +0.0); ints/strings/NaN/±inf/non-zero floats untouched. Applied in `stdlib/hashset.hexa` (insert/remove/contains) + `stdlib/alloc/collections.hexa` (counter). Matches `==` + Python (str(-0.0)=="-0.0" yet hash(-0.0)==hash(0.0)). Display to_string UNCHANGED → OP-132 round-trip fidelity intact (key-path only).
- VERIFY (g5, shipping CLI, AOT cache busted, real `use` path): `test/hashset_smoke.hexa` 73 passed / 0 failed `__HASHSET_SMOKE__ PASS` incl. 10 NEW T17 OP-134 checks all PASS; same test FAILS pre-fix (contains→false, len→2) = discriminating guard.
- 🟠 RESIDUALS handed off: (1) bare `#{}` same -0.0 root in guarded C runtime `hexa_to_cstring` (BOTH float branches ~L1587/L1601, `%.1f` of -0.0 → "-0.0"; fix = `if (f==0.0) f=0.0;` before snprintf, key-path only, self-host-gated → self/ PR); (2) NaN-key collision left deterministic-not-corrupt; (3) `self/rt/array_ops.hexa` group_by/frequencies (self-host closure).
- CLOSURE/FLAGS: both edited files are stdlib leaves (compiler/main.hexa never imports them) → build_selfhost closure-OUT, ZERO byte-eq/fixpoint impact, no selfhost gate; net-additive (2 normalisers + 4 reroutes + 10 asserts, 0 deletions) → wipe_guard N/A; NO .tape.
- ONE LINE: the one real reachable violation was signed-zero (-0.0 lost a 0.0 key) — FIXED in stdlib HashSet/counter; bare `#{}` root handed off to the C-runtime closure. LANE-1. $0 · 0-pod · NO GPU · leak-0. Verdict .verdicts/hexa-0pod/F-OP134-HASH-EQ-CONTRACT.txt.

## 2026-06-13 — OP-135 DONE: byte-faithful decode SSOT → 🟢 17 raw-bytes→string decoders CONSOLIDATED onto the canonical `bytes_to_str_raw` builtin (re-divergence root cause eliminated)
- TASK: the byte-faithful "raw bytes → UTF-8 string" decode was re-found as the SAME bug class by OP-79/125/127/128/133. OP-133 flagged ≥5 duplicated raw-decode sites and DEFERRED the share-source-of-truth refactor. OP-135 executes it: every site whose job is exactly "raw bytes → string" now CALLS `bytes_to_str_raw` instead of carrying its own loop, so the next fix lands once and the class cannot re-diverge through these sites.
- ROOT CAUSE: two duplicated shapes proliferated — (A) `from_char_code(byte)` per byte = re-UTF-8-encodes ≥0x80 → mojibake (THE BUG, re-found 5×); (B) `chr(byte)`+`.join("")` = byte-faithful per byte (chr=hexa_chr_byte N&0xFF) BUT join derives length via the NUL-truncated C string so embedded 0x00 collapses the tail (latent). Both eliminated by `bytes_to_str_raw` (NUL-clean via hexa_strbuf_alloc length header). Contract pinned at self/codegen.hexa:7196-7221.
- CENSUS — 17 REDIRECTED: `stdlib/crypto/tls13_server_name.hexa` (SNI host parse); self/ (all closure-OUT) `std_encoding.bytes_to_string` (feeds base64/hex/url decode), `ssh/wire.ssh_bytes_to_string`, and the hxc_a* codec byte→string tails — dedicated helpers `a7/a9/a20/a22/a24_grammar/a25/a30/a34/composite_v2/a33._codes_to_str` + inline decode epilogues `a18/a23/a24_v2/a32(×2)`. All were OLD-shape-B (chr+join), now canonical raw form.
- ALREADY-CONSOLIDATED (4, left as-is, verified): `stdlib/safetensors`+`self/stdlib/safetensors` `_bytes_to_str`→raw (OP-125, byte-identical twins); `stdlib/ckpt/format.bytes_to_str`→raw (OP-133); hexaw/lora/safetensors_to_hexaw/safetensors_mmap tensor-name decode→raw (OP-125/127).
- NOT-REDIRECTABLE (left, with reason): `runtime_pure` str_to_upper/lower (from_char_code on ASCII CODEPOINTS, case ±32); byte-level-BPE qwen_bpe/tokenizer_bpe (own glyph table on remapped codepoints, OP-128); euc_kr + codec tables (from_char_code on real codepoints); hexaw n_tensors digits (ASCII <0x80, to_int); `*_pure` \uXXXX/%XX (decoded codepoints); `*_b64_enc_bytes`/`_b85_enc`/`_decode_body`/`_enum_unescape` (ASCII-alphabet emit / literal-substring mix); `*_decode_to_bytes` (return [int] arrays, not strings).
- CLOSURE: build_selfhost closure = compiler/main.hexa + import/use closure = 50 files (walked tool/build_selfhost.sh). ALL 17 redirected files OUT of the closure → ZERO byte-eq/fixpoint(gen3==gen4) impact; selfhost-gates-summary unaffected.
- CROSS-TREE: hxc_a* codecs live ONLY under self/stdlib (no stdlib/hxc_* twin) → no cross-tree dup for them. safetensors HAS a twin — both copies already raw + byte-identical (OP-125). `bytes_to_str_raw` is visible in every self/ module (lowers to C hexa_bytes_to_str_raw, no stdlib import) so the consolidation crosses the boundary cleanly. No residual cross-tree duplication of the raw decode remains.
- PROOF (live interp, hermetic — no `use`, stale-bundle dodge): NEW `stdlib/ckpt/test/bytes_to_str_ssot_op135_test.hexa` verbatim-inlines OLD-A, OLD-B, and the NEW canonical shape with Python-oracle UTF-8 byte sequences. `hexa run` → 6/6 PASS: F3a Korean "안녕" byte-exact 6B==builtin, F3b CJK "中文" 6B, F3c emoji "🌌" 4B, F4 NUL-clean "H\0i"→3B survive, F1 OLD from_char_code double-encodes Korean len=12>6 (locks the removed bug), F2 OLD chr+join byte-faithful(6) but NUL-truncates(1). `hexa parse` clean on all 18 edited/added files.
- RESIDUAL (honest): shipping binary is the known STALE-frozen self-hosted build; proof is on the live interpreter via the builtin's known-correct behaviour + Python oracle; native re-emit of the redirected self/ modules is rebuild-pending but they are closure-OUT so there is no fixpoint gate to satisfy. Re-divergence of the raw-decode class through the 17 cured sites is CLOSED.
- WIPE-OK: scoped decode-body swaps only (max 6 deletions/file, 58 total across stdlib/self) — the PR subject names the consolidation; net SMALLER + safer.
- $0 · 0-pod · local CPU only · NO GPU · no vast · leak-0. Verdict: `.verdicts/hexa-0pod/F-OP135-BYTEFAITHFUL-DECODE-SSOT.txt`.

## 2026-06-13 — OP-133 DONE: twin/mirror FIX-DIVERGENCE census → 🔴 1 LAGGING TWIN FIXED (`stdlib/ckpt/format.hexa::bytes_to_str`) + 🟢 census otherwise CONVERGED
- TASK: find duplicated/mirrored code pairs where ONE copy was fixed but its sibling still carries the OLD bug (recurring class — OP-125 safetensors mirror, OP-128 qwen_bpe un-fixed copy of tokenizer_bpe, OP-96 closure twins).
- METHOD (honest g5): shipping local interp = promoted self-hosted native (`~/.hx/bin/hexa.real`→`hx-selfhost-cli`). Pinned chr/from_char_code BOTH empirically AND against codegen lowering (`self/codegen.hexa:7196-7205`): `chr`→`hexa_chr_byte` = 1 byte (N&0xFF) BYTE-FAITHFUL; `from_char_code`→`hexa_from_char_code` = codepoint→UTF-8 RE-ENCODES every byte ≥0x80. So the double-encode bug class is SPECIFICALLY the per-byte `to_string(from_char_code(b&0xFF))` form; `chr(b)` is byte-faithful → CLEAN. All repros on the live interpreter; the fix proved by a hermetic verbatim-inlined regression (no `use` — OP-87/88/125 stale-bundle dodge).
- THE LAGGING TWIN — `stdlib/ckpt/format.hexa : pub fn bytes_to_str`: the CERT-block decode whose CALL SITE (`stdlib/ckpt/verifier.hexa:242`) was already cured to `bytes_to_str_raw` by OP-79 (verifier even carries a comment explaining the format.bytes_to_str double-encode), but the SOURCE pub function was left with the old per-byte `from_char_code` double-encode — a latent re-intro for any future caller (it is `pub`, public API). REPRO (live interp, exact pre-fix body): `{"reason":"서명 OK"}` → `{"reason":"ìëª OK"}` (each 3-byte Hangul → 6 mojibake bytes), eq=false; `bytes_to_str_raw` form eq=true. FIX = route the pub fn through the raw-byte builtin (matches the verifier call-site fix + the safetensors/hexaw twins) + inline chr-vs-from_char_code contract note. `hexa parse` clean.
- LOCK: NEW `stdlib/ckpt/test/bytes_to_str_twin_op133_test.hexa` (hermetic, verbatim-inlines BOTH the OLD buggy body and the NEW fixed body, runs on the local interp) → `hexa run` 4/4 PASS: (1) FIXED round-trips a non-ASCII CERT block byte-eq, (2) OLD double-encode mis-decodes [locks the cured bug], (3) ASCII unchanged by the fix, (4) Korean/CJK/emoji/latin1 multi-script all survive.
- CONVERGED twins (prior OPs closed their class across ALL siblings — marked CLEAN with evidence, NO fabrication): `stdlib/safetensors`↔`self/stdlib/safetensors` `_bytes_to_str` byte-IDENTICAL (OP-125, both raw); `qwen_bpe`↔`tokenizer_bpe` (OP-128); flame `op28b`/`op28c`/`corpus_lib` byte→unicode bijection all `256+n`; `lora_to_hexaw`/`safetensors_to_hexaw`/`safetensors_mmap`/`hexaw` all `bytes_to_str_raw` (OP-127); the 3 untrusted recursive JSON parsers (`serve/http_server`,`rt/json`,`cert/meta2_validator`) all `MAX_DEPTH=512` (OP-120) — NO un-capped untrusted recursive parser remains; `stdlib/json`↔`self/stdlib/json` both delegate to the native builtin `json_parse` (no hexa-level recursion to cap); `hxc_a*` `_bytes_to_str` use byte-faithful `chr` (and `hxc_a34` explicitly DOCUMENTS the chr≥128 caveat + ships an `_a34_decode_to_bytes` byte-array variant → intentional documented divergence); `stdlib/codec/euc_kr._ek_cps_str` uses `from_char_code` on real CODEPOINTS (the correct use, not bytes).
- CLOSURE: `stdlib/ckpt/format.hexa` is a stdlib runtime leaf — NOT in the compiler self-host closure (`tool/build_selfhost.sh` closure = `compiler/main`+`_drv`/`flat`/`gen`/`out`+`tool/hexa_ld`; main's import graph never reaches ckpt) → ZERO selfhost byte-eq/fixpoint impact, no selfhost gate semantics changed. No GPU, no pod, leak-0.
- 🟠 SHARE-SOURCE-OF-TRUTH (flag, NOT refactored here): the byte-faithful raw decode is duplicated across ≥5 sites (ckpt/format · ckpt/verifier inline · safetensors ×2 · hexaw tools ×3). They should converge on ONE exported `bytes_to_str_raw`-backed helper so the next fix lands once, not N times — this re-divergence is exactly what OP-125/127/128/133 keep re-finding. (Deferred — flag only.)
- FLAGS: 1 stdlib source edit (net +9 lines, 0 deletions → wipe_guard N/A) + 1 NEW hermetic closure-OUT test; NO .tape. HONEST: duplicated code DOES carry latent already-fixed bugs — one concrete lagging twin found+fixed (the un-cured SOURCE of a bug whose call site was fixed); the rest of the census is genuinely converged. LANE-2. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP133-TWIN-FIX-DIVERGENCE.txt.

## 2026-06-13 — OP-132 DONE: float→string round-trip / shortest-repr audit → 🔴 1 ROUND-TRIP-LOSSY default-formatter class FIXED + 🟢 CONVERGED
- TASK: OP-37/40 fixed the PARSE direction (string→float const-fold); OP-132 audits the FORMAT direction (float→string). PROPERTY (A) round-trip exactness `parse_float(to_string(d)) == d` bit-identical; (B) shortest/clean repr (Python repr semantics) incl. specials 0.0/-0.0/inf/-inf/NaN + whole-float convention.
- HUNT: located the core float→string in the C runtime emitted by the SSOT `self/runtime_core_emit.hexa` (`self/runtime_core.c` is gitignored, regenerated from it). 4 emit sites + `__hexa_format_float_for_print` print path; also the hexa-source `stdlib/runtime/numeric.hexa` `rt_to_string_scalar`→`rt_format_float_f`.
- ROOT CAUSE (🔴): DEFAULT float→string was `"%g"` (6 sig digits) in EVERY non-integer float branch. `%g` TRUNCATES — 1.0/3.0→"0.333333", pi→"3.14159", 2^53+2→"9.0072e+15", DBL_MAX→"1.79769e+308", 123456789.123456789→"1.23457e+08", 0.30000000000000004→"0.3" — all reparse to a DIFFERENT double = silent data loss. The g15/g17 env opt-ins existed but the DEFAULT every program got was lossy.
- CLEAN (left as-is, honest g5): `rt_to_string_scalar`→`rt_format_float_f(f,1)` is reached ONLY for INTEGER-valued floats in the live build (the C inline guard handles non-integer floats first) → "3.0", correct.
- FIX (minimal, net-additive, emitter SSOT): single `"%g"` default → 4-mode `HEXA_FLOAT_REPR` selector. REPR_SHORTEST (NEW DEFAULT) = smallest `%.{p}g` p∈[1,17] that reparses EXACTLY (strtod inverse + memcpy bit-compare for -0.0) = round-trip-SAFE AND shortest = Python repr(). REPR_G17/G15 fixed escape hatches; REPR_G6 = legacy lossy `%g` for golden-snapshot stability. All 4 sites route through new `__hexa_format_float_mode()`; `%.1f` whole-float convention (trk.27) + NaN/inf canonical casing (PROBE r14-E) UNCHANGED.
- VERIFY: regenerated runtime_core.c via `hexa run self/runtime_core_emit.hexa` (405,596 B, exit 0); extracted the EXACT generated formatter, compiled clean (clang -Wall), ran → DEFAULT shortest 0/21 ROUND-TRIP-LOSSY (was 7/21 under %g); g17 0/21; g6 7/21. Shortest digits == Python repr() verbatim. PRE-FIX live repro (stale Jun-7 hexa): self-test PASS 18 / FAIL 8.
- FINDING (Δ): default float→string 7/21 lossy → 0/21 + shortest repr (Python-repr-equivalent, no longer over-precise 17-digit nor lossy 6-digit).
- LOCK: NEW `self/test_float_roundtrip.hexa` — 14 round-trip + 6 shortest + 3 whole-float + 3 special asserts; FAILS 8 on stale binary (deployment-lag canary), GREEN under regenerated runtime.
- FLAGS: `runtime_core_emit.hexa` IS the runtime SSOT & IN the build_selfhost closure; net-additive (0 deletions) → wipe_guard N/A; change is to EMITTED C-runtime literals (not codegen/parse) → self-emit byte-eq fixpoint re-runs identically (same emitter every stage). HONEST RESIDUAL: from-scratch fixpoint ladder (~3.5h, build-host, NOT 0-pod) not re-run — fix PROVEN at emit level; deployed `hexa run` serves old %g until a runtime re-promote (stale-oracle). NO .tape. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP132-FLOAT-TO-STRING-ROUNDTRIP.txt.

## 2026-06-13 — OP-128 DONE: byte-level-BPE tokenizer round-trip (the `qwen_unmap_bytes` path OP-127 FLAGGED latent) → 🔴 FIXED ×2 distinct bug classes in self/ml/qwen_bpe.hexa (class a: byte↔unicode bijection · class b: multibyte detokenize) — byte-level-BPE detokenize is now correct for multibyte cross-token text
- TASK: OP-127 flagged `self/ml/qwen_bpe.hexa::qwen_unmap_bytes` (byte-level-BPE detokenize) as a distinct latent path it did not audit. OP-128 audited the whole byte-level BPE encode/decode of the PRODUCTION Qwen2.5 tokenizer for the two classic bugs: (a) byte↔unicode bijection not exact, (b) detokenize reassembling per-token instead of collecting raw bytes then UTF-8-decoding once.
- METHOD (honest g5 + stale-oracle discipline): the shipping local hexa is a Jun-7 frozen-stdlib build, so correctness was established with a faithful Python PORT-OF-SOURCE oracle of the hexa bijection+decode (hexa string=raw-byte buffer, from_char_code=UTF-8 encoder, char_code=raw byte, slice=byte-indexed) cross-checked against the openai/HF GPT-2 `bytes_to_unicode()` reference, PLUS a pure hexa regression test run on the local interpreter (the bijection+unmap functions are pure/fast — no 7MB tokenizer.json load).
- BUG (a) — BIJECTION NOT EXACT: `qwen_build_byte_to_str` mapped each non-printable byte to codepoint `256 + i` (per-byte offset) instead of the canonical GPT-2 RUNNING COUNTER `256 + n`. AGREE for bytes 0..32, DIVERGE for the 35 non-printable bytes ≥127 (0x7F, 0x80..0xA0, 0xAD). 34 of those (0x80..0xA0, 0xAD) are UTF-8 CONTINUATION bytes present in EVERY Korean/CJK/emoji char → the wrong visible glyph never matched Qwen's vocab.json keys → encode produced wrong/missing tokens for ALL non-Latin multibyte text + decode of HF tokens carrying U+0122..U+0142 mojibaked. The internal decode∘encode was self-CONSISTENT (same wrong table both ways) which MASKED it; only interop with a real HF vocab exposes it. This is the EXACT defect OP-28b already cured in the tokenizer_bpe.hexa/op28b twin — qwen_bpe.hexa was the un-fixed copy. FIX = running-counter `256 + n`. ORACLE: buggy table mismatches-vs-GPT2=35 → fixed=0; '가'/'한글 테스트'/'中文'/'😀' encode against HF vocab missing-bytes []=OK.
- BUG (b) — DETOKENIZE corrupts multibyte glyphs (even within ONE token, an instance of the OP-127 from_char_code-over-raw-byte class): `qwen_unmap_bytes` (1) rebuilt the glyph lookup key per byte via `piece = piece + from_char_code(char_code(tok,j))` — but from_char_code re-UTF-8-encodes a raw byte ≥128 (0xC4 → "Ä" = 0xC3 0x84), so a 2-byte glyph became a 4-byte blob and str_to_byte MISSED; and (2) emitted the recovered raw byte via `from_char_code(byte_val)` — which for byte_val ≥128 (e.g. 0xEA, a Korean lead byte) emits the 2-byte UTF-8 of U+00EA, not the single raw byte. FIX (matching the proven tokenizer_bpe.hexa::bpe_decode twin): size the glyph by its lead byte (192/224/240→2/3/4), extract via `.slice` (raw-byte preserving), emit via `chr(byte_val)` (single raw byte), fall back to the raw slice on a miss. Cross-token correctness is then automatic: `out` is a raw-byte buffer and qwen_bpe_decode concatenates per-token unmaps, so a multibyte char whose UTF-8 bytes span 2+ tokens reassembles BEFORE it is decoded. PROOF on the local interp: BEFORE unmap(2-byte glyph)=4 bytes b0=195 (key 'Ä·' not found); AFTER unmap single glyph(149)=1 byte b0=149, unmap(한 packed)=[237,149,156] correct UTF-8.
- LOCK: NEW test/t54_qwen_bpe_multibyte_roundtrip.hexa — `hexa run` → 16/16 PASS: 256-glyph bijection distinct+inverse; canonical-codepoint spot-checks on the historically-broken bytes (0x20→U+0120, 0x7F→U+0121, 0x80→U+0122, 0xA0→U+0142, 0xAD→U+0143, + NOT the buggy 0x80→U+0180); and the CRITICAL multibyte CROSS-TOKEN cases — Korean 3-byte char split 1+1+1 across 3 tokens, 4-byte emoji split across 2/3 tokens, mixed ASCII+multibyte at widths 2/4 — all reassemble byte-exact. `hexa parse` clean on qwen_bpe.hexa + t54.
- CLOSURE: build_selfhost closure = import/use transitive closure of compiler/main.hexa + tool/hexa_ld.hexa (≈50 files). `self/ml/qwen_bpe.hexa` is imported ONLY by tests (t53/t54) + self/serve/serve_alm.hexa, never by compiler/main.hexa or tool/hexa_ld.hexa → closure-OUT → ZERO byte-eq/fixpoint impact, NO selfhost gate required. t54 is a closure-OUT leaf test.
- RESULT: 🔴 FIXED ×2 distinct bug classes (a: bijection · b: detokenize). Byte-level-BPE detokenize is NOW CORRECT for multibyte cross-token text. HONEST RESIDUALS: verified by Python-port-of-source oracle + pure-hexa interpreter test; the frozen Jun-7 binary carries the fix only after a rebuild (source correct — stale-oracle trap). t53 (ASCII-only integration) stalls in the dbg-println-spammed O(n) vocab-scan loop on the slow interpreter (>580s, never reached assertions) — unaffected by this fix (ASCII bytes <127 untouched). The earlier t54 FAIL was a stale-interp nested-struct-literal-in-helper SCOPING quirk (not a logic bug); hoisting the QwenBpe value → 16/16 GREEN.
- FLAGS: 1 source edit (self/ml/qwen_bpe.hexa) closure-OUT → ZERO selfhost-fixpoint impact, no selfhost gate; net-additive/substituting (no >50-line deletion) → wipe_guard N/A; 1 NEW leaf test closure-OUT; NO .tape. LANE-2. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP128-BYTELEVEL-BPE-ROUNDTRIP.txt.
## 2026-06-13 — OP-131 DONE: date/time arithmetic & parsing value-correctness audit (promotes OP-93's deferred 🟠) → 🔴 1 silent-normalisation bug FIXED + 🟢 CONVERGED
- TASK: OP-93 flagged the `civil` date helper does NOT validate field ranges (deferred 🟠 design note). PROMOTE to a real audit — hunt every date/time/calendar/duration helper for VALUE-correctness bugs (wrong results, not crashes) on edge inputs vs a Python datetime/calendar oracle.
- HUNT: `stdlib/time/civil.hexa` (Hinnant civil_from_days/days_from_civil + iso8601_format/parse + rfc3339), `self/runtime/datetime_mini_pure.hexa` (is_leap_year_pure/days_in_month_pure/day_of_year_pure/days_between_pure/weekday_zeller_pure), `self/std_time.hexa` (is_leap/parts_to_timestamp/timestamp_to_parts/time_parse). 8 helpers audited.
- METHOD: ran edge inputs through `hexa run` (over a patched-source copy, since deployed `use` resolves the stale ~/.hx frozen stdlib) and diffed each vs `python3 datetime.datetime(...,tzinfo=utc)` + a source-port of the Hinnant integer algorithm.
- CLEAN (value-correct): Hinnant civil_from_days/days_from_civil bit-exact vs Python over leap 2000/2024, NON-leap century 1900 (÷100 not ÷400), negative epoch 1969-12-31→-86400, Dec-31→Jan-1 boundary, day-of-year 366 — max|Δ|=0. datetime_mini_pure all DOCUMENT "no validity check — caller's responsibility" AND the only in-repo callers (its own days_between_pure + self-test) pass validated literals → documented precondition, NOT a bug. weekday_zeller_pure = OP-81 locked-clean leaf (not re-audited). std_time round-trips correctly for valid fields; is_leap rule correct.
- THE BUG (🔴, iso8601_parse_native): contract says "Returns -1 on a malformed input" but it NEVER range-checked the fields → every invalid date silently normalised through days_from_civil into a GARBAGE epoch. EDGE TABLE (input → BEFORE → Python → AFTER): "2024-13-05"→1736035200→ValueError→-1 ; "2024-02-30"→1709251200(=2024-03-01)→ValueError→-1 ; "2024-01-32"→1706745600→ValueError→-1 ; month 0 / day 0 / hour 25 / minute 60 → all garbage→ValueError→-1 ; "1900-02-29"→-2203891200→ValueError(1900 not leap)→-1. Valid UNCHANGED: 2024-02-29→1709208000, 2000-02-29→951782400, leap-sec :60 accepted, epoch0→0, roundtrip→1700000000.
- FIX (minimal, stdlib leaf): added month∈[1,12], hour≤23, minute≤59, second≤60 (leap-sec), day∈[1,_days_in_month(y,m)] checks (new leap-aware _days_in_month helper) before days_from_civil, returning the already-documented -1 sentinel.
- LOCK: stdlib/time/civil_test.hexa (@ci_gate) +11 asserts (8 rej_* invalid→-1 · 3 ok_* valid-edge→exact oracle epoch). `hexa run` patched source → civil self-test 21/21 PASS. Both files `hexa parse` clean.
- HONEST RESIDUAL: deployed `hexa run` delegates `use "stdlib/time/civil"` to the ~/.hx frozen stdlib so it still shows OLD garbage until the stdlib re-ships — stale-oracle trap, NOT a logic bug; fix logic proven by interpreting patched source directly.
- FLAGS: civil.hexa is build_selfhost closure-OUT (compiler/main.hexa never imports it) → ZERO byte-eq/fixpoint impact, no selfhost gate; net-additive (validation + 1 helper + 11 asserts, 0 deletions) → wipe_guard N/A; NO .tape. LANE-1. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP131-DATETIME-ARITHMETIC.txt.

## 2026-06-13 — OP-130 DONE: repo-wide sweep for the OP-129 NARROW error-mask class → 🔴×3 fragile diagnostic masks FIXED + 🟢 census CONVERGED
- TASK: OP-129 found `build_selfhost.sh`'s `grep -iE 'error:'` mask SWALLOWED clang's "Undefined symbols for architecture …" linker error (no `error:` token) → empty diagnostic before the artifact `fail`. Sweep ALL build/CI/tool shell scripts for the same dangerous class: a gate that decides pass/fail (or surfaces a failure) by grepping for a TOO-NARROW error pattern that a real failure with a different format can dodge.
- CENSUS: grep'd tool/*.sh (≈110) · .githooks/* · .github/workflows/*.yml for `grep -i*error`, `grep 'error:'`, `| grep -q`, `grep -c`, `if … grep`, `&& echo PASS`/`|| echo FAIL`, CI `::error::`. Classified ROBUST vs FRAGILE.
- ROBUST (the majority — pass/fail by EXIT CODE / ARTIFACT, not output-grep): `build_hexa_{atlas,calc,check,verify,module_loader,qmirror,qrng,sim_universe,test,convergence}.sh` decide on `rc=$?` from the build + a SECONDARY banner-smoke grep; `determinism_gate`/`miscompile_zero_gate`/`selfhost_codegen_guard`/`diff_fuzz`/`mczero_real_sweep` use `grep -c ENCODE-MISS`/`udf` as a COUNT METRIC alongside an exit check; `flame_phase4b3_verify_all.sh` PASS-marker grep over self-asserting verify steps; `dispatch_*.sh` `grep -q READY/OK` are retried remote-readiness probes; CI yml gates on step exit codes + `::error::`. The one grep-RESULT-gates-passfail site `build_aprime.sh:204` `[ -n "$CL_ERR" ]` is OR'd with `[ ! -x "$OUT" ]` → artifact backstop saves it.
- FRAGILE (the OP-129 class — 3 narrow `error:`-only DIAGNOSTIC masks OP-129's targeted fix didn't reach; its #3296 merge consolidated the hexat TU so the 4th narrow mask is gone): `build_selfhost.sh:173` (rt.o clang), `build_aprime.sh:222` (rt.o clang), `build_aprime.sh:223` (smoke .s clang). "Diagnostic-fragile": the GATE never silently passed (next line is `[ -s rt.o ] || fail` / `[ -x SMB ] || exit`), but the narrow mask SWALLOWED the real `Undefined symbols`/`ld:`/`fatal error:`/`cannot find` text → blank diagnostic before the failure (the confusing-failure half of OP-129).
- DEMONSTRATED MISSED MODE (`build_aprime.sh:223`): if the smoke .s fails with `fatal error: cannot find …` (or a driver-surfaced `ld:`), `grep -iE 'error:'` emits NO matching line → operator sees blank, then `[ ! -x "$SMB" ]` → "smoke link failed" with the real cause hidden.
- FIX (3 lines, 2 files, matches OP-129's `error:|undefined` widening): all three → `grep -iE 'error:|undefined|ld:|fatal|cannot find'`. Widening a DISPLAY-only grep can only print MORE real failure text → ZERO false-positive / CI-break hazard (the gate is the artifact/exit check, not the grep — so a widened mask can't flip a passing gate).
- VERIFY: `bash -n tool/build_selfhost.sh tool/build_aprime.sh` → both clean; `grep -rn "grep -iE 'error:'" tool/*.sh` → 0 narrow `error:`-only masks remain repo-wide.
- FLAGS: 2 BUILD SCRIPTS (`.sh`) only, NO compiler source → ZERO byte-eq/fixpoint impact, no selfhost gate semantics changed; net-additive (3 widened patterns, 0 deletions) → wipe_guard N/A; NO .tape. HONEST RESIDUAL: no known case remains where a NARROW grep DECIDES pass/fail — every grep gate is exit-code/artifact-backstopped or a count metric; class CONVERGED. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0 · LANE-1. Verdict .verdicts/hexa-0pod/F-OP130-SILENT-ERROR-MASK-SWEEP.txt.

## 2026-06-13 — OP-129 DONE: fix the from-scratch self-host bootstrap ROT in `build_selfhost.sh` (hexat link omitted self/runtime.c) → 🔴→FIXED + from-scratch build PROVEN green 0-pod; the manual one-TU workaround is no longer needed
- TASK: OP-126 surfaced and worked around a real rot — `build_selfhost.sh`'s hexat-build step compiled only `self/native/hexa_cc.c`, omitting `self/runtime.c`, so a from-scratch build failed at the clang link. Make the SCRIPT itself correct so the manual one-TU workaround is unnecessary; PROVE by actually running the build.
- REPRODUCE (verbatim, from scratch): restored frozen seeds (`tool/restore_frozen_seeds` → `self/native/hexa_cc.c` + `self/runtime.c`), then ran the EXACT broken line `clang … self/native/hexa_cc.c -o build/hexat` → `Undefined symbols for architecture arm64: ___hexa_fn_arena_enter / ___hexa_fn_arena_return / ___hexa_last_error / ___hexa_try_cleanup / ___hexa_try_push / ___hexa_try_top / ___hx_to_double …` → `ld: symbol(s) not found` → NO `build/hexat`. These are the runtime ABI symbols `hexa_cc.c` references, all defined in `self/runtime.c` (absent from the TU). NOTE the script's `grep -iE 'error:'` mask SWALLOWED this (clang says "Undefined symbols", not "error:") → silent fall-through to a confusing `fail seed hexat-build`.
- ROOT-CAUSE: `tool/build_selfhost.sh` hexat stage (formerly line 149-150) — single-file clang TU, no runtime, no `-lm`. The downstream `rt.o` stage compiles `self/runtime.c` correctly, but that's AFTER hexat and only feeds the gen2/gen3 LINK; hexat itself is a standalone bootstrap binary and must carry the runtime in its own link.
- FIX (minimal, convention-matching): add `self/runtime.c` + `-lm` to the hexat clang TU (matches the `rt.o` stage and the recipe's proven 1-TU line); require `self/runtime.c` in the seed guard (`[ ! -f self/native/hexa_cc.c ] || [ ! -f self/runtime.c ]` → restore); widen the error mask to `error:|undefined` so a future link regression SURFACES instead of being swallowed.
- PROVE (from scratch, LOCAL CPU, ~43 s, 0-pod): `rm -rf build/selfhost build/hexat` then `bash tool/build_selfhost.sh -r "$PWD" -w "$PWD/build/selfhost" -j --detached .../mark.txt` → EXIT=0; marker `EXIT=0 STAGE=stage0-only HEAD=8b89bbcc1 aprime=ok hexa_ld=ok rt=ok`; every stage this edit touches built clean WITHOUT the manual workaround — `build/hexat` 1,945,208 B Mach-O arm64 (the previously-FAILING step; sanity-run prints `hexa-cc: HEXA self-hosted compiler / Usage: …`, rc=0), `build/selfhost/aprime_cc` 1,571,560 B, `build/selfhost/rt.o` 525,536 B, `build/selfhost/hexa_ld` 359,656 B.
- DOC: `docs/selfhost-repromote-recipe.md` §1 step (2) — the "ROT WORKAROUND: build_selfhost.sh omits runtime.c" note replaced with "build_selfhost.sh now links the runtime correctly itself as of OP-129; the manual TU is shown only for the standalone proof".
- HONEST SCOPE: the fix targets ONLY the hexat link (the from-scratch blocker), proven end-to-end hexat→aprime_cc→rt.o→hexa_ld via the script's `-j` path. Stages 1-3 (THREE ~68-min self-emits + the cc-gen3.o==cc-gen4.o byte-eq fixpoint GATE, ~3.5h total) are UNCHANGED by this edit, inherently build-host (NOT 0-pod), and already link the runtime correctly via the existing `rt.o`/`selfemit`/link lines — not re-run here (out of 0-pod scope); no new breakage observed up to where they begin.
- FLAGS: BUILD SCRIPT (`.sh`) + recipe doc only, NO compiler-SOURCE change → ZERO byte-eq/selfhost-fixpoint impact, no selfhost gate semantics changed; net-additive (1 file + `-lm` + a guard clause, 0 deletions) → wipe_guard N/A; NO .tape. LANE-1. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict `.verdicts/hexa-0pod/F-OP129-BUILDSELFHOST-RUNTIME-LINK.txt`.

## 2026-06-13 — OP-126 DONE: VERIFY (no flip) a 0-pod LOCAL rebuild from current source CLOSES the deployed OP-124 front-end residual → 🟢 PROVEN; the actual default-flip remains a USER DECISION
- TASK: OP-124 closed both front-end bugs as CLOSED-STALE (fixed in source) but left an honest residual — the DEPLOYED `hexa run` still delegates to a stale pre-fix binary, so end-to-end `hexa run` STILL shows both bugs until a re-promote (recurs across OP-37/40/124). OP-126 = VERIFY that a 0-pod local rebuild from current source closes the residual + produce a turnkey promote recipe. MUST NOT flip the default (`~/.hx/bin` untouched, no `promote_selfhost.sh --default`) — the flip is user-gated.
- DISPATCH TOPOLOGY (measured, `~/.hx/bin`): `hexa`→exec `hexa.real`→symlink→`hx-selfhost-cli`. The shim routes `--emit=<x>`→`gen3` (`self/native/selfhost/gen3`, Jun-3 binary, compile surface only) but DELEGATES everything else — incl. `run`/`build`/`verify` — to `hexa.real.pre-selfhost.20260611-005522` (the Jun-7 SHIPPED dispatch binary, installed by `install.sh` from a prebuilt release tarball). `hexa run X` carries no `--emit` → DELEGATES → the stale Jun-7 dispatch binary. So `hexa run` NEVER touches gen3; both gen3 (Jun-3) and the dispatch binary (Jun-7) PRE-DATE the OP-37/OP-40 codegen fixes → both bugs survive end-to-end though current SOURCE + `gen3 --emit` are fixed.
- DEPLOYED (STALE) REPROS: BUG-1 `let f = 1 \n - 2 \n + 10; print(f)` → `hexa run`→**1** (drops `- 2 + 10`; correct=9). BUG-2 comptime `-1.7976931348623157e308` → emitted C `use1(hexa_float(-1.79769e+308))` (%g 6-sig-digit truncation, low mantissa DESTROYED; python truth `0xFFEFFFFFFFFFFFFF`).
- 0-POD REBUILD (fully local, ~32 s, NO gpu/pod/vast): (1) `tool/restore_frozen_seeds` → `self/native/hexa_cc.c` + `self/runtime.c`. (2) build `hexat` — ROT WORKAROUND: `build_selfhost.sh`'s hexat clang TU omits `self/runtime.c` → fails `Undefined symbols … ___hexa_last_error`; compiling `hexa_cc.c` + `runtime.c` in ONE TU yields a working `hexat` (1,945,208 B). (3) `tool/build_aprime.sh -v build/hexat` → `aprime_cc` (current `compiler/main.hexa` closure: flatten 48 files/44041 lines → transpile 47609 C lines → clang 1,571,560 B Mach-O arm64; atlas 17265 nodes; smoke exit(42)==42 PASS). (4) `clang -c self/runtime.c → rt.o`.
- PROOF the residual CLOSES on the current-source toolchain: BUG-1 — `aprime_cc --emit=obj` → `hexa_ld` link + run → **9** (CORRECT; deployed gave 1) ✅. BUG-2 — `aprime_cc --emit=obj` for `let a = -1.7976931348623157e308` bakes the comptime const as byte-EXACT IEEE-754 LE bytes `ff ff ff ff ff ff ef ff` = `0xFFEFFFFFFFFFFFFF` (proof: `xxd -p lit2.o | tr -d '\n' | grep -oi ffffffffffffefff` → MATCH; python truth `0xffefffffffffffff`) vs deployed `%g` truncation ✅. OP-40 bit-exact hex-float const-fold serialize is live in current source. NOTE: `f64_to_bytes_le` is a stdlib symbol absent from the bare `--emit` runtime (can't link the literal byte-dump on the bare compiler surface); the obj-disassembly of the baked const is the equivalent unambiguous proof — it's exactly what the byte-dump reads back at runtime.
- RESULT: 🟢 PROVEN — current source closes BOTH OP-124 repros where the deployed stale binary fails, and the proving build is fully 0-pod-feasible (~32 s, local CPU). USER DECISION (NOT done, by design): the actual DEFAULT FLIP. Closing `hexa run` end-to-end requires REFRESHING the DISPATCH binary that the shim delegates to (current-source dispatch build / re-install from a fresh release) and/or extending `hx-selfhost-cli` to route non-`--emit` `run`/`build` to a current-source compiler; the gen3 byte-eq fixpoint ladder (cc-gen3.o==cc-gen4.o, ~3.5h, THREE 68-min self-emits) + `FROZEN_SEED_REF` re-pin are the canonical build-host promote (CI-seed path, distinct — see `docs/selfhost-next-constfold-promote.md`). NEW `docs/selfhost-repromote-recipe.md` = turnkey promote + `--revert` rollback.
- FLAGS: NO source edit (both bugs already fixed in source; no fix to fabricate) → ZERO byte-eq/selfhost-fixpoint impact, no selfhost gate needed; recipe doc + verdict only; scratch `build/op126-scratch/` (gitignored region), `~/.hx/bin` UNTOUCHED, default NOT flipped; wipe_guard N/A (0 deletions); NO .tape edit. LANE-1. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict `.verdicts/hexa-0pod/F-OP126-SELFHOST-REPROMOTE-VERIFY.txt`.

## 2026-06-13 — OP-127 DONE: exhaustive census of the MODULE-LOCAL UTF-8 double-encoder class that OP-79's repo-wide `bytes_to_str(` sweep EXCLUDED → 🔴→FIXED ×4 files / 5 call-sites (hexaw/lora/mmap safetensors-header readers) + 🟢 CONVERGED census; the flat module-local double-encoder class is now DEFINITIVELY CLOSED
- TASK: OP-79 swept the GLOBAL `bytes_to_str(` repo-wide but EXPLICITLY EXCLUDED underscore/module-local helpers; OP-125 found one such missed site (safetensors `_bytes_to_str`, already FIXED & landed #3292 in this branch's base). OP-127 = census EVERY function (any name) that converts a byte array→string by the BUGGY "treat each raw byte as a codepoint and emit a char" pattern vs the correct `bytes_to_str_raw` (raw byte buffer, no codepoint re-encode), and fix every one that can receive non-ASCII file/wire data.
- METHOD (honest g5 + stale-oracle discipline): grep'd `fn _?bytes_to_str` / `fn .*to_str` / per-byte `chr()`/`from_char_code()` byte-walks across stdlib/ self/ compiler/ tool/. RUNTIME GROUND TRUTH localized the true bug primitive: `from_char_code(N)` (runtime.c:6035) = codepoint→UTF-8, expands any byte ≥0x80 to multi-byte = DOUBLE-ENCODE in both paths; `chr(N)` lowers to `hexa_chr_byte` (runtime.c:6067, codegen.hexa:7205) = raw N&0xFF = CORRECT under the shipping NATIVE codegen (compiler/PLAN.md:3399 — chr/from_char_code asymmetry DISSOLVED-BY-INTERP-RETIREMENT, @D g_interp_deprecated R7 CLOSED). So `chr(byte)` walks are NOT bugs; `from_char_code(rawbyte)` IS.
- BUGGY (from_char_code over RAW on-disk bytes, non-ASCII reachable) → FIXED: (1) self/tools/safetensors_to_hexaw.hexa::jc_parse_string, (2) self/alloc/safetensors_mmap.hexa::jc_parse_string, (3) self/tools/lora_to_hexaw.hexa::jc_parse_string — all three are safetensors JSON-header string readers (tensor names / __metadata__) → rewritten to accumulate raw bytes then `bytes_to_str_raw(sub)`; (4) self/tools/lora_to_hexaw.hexa::strip_peft_prefix + ::strip_suffix — rebuilt a name substring per byte via `from_char_code(char_code(name,i))` (split UTF-8 on byte boundaries + re-encode) → `name.substring()`; (5) self/ml/hexaw.hexa::hexaw_load_binary_from_bytes HEXAW2 tensor-NAME field → byte-accumulate + `bytes_to_str_raw` (ndim/dims/count fields LEFT per-byte = ASCII digits only, <0x80, documented inline).
- CENSUS-CLEAN (honest, nothing invented): stdlib codec decoders big5/gbk/cp949/euc_kr/shift_jis/unicode_normalize call from_char_code over a decoded Unicode CODEPOINT (correct); term_*/lsp/json escape constants are ASCII control bytes; self/stdlib/hxc_a{7,9,20,22,24,25,33,34}* compression atoms + ssh/wire + std_encoding use chr(byte) = raw-correct under native codegen. ONE distinct latent path FLAGGED (NOT a flat reader, out of OP-127 scope): self/ml/qwen_bpe.hexa::qwen_unmap_bytes `from_char_code(str_to_byte[s])` = byte-level-BPE detokenize inner loop — needs an accumulate-then-raw refactor of the whole path as a separate follow-on.
- CLOSURE: build_selfhost closure = import/use transitive closure of compiler/main.hexa + tool/hexa_ld.hexa (computed = 50 files, ~documented 48). ALL 4 fixed files are closure-OUT (compiler/main.hexa never imports them) → ZERO byte-eq/fixpoint impact, NO selfhost gate required.
- REPRO: Korean tensor name `층.가중치` = 13 UTF-8 bytes on disk. BUGGY per-byte from_char_code → 25 mojibake bytes != original; FIXED bytes_to_str_raw → 13 bytes == original. Confirmed by a Python UTF-8 oracle (faithful port of fixed source) AND the LIVE `hexa run` (bytes_to_str_raw is already raw-correct in the shipping binary): `층.가중치 / buggy_len=25 fixed_len=13 / ROUNDTRIP_OK`. `hexa parse` clean on all 4 edited files + the test.
- LOCK: NEW self/tools/double_encoder_op127_test.hexa HERMETIC (`hexa run` → `__HEXA_OP127_DOUBLE_ENCODER__ PASS 3/3`): fixed reader round-trips the Korean name byte-exact · old buggy reader mojibakes 13→25 != original · ASCII name "model.weight" unaffected (no regression).
- RESULT: 🔴→FIXED ×4 source files / 5 call-sites + 🟢 CONVERGED census. The FLAT module-local UTF-8 double-encoder (byte→string reader) class is DEFINITIVELY CLOSED: OP-79 cured the global helper, OP-125 the safetensors local, OP-127 the hexaw/lora/mmap tooling family. HONEST RESIDUAL: deployed `hexa run` delegates to the Jun-7 dispatch binary that freezes its embedded stdlib, so end-to-end of the tooling readers awaits the next interpreter rebuild — source is correct (the documented stale-oracle trap).
- FLAGS: 4 source edits all build_selfhost-closure-OUT → ZERO byte-eq/selfhost-fixpoint impact, no selfhost gate; net-additive/substituting (no >50-line deletion) → wipe_guard N/A; 1 NEW leaf test closure-OUT → fixpoint UNAFFECTED; NO .tape. LANE-2. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP127-DOUBLE-ENCODER-CENSUS.txt.

## 2026-06-13 — OP-124 DONE: TWO reported compiler FRONT-END bugs investigated → BOTH 🟢 CLOSED-STALE (already fixed in current origin/main SOURCE; the only surviving reproductions are STALE DEPLOYED-BINARY artifacts), guarded by 2 new hermetic regression tests
- TASK: backlog handoffs f37015ac ("front-end bug: multi-LINE arithmetic continuation drops trailing term") + 5fd409b8 ("BUG constant-folder: negating a large-magnitude float literal loses precision/term"). Reproduce on current main via the SHIPPING `hexa run`; if real, root-cause + fix + regression-guard; if stale, prove CLOSED-STALE honestly.
- METHOD (honest g5 + stale-oracle discipline): for each bug, (1) minimal `hexa run` repro, (2) when it reproduces, distinguish DEPLOYED-BINARY artifact vs current-SOURCE defect by reading the fixed function + checking git-ancestry + EXECUTING the fixed function's exact logic through `hexa run`. The deployed `hexa run` DELEGATES (no --emit) to hexa.real.pre-selfhost.20260611-005522 (Jun-7 dispatch binary); the freshest local hexat (Jun-13 mtime) is built off a Jun-1 hexa_v2/stage0 bootstrap — NEITHER carries the `INBOX #1703` marker → both are PRE-fix trees, NOT authority. The repo SOURCE at HEAD is the authority.
- BUG-1 (f37015ac) — multi-line arith continuation. REPRODUCES on deployed `hexa run`: `let f = 1 \n - 2 \n + 10` → f=1 (drops `- 2 + 10`). Controls isolate it: leading-PLUS (`1 \n + 2`)→13 OK, trailing-MINUS (`1 - \n 2`)→9 OK, only leading-MINUS (`1 \n - 2`) drops. Emitted C (hexat) confirms: `(-2)+10` is computed then DISCARDED, f stays 1. ROOT in SOURCE: parse_addition newline-continuation whitelist. FIXED by #1747 02616b75e (2026-05-28) self/parser.hexa:3377 `p_continue_bin_op(["Plus"])`→`(["Plus","Minus"])` (+ compiler/parse/parser.hexa additive loop covers Plus AND Minus). `git merge-base --is-ancestor 02616b75e origin/main` → TRUE. SOURCE PROOF: `/tmp/op124/unit_continue.hexa` (run via `hexa run`) executes the exact p_continue_bin_op decision on `[Newline,Minus,…]` — current whitelist `["Plus","Minus"]`→continues (=true, 1-2+10=9); pre-fix `["Plus"]`→drops (=false). → CLOSED-STALE.
- BUG-2 (5fd409b8) — large-float negation const-fold. REPRODUCES on deployed `hexa run` via f64_to_bytes_le byte-dump: `let a = -1.7976931348623157e308` → bytes corrupt in low mantissa vs python struct.pack('<d',-X); emitted C shows the comptime-const INLINE re-serialized as `hexa_float(-1.79769e+308)` (%g 6-sig-digit truncation), while the inline-call-arg `dump(-X)` and `0.0-X` forms are byte-exact. ROOT in SOURCE: comptime float-fold serialize. FIXED by OP-37 aab0c0891 + OP-37b 3e153225d + OP-40 9d3aee960 — self/codegen.hexa comptime_eval UnaryOp/`0.0-X` use exact-text `_cf_negate_float_text` (no lossy parse/serialize), computed folds use bit-exact hex-float `_cf_float_hexlit`. merge-base TRUE for both. SOURCE PROOF: `/tmp/op124/unit_negate.hexa` runs `_cf_negate_float_text("1.7976931348623157e308")` → `-1.7976931348623157e308` (all 17 digits, exact=true, roundtrip=true). → CLOSED-STALE. (NB: F-OP37-FLOAT-CONSTFOLD-VERIFY.txt already logged the "deployed `hexa run` still delegates to the pre-fix binary" residual.)
- RESULT: 🟢 BOTH CLOSED-STALE. No new source change (no fix to fabricate — both already landed). 2 NEW hermetic regression guards added: `test/t_op124_multiline_arith_continuation.hexa` (6/7 PASS on the stale deployed binary; the lone leading-minus FAIL is a deployment-lag canary that flips to PASS against a current-source build) + `test/t_op124_float_negate_constfold.hexa` (5/5 PASS — locks the inline-call-arg + `0.0-X` byte-exact forms vs IEEE-754 ground truth). Handoffs f37015ac + 5fd409b8 → DONE (source-fixed).
- HONEST RESIDUAL (deployment, NOT source): deployed `hexa run` still delegates to the pre-fix Jun-7 dispatch binary, so end-to-end `hexa run` keeps exhibiting both residuals until the self-host toolchain is re-promoted from current source (tool/promote_selfhost.sh / fresh aprime rebuild) — a PROMOTE step, out of scope for a 0-pod source-audit. Logged as a follow-up.
- FLAGS: NO source edit (both CLOSED-STALE) → ZERO byte-eq/selfhost-fixpoint impact, no selfhost gate needed; 2 NEW leaf tests build_selfhost-closure-OUT → fixpoint UNAFFECTED; wipe_guard N/A (2 new files, 0 deletions); NO .tape edit. LANE-1. $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict .verdicts/hexa-0pod/F-OP124-COMPILER-FRONTEND-BUGS.txt.
## 2026-06-13 — OP-125 DONE: LANE-2 Korean (multibyte UTF-8) bytes→str mojibake — 🔴→FIXED, ROOT-CAUSE = (c) str_to_bytes/bytes_to_str ASYMMETRY (OP-77/79 double-encode class); the pcc site was already fixed by OP-79/#3217, and OP-125 closes the OP-79 sweep GAP by fixing a MISSED same-class site in stdlib/safetensors.hexa `_bytes_to_str` (+self mirror)
- LANE: 2 (0-pod concrete-bug root-cause + class-sweep). LEAD: OP-123 found `stdlib/ckpt/test/pcc_roundtrip` fails 1/6 on the live binary — `한글`→`íê¸` (classic double-UTF-8-encode: each raw byte b>=0x80 rendered as to_string(from_char_code(b & 0xFF)), so 3-byte Hangul → 6 mojibake bytes).
- REPRO (verbatim, ~/.hx/bin/hexa): pcc roundtrip `FAIL cert_json mismatch / want 한글-fixture / got íê¸-fixture / FAIL: pcc roundtrip 5/6`.
- LOCALIZATION: NOT (a) bytes_to_str_raw — proven CORRECT in isolation on the SAME live binary (raw=[한글] RAW_OK · emoji=[🌌] EMOJI_OK · char_code→bytes→bytes_to_str_raw rt=[한글] RT_OK) AND class-wide by oracle; NOT (b) length/truncation — writer→file→bytes_slice→bytes_to_str_raw traced byte-correct on disk. Category (c): the verifier USED to call the double-encoder, but origin/main verifier.hexa:245 ALREADY reads `bytes_to_str_raw` (OP-79/#3217, landed 2026-06-13 08:34). The live repro is a STALE-BINARY artifact — the shipping Jun-7 `hexa.real.pre-selfhost.20260611-005522` dispatch binary embeds a FROZEN pre-#3217 stdlib (proven: a MARKER in ~/.hx/src/stdlib/ckpt/verifier.hexa, a full-file overlay, and HEXA_STDLIB_ROOT=<worktree> ALL failed to change runtime behavior). Documented stale-oracle (MEMORY "Local hexa is stale oracle").
- NEW FIND (OP-125's real contribution): the OP-79 sweep EXPLICITLY excluded `_bytes_to_str` identifiers ("DISTINCT identifiers, excluded"), so a same-class double-encoder was MISSED: `stdlib/safetensors.hexa::_bytes_to_str` (used at safetensors_read:417 to decode the on-disk UTF-8 safetensors JSON header) + its `self/stdlib/safetensors.hexa` mirror. Any safetensors file with non-ASCII `__metadata__` (Korean tensor labels; HF allows arbitrary str metadata) mojibakes on read.
- FIX (surgical): both `_bytes_to_str(bs,start,end)` → build the `[start,end)` sub-array then `bytes_to_str_raw(sub)`. pcc/ckpt path untouched (already correct). self/ copy is a mirror NOT in the build_selfhost closure (compiler/main.hexa never imports safetensors → walk() never reaches it) → ZERO selfhost byte-eq/fixpoint impact. No other caller repo-wide.
- CLASS SWEEP (Python UTF-8 oracle = faithful port of FIXED source): FIXED round-trips eq=True; STALE byte-identically reproduces the live `íê¸` mojibake (oracle faithful); SWEEP over Hangul U+AC00..U+D7A3 (11172) + CJK U+4E00..U+9FFF + combining U+0300..U+036F + FULL BMP minus surrogates + 4-byte astral/emoji (U+10000/U+10FFFF/U+2A6D6/U+1D11E) = 0 fails, CLASS CLEAN; ASCII headers unchanged under the fix (no regression).
- REGRESSION LOCK: `stdlib/test/test_safetensors.hexa` gains a Hangul+CJK+emoji `__metadata__` write→read→re-write round-trip assert. On the STALE binary the NEW lock FAILS (`FAIL read.meta.author Korean round-trip` + downstream `FAIL F-SAFETENSORS-1 byte-identical`, pass=32 fail=4) — proving the bug end-to-end through the REAL safetensors_write→safetensors_read path, not just the oracle. GREEN under FIXED source (oracle-proven); `hexa parse` clean on all 3 edited files. Live `hexa run` GREEN awaits the next interpreter rebuild.
- DISCIPLINE: $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0 · local CPU only · own worktree (no extra worktrees) · staged by explicit path. Verdict .verdicts/hexa-0pod/F-OP125-BYTESSTR-KOREAN-MOJIBAKE.txt.

## 2026-06-13 — OP-122 DONE: LANE-1 STATEFUL / STREAMING decoder cross-chunk state-corruption sweep — 🟢 CONVERGED, NO 🔴; every in-hexa carried-state decoder (websocket frame buf carry, SSE partial-event accumulator) passes streaming-equivalence at every chunk split, locked by a hermetic guard that decodes a multi-frame stream split at EVERY two-way offset
- LANE: 1 (0-pod correctness/security — carried-state streaming-decode axis). FRESH class: a decoder that maintains STATE ACROSS multiple input pieces (chunks/frames/feeds) where an adversarial chunk boundary corrupts the carried state → lost/duplicated bytes, frame desync. DISJOINT from OP-77 (single-buffer utf8 mojibake), OP-92 (single-buffer HTTP body split), OP-105 (single-buffer OOB-read), OP-120 (recursion DEPTH), OP-121 (comparators) — this axis = STATE carried between calls/chunks.
- PROPERTY (falsifier): streaming-equivalence — `decode(concat(chunks)) == concat(decode-per-chunk-with-carry)` for EVERY possible split. Split the SAME logical input at every byte offset and assert the streamed result == the one-shot result; a divergence at some split = real cross-chunk state corruption.
- METHOD: grep stdlib/ + self/serve + self/rt for carried-state fields (`carry|pending|remainder|partial|buf|leftover`) and feed-more decode fns; for each, identify the carried-state invariant; port the parser to Python (port-of-source oracle, stale-binary-immune) and split a multi-frame/multi-event stream at every offset, diffing vs one-shot.
- RESULT: 🟢 CONVERGED — NO 🔴. The in-hexa carried-state decoders whose chunk boundary is parsed IN HEXA all pass split-equivalence:
  - `stdlib/net/websocket_native.hexa` `ws_recv_native` — `h["buf"]` carry-over across frames via `_ws_fill`/`_ws_take`. CLEAN: every-offset split-equivalence holds (mid-header / mid-2-byte-length / mid-mask / mid-payload, across the 126 & 127 length-form boundaries that change header size mid-stream, plus zero-length, control (ping), and multibyte frames). The buf carry pops exactly the consumed bytes and stores the remainder back — correct by construction.
  - `stdlib/http_sse.hexa` `http_sse_parse_event`/`http_sse_feed` — the partial-event accumulator carried across `feed()` calls. CLEAN: the parser is PER-LINE and lines are atomic (the runtime's `exec_stream` fgets / `channel_recv_lines` hands whole lines), so the carried `event` map is invariant to poll-grouping; one-shot drive == incremental (1-line-per-poll) feed for multi-`data:` events, CRLF, comments, empty values, consecutive blanks.
  - `stdlib/jsonl_pool.hexa` `_jp_cell_next_line` (cell["lines"]/["lpos"]) — CLEAN: the line-split is delegated to `channel_recv_lines` (perl/runtime), not hexa; the cell carry only re-indexes already-split whole lines.
- OUT-OF-SCOPE / documented-precondition (boundary NOT parsed in hexa): `channel_recv_lines` does a single 1MB sysread + `split("\n")`; a line split across two reads fragments ONLY for a single write > PIPE_BUF (512 macOS / 4096 Linux), which is DOCUMENTED at `channel.hexa:400-404` ("atomic for lines ≤ PIPE_BUF; for larger use a dedicated channel pair") — documented precondition = not a bug, and the split itself is in perl. `websocket.hexa` python/websocat backends frame in the subprocess (hexa side carries no decode state). `http.hexa` chunked-transfer = curl de-chunks (no in-hexa decoder). `self/serve/http_server.hexa` `parse_request` is ONE-SHOT (whole `raw`, no feed-more API) = OP-92's single-buffer class, not OP-122.
- ORACLE: Python port-of-source of the ws frame parser (`_ws_fill`/`_ws_take` carry) exhaustively verified streaming-equivalence — one-shot vs all-1-byte chunks vs EVERY two-way split vs 2000 random three-way splits, 0 fails (boundary frames straddling the 126/127 header-length forms included). hexa int is 64-bit SIGNED (OP-118/121); the 127-form top-bit-set wrap is guarded upstream (`val<0` reject, locked by `websocket_native_len64_overflow_test`) and NOT re-tested here.
- LOCK: NEW `stdlib/net/streaming_state_equivalence_op122_test.hexa` HERMETIC guard (`hexa run`, 5/5 PASS): (1) one-shot decode has no short-read sentinel; (2) all-1-byte fragmentation == one-shot; (3) EVERY two-way split offset == one-shot; (4) three-way splits == one-shot; (5) SSE feed-with-carry event sequence == one-shot. Parser logic verbatim-inlined (stale-bundle-immune).
- HONEST g5: the in-hexa streaming-decode surface is split-equivalence-correct BY CONSTRUCTION → evidence-backed "class clean / LANE-1 converged" is the truthful SUCCESS; no 🔴 fabricated; the exhaustive every-offset two-way sweep + the Python oracle are the falsifier that found no divergence.
- FLAGS: NO source edit (CLEAN=🟢) → ZERO byte-eq/fixpoint impact; NEW leaf in `stdlib/net` is build_selfhost-closure-OUT (standalone test) → fixpoint UNAFFECTED; wipe_guard N/A (1 new file, 0 deletions); NO .tape; repro verbatim-INLINED. LANE-1 only — disjoint from sibling lanes (OP-120 recursion-depth, OP-121 comparator). $0 · 0-pod · NO GPU · no vast · no foreign-pod · leak-0. Verdict `.verdicts/hexa-0pod/F-OP122-STREAMING-STATE-CORRUPTION.txt`.

## 2026-06-13 — OP-123 DONE: LANE-2 malformed binary CONTAINER / media / archive parser robustness sweep (CWE-190 int-overflow → CWE-125/787 OOB / DoS) — 🔴×1 FIXED + 🟢 CONVERGED census; the .pcc proof-carrying-checkpoint header parser trusted chunk_count/payload_len/chunk_size/cert_block_len from an untrusted file and formed the layout via raw multiplication, so a 2^62 chunk_count WRAPS chunk_count*20 and slips a nonsense layout past the `expected_total == total_bytes` structural guard (then OOB-traps the dir walk / DoS); now wrap-safe-bounded before any multiply, locked by a hermetic 9/9 guard
- LANE: 2 (0-pod correctness/security — container/chunked-binary parser size-arithmetic int-overflow axis). Complements OP-118 (size-COMPUTATION wrap in crypto/protocol parsers): same CWE-190 root but on a MEDIA/ARCHIVE CONTAINER walker — disjoint from sibling lanes (OP-122 streaming/HTTP) and from the excluded surfaces (net/HTTP, crypto/TLS/x509/asn1 OP-105/118, JSON recursion OP-120).
- CLASS HUNTED: (magic)+repeated-(type/offset, length, payload)-record walkers — image (PNG/BMP/GIF/JPEG), audio (WAV/RIFF), archive (tar/zip/gzip), wasm sections, MessagePack/CBOR, font tables, or any `magic`+`len`+`body` reader — that TRUST a declared chunk length/count/offset from untrusted bytes then read/slice past the buffer, loop unboundedly, or WRAP on the length/offset arithmetic. hexa int = 64-bit SIGNED (wrap real, OP-118/121).
- METHOD: enumerated the parsers that ACTUALLY EXIST in-repo (grep stdlib for chunk/section/riff/magic/leb128/cbor/msgpack/wasm/safetensors readers); for each real parser fed MALFORMED input (declared length > remaining buffer, count huge, wrapping length) and asserted graceful fail vs a Python s64 oracle. Where a format has NO in-repo parser, it is OUT OF SCOPE (nothing invented).
- RESULT: 🔴×1 FIXED. `stdlib/ckpt/verifier.hexa` `verify_pcc` (.pcc header parser): the SOLVED instance is chunk_count = 4392081922311798020 (~2^62.4), chunk_size=1, cert_block_len=0, payload_len=chunk_count → dir_size = s64(chunk_count*20) = -6148914691236517072, payload_start = -6148914691236517036 (nonsense negative), payload_end = 176, expected_total = 200 — a 200-byte crafted file PASSES `expected_total == total_bytes` (the primary defense, DEFEATED by the wrap). Downstream the negative payload_start makes the per-chunk bounds checks no-ops and the `while ci < chunk_count` dir walk can index buf OOB. Runtime OOB = HARD FAULT (`index N out of bounds`, process abort) — so the wrap yields a graceful-but-WRONG reject or an UNCONTROLLED ABORT (DoS; verify_pcc's contract is a tamper-code DICT, never a crash).
- FIX (defense-in-depth, fail-closed, surgical): every declared region is a subset of the file → bound each field against total_bytes BEFORE any multiplication — chunk_count <= total_bytes/DIR_ENTRY_SIZE (wrap-free DIVISION), chunk_size/payload_len/cert_block_len <= total_bytes. Every subsequent product is then far below 2^62 → the layout arithmetic cannot wrap and the structural guard is no longer bypassable. Net-additive guard clauses, 0 deletions.
- CENSUS (rest CLEAN, honest g5): `stdlib/safetensors.hexa` safetensors_read ALREADY hardened (header_len<=0 · 8+header_len>n · per-tensor s>=0 && e>=s && data_start+e<=n before each slice); `stdlib/wasm/wasm_leb128` *_decode loop `while i<n` (buffer-bounded, no OOB) + doc'd @pure scaffold, ZERO alloc/offset-sink callers; wasm_section_header/_type_section/_export EMIT-ONLY; ckpt/merkle + chunk_store operate on in-memory leaf arrays / program-owned data, not a wire byte stream.
- LOCK: NEW `stdlib/ckpt/test/pcc_overflow_op123_test.hexa` — HERMETIC (`hexa run`, 9/9 PASS): valid-accept (clean .pcc still verifies) + 4 malformed-reject classes (2^62 wrap chunk_count, oversized payload_len, oversized chunk_size, oversized cert_block_len) each rejected GRACEFULLY (ok=false, ERR_HEADER_CORRUPT) — no crash, no wrong-region code. Existing pcc_roundtrip + pcc_tamper (5 distinct tamper codes) still GREEN.
- FLAGS: 1 source edit (`stdlib/ckpt/verifier.hexa`) build_selfhost-closure-OUT — verified not referenced by compiler/ or self/ → ZERO byte-eq / self-host-fixpoint impact, no selfhost gate. Net-additive (guard clauses only, 0 deletions) → wipe_guard N/A. 1 NEW leaf test closure-OUT. NO .tape. Repro Python-s64-oracle-modelled + verbatim-INLINED in the leaf (stale-bundle-immune). LANE-2 only — disjoint from sibling LANE-1.
