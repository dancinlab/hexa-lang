...
- [x] 🟡 parity 상속 명문화 — C-emit 경로가 clang -O2 와 동등(loop-unroll/reg-alloc/SIMD/sched 상속, 재구현 0) 임을 측정 확인 (M1 baseline 재사용) — **두 얼굴**: TU 내부 상속 ✅(O0→O2 1.2~1.8×, hash_range 155→87 instr) · raw C-parity 🔴(idiomatic-C 대비 7.9~1263×, 원인=HexaVal 박싱+runtime.o 벽). 상세=`UNSHADOW.bench.md §parity-attest`
