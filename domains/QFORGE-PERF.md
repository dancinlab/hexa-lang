...
- [x] **per-kernel roofline profiler** (QFORGE-PROCESS) 🟢bench-grounded ✅CLOSED-FORM — `qforge_roofline(name, gflops, bytes_s, AI, transc/inner, work)` (bench/qforge/roofline_profiler.hexa) names each kernel's BINDING resource — COMPUTE | MEMORY | TRANSCENDENTAL | LAUNCH — vs the Blackwell B200 roof (FP64 ~40 TFLOP/s · HBM3e ~8 TB/s, datasheet-cited; ridge AI*=5.0 flop/byte). Precedence LAUNCH > TRANSCENDENTAL > {MEMORY|COMPUTE}. Closes the m35 finding deterministically: α²F BZ-sum = **TRANSCENDENTAL-bound** (138.8 GFLOP/s = 0.347% of FMA roof, 3 exp()/inner ⇒ SFU/exp-rate ceiling, NOT FP64-FMA; next lever = fast-exp/poly) · Sternheimer dense-matvec (AI 0.25 < ridge) = **MEMORY** · batched GEMM (AI 64 > ridge) = **COMPUTE** · tiny 1D (work ≪ 1e5 crossover) = **LAUNCH**. d3 — m35 numbers read verbatim, no re-measure. **verdict: `.verdicts/qforge-roofline-profiler/F-QFORGE-ROOFLINE-PROFILER.txt` (🟢 SUPPORTED-NUMERICAL · g5 ALL_PASS)**.
- [x] **H_apply GPU-GEMM (batched)** ⚡hardware-PR 🟢bench-VERIFIED — **root-cause + FIX (2026-06-16, RTX 5070 sm_120)**: 단일-GEMV `qforge_h_apply_forge`(H[n,n]·v[n,1])는 memory-bound(AI≈0.25) + **per-band H2D 전송 지배** → CPU 못이김(K1 wall Δ<1 🔴 — single-GEMV 정직 ruled-out 유지). 근본원인 = Davidson/Sternheimer inner loop 이 m-band block 에 H 를 적용하는데 seam 이 m 개 독립 GEMV(= H[n,n] 를 m 번 재전송)로 했던 것. **FIX = `qforge_h_apply_forge_block`(assembler.hexa) — m apply 를 ONE GEMM `H[n,n]@Ψ[n,m]` 로 fold, H 1회 전송, GEMM(tensor-roof) AI≈m**. **실측(`nvptx_happly_block_host.cu` VERBATIM): batched GEMM 가 CPU 를 거의 전 size 에서 이김 — n=2048 m=64 cpu 310.5ms → GEMM 4.14ms (74.9×) · per-band GEMV 157.3ms(1.97×) 대비 37.97× 빠름 · parity max_rel 4.7e-11(FP64-tol) ALL PASS**. crossover: m≥32 면 n=256 부터 win(7.97×); m=8 작은 n 만 미달. CPU-build byte-eq gate(`happly_block_bench.hexa`): block ≡ m 스칼라 matvec, maxAbsDiff ≤3.2e-14 PASS. **falsifier(wall Δ≥1 at size ∧ fp-tol parity) 충족** — K2(Davidson VᵀHV 45-61×)가 연 batch-roof 를 H-apply 가 직접 실현. 잔여(d6): 엔진이 per-iter 로 batched seam 을 호출하도록 davidson/sternheimer 배선(별 항목 · seam+bench 박제 완료).
