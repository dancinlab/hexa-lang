...
and `tool/determinism_gate.sh` (M6 extends the latter to the gen3 DEFAULT path
and wires it as `.github/workflows/selfhost-determinism-gate.yml`). Both
classify a build-floor "cannot native-emit here" as exit 2 → CI-neutral
(#2547), so a runner that lacks the graduated gen3/`gen2_fix` never false-fails
a PR. SELFHOST-CI follows the exact same exit-2-neutral discipline.
...
- [x] determinism — `tool/determinism_gate.sh` EXTENDED to the gen3 DEFAULT promoted path + wired as `.github/workflows/selfhost-determinism-gate.yml` (per-PR + push(main)). The gate now auto-discovers the graduated gen3 slot `$HX_HOME/self/native/selfhost/{gen3,hexa_ld,rt.o}` (same slot as the byte-eq / promote-parity sisters) and drives the REAL promoted compile+link route: PHASE 1 `gen3 --emit=obj` each program twice → cmp byte-eq; PHASE 2 `hexa_ld -o out prog.o rt.o --lc-main _main` twice (same basename, two dirs, build-id isolated) → cmp byte-eq — not the legacy standalone hld_fixed bootstrap link. Real run PASS (exit 0) on ghost AND mini (Darwin arm64, graduated slot): 12/12 re-emit BYTE-EQ + 12/12 relink BYTE-EQ, FIRSTDIFF=0 on both axes (explicit cmp proof). exit-2-neutral canary verified by hiding the slot (rc=2 → workflow remaps to ::notice:: neutral, same #2600/#2605/#2607/#2608/#2609 shape). A TRUE nondeterminism (two runs differ byte-for-byte) is the only red (exit 1). Verdict `.verdicts/selfhost-ci/DETERMINISM.txt`.
