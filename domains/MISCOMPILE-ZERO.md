...
- [ ] (DEPENDS ↑ on the `hexa run` HANG fix) once `hexa run` terminates on the sgemm loop → finish the BLOCKED anima C-PORT sgemm RUNEQ: `hexa run sgemm_ref_native.hexa` on `cport/training-next`, diff vs the C reference (8 cases, e.g. 16³ tA0tB0 sum=5.7592 maxabs=6.2588), and if value-exact merge the anima PR — closes the last cross-repo C-PORT portable leaf (handoff `4b98348f`)
