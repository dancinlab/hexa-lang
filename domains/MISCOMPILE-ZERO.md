...
- [ ] `hexa run` HANG on compute-heavy loop (sgemm triple-loop) — `hexa run` on anima `cport/training-next:training/native/sgemm_ref_native.hexa` (169 LOC, GEMM up to 128³) NEVER TERMINATES (>44min, 0 stdout; reproduced both local mini AND pool summer/aiden). Candidate class: miscompiled loop-counter / regalloc-spill → non-terminating branch, or pathological interp on nested f32 accumulation. ACTION: capture a MINIMAL in-repo repro (nested int loop + f32 accum) under `self/test/miscompile_class/`, diagnose vs clang oracle, fix, byte-verify. Surfaced by cross-repo C-PORT (sidecar handoff `4b98348f`)
