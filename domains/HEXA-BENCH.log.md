...
(GEMM_BACKEND=5: cublasLtMatmulAlgoGetHeuristic, TF32 + -DLT_BF16, + CUBLASLT_EPILOGUE_GELU probe).
Real H100 80GB (vast 40078129, sm_90a, CUDA 12.4, torch 2.4.0; rented ONE, labeled hexa-bench9,
DESTROYED leak-0 tag-checked; foreign rtsc-li2mgh16-anchor untouched).
...
- Verdict .verdicts/hexa-bench/F-BENCH-9.txt (full matrix + root-cause + raw [RESULT] verbatim);
  files tool/bench/flame_bench_step_og.cu (GEMM_BACKEND=5) + tool/bench/run_bench9.sh.
