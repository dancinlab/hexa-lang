...
- [ ] **PRECISION-CHANGE — TF32/BF16 own-GEMM flame step, does precision uncap >3×?** 🟡 WIP (2026-06-08, F-FUSION-PRECISION-CHANGE): the FIRST honest uncap lever g85 names beyond the ~3× batch-fill cap. runtime_cuda.c PATCHED with env `HEXA_GEMM_PREC=fp64|tf32|bf16` in BOTH matmul launchers (single + strided-batched): TF32 = fp64→fp32 + cublasGemmEx COMPUTE_32F_FAST_TF32; BF16 = fp64→__nv_bfloat16 + cublasGemmEx CUDA_R_16BF/COMPUTE_32F; one-shot rel_rms-vs-FP64 gate print (W14 convention ≤1e-2, NOT byte-eq — precision changed deliberately). Sweep prec×B{1,4,8,16} on a real H100/H200, step/s vs FP64 + util-median. Honest dual outcome: UNCAP (quantify new ceiling) OR closed-neg (the wall is the serial PER-OP launch dispatch = precision-independent → only right-sized-GPU remains). verdict `.verdicts/hexa-fusion/F-FUSION-PRECISION-CHANGE.txt` (PENDING). build kit `.verdicts/hexa-fusion/precision-change-build/`.
