...
- [ ] **INTERP-ELIM — native/AOT-compile the per-step driver, removing the bytecode interpreter from the train-step hot path** — the ~1.3×→3× UNCAP lever named by both vs-PyTorch (#2912, ~1656× torch gap = interpreted driver) and BATCH-FILL (#2913, ≈3× self-speedup asymptote = interpreted per-position glue ∝B·Tw). Native-compile clm_prod.hexa's per-step driver (window t_get/t_set pack + im2col/col2im gather + CE softmax-grad + AdamW tail) via the proven `hexat` transpile → gcc -O2 native binary (the CC-NATIVE / R4-native path, PLAN-interp-retirement.md), removing the interp from between the GEMM bursts. GATE: native step == interpreted step byte-eq max|Δ|=0 (FP64, same weights/grad/loss after the step). MEASURE: native-compiled-driver step/s + util vs the bytecode-interpreted baseline (`hexa run`) at the batch sweep; report the new self-speedup ceiling vs the ≈3× batch-fill cap + % of the ~1656× torch gap closed. verdict `.verdicts/hexa-fusion/F-FUSION-INTERP-ELIM.txt`.
