...
- [x] **FF-GN-PARALLEL — fixed-order parallel-tree GN reduction (realizes FF-VALLEY's #1 reclaimable: the single-thread GN reduction)** 🟢 GREEN (2026-06-08, F-FUSION-FF-GN-PARALLEL, H100 SXM sm_90, vast 39970223 tag hexa-ffgn) — replaces the byte-eq-FORCED single-thread serial GN reduction (DUTYCYCLE: 90.5% of GPU-ACTIVE time, the two GN reductions @105-132ms EACH) with a **fixed-order block-tree** (NB=256×BS=256, ascending-index lane partition + fixed pairwise stride-halving combine + single-block finalize over partials, **NO atomics/NO race**) for fwd mean/var + bwd_dx s1/s2. The reference RE-BASELINES off the serial sum to the tree's fixed associativity. **VERBATIM (g5): GATE1 DETERMINISM run-to-run + cross-process max\|Δ\|=0 bitdiff=0 (bit-reproducible, preserves reproducibility north-star). GATE2 REBASELINE tree-vs-seq max\|Δ\| fwd 2.55351295663786e-14 / bwd_dx 1.2656542480726785e-14 = machine-eps FP64 reassoc (NOT accuracy regress). GATE3 SPEEDUP: GN fwd 93.9→0.026ms (3612x), bwd_dx 118→0.017ms (6838x), GN total 211.9→0.043ms ≈4900x.** FULL-STEP honest (g5): GN was 90.5% of GPU-ACTIVE but only 9.9% of SPAN → **GPU-active self-speedup 4.0x** (union 7571→1870ms), but **FULL-STEP wall self-speedup 1.11x** (span 57365→51664ms, gap unchanged). **DOES NOT break ~3x on the full step** — the 86.8% device-idle GAP is now the BINDING wall (untouched by this lever). FIRST lever attacking the RIGHT term (GN reduction, NOT GEMM/launch/dtype — INTERP-ELIM/PRECISION-CHANGE/RIGHT-SIZED-GPU all closed-neg on the WRONG term). => **FF-XSTREAM (cross-stream valley overlap + host-glue elim) is the CO-REQUISITE next lever**; only fast-GN + overlapped-valley together can move the full-step wall. pod DESTROYED leak-0 (tag confirmed; prior broken-sshd pod 39968941 also destroyed tag-confirmed). verdict `.verdicts/hexa-fusion/F-FUSION-FF-GN-PARALLEL.txt` · harness `.verdicts/hexa-fusion/ff-gn-parallel-build/{gn_tree.cu,build_run.sh}`.
