...
- [ ] **MEGA-OWNGEMM-INTEGRATE — wire the OG17 TF32-PARITY own-GEMM INTO the persistent whole-step megakernel** — the megakernel's in-kernel GEMM currently calls the OLD naive device `_hx_k_gemm`; swap it for the bit-exact OG17 canonical-atom + relaxed-pipeline wgmma kernel (the 1.24x cuBLAS-PARITY one) so the fully-realized cuBLAS-free whole-step graph runs the PARITY GEMM in-line. **honest (g5)**: VALUE = COMPLETENESS/own-ability (100% hexa-owned cuBLAS-free whole-step with a parity-class in-kernel GEMM), NOT a util win (util-via-megakernel is closed-neg, byte-eq perp util-lift). Gate: whole-step byte-eq max|delta|=0 (fused-with-OG17 == op-by-op ref) FIRST, then in-kernel GEMM TFLOP/s vs standalone OG17 280 (does megakernel context regress the parity kernel?). cuBLAS=roofline, no superiority. Builds on the megakernel 2nd-wall-closed (own-GEMM removed the cuBLAS-call wall #2697) + OG17 #2870.
