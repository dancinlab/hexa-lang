...
- [ ] **RIGHT-SIZED-GPU — the LAST honest uncap lever (g85, after precision REFUTED #2917)** — does the REAL flame CLMConvMoE step at the EXACT H100-baseline shape (D1536/T512/E2/K3, FP64) on a right-sized GPU (~50-80 SMs, but ≥24 GiB so D=1536 FITS — closing the P2 12-GiB OOM caveat) lift util MEDIAN off ~0%? falsifier: util MEDIAN climbs off the occupancy-wall floor (grid fills the smaller device = efficiency win) vs persists bimodal-low regardless of SM count (TERMINAL: launch-dispatch serial idle, only a fused megakernel fills it — structurally closed). HONEST: a right-sized GPU does NOT beat H100 in absolute step/s (fewer/slower SMs); the thesis is efficiency (high util at matched shape), not throughput. verdict `.verdicts/hexa-fusion/F-FUSION-RIGHT-SIZED-GPU.txt`. WIP — pod-fire pending (STORM backoff).
