...
- [ ] **BATCH-FILL throughput (samples/s) lever — the REAL ~1.3× the vs-PyTorch data pointed to** — M5-BATCHFILL closed-neg the *util* axis (MEAN flat ~0.45% B=1→32) but util=duty-cycle, NOT throughput. THIS measures **samples/sec** across batch {1,2,4,8,16,32,64} on real H100/H200: samples/s=(B·nbatch·epochs)/wall, 2-point timed (init/load-subtracted). Falsifier: find the batch where flame samples/s ≥1.3× the B=1 baseline + report FULL curve + saturation + whether the per-position interpreted glue (window t_get/t_set + CE-grad + AdamW, all ∝B·Tw) caps it. verdict `.verdicts/hexa-fusion/F-FUSION-BATCHFILL.txt`.
