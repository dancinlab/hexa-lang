...
## 2026-05-30 — 🔵 NaN-boxing HexaVal 표현 (DESIGN + feasibility + 최소측정 · 전역 flip 아님)

**의도적 honest 분해.** milestone 이 "블라스트 大 → C1·C13 이후 · GATED default OFF" 로 명시한
대로, 전역 ABI 변경을 1-batch 로 강행하지 않고 **설계 + 규모산정 + 최소 A/B 측정 + sub-task open**
으로 분해. 설계 SSOT = `domains/UNSHADOW.nanbox.md`.

- **사실 정정**: milestone "24B→8B" 의 24B 는 **stale**. 실측 `sizeof(HexaVal)=16B`(`{HexaTag
  tag; union}` = 4B+4B pad+8B). 밀도 이득 = **2× (16→8)**, 3× 아님. (`§unboxed-array` L701 이미 정정.)
- **레이아웃 설계**(nanbox.md §2): f64 quiet-NaN payload 에 tag+payload 패킹. box 마커 = sign|exp|
  qbit = `0xFFF8...`. **float 은 passthrough**(비트 그대로, 박싱 안 함) · int/bool/ptr 만 패킹.
  ptr=48비트 VA(arm64/x86-64 fit, LA57/52비트 VA 는 이식성 risk). 12 tag>3비트 → 2-tier 권장
  (포인터-카테고리는 deref 시점에 이미 앎).
- **규모 산정 = multi-session (1-batch 불가)**. NaN-box 는 union 을 **물리 제거** → 레이아웃 가정
  사이트 전부 동시 flip. worktree grep 실측: `HX_*` 매크로 **1151** use + emitted-C `TAG_*` **430**
  + `((HexaVal){.tag=..})` compound-literal **19**. 매크로 use 다수는 본문 교체로 흡수되나, 19개
  compound-literal construct(§hexaval-unbox·§native-arr·§B 가 emit 하는 형식)는 매크로 밖 = 전역
  flip 의 단일 토글 지점. 두 표현 공존 = dual ABI(runtime.o+user.c 함께 재빌드 필요 → B9 벽 충돌).
- **최소측정**(faithful A/B proxy, mini arm64, best-of-7 안정화 · `tool/unshadow_nanbox_bench.hexa`):
  3 축 = sequential traverse+sum **2.5–2.8× 느림** 🔴 · value-pass register-fit **30–36% 느림** 🔴 ·
  random/cache-pressure **7–11% 빠름** 🟢(유일 승). sizeof 16→8(2× denser) · int round-trip
  checksum match=YES.
- **정직 정정 (register-fit 가설 falsified)**: 초기 단발 측정의 "value-pass 37% 빠름" 은 측정
  artifact. best-of-7 안정화 시 일관 **30–36% 느림** — 매 호출 box/unbox(mask+shift+OR) 추출비용이
  8B=1 reg register-fit 이론 이득을 압도. NaN-box 우위는 **memory-bound random-access 단 하나**로 좁다.
- **NaN-collision 제약**(nanbox.md §5): 진짜 f64 NaN=`0x7ff8...`(sign=0)·box 마커=`0xFFF8...`
  (sign=1) → canonical positive qNaN 충돌 안 함(`is_boxed()=0` 실측). 단 negative/signaling/
  payload NaN 은 마커 영역과 겹침 → **모든 float store canonicalize 必**. raw f64 비트 관찰(bit-cast·
  직렬화·hash-of-double) 프로그램은 안전하게 못 덮음 = 이 표현의 **결정적 한계**.
- **C1 의존 정정**: milestone "C1 이후" 라 했으나 §4 가 **반전** 드러냄 — NaN-box 는 sequential-array
  에서 퇴보 → C1(native contiguous int64[]) 과 **상충**(의존 아니라 분리·종목 다름). C13(escape→
  stack)과는 밀도 8B 가 stack frame 절반으로 시너지.
- **결론**: 전역 flip 은 GATED default OFF sub-task 로 open 유지(nanbox.md §8 선결 5항). codegen
  무변경 = byte-diff N/A(전역 표현 미flip). milestone `- [ ]` 유지(설계+feasibility-only).
...
## 2026-05-30 · §native-arr sub-task: known-int ACCUMULATOR 일반화 (부분 착지 🟢)

§native-arr 가 등록한 #1 sub-task. B 가 native int64_t[] STORAGE(d_ideal 천장)는 착지했으나
LANDED 1.18× 만 잡은 이유 = read→sum 체인이 **누산기**에서 boxed HexaVal surface 재생성 →
`let mut acc=0`(untyped mut)이라 `acc += xs[i]` 가 out-of-line `hexa_add`(runtime.o C-ABI 벽)
→ clang -O2 vectorize 차단(vec-op 8<28).

**착지(`self/codegen.hexa`)** — known-int fact 를 accumulator 까지 전파:
- `_known_int_accum_set` 추적기 + `_known_int_accum_scan(fn-body 보수적 스캔)` (gen2_fn_decl
  진입에서 호출·fn 종료 reset). `let mut acc=<known-int init>` 이고 전 body 의 모든 write
  (`=` / `+= -= *= & | ^ << >>`)가 accum-int-safe(IntLit·known-int name·`acc` 자신·증명된
  int-literal-array `arr[i]` read·그들의 `+/-/*…` BinOp)면 등록. `/` `%` `**` 제외(div-by-zero
  throw 필요). closure-capture(boxed cell) 제외. scan-local int-literal-array set 로 RHS `arr[i]`
  증명을 self-contained(let-emit 전 스캔이므로 global `_known_intarr_set` 미사용).
- `_is_known_int` 가 accum 도 certify(Ident 분기, fn-local `let mut` 가 `_is_known_int_name` 의
  `_gen2_name_in_cur_lets` bail 에 걸리므로 BEFORE 에서 직접 체크).
- CompoundAssign 가 `acc<op>=rhs`(op∈safe·`_is_known_int(rhs)`)를 inline compound-literal `.i` 형
  으로 emit. AssignStmt `acc = acc <op> rhs` 는 기존 BinOp known-int fast-path 가 처리(accum
  certify 로 자동).
- GATED `HEXA_NATIVE_ARR`(§native-arr 플래그 재사용) default OFF=무회귀.

**버그 수정(같은 branch)**: `_is_accum_int_safe` 가 BinOp kind 를 `"BinaryOp"`로 잘못 검사(파서는
`"BinOp"` emit·codegen 핸들러 L5245 도 `"BinOp"`) → `acc = acc + xs[i]`(AssignStmt-BinOp RHS) /
`acc += <BinOp>` 형이 전부 reject → 누산기 미등록. 디버그 eprintln 으로 `writes_all_int=false`
확인 후 `"BinOp"`로 근본수정(순수 `acc += xs[i]` CompoundAssign+bare-Index RHS 는 통과했으나
BinOp RHS 경로가 죽어있었음).

**실측(mini Apple M4 arm64 · best-of-11 · faithful A/B proxy · B9 regen 벽 → proxy)**:
- g5 byte-diff: **4/4 IDENTICAL** (`35470124`). 추가 정확성 — 단순 read→sum(150) ON=OFF
  `176ef0…` · integrity(`+= -=` mixed · BinOp `acc=acc+x` · 음수 · 3×10⁹ overflow;
  1038601 / 2999999940) ON=OFF `9e3d23e4…`. 전부 byte-exact.
- codegen 발화 검증(real self-host hexat 재빌드 `/tmp/hexat.new`, /tmp transpile): FLAG ON =
  `acc=((HexaVal){.i=(HX_INT(acc)+HX_INT(((HexaVal){.i=data[i]}))})` · FLAG OFF =
  `acc=hexa_add(acc,items[i])`(boxed) 무변경.
- **e_boxed_acc(LANDED out-of-line hexa_add) 1.67s → f_inline_acc(THIS inline `.i`) 1.12s =
  1.49×**. asm `bl _hexa_add` **1→0**(총 bl 9→6) — runtime.o C-ABI 벽 호출 제거.
- d_ideal(raw `long acc`) 0.08s = 천장. 잔여 1.12→0.08s gap = HexaVal-struct 누산기(16B·step
  마다 compound-literal re-materialize·vec-op 8 vs d_ideal 18) → raw C `int64_t` local 강등이
  필요(다음 sub-task §typed-repr accumulator). **이 슬라이스 = 누산기 ABI-벽 닫힘 · register-pack open.**

**B9 노트**: `self/native/hexa_cc.c` 는 gitignored generated(SSOT=`self/codegen.hexa`) → 커밋 안 함.
regen 은 install 패키지 hexa_cc.c 직접 transpile($HEXA_LANG override 가 이 0.1.0-dispatch 바이너리엔
미적용 → install copy 임시 swap 으로 우회). 측정 빌드 sign-gate(mac fork-storm guard) → `pool on mini`.

verdict=`.verdicts/unshadow-knownint-accum/bench.txt`·bench=`UNSHADOW.bench.md §knownint-accum`·
재현=`tool/unshadow_knownint_accum_bench.hexa --rt ~/.hx/packages/hexa/self --runs 11`.

## 2026-05-31 — native-arr sub-task: F64 path (HexaArrF64) + no-escape auto-fire [~ code-landed, measure-pending]

Mirror of the landed §native-arr i64 path for doubles, PLUS unlock of the GATED
opt-in to default-ON auto-fire when whole-binding escape is statically proven safe.

PART 1 — F64 native storage (runtime + codegen):
- self/runtime_core_emit.hexa: `typedef struct HexaArrF64 { double* data; int len;
  int cap; }` (layout matches HexaArr/HexaArrI64 -> polymorphic hexa_len unchanged)
  + public-in-runtime.h decls/bodies hexa_arr_f64_new/push/len/box (pierces the
  runtime.o C-ABI wall, §c-class). box returns hexa_float(data[i]) at the boundary.
- self/codegen.hexa: _is_float_literal_array (monomorphic-f64 proof: every elem a
  FloatLit or UnaryOp(-, FloatLit)); _known_floatarr_set + add/void/is_name
  (lifecycle twin of _known_intarr_*); LetStmt lowers `let xs = [f64-lit…]` to
  native contiguous double[] (hexa_arr_f64_new/push) when auto-fire fires; the
  Index read arm emits the raw native double load as an INLINE TAG_FLOAT compound
  literal `((HexaVal){.tag=TAG_FLOAT,.f=(((HexaArrF64*)xs.arr_ptr)->data[i])})`
  (out-of-line hexa_float would re-box behind the wall — §hexaval-unbox lesson);
  _is_known_float gains an Index arm so a proven `xs[i]` read is statically
  TAG_FLOAT (surrounding sum extracts raw .f); AssignStmt/CompoundAssign reassign
  voids _known_floatarr alongside _known_intarr.

PART 2 — no-escape auto-fire (GATED opt-in -> default ON when proven safe):
- _native_arr_noescape_scan(body) pre-scan REUSES the §escape-stack analyzer
  _stmt_escapes_name / _expr_escapes_name over every sibling statement. The
  analyzer treats `xs[i]` element reads + `xs.len()` as local and every other
  mention (call-arg, return, let-flow, store, aggregate literal, bare operand) as
  an escape. A zero-escape binding is added to the proof set.
- _native_arr_autofire_ok(name) = proven-non-escape (AUTO-FIRE, no env flag) OR
  legacy HEXA_NATIVE_ARR flag (escape hatch). Wired into all four construct + read
  sites (replacing the bare flag gate). gen2_fn_decl runs the scan fn-scoped.
- Net: a read-only-loop monomorphic int/f64 array literal proven non-escaping now
  lowers to native contiguous storage BY DEFAULT; an escaping binding stays boxed
  (no regression, escape-driven not name-driven).

STATIC VALIDATION (this session): self/codegen.hexa + self/runtime_core_emit.hexa
brace and paren deltas both 0; all new F64 fns + the runtime struct/helpers
present; no leftover combined gate.

HONEST CAVEAT — MEASUREMENT PENDING. The pool A/B byte-diff (best-of-9, asm vec-op
counts, per-arm wall, the NEGATIVE-CONTROL grep) was NOT captured this session: the
harness stdout channel failed for extended stretches (Bash/Read tool calls returned
suppressed output), so the build/transpile/measure loop could not be observed. Per
claim_verify / paper_significance NO perf numbers are fabricated. The verdict (g5
byte-diff IDENTICAL across ref/a_boxed/b_native/d_ideal + boundary + negative
control) is the gate to flip the milestone to [x]; see
