...
## §knownint-rawlocal (native-arr sub-task — raw int64 local 강등) — 🔴 CLOSED-NEGATIVE

> milestone "🔵 native-arr sub-task: known-int accumulator → raw int64 local 강등 (§typed-repr accumulator)" 의 실측. **요지**: §knownint-accum(PR #2202)이 누산기 ABI-벽을 inline `.i` 슬라이스로 닫은 뒤, 남은 f_inline→d_ideal Δ 를 raw C `int64_t` local 강등(box surface 완전 제거)으로 회수해 register-pack 천장에 도달하는지 검증.
> SSOT 도구 = `tool/unshadow_knownint_rawlocal_bench.hexa` · verdict = `.verdicts/unshadow-knownint-rawlocal/`.

측정: `mini` (macOS arm64) · Apple clang 21.0.0 · -O2 · runtime.o ABI 벽 = hexart.c out-of-line(boxed arm 만 `bl _hexa_add` 지불) · faithful A/B proxy(B9 self-host rebuild 차단·스펙 허용) · 2026-05-31.

### 표 — hot-loop asm + wall

| arm | lowering | hot-loop asm md5 | `bl _hexa_add` | wall best-of-9 n=1e9 |
|-----|----------|------------------|----------------|----------------------|
| e_boxed_acc  | boxed HexaVal, out-of-line hexa_add | (links runtime.o) | 1 | **0.67s** (ABI 벽·fold 불가) |
| f_inline_acc | inline `.i` slice (PR #2202)        | `436ccab8` | 0 | **0.00s** (box-free → 완전 최적화) |
| g_raw_local  | raw int64 local (this sub-task)     | `436ccab8` | 0 | **0.00s** (== f_inline) |

- **THE gate**: f_inline vs g_raw hot-loop 정규화 asm **byte-IDENTICAL** (md5 `436ccab8` · diff exit 0).
- f==g wall (둘 다 0.00s = 동일 최적화 tier; e_boxed 만 opaque hexa_add 로 fold 불가 0.67s).
- g5 정확성 3-arm match: 단순(n=5/1e6) · 3×10⁹ overflow (int64 wrap 동일).

### 정직한 finding — 🔴 CLOSED-NEGATIVE (축 ruled-out)

clang -O2 의 **SROA/mem2reg 가 single-field `{.tag,.i}` box 를 이미 register 로 scalarize** 한다(.tag = loop 내 dead·.i = register promote). inline `.i` 슬라이스가 emit 하는 hot loop 가 **이미** raw int64 register add — 손작성 raw int64 local(g_raw)과 byte-identical. 따라서 raw 강등은 **추가 Δ 0**. box surface 는 -O2 를 넘어 **생존하지 않으므로** 제거할 대상이 애초에 없다 — f_inline IS the ceiling.

**RULED-OUT AXIS**: single-int accumulator 의 source-level box-stripping 은 clang -O2 하에서 dead lever(SROA 가 box-eliminator 역할). 유일 잔여 레버 = e_boxed 의 ABI-벽 tax(`bl _hexa_add`, 0.67s) = SROA 가 닫지 못하는 multi-field / escaping / aliased HexaVal(runtime.o 경계 통과)에서만 의미. §knownint-accum 이 남긴 "d_ideal 잔여 갭" 은 inline-.i arm 의 생존 box 가 아니라 e_boxed out-of-line ABI arm 이었음. codegen 무변경(확인된 no-op 을 GATED 로 심으면 dead complexity) → byte-diff = asm-IDENTICAL(no emit change). paper_negative_ok: 한 축(single-int accumulator raw-강등) 결정적 배제 = valid terminal.

> caveat: 단일 호스트(mini) 단일 세션 · 결정적 test = hot-loop 정규화 asm md5(constant-step wall 은 f/g 를 둘 다 0.00s 로 fold — asm-identity 와 일관, 동일 tier 확증) · runtime.o ABI 벽 = out-of-line hexart.c(boxed arm = 진짜 cross-TU call) · full self-host rebuild = B9 벽 차단 → faithful C A/B proxy(스펙 허용; C 문자열은 codegen 누산기 emit 형태와 byte-faithful) · repo 안 `.c` 0개(proxy = /tmp/unshadow_rl).
