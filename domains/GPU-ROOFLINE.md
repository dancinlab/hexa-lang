...
## milestones — deferred (다음 frontier)

- [ ] 🔵 hexa-emit GPU 커널 achieved roofline % (cuBLAS 가 아닌 hexa codegen 산출 커널) — 현재 hexa-emit wmma 는 256×256 shape-locked(GPU.md PR #214, ratio 0.767 vs cuBLAS). variable-shape 커널 emission(multi-session codegen) 후 hexa-emit 커널의 직접 achieved/peak % 측정. 게이트 = byte-eq + roofline %
- [ ] 🔵 PTX-diff → roofline 예측 (fire 없이) — hexa-emit vs nvcc -ptx instruction histogram 으로 AI(flops/bytes) 정적 추정 → binding-roof 예측 → $0 사전 roofline % 추정. 실측 fire 와 대조해 예측 정확도 게이트
