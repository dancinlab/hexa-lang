...
- **분모 = achieved-peak 실측**(멀티디바이스: ubu-2 RTX 5070 sm_120 + A100 80GB PCIe sm_80). theoretical 스펙 나란히. 디바이스별 분모 명시. 상세 = `GPU-ROOFLINE.bench.md §roofline · §multi-device-peak`.
...
- [ ] 1b. **HIR `[T; N]` fixed-array surface → shared-tile 크기 parametric** — 현 `.shared` staging 은 synthetic
  ...
  **🟠 2026-05-31 honest STOP — frontend+backend = >1 batch(flip 보류, fail 아님)**: backend SMEM 소비
  측은 이 도메인 자신의 선행 코드-감사(2026-05-30, 위 규모-산정 노트)가 이미 박제 — `_nvptx_shared_default_bytes`
  (`nvptx_target.hexa:3345-3352`)가 synthetic 고정 2048 B 를 반환하고 shape-param 진입점이 없다. 1b 가
  요구하는 frontend `[T; N]` fixed-array 크기 surface 는 도메인 노트가 명시하듯 "현재 없음 — codegen 이
  직접 명시". 착지하려면 lexer+parser 의 `[T; N]` 타입 문법 + (T,N) 운반 타입-AST 노드 + HIR/MIR 의 N-전파
  + backend `_nvptx_shared_default_bytes` 의 tile-size 산출식 교체 = frontend+backend 전면 변경 → 단일
  <200줄 batch 불가(규모-산정 경고 "frontend(HIR/MIR)+backend(NVPTX) 양쪽 변경 = multi-session"·1a
  honest-STOP 선례와 동일). **다음 세션 진입점**: HIR fixed-array `[T; N]` 크기 surface 착지 후
  `_nvptx_shared_default_bytes`(nvptx_target.hexa:3345)를 tile-size 산출식으로 교체. 상세 = log.md.
