...
\midrule
ThH$_{10}$        & $1.85$ & $-0.351$ & deep TRAPPED               & Semenok 2018                          & \tierGreen \\
...
LaBeH$_8$         & $2.40$ & $-0.042$ & \textbf{marginal TRAPPED}  & Belli--Bergara 2023                   & \tierGreen \\
YbH$_6$ (predict) & $2.50$ & $0.000$  & wall (boundary)            & clathrate prediction                  & \tierGreen \\
...
hydrides at $\lambda_{\text{target}}=2.5$. The classification map
spans deep-TRAPPED ($m\approx -0.35$, ThH$_{10}$) through the
$m=0$ wall (YbH$_6$ predicted) into deep-ESCAPE
($m\approx +0.17$, $\lambda=3.0$). Of the measured / predicted
candidates, only \textbf{YH$_{10}$ (Errea 2020) sits inside the ESCAPE
region}; CaH$_6$ is a Wang--Ma 2012 prediction anchor at $m=+0.074$.
LaBeH$_8$ (Belli--Bergara 2023, $\lambda=2.4$) at $m=-0.042$ is
\textbf{marginal TRAPPED} --- the closest measured candidate to the
$m=0$ wall on the TRAPPED side, requiring a $+5\%$ $\lambda$
enhancement to cross.}
...
$\operatorname{sign}(m)$ as the binary/ternary discriminant. Of the
eight measured / predicted candidates in
Table~\ref{tab:m-classification}, only \textbf{YH$_{10}$ at $m=+0.035$
sits inside the ESCAPE region} (Errea 2020, $\lambda=2.59$); the
CaH$_6$ prediction at $m=+0.074$ (Wang--Ma 2012) is the closest
clathrate-prediction anchor on the $m>0$ side. The most striking
near-boundary case is \textbf{LaBeH$_8$ at $m=-0.042$}
(Belli--Bergara 2023, $\lambda=2.4$), the closest measured candidate
to the wall on the TRAPPED side --- a $+5\%$ $\lambda$ enhancement
would cross it.

\medskip
\noindent The next most important
follow-up is therefore (a) the direct high-pressure synthesis
of YH$_{10}$ at the lowest pressure consistent with its parameter
tuple holding, and (b) a doping / pressure scan of LaBeH$_8$ aimed
at the $+5\%$ $\lambda$ enhancement to cross $m=0$. The pre-registered
falsifier is unchanged: any measured ternary hydride
that scores $m\leq 0$ under its self-consistent parameter tuple
$\{\lambda,\omega_{\log},\bar\omega_2,\mu^*\}$, and still reports
$T_c\geq 293$\,K at ambient pressure ($p<10$\,GPa),
falsifies Theorem~\ref{thm:unify}.
