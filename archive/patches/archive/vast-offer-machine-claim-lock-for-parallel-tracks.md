...
**Status**: SHIPPED (suggested-fix (1), best-effort) — `stdlib/cloud/vast.hexa::vast_offer_claim` implements the process-shared, file-backed claim ledger asked for in (1). hexa has no `flock` builtin, so the advisory mutex is a POSIX-atomic `mkdir` of `$HOME/.hexa-cloud/claims.lock` (mkdir of an existing dir fails — atomic test-and-set) guarding a read-modify-write of a newline-delimited `$HOME/.hexa-cloud/claims` file (`offer_id<TAB>claimed_at_sec`). Stale claims older than 300s are GC'd on each load so a crashed sibling cannot wedge the pool. `vast_search_offers` calls `vast_offer_claim` to pick the first un-claimed candidate (this also gives effect (2) — sibling processes diverge naturally because each accumulates claims — without a separate RNG); `vast_offer_release` returns an offer to the pool. A single-track caller is never blocked (falls back to the first id on lock-timeout / all-claimed). Suggested-fix (3) (tighter post-create detection) and (4) (Vast API fail-fast — external dependency) are out of scope for this lifecycle wrapper. Lock acquire/contains/release logic is in-file; the mkdir mutex is a standard POSIX idiom (live multi-process race-rate measurement is downstream — needs creds).
