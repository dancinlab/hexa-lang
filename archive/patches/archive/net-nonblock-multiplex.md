...
> **id**: `net-nonblock-multiplex` · **opened**: 2026-05-13 KST PM · **landed**: 2026-05-14 KST · **status**: `applied` — `hexa_net_set_nonblock(fd) -> 0/-errno` + `hexa_net_select(fds, timeout_ms) -> [ready_fds]` in `self/native/net.c`; codegen direct-emit at 1-arg / 2-arg + `_is_builtin_name` registration; shim globals `net_set_nonblock` / `net_select` already wired. Mac arm64 live PASS: empty select returns [], 2 in-proc clients accepted via select+accept loop, both messages read via multi-fd select, poll-only (timeout=0) returns immediately. Wilson 23/23 smoke PASS.
> **VERIFIED-CLOSED 2026-05-19**: SSOT grep cross-verified — `self/native/net.c` carries `hexa_net_set_nonblock` + `hexa_net_select` (grep ×5). Already `applied` 2026-05-14; this is a close-only marker (no source change, no fix re-run — fix already live in SSOT, dup-race precheck per @D g_inbox_processing_loop).
