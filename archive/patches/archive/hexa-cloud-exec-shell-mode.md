...
filed-at: 2026-05-24
resolved-at: 2026-05-25
related: hexa cloud (run · nohup · poll · copy-to · copy-from)
---

**Status**: resolved-2026-05-25 — `cloud_exec_opts` + `cloud_exec` added in `stdlib/cloud/cloud.hexa`, `exec` subcommand in `stdlib/cloud/cloud_cli.hexa`, byte-equivalent fallback help in `self/main.hexa` (per subcommand-help-scoping invariant). Shell-mode dispatch — post-`--` tokens joined with single spaces, sent to remote login shell AS-IS; caller owns quoting per patch §"제안 시그니처". `_ssh_capture` adds `__CLOUD_RC__=` marker for remote exit-code recovery same as `run`.
