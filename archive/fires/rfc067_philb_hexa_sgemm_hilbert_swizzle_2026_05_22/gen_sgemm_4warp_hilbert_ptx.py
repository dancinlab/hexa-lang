#!/usr/bin/env python3
"""RFC 067 PHILB -- 4-WARP 64x64 + Hilbert-curve CTA-swizzle (Pattern B) for L2 reuse.

Follow-on to N134 (rfc067_pswz, Pattern A 2D super-block 4x4). N134 recovered the
M>=6144 L2-thrash cliff:
    M=4096  ratio 0.828
    M=5120  ratio 0.717
    M=6144  ratio 0.655  (vs N130 0.234 -- +180% TFLOPS)
    M=8192  ratio 0.624  (vs N130 0.304 -- +218% TFLOPS)
N140 ncu confirmed L2 hit 56% @ M=6144, recovered by the swizzle.

Pattern B hypothesis (N134 honest scope): Pattern A's row-major super-block ordering
keeps a ~6 MB working set per 256x256 super-block, but adjacent super-blocks jump
across the grid (super-block i+1 is to the right; the last super-block in a row jumps
all the way back to column 0 of the next row). A Hilbert space-filling curve maps
ADJACENT linear CTA IDs to SPATIALLY-ADJACENT output tiles with Manhattan distance 1,
so the L2 working set of recently-scheduled CTAs is a tight contiguous 2D blob rather
than a row-strip -> potentially tighter L2 reuse at the cliff.

Mapping (Hilbert d2xy, replaces N134 super-block div+rem):
  - Launch grid = p x p where p = next_pow2(gx).  (64->64, 80->128, 96->128, 128->128)
  - d = ctaid.y * p + ctaid.x                       (linear Hilbert index in [0, p*p))
  - (sw_x, sw_y) = hilbert_d2xy(p, d)                (bijection over p x p square)
  - if sw_x >= gx OR sw_y >= gy: early-return        (drops padding tiles; preserves
                                                      bijection over the real gx x gy grid)
  - rest of kernel byte-identical to N134: uses (sw_x, sw_y) for A/B/C base addresses.

For pow2 grids (64, 128) the p x p Hilbert square == grid exactly, so NO CTA is dropped.
For non-pow2 grids (80, 96) the enclosing p=128 square is filtered; the kept (sw_x,sw_y)
set is exactly { (x,y) : x<gx, y<gy } (verified bijective in the generator), so every
real output tile is computed exactly once -> bit-exact preserved.

d2xy is fully UNROLLED at generation time (log2(p) = 6 or 7 iterations, compile-time
known per shape), so the PTX has NO runtime loop -- straight-line bit-twiddling. This is
the honest cost vs Pattern A's single div+rem: ~6-7 unrolled rounds of (shr/and/xor/
select/add) instead of 1 div.u32 + 1 rem.u32. Overhead is reported (regs/thd, prologue
instruction count) and amortised over K_TILES mma iterations.
...
Falsifier F-RFC067-HEXA-SGEMM-HILBERT-SWIZZLE:
  ...
  - Headline: does Hilbert beat N134 super-block at M=6144 / M=8192?
    * If Hilbert > super-block: tighter 2D L2 working set wins -> document L2 delta.
    * If Hilbert ~ super-block: super-block already sufficient for this L2 size
      (useful negative -- 32 MB L2 large enough that row-strip reuse is fine).
    * If Hilbert < super-block: prologue overhead or scheduler interaction dominates.

g3 honest caveats:
  - Both swizzles are semantic-preserving (only CTA->tile visitation order changes);
    bit-exact REQUIRED and asserted.
  - Hilbert prologue is longer (unrolled bit-twiddle) than super-block div+rem; we
    measure regs/thd and reason about prologue cost. Amortised, but reported.
  - Non-pow2 grids launch p*p CTAs (more than gx*gy) with early-return on padding:
    e.g. 80x80=6400 real but 128x128=16384 launched -> 9984 padding CTAs return
    immediately. These cost a few scheduler slots but no MMA work. This launch
    overhead is real and reported (it is part of the honest Hilbert cost on non-pow2).
"""
...
# --- PTX register budget ---
# Hilbert prologue uses %r120..%r159 scratch. Super-block had used up to %r111.
# We need: x in %r10 (sw_y replaced by ... see below), y in %r11. Original N134 used
#   %r10 = sw_y, %r11 = sw_x.  We compute (hx, hy) = d2xy and set sw_x=hx, sw_y=hy.
def emit_hilbert_prologue(p: int, gx: int, gy: int) -> str:
    ...
    Also sets predicate %phlb_oob true if (hx>=gx || hy>=gy) -> caller early-returns.

    Registers used (all in the .reg .u32 %r<160> pool):
      %r120 = d (running index t)
      %r121 = x (hx accumulator)
      %r122 = y (hy accumulator)
      %r123 = rx
      %r124 = ry
      %r125 = s-1  (mask for reflect: s is pow2 so s-1 is the low-bit mask)
      %r126,%r127,%r128,%r129 = scratch
    Predicate %prx for ry==0 branch is avoided by branchless select (selp).
    """
    log2p = p.bit_length() - 1
    lines = []
    lines.append("    // ---- CTA-swizzle: Hilbert-curve d2xy (Pattern B) for L2 reuse ----")
    ...
    for it in range(log2p):
        ...
        # rot(s,x,y,rx,ry): if ry==0 { if rx==1 { x=s-1-x; y=s-1-y } swap(x,y) }
        # branchless:
        #   need_reflect = (ry==0 && rx==1)
        #   need_swap    = (ry==0)
        # reflect first (only if need_reflect): x = sm1 - x ; y = sm1 - y
        ...
        # x += s*rx ; y += s*ry   (s known constant)
...
def gen(S: int) -> str:
    ...
    return f"""// RFC 067 PHILB perf HGEMM hexa-emit -- 4-WARP 64x64 + Hilbert-curve CTA-swizzle (Pattern B, L2-reuse) -- M=N=K={S}.
//
// CTA-swizzle Pattern B (Hilbert space-filling curve d2xy):
//   launch grid = p x p, p = {p} (= next_pow2(gx={gx}))
//   d = ctaid.y * p + ctaid.x
//   (sw_x, sw_y) = hilbert_d2xy({p}, d)  -- unrolled {p.bit_length()-1} rounds, no runtime loop
//   drop padding tiles where sw_x >= {gx} or sw_y >= {gy} (early-return; bijective over real grid)
//   adjacent CTA IDs -> Manhattan-adjacent output tiles -> tighter 2D L2 working set.
//
// Layout:
...
.visible .entry sgemm_4warp_hilbert_{S}x{S}_grid (
    .param .u64 a,
    .param .u64 b,
    .param .u64 c,
    .param .u64 k_tiles
...
{hilbert}
    ...
    // XOR swizzle slot (DISABLED -- matches N107 PY identity, single-axis test is CTA swizzle).
    mov.u32 %r80, 0;
    xor.b32 %r81, %r14, %r80;
    shl.b32 %r82, %r81, 4;
    ...
    //                   + sw_x * 256                   (64 N-cols * 4 B = 256 B per sw_x)
...
# Cliff regime + intermediate mapping (same shapes as N134 for direct comparison).
SHAPES = [4096, 5120, 6144, 8192]

if __name__ == "__main__":
    outdir = Path(__file__).resolve().parent
    for S in SHAPES:
        p = outdir / f"sgemm_4warp_hilbert_{S}x{S}_grid.ptx"
        p.write_text(gen(S))
