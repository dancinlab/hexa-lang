/* RFC 067 PT -- hexa-emit HGEMM with mma.m16n8k32 (2026-05-21)
 *
 * Comparator chain:
 *   N76 (PO): Path B FP16 HGEMM + ldmatrix.x4 + mma.m16n8k16          -> 31.28 TFLOPS @ M=1536 ratio 0.462
 *   N77 (PP): N76 + cp.async.cg vec16 producer                         -> 36.06 TFLOPS @ M=1536 ratio 0.533
 *   PT (this): N77 with consumer upgraded to 2x mma.m16n8k32           -> hypothesis ratio 0.65+
 *
 * Falsifier F-RFC067-HEXA-SGEMM-MMA-M16N8K32:
 *   - Numeric: per-element maxabs vs cuBLAS HGEMM within tolerance
 *     (Path-B math identical -> bit-exact match expected)
 *   - Per-shape median TFLOPS over 200 reps (20 warmup), cuEvent sync each iter
 *   - Compare to N77 PP baseline AND to cuBLAS HGEMM on same device
 *
 * Build:
 *   nvcc -O2 -arch=sm_90 -o host host.c -lcuda -lcublas -lm
 * Run:
 *   ./host sgemm_m16n8k32_256x256_grid.ptx ... 1536x1536_grid.ptx
 */
...
/* host-side float -> half packing.  Pure IEEE-754 binary16 (no denormal flush). */
...
static int run_hexa(CUfunction f_hexa,
                    ...
                    double *out_times_ms)
{
    const int K_PER_TILE = 32;   /* m16n8k32: K-step is 32 fp16 */
                           ...
                           size_t info_log_sz)
{
    ...
    /* big info-log so ptxas register count + spill info is captured. */
    static char log_info_static[32768];
    log_info_static[0] = 0;
    ...
    void *jit_vals[6] = {
        ...
        (void *)log_info_static,
        (void *)(uintptr_t)sizeof(log_info_static),
        (void *)(uintptr_t)1,
    };
    CUresult e = cuModuleLoadDataEx(out_mod, ptx, 6, jit_opts, jit_vals);
    if (out_info_log && info_log_sz > 0) {
        snprintf(out_info_log, info_log_sz, "%s", log_info_static);
    }
    if (e != CUDA_SUCCESS) {
        const char *s = NULL; cuGetErrorString(e, &s);
        fprintf(stderr, "cuModuleLoadDataEx %s: %s\n", ptx_path, s ? s : "?");
        if (log_err[0]) fprintf(stderr, "  ptxas err: %s\n", log_err);
        if (log_info_static[0]) fprintf(stderr, "  ptxas info: %s\n", log_info_static);
        free(ptx); return 1;
    }
    if (log_info_static[0]) {
        fprintf(stderr, "  ptxas info (%s): %s\n", entry, log_info_static);
    }
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_m16n8k32_256x256_grid.ptx ... 1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    char       ptxas_info[N_SHAPES][32768];
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_m16n8k32_%dx%d_grid", shapes[si], shapes[si]);
        ptxas_info[si][0] = 0;
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
                            ptxas_info[si], sizeof(ptxas_info[si])) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-m16n8k32 PTX shape %d (%s)\n", shapes[si], entry);
        } else {
            load_ok[si] = 0;
            snprintf(load_err[si], sizeof(load_err[si]),
                "PTX load/lookup failed for %s", argv[1 + si]);
            printf("FAILED hexa-m16n8k32 PTX shape %d -- skipping\n", shapes[si]);
        }
    }

    /* persist ptxas info log file for the artifact directory. */
    FILE *finfo = fopen("ptxas_info.log", "w");
    if (finfo) {
        fprintf(finfo, "# ptxas info captured via CU_JIT_INFO_LOG_BUFFER (verbose)\n");
        fprintf(finfo, "# device: %s sm_%d%d driver=%d runtime=%d\n",
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-m16n8k32 run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-m16n8k32 HGEMM        median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f\n",
            ...
            printf("[M=%4d] hexa-m16n8k32 HGEMM        SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "  \"variant\": \"PT (FP16 HGEMM + ldmatrix.x4 + cp.async.cg vec16 + 2x mma.sync.aligned.m16n8k32.row.col.f32.f16.f16.f32 -- K=32 per mma issue, half the mma issues vs N77's m16n8k16)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_m16n8k32_SxS_grid\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"stack_input\": {\n");
    fprintf(rj, "    \"N77_PP\": \"rfc067_pP_hexa_sgemm_ldmatrix_cpasync_2026_05_21 peak 36.06 TFLOPS @ M=1536 ratio 0.533 vs cuBLAS HGEMM 67.65\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"doubling K per mma -> halve mma issues -> ratio 0.65+ at M=1536\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_m16n8k32_tflops\":     %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_m16n8k32_median_ms\":  %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_m16n8k32_mean_ms\":    %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_m16n8k32_std_ms\":     %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_m16n8k32_min_ms\":     %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_m16n8k32_max_ms\":     %.6f,\n", res[si].hexa.max);
            ...
            fprintf(rj, "      \"hexa_m16n8k32_tflops\":     null,\n");
            fprintf(rj, "      \"hexa_m16n8k32_median_ms\":  null,\n");
            fprintf(rj, "      \"hexa_m16n8k32_mean_ms\":    null,\n");
            fprintf(rj, "      \"hexa_m16n8k32_std_ms\":     null,\n");
            fprintf(rj, "      \"hexa_m16n8k32_min_ms\":     null,\n");
            fprintf(rj, "      \"hexa_m16n8k32_max_ms\":     null,\n");
