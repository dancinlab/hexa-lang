/* RFC 067 PJ -- hexa-emit SGEMM + shared-mem prefetch (2026-05-21)
 *
 * Companion / direct comparator to:
 *   - PI (rfc067_pI_hexa_sgemm_2026_05_21 / N54) -- same kernel without
 *     shared-mem staging. Peak 8.88 TFLOPS @ M=1536 (ratio 0.27 vs cuBLAS).
 *
 * This fire adds cooperative shared-mem staging of the per-K-step A/B
 * tiles. Kernel geometry is identical (16 warps in a 4x4 layout,
 * 64x64 output tile, TF32 wmma m16n16k8 inner loop). Single biggest
 * optimization from N54's report: "cuBLAS shared-mem swizzled prefetch".
 *
 * Layout (same as PI):
 *   We compute C[M,N] = A[M,K] * B[K,N] in row-major.
 *   cuBLAS is col-major, so we ask it to compute
 *     C^T[N,M] = B^T[N,K] * A^T[K,M]
 *   via (op_N, op_N, N, M, K, B, ldb=N, A, lda=K, C, ldc=N).
 *
 * Measurement protocol (identical to PG/PH/PI):
 ...
 * Run:
 *   ./host sgemm_shared_256x256_grid.ptx ... sgemm_shared_1536x1536_grid.ptx
 */
                           ...
                           CUfunction *out_fn)
{
    ...
    /* JIT with verbose log so ptxas errors surface to fire.log */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_shared_256x256_grid.ptx ... sgemm_shared_1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_shared_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si]) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-shared PTX shape %d (%s)\n", shapes[si], entry);
        } else {
            load_ok[si] = 0;
            snprintf(load_err[si], sizeof(load_err[si]),
                "PTX load/lookup failed for %s", argv[1 + si]);
            printf("FAILED hexa-shared PTX shape %d -- skipping\n", shapes[si]);
        }
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-shared run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-shared SGEMM   median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.6f\n",
            ...
            printf("[M=%4d] hexa-shared SGEMM   SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PJ-hexa-sgemm-shared\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PJ (TF32 wmma m16n16k8 + cooperative shared-mem A/B staging, single-buffer)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_shared_SxS_grid -- 16-warp 4x4 grid 64x64 output tile, TF32 wmma K=8 inner loop, per-K-step _tg_a/_tg_b shared staging (2048 B each)\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (TF32 dispatch on sm_80+)\",\n");
    fprintf(rj, "  \"baseline_pI_n54\": \"rfc067_pI_hexa_sgemm_2026_05_21 (no shared-mem, peak 8.88 TFLOPS @ M=1536 ratio 0.27)\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        fprintf(rj, "    {\n");
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_shared_tflops\":      %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_shared_median_ms\":   %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_shared_mean_ms\":     %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_shared_std_ms\":      %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_shared_min_ms\":      %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_shared_max_ms\":      %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"ratio\":                   %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_shared_tflops\":      null,\n");
            fprintf(rj, "      \"hexa_shared_median_ms\":   null,\n");
            fprintf(rj, "      \"hexa_shared_mean_ms\":     null,\n");
            fprintf(rj, "      \"hexa_shared_std_ms\":      null,\n");
            fprintf(rj, "      \"hexa_shared_min_ms\":      null,\n");
            fprintf(rj, "      \"hexa_shared_max_ms\":      null,\n");
