/* RFC 067 PU -- DIRECT-D-STORE vec2 epilogue on top of N89 (PS) stack (2026-05-21)
 *
 * Comparator chain:
 *   N77 (PP):  ldmatrix.x4 + mma.m16n8k16 + cp.async.cg vec16, 64x64 tile
 *              -> peak 36.06 TFLOPS @ M=1536 ratio 0.533 vs cuBLAS HGEMM 67.65
 *   N89 (PS):  same primitives, 128x128 output per CTA, 32 warps (1024 thd/CTA)
 *              -> peak 37.07 TFLOPS @ M=1536 ratio 0.557 vs cuBLAS HGEMM 66.60 (+2.81% over N77)
 *   PU (this): identical to PS except epilogue uses st.global.v2.f32 (16 -> 8 store instr / warp)
 *              -> hypothesis: marginal epilogue speedup (epilogue is one-shot, amortised)
 *
 * Falsifier F-RFC067-HEXA-SGEMM-DIRECT-D-STORE:
 ...
 *   - Compare to N89 (PS) baseline AND to cuBLAS HGEMM on same device
 *
 * Build:  nvcc -O2 -arch=sm_90 -o host host.c -lcuda -lcublas -lm
 * Run:    ./host sgemm_direct_d_256x256_grid.ptx ... 1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_direct_d_256x256_grid.ptx ... 1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_direct_d_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
                            load_info[si], sizeof(load_info[si])) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-direct_d PTX shape %d (%s)\n", shapes[si], entry);
            if (fp_info) {
            ...
            printf("FAILED hexa-direct_d PTX shape %d -- skipping\n", shapes[si]);
        }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-direct_d run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-direct_d HGEMM     median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f\n",
            ...
            printf("[M=%4d] hexa-direct_d HGEMM     SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "  \"variant\": \"PU (PS+epilogue st.global.v2.f32 vec-2 stores; 16->8 store instr per warp)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_direct_d_SxS_grid -- N89 (PS tile128 1024thd) consumer, epilogue vec-2 store optimisation\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"stack_inputs\": {\n");
    fprintf(rj, "    \"N77_PP\": \"rfc067_pP_hexa_sgemm_ldmatrix_cpasync_2026_05_21 peak 36.06 TFLOPS @ M=1536\",\n");
    fprintf(rj, "    \"N89_PS\": \"rfc067_pS_hexa_sgemm_tile128_2026_05_21 peak 37.07 TFLOPS @ M=1536 ratio 0.557\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"vec-2 store epilogue reduces 16->8 store instr/warp; expect marginal (<=1%%) delta over N89 because epilogue is one-shot (amortised over K-loop)\",\n");
    fprintf(rj, "  \"discovery\": \"N89 (PS) already implements direct register->global store. Confirmed via PTX inspection (no D-output shared-mem slab, shmem only 16KB for A/B inputs). Both bar.sync 0 per K-iter are A/B WAR barriers; NONE are D-output staging artefacts. PU explores the only remaining direct-D-store delta available: store vectorisation.\",\n");
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_direct_d_tflops\":    %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_direct_d_median_ms\": %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_direct_d_mean_ms\":   %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_direct_d_std_ms\":    %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_direct_d_min_ms\":    %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_direct_d_max_ms\":    %.6f,\n", res[si].hexa.max);
            ...
            fprintf(rj, "      \"hexa_direct_d_tflops\":    null,\n");
            fprintf(rj, "      \"hexa_direct_d_median_ms\": null,\n");
            fprintf(rj, "      \"hexa_direct_d_mean_ms\":   null,\n");
            fprintf(rj, "      \"hexa_direct_d_std_ms\":    null,\n");
            fprintf(rj, "      \"hexa_direct_d_min_ms\":    null,\n");
            fprintf(rj, "      \"hexa_direct_d_max_ms\":    null,\n");
