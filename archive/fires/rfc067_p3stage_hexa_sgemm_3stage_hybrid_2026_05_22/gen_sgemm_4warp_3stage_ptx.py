...
Goal:
  Sweet-spot between N107 PY (2-stage DB, peak 51.65 TFLOPS @ M=1536 ratio 0.777,
  8192 B shmem, ~8 CTAs/SM) and N121 PZ (6-stage, peak 50.46 ratio 0.760, 24576 B
  shmem, 4 CTAs/SM regresses peak but wins small shapes M=256 ratio 1.161 vs PY 1.053).
  3-stage halves PZ's shmem cost (12288 B per CTA) -> ~6 CTAs/SM, intermediate
  occupancy. Tests whether moderate pipeline depth captures small-shape uplift
  without occupancy regression at peak.

Design (consumer = N107 PY, producer = 3-slot ring):
  Output tile      = 64 x 64 (M x N) per CTA          (PY)
  Warps / CTA      = 4 (2x2 grid)                      (PY)
  Threads / CTA    = 128                               (PY)
  Per-warp output  = 32 M x 32 N                       (PY)
  mma per K-step   = 8 (2 M sub-tiles x 4 N sub-8)     (PY)
  Acc f32 / lane   = 32                                (PY)

  Pipeline stages  = 3                                  (PZ 6 / 2)
  In-flight groups = 2                                  (commit on each issue)
  wait_group N     = 1                                  (consume k => wait <=1 in flight)

Shared-mem budget:
  Per-slab A = 64 rows x 16 K-elem fp16 = 2048 B
  Per-slab B = 16 K-elem x 64 cols fp16 = 2048 B
  3 stages x 2048 B = 6144 B per array
  Total per CTA = 12288 B   (between PY 8192 and PZ 24576)
  Occupancy expected = ~6 CTAs/SM (sm_120 128 KB shmem / 12288 ~= 10.6, but reg/warp
  budget likely caps at 6).

Prologue:
  Stage 0: unconditional cp.async A + B at slab[0], commit_group
  Stage 1: predicated on K_total > 1 -- issue cp.async + commit_group
  After prologue: 2 committed groups in flight.

Steady-state loop (k_iter = 0..K_total - 1, consume_stage = k % 3, produce_stage = (k+2) % 3):
  cp.async.wait_group 1         (wait until <=1 in flight; consume_slot ready)
  bar.sync 0
  ldmatrix.x4 (A_lo + A_hi)
  ldmatrix.x4.trans (B_lo + B_hi)
  8x mma.m16n8k16
  bar.sync 0
  if k + 2 < K_total: issue cp.async into slab[produce_stage]
  cp.async.commit_group         (always commit -- accounting stays consistent)
  advance consume_stage = (consume_stage + 1) % 3
  advance produce_stage = (produce_stage + 1) % 3

Epilogue:
  cp.async.wait_all
  st.global.f32 x 32 (PY identical)
...
Falsifier F-RFC067-HEXA-SGEMM-3STAGE-HYBRID:
  ...
  - compare against PY (51.65 @ M=1536) and PZ (50.46 @ M=1536)

Pareto curve interpretation:
  - If 3-stage > PY peak: sweet spot found between 2 and 6.
  - If 3-stage matches PY peak (within stat noise): pipeline depth saturates at 2;
    occupancy is the dominant cost.
  - If 3-stage shows PZ's small-shape uplift at lower cost: partial win.

PTX gotchas (already encountered in PW/PZ):
...
def gen(S: int) -> str:
    ...
    assert S >= 32, f"S={S} too small for 2-stage prologue (K_total >= 2 -> K >= 32)"
    ...
    SLAB_BYTES   = 2048
    STAGES       = 3
    SHMEM_PER_ARRAY = STAGES * SLAB_BYTES   # 6144

    return f"""// RFC 067 P3STAGE perf HGEMM hexa-emit -- 4-WARP 64x64 + 3-STAGE PIPELINE -- M=N=K={S}.
//
// Hybrid:
//   N107 (PY): 4-warp 64x64 tile, 128 thd/CTA, 8 mma/warp/K, 2-slot ring (DB)
//              -> 51.65 TFLOPS @ M=1536 ratio 0.777, 8192 B shmem
//   N121 (PZ): N107 consumer + 6-stage cp.async pipeline
//              -> 50.45 TFLOPS @ M=1536 ratio 0.760, 24576 B shmem (regresses peak,
//                                                                   wins M=256 ratio 1.161)
//   P3STAGE  : N107 consumer + 3-stage cp.async pipeline
//              -> Pareto middle: 12288 B shmem, ~6 CTAs/SM expected
//
// Shared-mem per CTA: 3 stages * 2 arrays * 2048 B = 12288 B (sm_90 100 KB cap).
...
.visible .entry sgemm_4warp_3stage_{S}x{S}_grid (
    .param .u64 a,
    .param .u64 b,
    .param .u64 c,
    .param .u64 k_tiles
...
{{
    ...
    // ===========================================================
    // PROLOGUE: issue up to 2 cp.async groups (stages 0..1).
    // ===========================================================

    // Prologue stage 0 (slab byte offset 0) -- unconditional (K_total >= 1).
    add.u32 %r40, %r18, 0;
    add.u32 %r40, %r40, %r17;
    ...
    // Prologue stage 1 (slab byte offset 2048) -- predicated on K_total > 1.
    setp.gt.s32 %pissue, %r0, 1;
    ...
    //   %r30 = consume stage = k % 3
    //   %r31 = produce stage = (k + 2) % 3
    //   %r34 = K-blocks remaining to issue (= K_total - 2)
...
$kloop:
    ...
    add.u32 %r30, %r30, 1;
    setp.eq.s32 %p1, %r30, 3;
    @%p1 mov.u32 %r30, 0;

    add.u32 %r31, %r31, 1;
    setp.eq.s32 %p1, %r31, 3;
    @%p1 mov.u32 %r31, 0;
...
if __name__ == "__main__":
    ...
    for S in SHAPES:
        ...
        p = outdir / f"sgemm_4warp_3stage_{S}x{S}_grid.ptx"
        p.write_text(gen(S))
