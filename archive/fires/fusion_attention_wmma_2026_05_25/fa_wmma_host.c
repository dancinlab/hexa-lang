...
int main(int argc, char **argv) {
    ...
    unsigned grid = (unsigned)(N / 16);
    unsigned block = 32;             /* one warp per 16-query-row tile */
    ...
    printf("N=%d d=%d  max_abs=%.6g  max_rel=%.6g  numeric=%s (tol rel<=1e-2)\n",
           N, d, max_abs, max_rel, (max_rel <= 1e-2) ? "PASS" : "FAIL");
