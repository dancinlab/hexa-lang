/* RFC 067 PT128H -- 128x128 output tile (N89 PS) + Hilbert CTA-swizzle (N149 PHILB) -- large-M sweep (2026-05-22)
 *
 * Goal: combine N89's 128x128 output tile with N149's Hilbert-curve CTA-swizzle. N89 found
 *       the 128x128 tile lost to the 64x64 4-warp baseline at M<=1536 due to 1-CTA/SM
 *       occupancy collapse (47 regs * 1024 thd = 48128 regs/CTA). N149 found a Hilbert d2xy
 *       CTA-swizzle flattened the large-M L2-thrash cliff (ratio held ~0.82-0.85 across
 *       M=4096..8192, M=8192 0.847) on the 64x64 tile. This fire tests whether the bigger
 *       tile + Hilbert's L2 locality pushes the large-M ratio PAST N149's 0.847.
 *
 * Launch grid: p x p where p = next_pow2(side), side = N/128. Kernel computes
 *       (sw_x,sw_y)=d2xy(p,d) from linear CTA id d = ctaid.y*p + ctaid.x, then
 *       EARLY-RETURNS padding CTAs (sw_x>=side || sw_y>=side). Bijective over the real
 *       side x side grid -> bit-exact.
 *         4096 -> grid 32x32 -> p=32  (no padding, 1024 real)
 *         5120 -> grid 40x40 -> p=64  (4096 launched, 1600 real, 2496 padding-return)
 *         6144 -> grid 48x48 -> p=64  (4096 launched, 2304 real, 1792 padding-return)
 *         8192 -> grid 64x64 -> p=64  (no padding, 4096 real)
 *
 * Sweep (same shapes as N149 for direct comparison):
 ...
 *   - M = 8192  (N149 64x64+Hilbert 59.48 / ratio 0.847 -- headline: does 128+Hilbert beat it?)
 *
 ...
 * Run:    ./host sgemm_tile128_hilbert_4096x4096_grid.ptx \
 *                sgemm_tile128_hilbert_5120x5120_grid.ptx \
 *                sgemm_tile128_hilbert_6144x6144_grid.ptx \
 *                sgemm_tile128_hilbert_8192x8192_grid.ptx
 */
                    ...
                    double *out_times_ms)
{
    ...
    /* PT128H: launch p x p where p = next_pow2(side), side = N/128 (128-tile grid). */
    unsigned int side = (unsigned int)(N / 128);
                        ...
                        float *out_maxabs, float *out_maxrel)
{
    ...
    unsigned long long k_arg = (unsigned long long)K_TILES_TOTAL;
    void *kargs[4] = { &da, &db, &dc, &k_arg };
    /* PT128H: launch p x p (next_pow2 of side=N/128); kernel d2xy + padding return. */
    unsigned int side = (unsigned int)(N / 128);
    unsigned int p = next_pow2_u(side);
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr,
            "usage: %s sgemm_tile128_hilbert_4096x4096_grid.ptx ... 8192x8192_grid.ptx\n",
            argv[0]);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_tile128_hilbert_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
                            load_info[si], sizeof(load_info[si])) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-PT128H PTX shape %d (%s)\n", shapes[si], entry);
            if (fp_info) {
            ...
            printf("FAILED hexa-PT128H PTX shape %d -- skipping\n", shapes[si]);
        }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        res[si].num_ctas = (M / 128) * (N / 128);  /* 128-tile real CTA count */
        if (load_ok[si]) {
            int rpt = 0;
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-PT128H run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                snprintf(res[si].note, sizeof(res[si].note),
                ...
                printf("[M=%4d] hexa-PT128H 128tile+Hilbert median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f  CTAs=%d  regs=%d\n",
            ...
            printf("[M=%4d] hexa-PT128H SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    /* N149 PHILB baselines: 4-warp 64x64 + Hilbert CTA-swizzle (Pattern B).
       Headline comparison: does 128x128 + Hilbert beat 64x64 + Hilbert?
       Per-shape order matches shapes[] = {4096, 5120, 6144, 8192}. From
       rfc067_philb_hexa_sgemm_hilbert_swizzle_2026_05_22/analysis.txt. */
    double n149_hexa_tflops[N_SHAPES]   = { 56.99, 57.69, 58.49, 59.48 };
    double n149_cublas_tflops[N_SHAPES] = { 69.39, 69.72, 70.13, 70.20 };
    double n149_ratio[N_SHAPES]         = {  0.821, 0.827, 0.834, 0.847 };
    /* N89 PS 128x128 (NO swizzle) -- only measured M<=1536; peak 37.07 @ M=1536 ratio 0.557.
       NOT measured at these large shapes (context: bigger tile lost everywhere). */

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PT128H-hexa-hgemm-tile128-hilbert\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PT128H -- N89 PS 128x128 output tile (32 warps, 1024 thd/CTA, 16 f32 acc/lane, 4 mma.m16n8k16/warp/K-step) + N149 PHILB Hilbert d2xy CTA-swizzle. Launch grid p x p, p=next_pow2(side), side=N/128; d=ctaid.y*p+ctaid.x; (sw_x,sw_y)=hilbert_d2xy(p,d) unrolled log2(p) rounds (no runtime loop); early-return padding CTAs (sw_x>=side||sw_y>=side). 128x128-tile MMA body byte-identical to N89.\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_tile128_hilbert_SxS_grid\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"swizzle_pattern\": \"Hilbert space-filling curve d2xy on 128-tile grid (side=N/128). Launch p x p (p=next_pow2(side)), d2xy(p, ctaid.y*p+ctaid.x), drop padding tiles outside side x side. Bijective over real grid -> bit-exact. d2xy unrolled at gen time.\",\n");
    fprintf(rj, "  \"l2_size_mb\": 32,\n");
    fprintf(rj, "  \"stack_inputs\": {\n");
    fprintf(rj, "    \"N89_PS\":   \"rfc067_pS_hexa_sgemm_tile128_2026_05_21 128x128 tile NO swizzle: peak 37.07 @ M=1536 ratio 0.557, lost to 64x64 baseline at M<=1536 (1 CTA/SM occupancy collapse, 47 regs). Not measured at M>=4096.\",\n");
    fprintf(rj, "    \"N149_PHILB\": \"rfc067_philb_hexa_sgemm_hilbert_swizzle_2026_05_22 64x64 + Hilbert: M=4096 0.821, M=5120 0.827, M=6144 0.834, M=8192 0.847 (best large-M ratio in RFC 067 SGEMM line).\",\n");
    fprintf(rj, "    \"N107_PY\":  \"4-warp 64x64 canonical: 51.65 TFLOPS @ M=1536 ratio 0.777\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"N89 found the 128x128 tile lost to 64x64 at M<=1536 due to 1-CTA/SM occupancy collapse. N149 found a Hilbert CTA-swizzle flattened the large-M L2 cliff on the 64x64 tile (M=8192 0.847). PT128H tests if the bigger tile's larger L2-resident working set per CTA + Hilbert visitation order pushes the large-M ratio PAST 0.847. Two-sided: if 128x128+Hilbert beats 64x64+Hilbert at large M, L2-locality changes the tile-size tradeoff; if it still loses, N89's occupancy-collapse finding holds and bigger tile is the wrong knob on RTX 5070.\",\n");
    ...
    fprintf(rj, "    \"shmem_bytes_per_cta\": 16384\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        unsigned int side = (unsigned int)(res[si].M / 128);
        unsigned int p = next_pow2_u(side);
        fprintf(rj, "    {\n");
        ...
        fprintf(rj, "      \"pt128h_regs_per_thd\":      %d,\n", res[si].regs_per_thd);
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_pt128h_tflops\":       %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_pt128h_median_ms\":    %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_pt128h_mean_ms\":      %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_pt128h_std_ms\":       %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_pt128h_min_ms\":       %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_pt128h_max_ms\":       %.6f,\n", res[si].hexa.max);
            ...
            fprintf(rj, "      \"tile128_hilbert_beats_hilbert64\": %s,\n", beats_n149 ? "true" : "false");
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_pt128h_tflops\":       null,\n");
            fprintf(rj, "      \"hexa_pt128h_median_ms\":    null,\n");
            fprintf(rj, "      \"hexa_pt128h_mean_ms\":      null,\n");
            fprintf(rj, "      \"hexa_pt128h_std_ms\":       null,\n");
            fprintf(rj, "      \"hexa_pt128h_min_ms\":       null,\n");
            fprintf(rj, "      \"hexa_pt128h_max_ms\":       null,\n");
            ...
            fprintf(rj, "      \"tile128_hilbert_beats_hilbert64\": null,\n");
            fprintf(rj, "      \"note\":                    \"%s\"\n", res[si].note);
        }
        fprintf(rj, "    }%s\n", (si == N_SHAPES - 1) ? "" : ",");
    }
