/* RFC 067 N203 -- TMA + warp-spec (2P+2C) + Hilbert SGEMM sweep host driver (2026-05-22).
 *
...
int main(int argc, char **argv) {
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_tma_warp_spec_%dx%d_grid", shapes[si], shapes[si]);
        load_err[si][0] = 0;
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            res[si].hexa_valid = 1;
            printf("[M=%4d] N203 TMA+warp-spec       median=%.6f ms  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  CTAs=%u  regs=%d  shmem=%d\n",
     ...
     * N200-full = rfc067_ptma_mma_hilbert (TMA + mma, no warp split). 6 shapes.
     * N197      = rfc067_pwspec_named_bar (named-bar warp-spec, NO TMA). 4 shapes (1024,2048,4096,8192).
     * Indices align with shapes[]: { 512, 1024, 2048, 4096, 6144, 8192 }.
     */
    double n200_hexa_tflops[N_SHAPES] = { 15.169273, 39.394694, 51.901676, 55.979814, 56.455059, 57.984318 };
    double n200_ratio[N_SHAPES]       = {  0.654611,  0.738480,  0.777262,  0.799475,  0.797622,  0.819045 };
    double n197_hexa_tflops[N_SHAPES] = { 0.0, 24.555017, 28.384844, 20.695845, 0.0,  9.637833 };
    double n197_ratio[N_SHAPES]       = { 0.0,  0.462312,  0.423866,  0.295469, 0.0,  0.136120 };

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-N203-tma-warp-spec\",\n");
    ...
    fprintf(rj, "  \"variant\": \"N203: TMA cp.async.bulk.tensor.2d (mbarrier::complete_tx::bytes, expect_tx 4096/K-iter) + 2P+2C warp specialization (warps 0,1 = producer lanes; warps 2,3 = consumer mma) + Hilbert d2xy CTA-swizzle. 4 mbarriers (full[2] + empty[2]), per-stage parity tracked. DMA-driven mbarrier completion replaces N197's cp.async.wait_all (root cause #1 fix). 128 thd/CTA preserves occupancy vs N197's 256. sm_120a + .version 8.7 + ASCII-only PTX.\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_tma_warp_spec_SxS_grid\",\n");
    ...
    fprintf(rj, "    \"N197\":      \"named-bar warp-spec 4P+4C, NO TMA -- FAILED (-34% to -84%% vs N149)\",\n");
    fprintf(rj, "    \"N200_FULL\": \"TMA + mma all-threads, NO warp split -- ratio 0.819 @ M=8192\",\n");
    ...
    fprintf(rj, "    \"tma_swizzle\": \"CU_TENSOR_MAP_SWIZZLE_NONE\",\n");
    fprintf(rj, "    \"per_cta_output\": \"64x64\",\n");
    fprintf(rj, "    \"warps_per_cta\": 4,\n");
    fprintf(rj, "    \"producer_warps\": 2,\n");
    fprintf(rj, "    \"consumer_warps\": 2,\n");
    fprintf(rj, "    \"pipeline_stages\": 2,\n");
    fprintf(rj, "    \"mma_per_consumer_warp_per_kstep\": 16,\n");
    fprintf(rj, "    \"acc_f32_per_consumer_lane\": 64,\n");
    fprintf(rj, "    \"smem_a_bytes\": 4096,\n");
    fprintf(rj, "    \"smem_b_bytes\": 4096,\n");
    fprintf(rj, "    \"smem_mbar_bytes\": 32,\n");
    fprintf(rj, "    \"smem_total_per_cta\": 8224,\n");
    fprintf(rj, "    \"mbarriers\": \"full[0]@+0, full[1]@+8, empty[0]@+16, empty[1]@+24; arrive_count: full=1 (TMA-driven), empty=64 (2 consumer warps * 32 lanes)\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        double pct_over_n200 = 0.0, pct_over_n197 = 0.0;
        double ratio_delta_n200 = 0.0, ratio_delta_n197 = 0.0;
        int beats_n200 = 0, beats_n197 = 0;
        int has_n200 = (n200_hexa_tflops[si] > 0.0);
        int has_n197 = (n197_hexa_tflops[si] > 0.0);
        if (res[si].hexa_valid && has_n200) {
        ...
        if (res[si].hexa_valid && has_n197) {
            pct_over_n197 = 100.0 * (res[si].hexa_tflops - n197_hexa_tflops[si]) / n197_hexa_tflops[si];
            ratio_delta_n197 = res[si].ratio - n197_ratio[si];
            beats_n197 = (res[si].hexa_tflops > n197_hexa_tflops[si]);
        }
        fprintf(rj, "    {\n");
        ...
        if (has_n200) {
            ...
            fprintf(rj, "      \"n200_full_hexa_tflops\":    null,\n");
            fprintf(rj, "      \"n200_full_ratio\":          null,\n");
        }
        if (has_n197) {
            fprintf(rj, "      \"n197_pwspec_hexa_tflops\": %.6f,\n", n197_hexa_tflops[si]);
            fprintf(rj, "      \"n197_pwspec_ratio\":       %.6f,\n", n197_ratio[si]);
        } else {
            fprintf(rj, "      \"n197_pwspec_hexa_tflops\": null,\n");
            fprintf(rj, "      \"n197_pwspec_ratio\":       null,\n");
        }
        if (res[si].hexa_valid) {
            ...
            fprintf(rj, "      \"pct_over_n197\":            %.6f,\n", pct_over_n197);
            fprintf(rj, "      \"ratio_delta_vs_n197\":      %.6f,\n", ratio_delta_n197);
            fprintf(rj, "      \"beats_n197\":               %s,\n", beats_n197 ? "true" : "false");
            ...
            fprintf(rj, "      \"pct_over_n197\":            null,\n");
            fprintf(rj, "      \"ratio_delta_vs_n197\":      null,\n");
            fprintf(rj, "      \"beats_n197\":               null,\n");
            fprintf(rj, "      \"note\":                     \"%s\"\n", res[si].note);
        }
        fprintf(rj, "    }%s\n", (si == N_SHAPES - 1) ? "" : ",");
    }
