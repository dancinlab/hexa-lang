/* RFC 067 PM -- hexa-emit SGEMM + 2-stage cp.async size=16 vectorised (2026-05-21)
 *
 * Direct comparator to:
 *   - PK (rfc067_pK_hexa_sgemm_multistage_2026_05_21 / N66) -- 2-stage
 *     cp.async.ca size=4. Peak 13.35 TFLOPS @ M=1536 / ratio 0.406 on RTX 5070.
 *
 * This fire keeps the PK kernel geometry verbatim (16 warps 4x4, 64x64 output
 * tile, m16n16k8 TF32 wmma, K=8, 2-slab shared double-buffer) but switches the
 * cooperative-load instruction from cp.async.ca.shared.global size=4 (per-fp32
 * scalar, 1024 instructions per K-step) to cp.async.cg.shared.global size=16
 * (4-fp32 vector, 256 instructions per K-step -- 4x fewer).
 *
 * N71's instrumented analysis identified this size=4 -> size=16 switch as the
 * largest unfalsified perf candidate after the 2-stage SW pipeline.
 *
 * Layout (same as PI/PJ):
 *   C[M,N] = A[M,K] * B[K,N] row-major. cuBLAS gets the transposed form.
 *
 * Measurement protocol (identical to PG/PH/PI/PJ):
 ...
 * Run:
 *   ./host sgemm_cpasync_vec16_256x256_grid.ptx ... sgemm_cpasync_vec16_1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_cpasync_vec16_256x256_grid.ptx ... sgemm_cpasync_vec16_1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_cpasync_vec16_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si]) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-vec16 PTX shape %d (%s)\n", shapes[si], entry);
        } else {
            load_ok[si] = 0;
            snprintf(load_err[si], sizeof(load_err[si]),
                "PTX load/lookup failed for %s", argv[1 + si]);
            printf("FAILED hexa-vec16 PTX shape %d -- skipping\n", shapes[si]);
        }
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-vec16 run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-vec16 SGEMM        median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.6f\n",
            ...
            printf("[M=%4d] hexa-vec16 SGEMM        SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PM-hexa-sgemm-cpasync-vec16\",\n");
    fprintf(rj, "  \"date_utc\": \"2026-05-21\",\n");
    fprintf(rj, "  \"host\": \"%s\",\n", getenv("PM_HOST") ? getenv("PM_HOST") : "ubu-2");
    fprintf(rj, "  \"device\": \"%s\",\n", dev_name);
    ...
    fprintf(rj, "  \"variant\": \"PM (TF32 wmma m16n16k8 + cp.async.cg.shared.global size=16 vectorised 2-stage double-buffered pipeline)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_cpasync_vec16_SxS_grid -- 16-warp 4x4 grid 64x64 output tile, TF32 wmma K=8 inner loop, 2-slab _tg_a/_tg_b shared staging (4096 B each), cp.async.cg size=16 (4-fp32 vec) producer + wmma.load consumer; tid[0,128) load A, tid[128,256) load B, tid[256,512) idle during load\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (TF32 dispatch on sm_80+)\",\n");
    fprintf(rj, "  \"baseline_pK_n66\": \"rfc067_pK_hexa_sgemm_multistage_2026_05_21 (2-stage cp.async.ca size=4, peak 13.35 TFLOPS @ M=1536 ratio 0.406 on RTX 5070 sm_120)\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        fprintf(rj, "    {\n");
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_vec16_tflops\":      %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_vec16_median_ms\":   %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_vec16_mean_ms\":     %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_vec16_std_ms\":      %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_vec16_min_ms\":      %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_vec16_max_ms\":      %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"ratio\":                   %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_vec16_tflops\":      null,\n");
            fprintf(rj, "      \"hexa_vec16_median_ms\":   null,\n");
            fprintf(rj, "      \"hexa_vec16_mean_ms\":     null,\n");
            fprintf(rj, "      \"hexa_vec16_std_ms\":      null,\n");
            fprintf(rj, "      \"hexa_vec16_min_ms\":      null,\n");
            fprintf(rj, "      \"hexa_vec16_max_ms\":      null,\n");
