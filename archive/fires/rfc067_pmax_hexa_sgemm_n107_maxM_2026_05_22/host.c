/* RFC 067 Pmax -- 4-WARP 64x64 + XOR-swizzle MAX-M extension (N124 -> M=6144 + M=8192) (2026-05-22)
 *
 * Goal: extend N124 PZbig 3-shape big-M sweep (M=2048/3072/4096) to M=6144 + M=8192.
 *       Test whether structural ceiling ~0.82 / ~58 TFLOPS observed at M=4096 (N124)
 *       breaks past or stays flat at even larger M.
 *
 * Sweep:
 *   - M=4096  reproducer (N124 baseline 57.33 TFLOPS / ratio 0.819)
 *   - M=6144  NEW
 *   - M=8192  NEW
 *
 * Falsifier F-RFC067-HEXA-SGEMM-MAXM-STRESS:
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_4warp_swizzle_4096x4096_grid.ptx sgemm_4warp_swizzle_6144x6144_grid.ptx sgemm_4warp_swizzle_8192x8192_grid.ptx\n",
                argv[0]);
        return 2;
    }
    ...
    /* Prior baselines. N124 only had data for M=4096; M=6144 + M=8192 are NEW. */
    double n107_tflops[N_SHAPES] = { 57.329673, 0.0, 0.0 };   /* N124 M=4096 reproducer baseline */
    double n124_cublas_tflops[N_SHAPES] = { 70.043008, 0.0, 0.0 };
    double n124_ratio[N_SHAPES] = { 0.818492, 0.0, 0.0 };

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-Pmax-hexa-hgemm-n107-maxM\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PY V1 -- 4 warps (2x2 warp grid) 64x64 output / CTA, 8 mma.m16n8k16 per warp per K-step (32 f32 acc/lane), XOR swizzle wired but masked to identity (ldmatrix.x4 coherence -- g3 caveat). Kernel byte-identical to N107 PY / N124 PZbig; only shapes-list extends to M=4096/6144/8192.\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_4warp_swizzle_SxS_grid -- N107 PY 4-warp 64x64 tile pushed to M=6144 + M=8192\",\n");
    ...
    fprintf(rj, "    \"N124_PZbig\": \"rfc067_pZbig_hexa_sgemm_n107_bigshape_2026_05_22 M=4096 57.33 TFLOPS ratio 0.819 -- big-M sweep up to M=4096\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"Extend N124 PZbig sweep to M=6144 + M=8192 to determine structural ceiling boundary. N124 showed ratio plateau ~0.82 at M=4096. Does larger M (more compute saturation) break past ceiling (ratio improves to 0.85+) or confirm it (~0.82 / ~58 TFLOPS holds)?\",\n");
    ...
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        int has_prior = (n107_tflops[si] > 0.0);
        double pct_over_n107 = (res[si].hexa_valid && has_prior)
        ...
        if (has_prior) {
            fprintf(rj, "      \"n124_baseline_cublas_tflops\": %.6f,\n", n124_cublas_tflops[si]);
            fprintf(rj, "      \"n124_baseline_hexa_tflops\":   %.6f,\n", n107_tflops[si]);
            fprintf(rj, "      \"n124_baseline_ratio\":         %.6f,\n", n124_ratio[si]);
        } else {
            fprintf(rj, "      \"n124_baseline_cublas_tflops\": null,\n");
            fprintf(rj, "      \"n124_baseline_hexa_tflops\":   null,\n");
            fprintf(rj, "      \"n124_baseline_ratio\":         null,\n");
        }
