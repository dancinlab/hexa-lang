#!/usr/bin/env python3
"""RFC 067 Pmax -- N107 PY 4-warp 64x64 kernel pushed to M=6144 and M=8192.

Identical generator as N124 PZbig (rfc067_pZbig_hexa_sgemm_n107_bigshape_2026_05_22).
Only the SHAPES list changes:
  - M=4096 reproducer baseline (N124 reported 57.33 TFLOPS / ratio 0.819)
  - M=6144 NEW   (96 * 96 = 9216 CTAs, fp16 mem 3 * 6144^2 * 2 = 226 MB, fits 12 GB)
  - M=8192 NEW   (128 * 128 = 16384 CTAs, mem 3 * 8192^2 * 2 = 402 MB, fits 12 GB)

Hypothesis (per parent task):
  - N124 showed structural ceiling ratio ~0.82 at M=4096 (57.33 TFLOPS vs 70.04).
  - cuBLAS may grow toward 70-75 TFLOPS at M=8192 (kernel-variant dispatch on
    larger shapes), while hexa stays at ~58 TFLOPS -> ceiling confirmed.
  - If hexa breaks past 60 TFLOPS, ceiling not yet hit.

The kernel itself is byte-identical (single SHAPES-list change) to N124. See
N124 docstring for full PY V1 layout, ldmatrix, mma, swizzle (masked-identity)
plumbing.
"""
...
def gen(S: int) -> str:
    ...
    return f"""// RFC 067 Pmax perf HGEMM hexa-emit -- 4-WARP 64x64 + XOR swizzle -- M=N=K={S}.
//
// N124 PZbig extension to M=6144/8192 -- single SHAPES-list change, kernel
// byte-identical to N124. Goal = structural ceiling boundary determination.
//
// Layout:
...
# RFC 067 Pmax: structural-ceiling boundary determination.
#   M=4096 reproducer (N124 baseline 57.33 TFLOPS / ratio 0.819)
#   M=6144 NEW (3 * 6144^2 * 2 = 226 MB fp16  +  6144^2 * 4 = 151 MB f32)
#   M=8192 NEW (3 * 8192^2 * 2 = 402 MB fp16  +  8192^2 * 4 = 268 MB f32)
# Total max VRAM ~= 670 MB at M=8192, well under 12 GB.
