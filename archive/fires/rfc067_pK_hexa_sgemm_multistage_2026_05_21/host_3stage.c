 ...
 * Run:
 *   ./host sgemm_3stage_256x256_grid.ptx ... sgemm_3stage_1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_3stage_256x256_grid.ptx ... sgemm_3stage_1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_3stage_%dx%d_grid", shapes[si], shapes[si]);
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                ...
                printf("[M=%4d] hexa-3stage SGEMM   median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.6f\n",
            ...
            printf("[M=%4d] hexa-3stage SGEMM   SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PK-hexa-sgemm-3stage\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PK-3stage (TF32 wmma m16n16k8 + cp.async.ca 3-stage triple-buffered pipeline)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_3stage_SxS_grid -- 16-warp 4x4 grid 64x64 output tile, TF32 wmma K=8 inner loop, 2-slab _tg_a/_tg_b shared staging (4096 B each), cp.async.ca producer + wmma.load consumer (size=4 per-fp32; cg requires size=16)\",\n");
