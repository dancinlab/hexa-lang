/* RFC 067 PK -- hexa-emit SGEMM + 2-stage cp.async pipeline (2026-05-21)
 *
 * Companion / direct comparator to:
 *   - PI (rfc067_pI_hexa_sgemm_2026_05_21 / N54) -- baseline (no shared-mem)
 *   - PJ (rfc067_pJ_hexa_sgemm_shared_2026_05_21 / N60) -- single-buffer shared-mem.
 *     Peak 22.36 TFLOPS @ M=1536 (ratio 0.255 vs cuBLAS on Blackwell sm_120).
 *
 * This fire adds 2-stage software pipelining via cp.async.cg.shared.global:
 * each K=k+1 tile prefetch is issued BEFORE the K=k mma compute, overlapping
 * global-load latency with TF32 mma latency. Kernel geometry is identical
 * to PJ (16 warps in 4x4, 64x64 tile, m16n16k8 wmma, 512 threads, K=8).
 *
 ...
 * Run:
 *   ./host sgemm_multistage_256x256_grid.ptx ... sgemm_multistage_1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_multistage_256x256_grid.ptx ... sgemm_multistage_1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_multistage_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si]) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-multistage PTX shape %d (%s)\n", shapes[si], entry);
        } else {
            load_ok[si] = 0;
            snprintf(load_err[si], sizeof(load_err[si]),
                "PTX load/lookup failed for %s", argv[1 + si]);
            printf("FAILED hexa-multistage PTX shape %d -- skipping\n", shapes[si]);
        }
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-multistage run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-multistage SGEMM   median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.6f\n",
            ...
            printf("[M=%4d] hexa-multistage SGEMM   SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PK-hexa-sgemm-multistage\",\n");
    fprintf(rj, "  \"date_utc\": \"2026-05-21\",\n");
    fprintf(rj, "  \"host\": \"%s\",\n", getenv("PK_HOST") ? getenv("PK_HOST") : "ubu-2");
    fprintf(rj, "  \"device\": \"%s\",\n", dev_name);
    ...
    fprintf(rj, "  \"variant\": \"PK (TF32 wmma m16n16k8 + cp.async.ca.shared.global 2-stage double-buffered pipeline)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_multistage_SxS_grid -- 16-warp 4x4 grid 64x64 output tile, TF32 wmma K=8 inner loop, 2-slab _tg_a/_tg_b shared staging (4096 B each), cp.async.ca producer + wmma.load consumer (size=4 per-fp32; cg requires size=16)\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (TF32 dispatch on sm_80+)\",\n");
    fprintf(rj, "  \"baseline_pJ_n60\": \"rfc067_pJ_hexa_sgemm_shared_2026_05_21 (single-buffer shared, peak 22.36 TFLOPS @ M=1536 ratio 0.255 on Blackwell PRO 4500)\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        fprintf(rj, "    {\n");
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_multistage_tflops\":      %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_multistage_median_ms\":   %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_multistage_mean_ms\":     %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_multistage_std_ms\":      %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_multistage_min_ms\":      %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_multistage_max_ms\":      %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"ratio\":                   %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_multistage_tflops\":      null,\n");
            fprintf(rj, "      \"hexa_multistage_median_ms\":   null,\n");
            fprintf(rj, "      \"hexa_multistage_mean_ms\":     null,\n");
            fprintf(rj, "      \"hexa_multistage_std_ms\":      null,\n");
            fprintf(rj, "      \"hexa_multistage_min_ms\":      null,\n");
            fprintf(rj, "      \"hexa_multistage_max_ms\":      null,\n");
