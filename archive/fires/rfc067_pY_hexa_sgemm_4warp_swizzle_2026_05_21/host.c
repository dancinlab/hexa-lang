/* RFC 067 PY -- 4-WARP 64x64 + XOR-swizzle (single-axis isolation per N104) (2026-05-22)
 *
 * Comparator chain:
 ...
 *   PY (this): revert tile to 64x64, 4 warps (128 thd/CTA), keep N89's swizzle plumbing.
 *              CTA grid restored 144 -> 576 for M=1536, hypothesis = inter-CTA latency overlap.
 *
 * Falsifier F-RFC067-HEXA-SGEMM-4WARP-SWIZZLE:
 ...
 *   - Compare to N89 PS baseline AND to N93 PU baseline
 *
 * Launch config (V1):
 ...
 * Run:
 *   ./host sgemm_4warp_swizzle_256x256_grid.ptx ... 1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_4warp_swizzle_256x256_grid.ptx ... 1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    /* Reference baselines (RTX 5070 sm_120 2026-05-21). */
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PY-hexa-hgemm-4warp-swizzle-V1\",\n");
    ...
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"Reverting to 64x64 output tile restores CTA count 144 -> 576 @ M=1536 for inter-CTA latency overlap; N104 projection +0.05-0.10 TFLOPS @ M=1536 (larger gain for small M)\",\n");
