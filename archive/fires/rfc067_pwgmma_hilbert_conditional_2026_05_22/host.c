/* RFC 067 PWGMMA-HILBERT-CONDITIONAL (N199) -- wgmma.mma_async + Hilbert + identity-at-small-M
 *                                              full-range sweep (2026-05-22)
 *
 * Goal: single "best everywhere" SGEMM kernel = N171 PCOND conditional dispatch + N149 Hilbert
 *       d2xy + N172 64x64 tile + Hopper wgmma.mma_async.sync.aligned.m64n64k16.f32.f16.f16
 *       replacing N171's 8x mma.sync.aligned.m16n8k16 with a single warpgroup-level wgmma
 *       instruction per K-step.
 *
 *   At kernel entry the kernel branches on the launch grid CTA count (gridDim.x * gridDim.y)
 *   -- a uniform grid-constant, no warp divergence -- and selects:
 *       grid_ctas <= THRESHOLD (4096)  -> IDENTITY map  (sw=ctaid; NO Hilbert prologue)
 *       grid_ctas >  THRESHOLD          -> HILBERT d2xy  (L2-locality recovery)
 *
 * HARDWARE BLOCKER (@D g3): this host REQUIRES sm_90a (Hopper). The PTX uses wgmma which
 * is REJECTED by ptxas/driver on sm_120 (Blackwell consumer / RTX 5070, the only GPU on
 * ubu-2). N195 ("wgmma.async STRUCTURAL IMPOSSIBILITY on RTX 5070 sm_120", commit 7e26b7b8)
 * already established this for the m64n16k16 scaffold; this N199 full-kernel variant
 * reproduces the same boundary. For silicon fire, run on H100/H200/GH200; PTX is verified
 * ptxas-PASS on sm_90a. Host hard-gates on sm_major != 9: prints canonical ptxas errors
 * and aborts with no measure (no shape rows in result.json).
 *
 ...
 *
 * Build:  nvcc -O2 -arch=sm_90a -o host host.c -lcuda -lcublas -lm
 *         (needs sm_90a-capable nvcc / CUDA 12.9+. device-side PTX requires Hopper.)
 */
...
int main(int argc, char **argv) {
    ...
    /* ===== N199 wgmma HARD-GATE: require sm_9x (Hopper) =====
     * wgmma is Hopper-only (sm_90a). Ampere/Ada (sm_80/sm_86/sm_89) lacks the wgmma unit;
     * Blackwell consumer (sm_120) replaces it with tcgen05.mma -- ptxas REJECTS wgmma on
     * sm_120a. The only family that natively executes wgmma is sm_90 series. */
    if (sm_major != 9) {
        fprintf(stderr, "\n");
        fprintf(stderr, "============================================================\n");
        fprintf(stderr, "  N199 PWGMMA-HILBERT-CONDITIONAL: HARDWARE BLOCKER\n");
        fprintf(stderr, "============================================================\n");
        fprintf(stderr, "  This kernel uses wgmma.mma_async.sync.aligned.m64n64k16.f32.f16.f16\n");
        fprintf(stderr, "  which is a Hopper-only (sm_90a) PTX instruction.\n");
        fprintf(stderr, "  Detected device sm_%d%d (not sm_9x) -- driver will reject:\n", sm_major, sm_minor);
        fprintf(stderr, "    * Instruction 'wgmma.fence' cannot be compiled for arch 'sm_%d%d'\n", sm_major, sm_minor);
        fprintf(stderr, "    * Instruction 'wgmma.mma_async with floating point types' cannot be compiled\n");
        fprintf(stderr, "    * Instruction 'wgmma.commit_group' cannot be compiled for arch 'sm_%d%d'\n", sm_major, sm_minor);
        fprintf(stderr, "    * Instruction 'wgmma.wait_group' cannot be compiled for arch 'sm_%d%d'\n", sm_major, sm_minor);
        fprintf(stderr, "  Aborting with no measurement run. Re-run on H100/H200/GH200.\n");
        fprintf(stderr, "  sm_120 (Blackwell consumer) needs tcgen05.mma instead of wgmma.\n");
        fprintf(stderr, "============================================================\n");
        return 3;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_wgmma_cond_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
                            load_info[si], sizeof(load_info[si])) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-PWGMMA PTX shape %d (%s)\n", shapes[si], entry);
            ...
            printf("FAILED hexa-PWGMMA PTX shape %d -- skipping\n", shapes[si]);
        }
        if (load_ok[si]) {
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                ...
                printf("[M=%4d] hexa-PWGMMA %-8s     median=%.6f ms  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  CTAs=%u  regs=%d\n",
            ...
            printf("[M=%4d] hexa-PWGMMA SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PWGMMA-HILBERT-CONDITIONAL-hexa-hgemm-N199\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PWGMMA-HILBERT-CONDITIONAL (N199) -- N171 conditional CTA-swizzle (identity-at-small-M + Hilbert-at-large-M) + 64x64 tile + WGMMA. Replaces N171's 8x mma.sync.aligned.m16n8k16 per K-step with a single wgmma.mma_async.sync.aligned.m64n64k16.f32.f16.f16 warpgroup-level instruction (4 warps' worth of mma in one issue). B descriptors point to shared memory (no ldmatrix); cp.async.cg double-buffer remains. REQUIRES sm_90a (Hopper) -- ptxas+driver REJECT on sm_120 Blackwell consumer (N195 falsified for scaffold; N199 reproduces for full kernel).\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_wgmma_cond_SxS_grid\",\n");
