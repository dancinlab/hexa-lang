/* RFC 067 PR -- STACK N77 (ldmatrix HGEMM + cp.async.cg vec16) + K-UNROLL 2x (2026-05-21)
 *
 * Comparator chain:
 *   N77 (PP):  ldmatrix.x4 + cp.async.cg vec16  -> peak 36.06 TFLOPS @ M=1536 ratio 0.533
 *   PR (this): N77 stack + K-loop unroll 2x      -> hypothesis ratio 0.60+
 *
 * Falsifier F-RFC067-HEXA-SGEMM-K-UNROLL:
 ...
 * Run:
 *   ./host sgemm_kunroll_256x256_grid.ptx ... 1536x1536_grid.ptx
 */
...
static int run_hexa(CUfunction f_hexa,
                    ...
                    double *out_times_ms)
{
    /* K-UNROLL 2x: each K-step processes 32 K-elements (was 16 in N77). */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_kunroll_256x256_grid.ptx ... 1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_kunroll_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si]) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-kunroll PTX shape %d (%s)\n", shapes[si], entry);
        } else {
            load_ok[si] = 0;
            snprintf(load_err[si], sizeof(load_err[si]),
                "PTX load/lookup failed for %s", argv[1 + si]);
            printf("FAILED hexa-kunroll PTX shape %d -- skipping\n", shapes[si]);
        }
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-kunroll run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-kunroll HGEMM      median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f\n",
            ...
            printf("[M=%4d] hexa-kunroll HGEMM      SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PR-hexa-hgemm-ldmatrix-cpasync-kunroll2-STACK\",\n");
    fprintf(rj, "  \"date_utc\": \"2026-05-21\",\n");
    fprintf(rj, "  \"host\": \"%s\",\n", getenv("PR_HOST") ? getenv("PR_HOST") : "ubu-2");
    fprintf(rj, "  \"device\": \"%s\",\n", dev_name);
    ...
    fprintf(rj, "  \"variant\": \"PR (N77 PP stack + K-loop unroll 2x: K-block 16->32, 2x ldmatrix+4x mma per K-step, halved bar.sync)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_kunroll_SxS_grid -- N77 consumer doubled to 2 ldmatrix sets + 4 mma per K-step; K_TILES_TOTAL = K/32 (was K/16)\",\n");
    ...
    fprintf(rj, "    \"N79_PQ\": \"rfc067_pQ_hexa_sgemm_swpipe_2026_05_21 SW pipeline 0%% gain (refuted; ptxas auto-reorders)\",\n");
    fprintf(rj, "    \"N48_Apple_M3\": \"K-unroll 2x measured +2.4%% on Apple M3 (architecturally similar lever)\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"K-unroll 2x halves bar.sync, doubles mma per sync -> hexa ratio >= 0.6 at M=1536\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_kunroll_tflops\":         %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_kunroll_median_ms\":      %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_kunroll_mean_ms\":        %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_kunroll_std_ms\":         %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_kunroll_min_ms\":         %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_kunroll_max_ms\":         %.6f,\n", res[si].hexa.max);
            ...
            fprintf(rj, "      \"hexa_kunroll_tflops\":         null,\n");
            fprintf(rj, "      \"hexa_kunroll_median_ms\":      null,\n");
            fprintf(rj, "      \"hexa_kunroll_mean_ms\":        null,\n");
            fprintf(rj, "      \"hexa_kunroll_std_ms\":         null,\n");
            fprintf(rj, "      \"hexa_kunroll_min_ms\":         null,\n");
            fprintf(rj, "      \"hexa_kunroll_max_ms\":         null,\n");
