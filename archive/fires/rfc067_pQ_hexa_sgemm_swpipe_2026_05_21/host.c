/* RFC 067 PQ -- hexa-emit HGEMM with SW-pipelined ldmatrix/mma overlap (2026-05-21)
 *
 * Comparator chain:
 *   - PO (N76, FP16 ldmatrix.x4 + mma.m16n8k16 + 2-stage cp.async) -- peak 31.28 TFLOPS @ M=1536, ratio 0.462
 *   - This (PQ): same kernel structure as PO; K-loop rearranged so ldmatrix for
 *     K+1 issues BEFORE mma reads the CUR (K) frag. Latency hides behind mma.
 *
 * Falsifier F-RFC067-HEXA-SGEMM-LDMATRIX-SWPIPE:
 *   - Bit-exact-ish vs cuBLAS HGEMM (SW pipeline is semantic preserving)
 *   - Per-shape median TFLOPS recorded over 200 reps (20 warmup)
 *   - Compare to PO (N76-retry) baseline AND to cuBLAS HGEMM on same device
 *
 * Build:
 *   nvcc -O2 -arch=sm_90 -o host host.c -lcuda -lcublas -lm
 * Run:
 *   ./host sgemm_swpipe_256x256_grid.ptx ... sgemm_swpipe_1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_swpipe_256x256_grid.ptx ... sgemm_swpipe_1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_swpipe_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si]) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-swpipe PTX shape %d (%s)\n", shapes[si], entry);
        } else {
            load_ok[si] = 0;
            snprintf(load_err[si], sizeof(load_err[si]),
                "PTX load/lookup failed for %s", argv[1 + si]);
            printf("FAILED hexa-swpipe PTX shape %d -- skipping\n", shapes[si]);
        }
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-swpipe run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-swpipe HGEMM     median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f\n",
            ...
            printf("[M=%4d] hexa-swpipe HGEMM     SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PQ-hexa-hgemm-swpipe-ldmatrix-mma\",\n");
    fprintf(rj, "  \"date_utc\": \"2026-05-21\",\n");
    fprintf(rj, "  \"host\": \"%s\",\n", getenv("PQ_HOST") ? getenv("PQ_HOST") : "ubu-2");
    fprintf(rj, "  \"device\": \"%s\",\n", dev_name);
    ...
    fprintf(rj, "  \"variant\": \"PQ (PO baseline + SW-pipelined ldmatrix/mma overlap: ldmatrix(K+1) issues BEFORE mma reads CUR frag)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_swpipe_SxS_grid -- PO geometry + 2-deep K-loop pipeline (ra,rb=CUR, rA,rB=NXT, explicit b32 mov rename)\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"path_chosen\": \"PQ over PO -- SW pipeline (N76 honest scope: 'lack of software-pipelined mma/ldmatrix overlap')\",\n");
    fprintf(rj, "  \"baseline_pO_n76_retry\": \"rfc067_pO_hexa_sgemm_ldmatrix_2026_05_21 (FP16 ldmatrix.x4 + mma.m16n8k16, peak 31.28 TFLOPS @ M=1536 ratio 0.462 vs cuBLAS HGEMM 67.65)\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        fprintf(rj, "    {\n");
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_swpipe_tflops\":      %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_swpipe_median_ms\":   %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_swpipe_mean_ms\":     %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_swpipe_std_ms\":      %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_swpipe_min_ms\":      %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_swpipe_max_ms\":      %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"hexa_vs_cublas_maxrel\":   %.6f,\n", res[si].hexa_maxrel);
            fprintf(rj, "      \"ratio\":                   %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_swpipe_tflops\":      null,\n");
            fprintf(rj, "      \"hexa_swpipe_median_ms\":   null,\n");
            fprintf(rj, "      \"hexa_swpipe_mean_ms\":     null,\n");
            fprintf(rj, "      \"hexa_swpipe_std_ms\":      null,\n");
            fprintf(rj, "      \"hexa_swpipe_min_ms\":      null,\n");
            fprintf(rj, "      \"hexa_swpipe_max_ms\":      null,\n");
