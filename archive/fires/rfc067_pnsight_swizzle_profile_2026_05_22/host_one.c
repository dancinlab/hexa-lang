/* RFC 067 P-nsight -- Single-shape profile driver for ncu/nsys (2026-05-22)
 *
 * Goal: profile a SINGLE N107 4-warp 64x64 hexa SGEMM kernel launch
 *       at a given M=N=K (M chosen at runtime by argv) so ncu kernel-replay
 *       collects metrics for ONE shape per invocation.
 *
 * Strategy:
 *   - One PTX file + one entry name + one shape (M=N=K square).
 *   - argv: ./host_one  <ptx_path>  <M>  <entry_name>  <nreps>
 *   - Allocate matrices, init, do `warmup` warmup launches, then `nreps` measured launches.
 *   - For ncu: nreps small (1-5) since ncu replays kernels; we just need a single
 *     successful launch for ncu to lock onto.
 *   - For nsys: nreps moderate (20-50) so trace shows multiple launches.
 *
 * Bit-identical kernel to N130 host.c (same launch config, same args).
 *
 * Build:
 *   nvcc -O2 -arch=sm_90 -o host_one host_one.c -lcuda -lcublas -lm
 *
 * Run:
 *   ./host_one  sgemm_4warp_swizzle_4096x4096_grid.ptx  4096  sgemm_4warp_swizzle_4096x4096_grid  10
 */
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "launch grid=(%u,%u,1) block=(128,1,1) K_TILES=%d\n", gx, gy, K_TILES_TOTAL);
