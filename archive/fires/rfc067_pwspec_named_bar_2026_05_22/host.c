 ...
 *
 * Sequel to N127 (2P+2C with full-CTA bar.sync 0 -> -6.8% vs N107).
 * Uses named bar.arrive (non-blocking) + bar.sync (blocking) to decouple
 * producer-consumer pipeline and avoid serializing producer slot reuse
 * with consumer mma chain.
 *
 * Comparator chain:
 *   N127 PZ-spec:    2P+2C bar.sync 0 -- M=1024 ratio 0.686, M=1536 ratio 0.719
 *   N149 PHILB:      4-warp 64x64 + Hilbert swizzle -- M=4096 0.821, M=8192 0.847
 *   This (V2):       4P+4C named bars asymmetric -- can it beat both?
 *
 * Launch config:
 *   block = (256, 1, 1)              -- 8 warps (4P + 4C)
 *   grid  = (N/64, M/64, 1)
 *
 * Build:
 *   nvcc -O2 -arch=sm_90 -o host host.c -lcuda -lcublas -lm
 *
 * Shapes: M=N=K in {1024, 2048, 4096, 8192} per task spec.
 */
                    ...
                    double *out_times_ms)
{
    ...
    /* 4P+4C named-bar: grid = (N/64, M/64, 1), block = 256 thd (8 warps). */
    ...
    for (int i = 0; i < warmup; ++i) {
        CHECK_CU(cuLaunchKernel(f_hexa,
            gx, gy, 1, 256, 1, 1, 0, NULL, kargs, NULL));
    }
    CHECK_CU(cuCtxSynchronize());
                        ...
                        float *out_maxabs, float *out_maxrel)
{
    ...
    unsigned int gx = (unsigned int)(N / 64);
    unsigned int gy = (unsigned int)(M / 64);
    CHECK_CU(cuLaunchKernel(f_hexa, gx, gy, 1, 256, 1, 1, 0, NULL, kargs, NULL));
    CHECK_CU(cuCtxSynchronize());
...
int main(int argc, char **argv) {
    int shapes[N_SHAPES] = { 1024, 2048, 4096, 8192 };

    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_pwspec_nb_1024x1024_grid.ptx ... 8192x8192_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_pwspec_nb_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
                            load_info[si], sizeof(load_info[si])) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-pwspec-nb PTX shape %d (%s)\n", shapes[si], entry);
            if (fp_info) {
            ...
            printf("FAILED hexa-pwspec-nb PTX shape %d -- skipping\n", shapes[si]);
        }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-pwspec-nb run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-PWSPEC-NB 4P+4C named-bar  median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f  CTAs=%d  regs=%d\n",
            ...
            printf("[M=%4d] hexa-PWSPEC-NB                  SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    /* N127 PZ-spec (RTX 5070 sm_120 2026-05-22) -- 2P+2C bar.sync 0 baseline.
       Only M=1024 measured (others were 256..1536). Use NaN-ish for missing. */
    double n127_tflops_1024 = 37.282703;  /* ratio 0.686 */

    /* N149 PHILB (RTX 5070 sm_120 2026-05-22) -- 4-warp 64x64 + Hilbert swizzle.
       M=1024 not measured in N149 (N149 was 4096..8192). */
    double n149_tflops_4096 = 56.991156;  /* ratio 0.821 */
    double n149_tflops_8192 = 59.477660;  /* ratio 0.847 */

    /* N107 PY 4-warp baseline @ M=1024 = 40.017213 TFLOPS ratio ~0.69-0.74. */
    double n107_tflops_1024 = 40.017213;

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PWSPEC-NB-hexa-hgemm-warp-spec-named-bar\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PWSPEC-NB V2 -- 4 PRODUCER warps + 4 CONSUMER warps (256 thd/CTA). Named bar.arrive (non-blocking) + bar.sync (blocking) with rotating bar indices per slot. bar 1=slot 0 ready, bar 2=slot 0 consumed, bar 3=slot 1 ready, bar 4=slot 1 consumed. Each bar count=256 (full CTA). Per consumer: 1 M-sub (16M) x 8 N-sub-8 = 8 mma.m16n8k16, 32 f32 acc/lane (back to N107 reg pressure, NOT N127's 64). Producer races ahead via bar.arrive; consumer races ahead via bar.arrive on consumed signal.\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_pwspec_nb_SxS_grid -- N127 sequel with named asymmetric barriers\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"stack_inputs\": {\n");
    fprintf(rj, "    \"N127_PZ_spec\":  \"rfc067_pZspec_hexa_sgemm_warp_spec_2026_05_22 -- 2P+2C bar.sync 0 (full-CTA), M=1024 37.28 TFLOPS ratio 0.686 (-6.8%% vs N107)\",\n");
    fprintf(rj, "    \"N149_PHILB\":    \"rfc067_philb_hexa_sgemm_hilbert_swizzle_2026_05_22 -- 4-warp 64x64 + Hilbert swizzle, M=4096 0.821, M=8192 0.847\",\n");
    fprintf(rj, "    \"N107_PY\":       \"rfc067_pY_hexa_sgemm_4warp_swizzle_2026_05_21 -- M=1024 40.02 TFLOPS\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"Named-barrier asymmetric arrive/sync removes the full-CTA blocking sync that hobbled N127. Producer issues bar.arrive (non-blocking) for 'slot ready' and 'gets out of the way'; consumer's bar.arrive for 'slot consumed' lets it race to next iter's wait. Producer's bar.sync only blocks before reusing a slot two iters back. Expected: positive delta vs N127 at all shapes; competitive with N149 at large M (4096+) if decoupled pipeline beats simple per-warp interleaving.\",\n");
    fprintf(rj, "  \"launch_config\": {\n");
    fprintf(rj, "    \"threads_per_block\": 256,\n");
    fprintf(rj, "    \"warps_per_block\": 8,\n");
    fprintf(rj, "    \"producer_warps\": 4,\n");
    fprintf(rj, "    \"consumer_warps\": 4,\n");
    fprintf(rj, "    \"output_tile_M\": 64,\n");
    fprintf(rj, "    \"output_tile_N\": 64,\n");
    fprintf(rj, "    \"per_consumer_M\": 16,\n");
    fprintf(rj, "    \"per_consumer_N\": 64,\n");
    fprintf(rj, "    \"mma_per_consumer_per_kstep\": 8,\n");
    fprintf(rj, "    \"acc_f32_per_consumer_lane\": 32,\n");
    fprintf(rj, "    \"named_bars_used\": [1, 2, 3, 4],\n");
    fprintf(rj, "    \"named_bar_count\": 256,\n");
    fprintf(rj, "    \"shmem_bytes_per_cta\": 8192\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        double pct_over_n127 = 0.0;
        double pct_over_n149 = 0.0;
        int has_n127 = 0, has_n149 = 0;
        if (res[si].hexa_valid) {
            if (res[si].M == 1024) { pct_over_n127 = 100.0 * (res[si].hexa_tflops - n127_tflops_1024) / n127_tflops_1024; has_n127 = 1; }
            if (res[si].M == 4096) { pct_over_n149 = 100.0 * (res[si].hexa_tflops - n149_tflops_4096) / n149_tflops_4096; has_n149 = 1; }
            if (res[si].M == 8192) { pct_over_n149 = 100.0 * (res[si].hexa_tflops - n149_tflops_8192) / n149_tflops_8192; has_n149 = 1; }
        }
        fprintf(rj, "    {\n");
        ...
        fprintf(rj, "      \"pwspec_nb_num_ctas\":      %d,\n", res[si].num_ctas);
        fprintf(rj, "      \"pwspec_nb_regs_per_thd\":  %d,\n", res[si].regs_per_thd);
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_pwspec_nb_tflops\":   %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_pwspec_nb_median_ms\": %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_pwspec_nb_mean_ms\":  %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_pwspec_nb_std_ms\":   %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_pwspec_nb_min_ms\":   %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_pwspec_nb_max_ms\":   %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"hexa_vs_cublas_maxrel\":   %.6f,\n", res[si].hexa_maxrel);
            fprintf(rj, "      \"ratio_vs_cublas\":         %.6f,\n", res[si].ratio);
            if (has_n127) fprintf(rj, "      \"pct_over_n127\":           %.4f,\n", pct_over_n127);
            if (has_n149) fprintf(rj, "      \"pct_over_n149\":           %.4f,\n", pct_over_n149);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_pwspec_nb_tflops\":   null,\n");
            fprintf(rj, "      \"hexa_pwspec_nb_median_ms\": null,\n");
            fprintf(rj, "      \"hexa_pwspec_nb_mean_ms\":  null,\n");
            fprintf(rj, "      \"hexa_pwspec_nb_std_ms\":   null,\n");
            fprintf(rj, "      \"hexa_pwspec_nb_min_ms\":   null,\n");
            fprintf(rj, "      \"hexa_pwspec_nb_max_ms\":   null,\n");
    ...
    /* Suppress unused-var warnings for baselines that weren't applicable. */
    (void)n107_tflops_1024;
