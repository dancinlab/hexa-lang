...
extern "C" __global__ void fa_mw_cpasync(const __half* q, const __half* k,
                                         const __half* v, __half* o,
                                         int N, float scale) {
    /* double-buffered K/V tiles in smem */
    __shared__ __half k_sm[2][KTILE*D];   // 2 * 2048 B
    __shared__ __half v_sm[2][KTILE*D];   // 2 * 2048 B
    ...
    for (int kt=0; kt<n_tiles; ++kt) {
        ...
        /* wait until the CURRENT tile is in smem. With 2 groups in flight
         * (current + next), wait_group<1> keeps the next one outstanding. */
...
int main(int argc,char**argv){
    ...
    fa_mw_cpasync<<<grid,block>>>(dq,dk,dv,dO,N,scale);
    ...
    printf("mw_cpasync N=%d grid=%d block=%d max_abs=%.4g rel_rowscale=%.4g rms_rel=%.4g naninf=%ld numeric=%s",
           N,grid,block,maxa,relrs,rms,nanc,pass?"PASS":"FAIL");
    if(do_time){cudaEvent_t s,n;cudaEventCreate(&s);cudaEventCreate(&n);
        for(int w=0;w<20;++w)fa_mw_cpasync<<<grid,block>>>(dq,dk,dv,dO,N,scale);cudaDeviceSynchronize();
        int reps=200;double *ms=(double*)malloc(reps*8);
        for(int r=0;r<reps;++r){cudaEventRecord(s,0);fa_mw_cpasync<<<grid,block>>>(dq,dk,dv,dO,N,scale);cudaEventRecord(n,0);cudaEventSynchronize(n);float t;cudaEventElapsedTime(&t,s,n);ms[r]=t;}
