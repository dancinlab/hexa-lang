/* RFC 067 N200 TMA SMOKE host driver
 *
 * Fires sgemm_tma_smoke.ptx on RTX 5070 sm_120.
 * Builds 2 TMA descriptors (one for A 64x64 fp16, one for B 64x64 fp16),
 * launches kernel, reads back C (256 f32), verifies first 256 elements
 * match A's top-left 16x16 (cast f16 -> f32).
 *
 * Falsifier F-RFC067-TMA-SMOKE: byte_mismatch == 0 AND no driver error.
 */
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: %s sgemm_tma_smoke.ptx\n", argv[0]);
        return 2;
    }
    ...
    /* Allocate A 64x64 fp16, B 64x64 fp16, C 64x64 f32 */
    const int M = 64;
    const int K = 64;
    const int N = 64;
    ...
    /* Fill A with deterministic pattern: a[i,j] = (i+j)/16.0 */
    for (int i = 0; i < M; ++i)
    ...
    /* Build TMA descriptors. A: 64x64 fp16, tile 64x16 (M rows x K_TILE cols).
       B: 64x64 fp16, tile 16x64 (K_TILE rows x N cols). */
    CUtensorMap tmap_a, tmap_b;

    /* A descriptor: load 64x16 tile (TILE_K cols) from (col=k_off, row=0) */
    {
        cuuint64_t globalDim[2]   = { (cuuint64_t)K, (cuuint64_t)M };  // innermost first (K cols, M rows)
        cuuint64_t globalStride[1] = { (cuuint64_t)K * 2 };            // K * 2 bytes per row
        cuuint32_t boxDim[2]      = { 16, 64 };                         // 16 cols, 64 rows
    ...
    /* B descriptor: load 16x64 tile (TILE_K rows x N cols) from (col=0, row=k_off) */
    {
    ...
    CUfunction kernel;
    CHECK_CU(cuModuleGetFunction(&kernel, mod, "sgemm_tma_smoke"));

    /* Kernel param order: tmap_a (128 B by value), tmap_b (128 B by value), c_ptr (u64). */
    ...
    /* SMOKE verification: kernel writes smem_a back to C as fp32.
       smem_a holds the LAST K-tile (k_iter=3, k_off=48..63) of A:
       a[i, k_off+j] for i in 0..63, j in 0..15 = a[i, 48..63].
       Each thread tx writes 2 f32 at C[tx*2 .. tx*2+1].
       The smem layout is 64 rows x 16 cols stride 16 (= 32 bytes per row).
       So thread tx reads smem_a at byte offset tx*8, which is element index tx*4.
       => C[2*tx] = a[ (tx*4)/16,   (48 + (tx*4)%16) ]
          C[2*tx+1] = a[ (tx*4+2)/16, (48 + (tx*4+2)%16) ] (approximately; depends on row layout)

       Simpler verify: read each C cell, check it equals the corresponding a[row,col]
       at the LAST k-tile (k_off=48..63). Since we wrote 256 f32 cells (128 threads x 2),
       cells 0..255 cover the first 256 fp16 elements of smem_a = a[0,48..63] then a[1,48..63] etc. */

    int mismatch = 0;
    int first_mismatch_idx = -1;
    float first_got = 0, first_exp = 0;
    for (int tx = 0; tx < 128; ++tx) {
        /* thread tx wrote C[2*tx], C[2*tx+1]; sourced from smem_a at byte tx*8
           which is elem tx*4 .. tx*4+1 in the 64x16=1024-element smem_a buffer.
           That buffer holds A[row, 48..63] for row=0..63, stride 16, total 1024 elements. */
        int e0 = tx * 4;
        int e1 = tx * 4 + 1;
    ...
    printf("  \"verdict\": \"%s\"\n", mismatch == 0 ? "TMA_SMOKE_PASS" : "TMA_SMOKE_MISMATCH");
    printf("}\n");
