#!/usr/bin/env bash
# RFC 067 PP -- STACK ldmatrix HGEMM + cp.async.cg vec16.
#
...
ART_DIR_BASENAME="rfc067_pP_hexa_sgemm_ldmatrix_cpasync_2026_05_21"

echo "== STEP 0: regen PTX (idempotent) ==" >&2
python3 gen_sgemm_ldmatrix_cpasync_ptx.py

echo "== STEP 1: rsync to ubu-2:/tmp/${ART_DIR_BASENAME}/ ==" >&2
ssh ubu-2 "mkdir -p /tmp/${ART_DIR_BASENAME}"
scp -q host.c gen_sgemm_ldmatrix_cpasync_ptx.py *.ptx "ubu-2:/tmp/${ART_DIR_BASENAME}/"
...
ssh ubu-2 "cd /tmp/${ART_DIR_BASENAME} && ./host \
    sgemm_ldmatrix_cpasync_256x256_grid.ptx \
    sgemm_ldmatrix_cpasync_384x384_grid.ptx \
    sgemm_ldmatrix_cpasync_512x512_grid.ptx \
    sgemm_ldmatrix_cpasync_768x768_grid.ptx \
    sgemm_ldmatrix_cpasync_1024x1024_grid.ptx \
    sgemm_ldmatrix_cpasync_1536x1536_grid.ptx 2>&1" | tee fire.log
