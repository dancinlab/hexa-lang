/* RFC 067 PP -- STACK N76 ldmatrix HGEMM + N74 cp.async.cg vec16 (2026-05-21)
 *
 * Comparator chain:
 *   N76 (PO):  Path B FP16 HGEMM + ldmatrix.x4 + mma.m16n8k16  -> peak 31.28 TFLOPS @ M=1536 ratio 0.462
 *   N74 (PM):  TF32 SGEMM + cp.async.cg vec16                  -> +14.2% over N66 scalar cp.async
 *   PP (this): N76 layout + N74's cp.async.cg vec16 producer    -> hypothesis ratio 0.55+
 *
 * Falsifier F-RFC067-HEXA-SGEMM-LDMATRIX-CPASYNC-STACK:
 *   - Numeric: per-element maxabs vs cuBLAS HGEMM within tolerance
 *     (Path-B consumer is identical to N76 -> bit-exact match to N76 expected)
 *   - Per-shape median TFLOPS over 200 reps (20 warmup), cuEvent sync each iter
 *   - Compare to N76 PO baseline AND to cuBLAS HGEMM on same device
 *
 * Build:
 *   nvcc -O2 -arch=sm_90 -o host host.c -lcuda -lcublas -lm
 * Run:
 *   ./host sgemm_ldmatrix_cpasync_256x256_grid.ptx ... 1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_ldmatrix_cpasync_256x256_grid.ptx ... 1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_ldmatrix_cpasync_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si]) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-stack PTX shape %d (%s)\n", shapes[si], entry);
        } else {
            load_ok[si] = 0;
            snprintf(load_err[si], sizeof(load_err[si]),
                "PTX load/lookup failed for %s", argv[1 + si]);
            printf("FAILED hexa-stack PTX shape %d -- skipping\n", shapes[si]);
        }
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-stack run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-stack HGEMM        median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f\n",
            ...
            printf("[M=%4d] hexa-stack HGEMM        SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    /* N76 baseline (PO, 2026-05-21 RTX 5070 sm_120) for inline % delta. */
    double n76_tflops[N_SHAPES] = { 4.405782, 11.718358, 15.033348, 26.459394, 25.224155, 31.283482 };

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PP-hexa-hgemm-ldmatrix-cpasync-vec16-STACK\",\n");
    fprintf(rj, "  \"date_utc\": \"2026-05-21\",\n");
    fprintf(rj, "  \"host\": \"%s\",\n", getenv("PP_HOST") ? getenv("PP_HOST") : "ubu-2");
    fprintf(rj, "  \"device\": \"%s\",\n", dev_name);
    ...
    fprintf(rj, "  \"variant\": \"PP (FP16 HGEMM + ldmatrix.x4 + mma.m16n8k16 consumer (from N76) + cp.async.cg.shared.global size=16 vectorised producer (from N74))\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_ldmatrix_cpasync_SxS_grid -- N76 consumer identical (ldmatrix.x4 + 2x mma.m16n8k16) with N74-style vec16 cp.async producer; 4x fewer async-issue instr/K-step vs N76 (256 vs 1024)\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"stack_inputs\": {\n");
    fprintf(rj, "    \"N76_PO\": \"rfc067_pO_hexa_sgemm_ldmatrix_2026_05_21 peak 31.28 TFLOPS @ M=1536 ratio 0.462 vs cuBLAS HGEMM 67.65\",\n");
    fprintf(rj, "    \"N74_PM\": \"rfc067_pM_hexa_sgemm_cpasync_vec16_2026_05_21 +14.2%% over N66 (TF32 SGEMM)\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"compound multiplicative -> ratio ~0.55+ at M=1536 (~37 TFLOPS)\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        double pct_over_n76 = res[si].hexa_valid
            ? 100.0 * (res[si].hexa_tflops - n76_tflops[si]) / n76_tflops[si]
            : 0.0;
        fprintf(rj, "    {\n");
        ...
        fprintf(rj, "      \"n76_baseline_tflops\":     %.6f,\n", n76_tflops[si]);
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_stack_tflops\":         %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_stack_median_ms\":      %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_stack_mean_ms\":        %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_stack_std_ms\":         %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_stack_min_ms\":         %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_stack_max_ms\":         %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"hexa_vs_cublas_maxrel\":   %.6f,\n", res[si].hexa_maxrel);
            fprintf(rj, "      \"ratio_vs_cublas\":         %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"pct_over_n76\":            %.4f,\n", pct_over_n76);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_stack_tflops\":         null,\n");
            fprintf(rj, "      \"hexa_stack_median_ms\":      null,\n");
            fprintf(rj, "      \"hexa_stack_mean_ms\":        null,\n");
            fprintf(rj, "      \"hexa_stack_std_ms\":         null,\n");
            fprintf(rj, "      \"hexa_stack_min_ms\":         null,\n");
            fprintf(rj, "      \"hexa_stack_max_ms\":         null,\n");
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   null,\n");
            fprintf(rj, "      \"hexa_vs_cublas_maxrel\":   null,\n");
            fprintf(rj, "      \"ratio_vs_cublas\":         null,\n");
            fprintf(rj, "      \"pct_over_n76\":            null,\n");
            fprintf(rj, "      \"note\":                    \"%s\"\n", res[si].note);
        }
        fprintf(rj, "    }%s\n", (si == N_SHAPES - 1) ? "" : ",");
    }
