#!/usr/bin/env bash
# RFC 067 PS -- TILE128 (128x128 output per CTA, 32 warps).
#
...
ART_DIR_BASENAME="rfc067_pS_hexa_sgemm_tile128_2026_05_21"

echo "== STEP 0: regen PTX (idempotent) ==" >&2
python3 gen_sgemm_tile128_ptx.py

echo "== STEP 1: rsync to ubu-2:/tmp/${ART_DIR_BASENAME}/ ==" >&2
ssh ubu-2 "mkdir -p /tmp/${ART_DIR_BASENAME}"
scp -q host.c gen_sgemm_tile128_ptx.py *.ptx "ubu-2:/tmp/${ART_DIR_BASENAME}/"
...
ssh ubu-2 "cd /tmp/${ART_DIR_BASENAME} && ./host \
    sgemm_tile128_256x256_grid.ptx \
    sgemm_tile128_384x384_grid.ptx \
    sgemm_tile128_512x512_grid.ptx \
    sgemm_tile128_768x768_grid.ptx \
    sgemm_tile128_1024x1024_grid.ptx \
    sgemm_tile128_1536x1536_grid.ptx 2>&1" | tee fire.log
