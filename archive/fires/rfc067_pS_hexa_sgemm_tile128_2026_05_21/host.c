/* RFC 067 PS -- TILE128 on top of N77 stack (2026-05-21)
 *
 * Comparator chain:
 *   N77 (PP):  ldmatrix.x4 + mma.m16n8k16 + cp.async.cg vec16, 64x64 tile
 *              -> peak 36.06 TFLOPS @ M=1536 ratio 0.533 vs cuBLAS HGEMM 67.65
 *   PS (this): same primitives, 128x128 output per CTA, 32 warps (1024 thd/CTA)
 *              -> hypothesis: more output/CTA -> better SM utilisation at large M
 *
 * Falsifier F-RFC067-HEXA-SGEMM-TILE128:
 *   - Numeric: per-element maxabs vs cuBLAS HGEMM (consumer mma path identical to N77 -> bit-exact)
 ...
 * Run:
 *   ./host sgemm_tile128_256x256_grid.ptx ... 1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_tile128_256x256_grid.ptx ... 1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    /* Write ptxas info log to a separate file for register/occupancy analysis. */
    FILE *fp_info = fopen("ptxas_info.log", "w");
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_tile128_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
                            load_info[si], sizeof(load_info[si])) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-tile128 PTX shape %d (%s)\n", shapes[si], entry);
            if (fp_info) {
            ...
            printf("FAILED hexa-tile128 PTX shape %d -- skipping\n", shapes[si]);
        }

        /* Also query device attrs for fns[si] if loaded. */
        if (load_ok[si]) {
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-tile128 run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-tile128 HGEMM      median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f\n",
            ...
            printf("[M=%4d] hexa-tile128 HGEMM      SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    /* N77 PP baseline (2026-05-21 RTX 5070 sm_120) for inline % delta. */
    double n77_tflops[N_SHAPES] = { 4.619278, 12.245481, 17.331835, 29.896042, 29.038887, 36.059929 };

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PS-hexa-hgemm-tile128-1024thd-STACK\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PS (V1 128x128 output tile per CTA, 32 warps = 1024 thd/CTA, 4x mma.m16n8k16 per warp per K-step)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_tile128_SxS_grid -- N77 (ldmatrix.x4 + mma.m16n8k16 + cp.async.cg vec16) consumer, scaled to 128x128 output per CTA\",\n");
    ...
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"128x128 output per CTA -> 4x fewer CTAs -> better SM utilisation @ large M; trade-off vs occupancy collapse from 1024 thd/CTA\",\n");
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_tile128_tflops\":     %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_tile128_median_ms\":  %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_tile128_mean_ms\":    %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_tile128_std_ms\":     %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_tile128_min_ms\":     %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_tile128_max_ms\":     %.6f,\n", res[si].hexa.max);
            ...
            fprintf(rj, "      \"hexa_tile128_tflops\":     null,\n");
            fprintf(rj, "      \"hexa_tile128_median_ms\":  null,\n");
            fprintf(rj, "      \"hexa_tile128_mean_ms\":    null,\n");
            fprintf(rj, "      \"hexa_tile128_std_ms\":     null,\n");
            fprintf(rj, "      \"hexa_tile128_min_ms\":     null,\n");
            fprintf(rj, "      \"hexa_tile128_max_ms\":     null,\n");
