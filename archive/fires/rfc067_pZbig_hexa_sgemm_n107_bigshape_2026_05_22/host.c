/* RFC 067 PZbig -- 4-WARP 64x64 + XOR-swizzle BIG-SHAPE extension (N107 PY @ M=2048/3072/4096) (2026-05-22)
 *
 * Comparator chain:
 *   N77 (PP):  ldmatrix.x4 + mma.m16n8k16 + cp.async.cg vec16, 64x64 tile, 16 warps
 *              -> peak 36.06 TFLOPS @ M=1536 ratio 0.533 vs cuBLAS HGEMM 67.65
 *   N89 (PS):  same primitives, 128x128 output / CTA, 32 warps (1024 thd/CTA)
 *              -> peak 37.07 TFLOPS @ M=1536 ratio 0.557
 *   N93 (PU):  N89 + epilogue vec-2 stores                       -> 37.996 TFLOPS @ M=1536
 *   N107 (PY): revert tile to 64x64, 4 warps (128 thd/CTA), keep N89's swizzle plumbing.
 *              peak 51.65 TFLOPS @ M=1536 ratio 0.777; M=256 ratio 1.053 (cuBLAS-BEAT).
 *   PZbig (this): extend N107 PY sweep to M=2048/3072/4096 (compute-bound regime).
 *                 Hypothesis: cuBLAS large-M peak ~67 TFLOPS; does N107 close ratio further
 *                 (more arithmetic intensity hides launch overhead) or saturate at structural ceiling?
 *
 * Falsifier F-RFC067-HEXA-SGEMM-N107-BIGSHAPE:
 *   - Numeric: per-element maxabs vs cuBLAS HGEMM (must be 0.0 - bit-exact bound on HGEMM-f32-acc)
 *   - Per-shape median TFLOPS over 200 reps (20 warmup), cuEvent sync each iter
 *   - Compare to N107 PY baseline; report ratio at large M
 *
 * Launch config (V1):
 ...
 * Run:
 *   ./host sgemm_4warp_swizzle_256x256_grid.ptx ... 4096x4096_grid.ptx (9 args)
 */
                    ...
                    double *out_times_ms)
{
    ...
    /* PY V1: grid = (N/64, M/64, 1), block = 128 thd (4 warps). */
...
int main(int argc, char **argv) {
    int shapes[N_SHAPES] = { 256, 384, 512, 768, 1024, 1536, 2048, 3072, 4096 };

    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_4warp_swizzle_256x256_grid.ptx ... 4096x4096_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_4warp_swizzle_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
                            load_info[si], sizeof(load_info[si])) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-py PTX shape %d (%s)\n", shapes[si], entry);
            if (fp_info) {
            ...
            printf("FAILED hexa-py PTX shape %d -- skipping\n", shapes[si]);
        }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-py run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-PY 4warp+swizzle   median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f  CTAs=%d  regs=%d\n",
            ...
            printf("[M=%4d] hexa-PY 4warp+swizzle   SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    /* Reference baselines (RTX 5070 sm_120 2026-05-21/22). Trailing zeros = no prior baseline. */
    /* N89 PS (128x128 tile, 1024 thd) -- peak 37.07 @ M=1536 */
    double n89_tflops[N_SHAPES] = { 2.179992, 5.577532, 10.000000, 19.000000, 25.000000, 37.070000,
                                    0.0, 0.0, 0.0 };
    /* N93 PU (N89 + vec-2 stores) -- peak 37.996 @ M=1536 */
    double n93_tflops[N_SHAPES] = { 2.180000, 5.580000, 10.000000, 19.000000, 25.000000, 37.996000,
                                    0.0, 0.0, 0.0 };
    /* N77 PP -- 64x64 16-warp baseline */
    double n77_tflops[N_SHAPES] = { 4.619278, 12.245481, 17.331835, 29.896042, 29.038887, 36.059929,
                                    0.0, 0.0, 0.0 };
    /* N107 PY (this kernel) prior measurement on M<=1536 -- regression-sanity for big-shape extension */
    double n107_tflops[N_SHAPES] = { 5.2825, 14.5636, 22.6108, 39.8754, 40.0172, 51.6516,
                                     0.0, 0.0, 0.0 };
    /* N89 CTA count for inline reporting (M/128)^2 */
    int    n89_ctas[N_SHAPES]   = { 4, 9, 16, 36, 64, 144, 256, 576, 1024 };

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PZbig-hexa-hgemm-n107-bigshape\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PY V1 -- 4 warps (2x2 warp grid) 64x64 output / CTA, 8 mma.m16n8k16 per warp per K-step (32 f32 acc/lane), XOR swizzle wired but masked to identity (ldmatrix.x4 coherence -- g3 caveat)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_4warp_swizzle_SxS_grid -- N77 (ldmatrix.x4 + mma.m16n8k16 + cp.async.cg vec16) primitives with 4-warp 64x64 tile per N104 #3 recommendation\",\n");
    ...
    fprintf(rj, "    \"N89_PS\": \"rfc067_pS_hexa_sgemm_tile128_2026_05_21 peak 37.07 TFLOPS @ M=1536 ratio 0.557 -- 128x128 tile 32 warps\",\n");
    fprintf(rj, "    \"N93_PU\": \"rfc067_pU_hexa_sgemm_direct_d_2026_05_21 peak 37.996 TFLOPS @ M=1536 -- N89 + vec-2 stores\",\n");
    fprintf(rj, "    \"N107_PY\": \"rfc067_pY_hexa_sgemm_4warp_swizzle_2026_05_21 peak 51.65 TFLOPS @ M=1536 ratio 0.777; M=256 ratio 1.053 cuBLAS-BEAT -- 4 warps 64x64 tile\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"Extend N107 PY 4-warp 64x64 kernel to M=2048/3072/4096 (compute-bound regime). cuBLAS HGEMM large-M peak ~67 TFLOPS on RTX 5070 sm_120. Question: does N107 ratio improve (more arithmetic intensity hides launch overhead) -> cuBLAS-BEAT, match, or saturate at structural ceiling 0.77-0.85?\",\n");
    ...
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        int has_prior = (n89_tflops[si] > 0.0);
        double pct_over_n89 = (res[si].hexa_valid && has_prior)
            ? 100.0 * (res[si].hexa_tflops - n89_tflops[si]) / n89_tflops[si]
            : 0.0;
        double pct_over_n93 = (res[si].hexa_valid && has_prior)
            ? 100.0 * (res[si].hexa_tflops - n93_tflops[si]) / n93_tflops[si]
            : 0.0;
        double pct_over_n77 = (res[si].hexa_valid && has_prior)
            ? 100.0 * (res[si].hexa_tflops - n77_tflops[si]) / n77_tflops[si]
            : 0.0;
        double pct_over_n107 = (res[si].hexa_valid && n107_tflops[si] > 0.0)
        ...
        if (has_prior) {
            ...
            fprintf(rj, "      \"n77_baseline_tflops\":     null,\n");
            fprintf(rj, "      \"n89_baseline_tflops\":     null,\n");
            fprintf(rj, "      \"n93_baseline_tflops\":     null,\n");
            fprintf(rj, "      \"n107_baseline_tflops\":    null,\n");
        }
        fprintf(rj, "      \"py_num_ctas\":             %d,\n", res[si].num_ctas);
        fprintf(rj, "      \"n89_num_ctas\":            %d,\n", n89_ctas[si]);
        fprintf(rj, "      \"py_regs_per_thd\":         %d,\n", res[si].regs_per_thd);
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_py_tflops\":          %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_py_median_ms\":       %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_py_mean_ms\":         %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_py_std_ms\":          %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_py_min_ms\":          %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_py_max_ms\":          %.6f,\n", res[si].hexa.max);
            ...
            fprintf(rj, "      \"hexa_py_tflops\":          null,\n");
            fprintf(rj, "      \"hexa_py_median_ms\":       null,\n");
            fprintf(rj, "      \"hexa_py_mean_ms\":         null,\n");
            fprintf(rj, "      \"hexa_py_std_ms\":          null,\n");
            fprintf(rj, "      \"hexa_py_min_ms\":          null,\n");
            fprintf(rj, "      \"hexa_py_max_ms\":          null,\n");
