#!/usr/bin/env python3
"""RFC 067 PBEAT -- N121 (4-warp 64x64 + 6-stage cp.async pipeline) BEAT-ENVELOPE sweep (2026-05-22).

Identical kernel body to N121 (rfc067_pZ_hexa_sgemm_4warp_6stage_2026_05_21);
ONLY the SHAPES list differs -- we sweep the small-M cuBLAS-launch-bound regime
M = 192 / 256 / 320 / 384 / 448 / 512 (all 64-aligned) to map the exact M where
hexa STOPS beating cuBLAS HGEMM.

N121 prior measurement (6 large shapes, 2026-05-22):
  M=256 ratio 1.1611 (BEAT), M=384 ratio 0.9799, M=512 ratio 0.8768.
  -> BEAT envelope clearly ends somewhere in (256, 384]. This sweep pins it.

cuBLAS-BEAT happens at small shapes because cuBLAS itself is launch-overhead-bound
there (the under-subscribed-grid regime). These BEAT shapes are NOT a compute-bound
signal -- they characterize WHERE the smaller-tile hexa kernel wins on a small grid.

PZ/N121 design (PY consumer + PW producer):
  Output tile      = 64 x 64 (M x N) per CTA
  Warps / CTA      = 4 (2x2 grid)        Threads/CTA = 128
  Per-warp output  = 32 M x 32 N         mma per K-step = 8 (2 M-stacks x 4 N-cols)
  Pipeline stages  = 6                   In-flight groups = 5    wait_group(4)
  Shared-mem/CTA   = 6 * 2 * 2048 = 24576 B (fits sm_90 100 KB carveout)
...
{adv}"""
    ...
    return f"""// RFC 067 PBEAT (N121 body) perf HGEMM hexa-emit -- 4-WARP 64x64 + 6-STAGE PIPELINE -- M=N=K={S}.
//
// Stack:
//   N107 (PY): 4-warp 64x64 tile, 128 thd/CTA, 8 mma/warp/K, 2-slot ring
//   N105 (PW): 32-warp 128x128 tile, 6-stage cp.async pipeline
//   N121 (PZ): N107 consumer + N105 producer = 4-warp 64x64 tile with 6-stage pipeline
//
...
# BEAT-envelope sweep -- 64-aligned (192/256/320/384/448/512 all divisible by 64).
SHAPES = [192, 256, 320, 384, 448, 512]
