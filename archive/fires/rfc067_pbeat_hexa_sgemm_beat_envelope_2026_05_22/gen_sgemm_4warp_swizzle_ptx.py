#!/usr/bin/env python3
"""RFC 067 PBEAT -- N107 (4-warp 64x64 + 2-stage swizzle) BEAT-ENVELOPE sweep (2026-05-22).

Identical kernel body to N107 (rfc067_pY_hexa_sgemm_4warp_swizzle_2026_05_21);
ONLY the SHAPES list differs -- we sweep M = 192/256/320/384/448/512 (all 64-aligned)
to map the exact M where hexa STOPS beating cuBLAS HGEMM.

N107 prior measurement (6 large shapes, 2026-05-22):
  M=256 ratio 1.0529 (BEAT), M=384 ratio 0.8724, M=512 ratio 0.9111.
  (the 0.91 at 512 > 0.87 at 384 is small-M cuBLAS launch-noise non-monotonicity;
   3x repeat in this fire captures variance.)

cuBLAS-BEAT happens at small shapes because cuBLAS itself is launch-overhead-bound
there. These BEAT shapes are NOT a compute-bound signal -- they characterize WHERE
the smaller-tile hexa kernel wins on a small grid (small-shape inference relevance).

PY/N107 design:
  Output tile = 64 x 64; 4 warps (2x2), 128 thd/CTA, 8 mma/warp/K, 32 f32 acc/lane.
  Double-buffered 2-slot ring (shmem 8192 B/CTA). XOR swizzle wired but masked to
  identity (ldmatrix.x4 coherence) -- single-axis test of the tile shrink.
...
def gen(S: int) -> str:
    ...
    return f"""// RFC 067 PBEAT (N107 body) perf HGEMM hexa-emit -- 4-WARP 64x64 + XOR swizzle -- M=N=K={S}.
//
//   N107 (PY): 64x64 tile, 4 warps (128 thd/CTA), 8 mma/warp, 2-slot ring buffer,
//              2-phase XOR shared-mem swizzle (masked to identity for ldmatrix coherence).
//
// Layout:
