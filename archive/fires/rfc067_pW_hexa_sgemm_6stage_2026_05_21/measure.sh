#!/usr/bin/env bash
# RFC 067 PW -- 6-STAGE cp.async PIPELINE on top of N93/PU stack (2026-05-22).
#
...
ART_DIR_BASENAME="rfc067_pW_hexa_sgemm_6stage_2026_05_21"

echo "== STEP 0: regen PTX (idempotent) ==" >&2
python3 gen_sgemm_6stage_ptx.py

echo "== STEP 1: rsync to ubu-2:/tmp/${ART_DIR_BASENAME}/ ==" >&2
ssh ubu-2 "mkdir -p /tmp/${ART_DIR_BASENAME}"
scp -q host.c gen_sgemm_6stage_ptx.py *.ptx "ubu-2:/tmp/${ART_DIR_BASENAME}/"
...
ssh ubu-2 "cd /tmp/${ART_DIR_BASENAME} && ./host \
    sgemm_6stage_256x256_grid.ptx \
    sgemm_6stage_384x384_grid.ptx \
    sgemm_6stage_512x512_grid.ptx \
    sgemm_6stage_768x768_grid.ptx \
    sgemm_6stage_1024x1024_grid.ptx \
    sgemm_6stage_1536x1536_grid.ptx 2>&1" | tee fire.log
