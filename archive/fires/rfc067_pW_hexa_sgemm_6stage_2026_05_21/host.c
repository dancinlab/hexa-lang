/* RFC 067 PW -- 6-STAGE cp.async SOFTWARE PIPELINE on top of N93/PU stack (2026-05-22)
 *
 * Comparator chain:
 *   N89 (PS):  128x128 tile, 32 warps, 2-slot pipeline
 *              -> peak 37.07 TFLOPS @ M=1536 ratio 0.557 vs cuBLAS
 *   N93 (PU):  PS + vec2 epilogue stores
 *              -> peak 37.996 TFLOPS @ M=1536 ratio 0.5705 vs cuBLAS HGEMM 66.60
 *   PW (this): PU + 6-stage cp.async pipeline (5 in-flight + 1 compute)
 *              -> hypothesis (per N104 SASS-diff): +0.20-0.30 ratio (53-57 TFLOPS @ M=1536)
 *
 ...
 * Run:    ./host sgemm_6stage_256x256_grid.ptx ... 1536x1536_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_6stage_256x256_grid.ptx ... 1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
            ...
            printf("FAILED hexa-6stage PTX shape %d -- skipping\n", shapes[si]);
        }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-6stage run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-6stage HGEMM     median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f\n",
            ...
            printf("[M=%4d] hexa-6stage HGEMM     SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "  \"variant\": \"PW (PU + 6-stage cp.async software pipeline: 5-stage prologue + steady-state wait_group(4) + tail drain)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_6stage_SxS_grid -- N93 (PU tile128 1024thd vec2-epilogue) consumer, 6-slot ring buffer\",\n");
    ...
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"6-stage cp.async pipeline (5 in-flight + 1 compute) overlaps gmem load latency across K-steps; expect +0.20-0.30 ratio (37 -> 53-57 TFLOPS @ M=1536) per N104 SASS-diff\",\n");
    fprintf(rj, "  \"design\": \"shmem _tg_a expanded 8192 B -> 24576 B (6 slabs * 4096 B); _tg_b same. PROLOGUE: 5 cp.async issues each with own commit_group. STEADY-STATE: wait_group(4) + bar.sync + ldmatrix+4xmma + bar.sync + issue+commit (predicated when K-blocks remain). DRAIN: cp.async.wait_all.\",\n");
    ...
    fprintf(rj, "    \"shmem_bytes_per_cta\": 49152\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_6stage_tflops\":    %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_6stage_median_ms\": %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_6stage_mean_ms\":   %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_6stage_std_ms\":    %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_6stage_min_ms\":    %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_6stage_max_ms\":    %.6f,\n", res[si].hexa.max);
            ...
            fprintf(rj, "      \"hexa_6stage_tflops\":    null,\n");
            fprintf(rj, "      \"hexa_6stage_median_ms\": null,\n");
            fprintf(rj, "      \"hexa_6stage_mean_ms\":   null,\n");
            fprintf(rj, "      \"hexa_6stage_std_ms\":    null,\n");
            fprintf(rj, "      \"hexa_6stage_min_ms\":    null,\n");
            fprintf(rj, "      \"hexa_6stage_max_ms\":    null,\n");
