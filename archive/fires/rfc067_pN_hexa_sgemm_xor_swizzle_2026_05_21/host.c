/* RFC 067 PN -- hexa-emit SGEMM + XOR-swizzle shared-mem (2026-05-21)
 *
 * Direct comparator to:
 *   - PK (rfc067_pK_hexa_sgemm_multistage_2026_05_21 / N66) baseline
 *     -- 2-stage cp.async pipeline + plain row-major shared layout.
 *     -- Peak 13.35 TFLOPS @ M=1536 ratio 0.406 on RTX 5070 sm_120.
 *   - N71 instrumented analysis flagged 'shared-mem bank-conflict
 *     swizzling' as one of 3 unfalsified candidates.
 *
 * This fire adds an XOR row-permutation swizzle:
 *   phys_col = log_col XOR (log_row & 7)
 * applied symmetrically on cooperative cp.async stores and per-lane
 * shared loads.  Consumer switches from wmma.load.shared.tf32 +
 * wmma.mma.m16n16k8 to mma.sync.aligned.m16n8k8 with manual fragment
 * loads (PTX-documented lane layout), and two mmas per warp cover the
 * 16x16 tile to keep PK's geometry.
 *
 * Measurement protocol identical to PK:
 *   - 20 warmup launches per (kernel, shape)
 ...
 * Run:   ./host sgemm_xor_swizzle_256x256_grid.ptx ... 1536x1536
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_xor_swizzle_256x256_grid.ptx ... sgemm_xor_swizzle_1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_xor_swizzle_%dx%d_grid", shapes[si], shapes[si]);
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si]) == 0) {
            load_ok[si] = 1; load_err[si][0] = 0;
            printf("loaded hexa-xor-swizzle PTX shape %d (%s)\n", shapes[si], entry);
        } else {
            load_ok[si] = 0;
            snprintf(load_err[si], sizeof(load_err[si]),
                "PTX load/lookup failed for %s", argv[1 + si]);
            printf("FAILED hexa-xor-swizzle PTX shape %d -- skipping\n", shapes[si]);
        }
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-xor-swizzle run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-xor-swizzle SGEMM   median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.6f\n",
            ...
            printf("[M=%4d] hexa-xor-swizzle SGEMM   SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PN-hexa-sgemm-xor-swizzle\",\n");
    fprintf(rj, "  \"date_utc\": \"2026-05-21\",\n");
    fprintf(rj, "  \"host\": \"%s\",\n", getenv("PN_HOST") ? getenv("PN_HOST") : "ubu-2");
    fprintf(rj, "  \"device\": \"%s\",\n", dev_name);
    ...
    fprintf(rj, "  \"variant\": \"PN (TF32 mma m16n8k8 x 2 per warp + cp.async.ca.shared.global 2-stage + XOR row-permutation swizzle phys_col=log_col XOR (row & 7))\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_xor_swizzle_SxS_grid -- 16-warp 4x4 grid 64x64 output tile, 2-slab _tg_a/_tg_b (4096 B each) XOR-swizzled, cp.async.ca producer + per-lane ld.shared.b32 consumer feeding 2x mma.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32 per K-step\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (TF32 dispatch on sm_80+)\",\n");
    fprintf(rj, "  \"baseline_pK_n66\": \"rfc067_pK_hexa_sgemm_multistage_2026_05_21 (plain layout, peak 13.35 TFLOPS @ M=1536 ratio 0.406 on RTX 5070 sm_120)\",\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        fprintf(rj, "    {\n");
        fprintf(rj, "      \"M\": %d, \"N\": %d, \"K\": %d,\n", res[si].M, res[si].N, res[si].K);
        fprintf(rj, "      \"cublas_sgemm_tflops\":     %.6f,\n", res[si].cublas_tflops);
        fprintf(rj, "      \"cublas_sgemm_median_ms\":  %.6f,\n", res[si].cublas.median);
        fprintf(rj, "      \"cublas_sgemm_mean_ms\":    %.6f,\n", res[si].cublas.mean);
        fprintf(rj, "      \"cublas_sgemm_std_ms\":     %.6f,\n", res[si].cublas.std);
        fprintf(rj, "      \"cublas_sgemm_min_ms\":     %.6f,\n", res[si].cublas.min);
        fprintf(rj, "      \"cublas_sgemm_max_ms\":     %.6f,\n", res[si].cublas.max);
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_xor_swizzle_tflops\":      %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_xor_swizzle_median_ms\":   %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_xor_swizzle_mean_ms\":     %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_xor_swizzle_std_ms\":      %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_xor_swizzle_min_ms\":      %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_xor_swizzle_max_ms\":      %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"ratio\":                   %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_xor_swizzle_tflops\":      null,\n");
            fprintf(rj, "      \"hexa_xor_swizzle_median_ms\":   null,\n");
            fprintf(rj, "      \"hexa_xor_swizzle_mean_ms\":     null,\n");
            fprintf(rj, "      \"hexa_xor_swizzle_std_ms\":      null,\n");
            fprintf(rj, "      \"hexa_xor_swizzle_min_ms\":      null,\n");
            fprintf(rj, "      \"hexa_xor_swizzle_max_ms\":      null,\n");
