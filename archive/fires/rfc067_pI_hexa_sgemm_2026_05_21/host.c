/* RFC 067 PI -- hexa-emit SGEMM (TF32 WMMA) vs cuBLAS SGEMM (2026-05-21)
 *
 * Companion to:
 *   - PG (HGEMM via cublasGemmEx FP16-in/FP32-acc + 23-shape hexa wmma)
 *   - PH (cuBLAS SGEMM FP32 baseline 23-shape)
 *
 * This fire adds the missing SGEMM column for the hexa-emit family. The
 * hand-emit PTX kernels (gen_sgemm_ptx.py) use the TF32 wmma shape
 *   wmma.mma.sync.aligned.row.col.m16n16k8.f32.tf32.tf32.f32
 * which mirrors cuBLAS SGEMM under CUBLAS_TENSOR_OP_MATH on sm_80+ (the
 * exact math mode PH used). Both columns therefore measure the TF32
 * tensor-core SGEMM path.
 *
 * Layout / convention (matches PG):
 *   We compute C[M,N] = A[M,K] * B[K,N] in row-major.
 *   cuBLAS is col-major, so we ask it to compute
 *     C^T[N,M] = B^T[N,K] * A^T[K,M]
 *   by passing (op_N, op_N, N, M, K, B, ldb=N, A, lda=K, C, ldc=N).
 *
 * Measurement protocol (identical to PG/PH):
 ...
 * Run:
 *   ./host sgemm_256x256_grid.ptx sgemm_384x384_grid.ptx \
 *          sgemm_512x512_grid.ptx sgemm_768x768_grid.ptx \
 *          sgemm_1024x1024_grid.ptx sgemm_1536x1536_grid.ptx
 */
...
static int run_hexa(CUfunction f_hexa,
                    ...
                    double *out_times_ms)
{
    const int K_PER_TILE = 8;                 /* TF32 wmma K=8 */
...
/* Sanity check: compare hexa output vs cuBLAS output. Both use TF32 input
 * rounding, so per-element delta should be small (~K * eps_tf32, where
 * eps_tf32 ~ 2^-10). For modular sawtooth inputs we expect maxabs ~ 0
 * because the sawtooth values fit exactly in TF32 (no low mantissa bits).
 */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_256x256_grid.ptx ... sgemm_1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_%dx%d_grid", shapes[si], shapes[si]);
    ...
    /* CUBLAS_TENSOR_OP_MATH so cuBLAS SGEMM dispatches to TF32 tensor cores
     * on sm_80+ -- this matches the math regime of the hexa-emit kernel
     * (which uses wmma TF32 directly) and is the same mode used in PH. */
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        /* Same modular sawtooth as PG/PH for parity. Values are exact in TF32. */
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                ...
                printf("[M=%4d] hexa SGEMM    median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.6f\n",
            ...
            printf("[M=%4d] hexa SGEMM    SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PI-hexa-sgemm\",\n");
    ...
    fprintf(rj, "  \"variant\": \"B (TF32 wmma m16n16k8.f32.tf32.tf32.f32)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_SxS_grid -- 16-warp 4x4 grid 64x64 output tile, TF32 wmma K=8 inner loop, hand-port of pG HGEMM template\",\n");
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_sgemm_tflops\":       %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_sgemm_median_ms\":    %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_sgemm_mean_ms\":      %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_sgemm_std_ms\":       %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_sgemm_min_ms\":       %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_sgemm_max_ms\":       %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"ratio\":                   %.6f,\n", res[si].ratio);
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_sgemm_tflops\":       null,\n");
            fprintf(rj, "      \"hexa_sgemm_median_ms\":    null,\n");
            fprintf(rj, "      \"hexa_sgemm_mean_ms\":      null,\n");
            fprintf(rj, "      \"hexa_sgemm_std_ms\":       null,\n");
            fprintf(rj, "      \"hexa_sgemm_min_ms\":       null,\n");
            fprintf(rj, "      \"hexa_sgemm_max_ms\":       null,\n");
