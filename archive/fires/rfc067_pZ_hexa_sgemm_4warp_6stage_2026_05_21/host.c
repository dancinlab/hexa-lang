 ...
 *   N93 (PU):  N89 + vec-2 stores                                  -> 37.996 TFLOPS @ M=1536 ratio 0.570
 *   N105 (PW): N93 + 6-stage cp.async pipeline                     -> 40.90 TFLOPS @ M=1536 ratio 0.614
 *   N107 (PY): 4-warp 64x64 tile, 128 thd/CTA, 8 mma/warp/K        -> 51.65 TFLOPS @ M=1536 ratio 0.777
 *   PZ (this): PY consumer + PW producer = stack both axes         -> hypothesis: ratio 0.82+
 *
 ...
 *   - Compare to PY and PW baselines
 *
 ...
 * Run:
 *   ./host sgemm_4warp_6stage_256x256_grid.ptx ... 1536x1536_grid.ptx
 */
                    ...
                    double *out_times_ms)
{
    ...
    /* PZ: grid = (N/64, M/64, 1), block = 128 thd (4 warps). */
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_4warp_6stage_256x256_grid.ptx ... 1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        char entry[64];
        snprintf(entry, sizeof(entry), "sgemm_4warp_6stage_%dx%d_grid", shapes[si], shapes[si]);
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                ...
                printf("[M=%4d] hexa-PZ 4warp+6stage     median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f  CTAs=%d  regs=%d  shmem=%d\n",
            ...
            printf("[M=%4d] hexa-PZ 4warp+6stage     SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    /* N105 PW (N93 + 6-stage)                                 */
    double pw_tflops[N_SHAPES]  = { 2.496610, 6.285869, 11.436412, 27.742825, 26.102242, 40.897874 };
    /* N107 PY (4-warp 64x64 tile)                             */
    double py_tflops[N_SHAPES]  = { 5.282499, 14.563555, 22.610803, 39.875426, 40.017213, 51.651633 };

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PZ-hexa-hgemm-4warp-6stage-STACK\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PZ -- STACK: N107 consumer (4-warp 2x2 grid, 64x64 tile, 8 mma/warp/K, 32 f32 acc/lane, 128 thd/CTA) + N105 producer (6-stage cp.async software pipeline, 5 in-flight, wait_group(4))\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_4warp_6stage_SxS_grid -- PY consumer + PW producer\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"stack_inputs\": {\n");
    fprintf(rj, "    \"N93_PU\":  \"rfc067_pU_hexa_sgemm_direct_d_2026_05_21 peak 37.996 TFLOPS @ M=1536 ratio 0.5705\",\n");
    fprintf(rj, "    \"N105_PW\": \"rfc067_pW_hexa_sgemm_6stage_2026_05_21 peak 40.898 TFLOPS @ M=1536 ratio 0.6141 (+7.6%% vs N93)\",\n");
    fprintf(rj, "    \"N107_PY\": \"rfc067_pY_hexa_sgemm_4warp_swizzle_2026_05_21 peak 51.652 TFLOPS @ M=1536 ratio 0.7770 (+35.9%% vs N93, +0.220 ratio uplift)\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"PY consumer + PW producer = stacked compound. If additive: ratio ~ 0.78 + 0.04 = 0.82. If multiplicative: ratio could hit 0.85-0.95. If pipeline cost cuts CTA/SM occupancy, useful negative.\",\n");
    fprintf(rj, "  \"design\": \"PY's 4-warp consumer + PW's 6-slot ring buffer. Slab=2048 B (64 rows x 16 K-elem x 2 B). 6 stages x 2 arrays = 24576 B shmem/CTA. Prologue issues 5 cp.async groups (commit_group each). Steady-state: wait_group(4) + bar.sync + ldmatrix.x4 (A+B) + 8 mma + bar.sync + cp.async/commit. Drain via wait_all.\",\n");
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        double pct_over_n93 = res[si].hexa_valid
            ...
            : 0.0;
        double pct_over_pw = res[si].hexa_valid
            ? 100.0 * (res[si].hexa_tflops - pw_tflops[si]) / pw_tflops[si]
            : 0.0;
