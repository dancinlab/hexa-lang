/* RFC 067 PY-w8 -- 8-WARP 64x64 occupancy-isolation (2026-05-22)
 *
 * Isolation cell vs N107 PY and N89 PS:
 *   N89  (PS):  128x128 output / CTA, 32 warps (1024 thd/CTA), 4 mma/warp/K-step
 *               -> 37.07 TFLOPS @ M=1536 ratio 0.557 vs cuBLAS HGEMM 66.48
 *   N107 (PY):  64x64 tile, 4 warps (128 thd/CTA), 8 mma/warp/K-step
 *               -> 51.65 TFLOPS @ M=1536 ratio 0.777
 *   PY-w8 (this): 64x64 tile, 8 warps (256 thd/CTA), 4 mma/warp/K-step
 *               -> isolation cell -- tests warp-count axis (4->8) at fixed tile
 *
 * Falsifier F-RFC067-HEXA-SGEMM-W8-OCCUPANCY:
 ...
 *   - Compare to N107 PY baseline AND to N89 PS baseline
 *
 * Launch config (w8):
 *   block = (256, 1, 1)              -- 8 warps
 *   grid  = (N/64, M/64, 1)          -- identical to N107
 *
 * Build:
 *   nvcc -O2 -arch=sm_90 -o host host.c -lcuda -lcublas -lm
 * Run:
 *   ./host sgemm_w8_256x256_grid.ptx ... 1536x1536_grid.ptx
 */
                    ...
                    double *out_times_ms)
{
    ...
    /* w8: grid = (N/64, M/64, 1), block = 256 thd (8 warps). */
    ...
    for (int i = 0; i < warmup; ++i) {
        CHECK_CU(cuLaunchKernel(f_hexa,
            gx, gy, 1, BLOCK_THD, 1, 1, 0, NULL, kargs, NULL));
    }
    CHECK_CU(cuCtxSynchronize());
                        ...
                        float *out_maxabs, float *out_maxrel)
{
    ...
    unsigned int gx = (unsigned int)(N / 64);
    unsigned int gy = (unsigned int)(M / 64);
    CHECK_CU(cuLaunchKernel(f_hexa, gx, gy, 1, BLOCK_THD, 1, 1, 0, NULL, kargs, NULL));
    CHECK_CU(cuCtxSynchronize());
...
int main(int argc, char **argv) {
    ...
    if (argc < 1 + N_SHAPES) {
        fprintf(stderr, "usage: %s sgemm_w8_256x256_grid.ptx ... 1536x1536_grid.ptx (%d args)\n",
                argv[0], N_SHAPES);
        return 2;
    }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
            ...
            printf("FAILED hexa-w8 PTX shape %d -- skipping\n", shapes[si]);
        }
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-w8 run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                ...
                printf("[M=%4d] hexa-w8 8warp 64x64       median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f  CTAs=%d  regs=%d\n",
            ...
            printf("[M=%4d] hexa-w8 8warp 64x64       SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    /* N89 PS  (128x128 tile, 32 warps) -- peak 37.07 @ M=1536 */
    double n89_tflops[N_SHAPES]  = { 2.179992, 5.577532, 10.000000, 19.000000, 25.000000, 37.070000 };
    /* N107 PY (64x64 tile, 4 warps)   -- peak 51.65 @ M=1536 (rfc067_pY 2026-05-22 fire.log) */
    double n107_tflops[N_SHAPES] = { 5.282500, 14.563600, 22.610800, 39.875400, 40.017200, 51.651600 };
    /* cuBLAS HGEMM reference from N107 fire */
    double n107_cublas_tflops[N_SHAPES] = { 5.017100, 16.693100, 24.818400, 47.662500, 54.295200, 66.478500 };
    /* N107 CTA count -- identical to PY-w8 since same 64x64 tile */
    int    n107_ctas[N_SHAPES]   = { 16, 36, 64, 144, 256, 576 };
    /* N89 CTA count (M/128)^2 */
    int    n89_ctas[N_SHAPES]    = { 4, 9, 16, 36, 64, 144 };

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PY-w8-hexa-hgemm-8warp-occupancy-iso\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PY-w8 -- 8 warps (4x2 warp grid) 64x64 output / CTA, 4 mma.m16n8k16 per warp per K-step (16 f32 acc/lane), 256 thd/CTA (only first 128 predicated load), swizzle identity (matches N107 PY)\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_w8_SxS_grid -- single-axis isolation of warp-count axis between N107 PY (4 warps) and N89 PS (32 warps) at fixed 64x64 tile\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"comparators\": {\n");
    fprintf(rj, "    \"N89_PS\":  \"128x128 tile, 32 warps (1024 thd/CTA), 4 mma/warp/K-step, 144 CTAs @ M=1536, peak 37.07 TFLOPS ratio 0.557\",\n");
    fprintf(rj, "    \"N107_PY\": \"64x64 tile,   4 warps ( 128 thd/CTA), 8 mma/warp/K-step, 576 CTAs @ M=1536, peak 51.65 TFLOPS ratio 0.777\",\n");
    fprintf(rj, "    \"PY-w8\":   \"64x64 tile,   8 warps ( 256 thd/CTA), 4 mma/warp/K-step, 576 CTAs @ M=1536, THIS FIRE\"\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"isolation_design\": \"Holds axis (1) tile=64x64 from N107. Axis (2) warps moves 4->8 (toward N89's 32). Axis (3) mma/warp/K-step moves 8->4 (matches N89). Tests whether warp doubling helps (occupancy lift) or hurts (per-warp ILP collapse) at fixed tile/CTA count.\",\n");
    fprintf(rj, "  \"launch_config\": {\n");
    fprintf(rj, "    \"threads_per_block\": 256,\n");
    fprintf(rj, "    \"warps_per_block\": 8,\n");
    fprintf(rj, "    \"warp_grid_M\": 4,\n");
    fprintf(rj, "    \"warp_grid_N\": 2,\n");
    ...
    fprintf(rj, "    \"load_predicated_first_128_threads\": true\n");
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        fprintf(rj, "      \"n107_cublas_tflops\":      %.6f,\n", n107_cublas_tflops[si]);
        fprintf(rj, "      \"w8_num_ctas\":             %d,\n", res[si].num_ctas);
        fprintf(rj, "      \"n107_num_ctas\":           %d,\n", n107_ctas[si]);
        fprintf(rj, "      \"n89_num_ctas\":            %d,\n", n89_ctas[si]);
        fprintf(rj, "      \"w8_regs_per_thd\":         %d,\n", res[si].regs_per_thd);
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_w8_tflops\":          %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_w8_median_ms\":       %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_w8_mean_ms\":         %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_w8_std_ms\":          %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_w8_min_ms\":          %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_w8_max_ms\":          %.6f,\n", res[si].hexa.max);
            ...
            fprintf(rj, "      \"hexa_w8_tflops\":          null,\n");
            fprintf(rj, "      \"hexa_w8_median_ms\":       null,\n");
            fprintf(rj, "      \"hexa_w8_mean_ms\":         null,\n");
            fprintf(rj, "      \"hexa_w8_std_ms\":          null,\n");
            fprintf(rj, "      \"hexa_w8_min_ms\":          null,\n");
            fprintf(rj, "      \"hexa_w8_max_ms\":          null,\n");
