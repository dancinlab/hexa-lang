#!/usr/bin/env bash
# RFC 067 PY-w8 -- 8-WARP 64x64 occupancy-isolation fire.
#
...
ART_DIR_BASENAME="rfc067_w8_hexa_sgemm_occupancy_iso_2026_05_22"

echo "== STEP 0: regen PTX (idempotent) ==" >&2
python3 gen_sgemm_w8_ptx.py

echo "== STEP 1: rsync to ubu-2:/tmp/${ART_DIR_BASENAME}/ ==" >&2
ssh ubu-2 "mkdir -p /tmp/${ART_DIR_BASENAME}"
scp -q host.c gen_sgemm_w8_ptx.py *.ptx "ubu-2:/tmp/${ART_DIR_BASENAME}/"
...
ssh ubu-2 "cd /tmp/${ART_DIR_BASENAME} && ./host \
    sgemm_w8_256x256_grid.ptx \
    sgemm_w8_384x384_grid.ptx \
    sgemm_w8_512x512_grid.ptx \
    sgemm_w8_768x768_grid.ptx \
    sgemm_w8_1024x1024_grid.ptx \
    sgemm_w8_1536x1536_grid.ptx 2>&1" | tee fire.log
