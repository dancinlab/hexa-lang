/* RFC 067 PSWZ -- 4-WARP 64x64 + CTA-swizzle (2D super-block) cliff sweep (2026-05-22)
 *
 * Goal: test CTA-swizzle hypothesis for the M >= 6144 ratio cliff observed in N130
 *       (rfc067_pmax). N130 row-major CTA visitation thrashes L2 past M=4096; this
 *       fire adds 4 x 4 super-block CTA-swizzle to improve L2 reuse.
 *
 * Sweep:
 *   - M = 4096  (N107 baseline 57.33 / ratio 0.819)
 *   - M = 5120  NEW intermediate cliff-boundary
 *   - M = 6144  (N130 cliff 16.55 / 0.234 -- does swizzle recover?)
 *   - M = 8192  (N130 cliff 13.91 / 0.304)
 *
 * Falsifier F-RFC067-HEXA-SGEMM-CTA-SWIZZLE:
 ...
 * Run:
 *   ./host sgemm_4warp_swizzle_4096x4096_grid.ptx \
 *          sgemm_4warp_swizzle_5120x5120_grid.ptx \
 *          sgemm_4warp_swizzle_6144x6144_grid.ptx \
 *          sgemm_4warp_swizzle_8192x8192_grid.ptx
 */
...
int main(int argc, char **argv) {
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ptx_kernel(argv[1 + si], entry, &mods[si], &fns[si],
            ...
            printf("loaded hexa-PSWZ PTX shape %d (%s)\n", shapes[si], entry);
            if (fp_info) {
    ...
    for (int si = 0; si < N_SHAPES; ++si) {
        ...
        if (load_ok[si]) {
            ...
            if (run_hexa(fns[si], M, N, K, da, db, dc, WARMUP, REPS, times) != 0) {
                fprintf(stderr, "hexa-PSWZ run failed at shape %d\n", M);
                res[si].hexa_valid = 0;
                snprintf(res[si].note, sizeof(res[si].note),
                ...
                printf("[M=%4d] hexa-PSWZ 4warp+CTA-swz median=%.6f ms  std=%.6f  min=%.6f  max=%.6f  TFLOPS=%8.4f  ratio=%.4f  maxabs=%.4f  maxrel=%.4f  CTAs=%d  regs=%d\n",
            ...
            printf("[M=%4d] hexa-PSWZ 4warp+CTA-swz SKIPPED -- %s\n", M, res[si].note);
        }
    ...
    /* N130 baselines (4-warp 64x64, NO swizzle).
       Per-shape order matches shapes[] = {4096, 5120, 6144, 8192}.
       M=5120 not measured in N130 -> 0.0 placeholder (delta reported only where available). */
    double n130_hexa_tflops[N_SHAPES]   = { 57.330442, 0.0, 16.552417, 13.908097 };
    double n130_cublas_tflops[N_SHAPES] = { 70.046438, 0.0, 70.820378, 45.725131 };
    double n130_ratio[N_SHAPES]         = { 0.818463,  0.0,  0.233724,  0.304167 };

    FILE *rj = fopen("result.json", "w");
    fprintf(rj, "{\n");
    fprintf(rj, "  \"rfc\": \"067-PSWZ-hexa-hgemm-cta-swizzle\",\n");
    ...
    fprintf(rj, "  \"variant\": \"PSWZ -- N107 PY 4-warp 64x64 + Pattern A 2D super-block CTA-swizzle (4x4) for L2 reuse. raw (ctaid.x, ctaid.y) -> (sw_x, sw_y) via linear/16, %% sb_per_row. Adds 1 div.u32 + 1 rem.u32 per CTA at entry (amortised). Rest of kernel byte-identical to N107 PY.\",\n");
    fprintf(rj, "  \"hexa_kernel_family\": \"sgemm_4warp_swizzle_SxS_grid -- N107 PY + CTA swizzle 4x4 super-block\",\n");
    fprintf(rj, "  \"cublas_math_mode\": \"CUBLAS_TENSOR_OP_MATH (HGEMM via cublasGemmEx)\",\n");
    fprintf(rj, "  \"swizzle_pattern\": \"2D super-block (SUPER_BLOCK_SIZE=4) -- super-block 4x4 CTAs covers 256x256 output tile, raw (ctaid.x, ctaid.y) remapped to (sw_x, sw_y) via div+rem on linear/16 vs gridDim.x/4\",\n");
    fprintf(rj, "  \"super_block_size\": 4,\n");
    fprintf(rj, "  \"super_block_output_tile\": 256,\n");
    ...
    fprintf(rj, "  },\n");
    fprintf(rj, "  \"hypothesis\": \"Row-major CTA visitation thrashes L2 past M=4096 (working set 64 MB -> 144 MB @ M=6144 -> 256 MB @ M=8192 vs 32 MB L2). 4x4 super-block CTA-swizzle keeps super-block working set ~6 MB at K=6144 (well under L2), restoring B/A reuse across super-block. If ratio @ M=6144 recovers toward 0.5+, L2-thrash CONFIRMED as primary cliff mechanism. If ratio stays ~0.25, L2-thrash partially refuted -- DRAM bandwidth limit also contributes.\",\n");
    ...
    fprintf(rj, "  \"shapes\": [\n");
    for (int si = 0; si < N_SHAPES; ++si) {
        int has_prior = (n130_hexa_tflops[si] > 0.0);
        double pct_over_n130 = (res[si].hexa_valid && has_prior)
            ? 100.0 * (res[si].hexa_tflops - n130_hexa_tflops[si]) / n130_hexa_tflops[si]
            : 0.0;
        double ratio_delta = (res[si].hexa_valid && has_prior)
            ? (res[si].ratio - n130_ratio[si])
            : 0.0;
        fprintf(rj, "    {\n");
        ...
        if (has_prior) {
            fprintf(rj, "      \"n130_baseline_cublas_tflops\": %.6f,\n", n130_cublas_tflops[si]);
            fprintf(rj, "      \"n130_baseline_hexa_tflops\":   %.6f,\n", n130_hexa_tflops[si]);
            fprintf(rj, "      \"n130_baseline_ratio\":         %.6f,\n", n130_ratio[si]);
        } else {
            fprintf(rj, "      \"n130_baseline_cublas_tflops\": null,\n");
            fprintf(rj, "      \"n130_baseline_hexa_tflops\":   null,\n");
            fprintf(rj, "      \"n130_baseline_ratio\":         null,\n");
        }
        fprintf(rj, "      \"pswz_num_ctas\":           %d,\n", res[si].num_ctas);
        fprintf(rj, "      \"pswz_regs_per_thd\":       %d,\n", res[si].regs_per_thd);
        if (res[si].hexa_valid) {
            fprintf(rj, "      \"hexa_pswz_tflops\":        %.6f,\n", res[si].hexa_tflops);
            fprintf(rj, "      \"hexa_pswz_median_ms\":     %.6f,\n", res[si].hexa.median);
            fprintf(rj, "      \"hexa_pswz_mean_ms\":       %.6f,\n", res[si].hexa.mean);
            fprintf(rj, "      \"hexa_pswz_std_ms\":        %.6f,\n", res[si].hexa.std);
            fprintf(rj, "      \"hexa_pswz_min_ms\":        %.6f,\n", res[si].hexa.min);
            fprintf(rj, "      \"hexa_pswz_max_ms\":        %.6f,\n", res[si].hexa.max);
            fprintf(rj, "      \"hexa_vs_cublas_maxabs\":   %.6f,\n", res[si].hexa_maxabs);
            fprintf(rj, "      \"hexa_vs_cublas_maxrel\":   %.6f,\n", res[si].hexa_maxrel);
            fprintf(rj, "      \"ratio_vs_cublas\":         %.6f,\n", res[si].ratio);
            if (has_prior) {
                fprintf(rj, "      \"pct_over_n130\":           %.4f,\n", pct_over_n130);
                fprintf(rj, "      \"ratio_delta_vs_n130\":     %.6f,\n", ratio_delta);
            } else {
                fprintf(rj, "      \"pct_over_n130\":           null,\n");
                fprintf(rj, "      \"ratio_delta_vs_n130\":     null,\n");
            }
            fprintf(rj, "      \"note\":                    null\n");
        } else {
            fprintf(rj, "      \"hexa_pswz_tflops\":        null,\n");
            fprintf(rj, "      \"hexa_pswz_median_ms\":     null,\n");
            fprintf(rj, "      \"hexa_pswz_mean_ms\":       null,\n");
            fprintf(rj, "      \"hexa_pswz_std_ms\":        null,\n");
            fprintf(rj, "      \"hexa_pswz_min_ms\":        null,\n");
            fprintf(rj, "      \"hexa_pswz_max_ms\":        null,\n");
