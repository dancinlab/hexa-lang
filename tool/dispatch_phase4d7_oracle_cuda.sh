...
#     tool/flame_phase4d7_gpu_path_oracle.c
...
#
# Falsifier:
#   F-RFC058-GPU-PATH-ORACLE-CUDA  the d768 GPU-path projection
...
PHASE_ID="flame_phase4d7_oracle_cuda_2026_05_18"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOCAL_DIR="${REPO_ROOT}/state/${PHASE_ID}"

ORACLE_C="${REPO_ROOT}/tool/flame_phase4d7_gpu_path_oracle.c"
ORACLE_SH="${REPO_ROOT}/tool/flame_phase4d7_gpu_path_oracle.sh"
PRIM_C="${REPO_ROOT}/tool/flame_phase4d6_matmul_primitives.c"
RUNTIME_CUDA_C="${REPO_ROOT}/self/cuda/runtime_cuda.c"

PHASE_LABEL="flame-phase4d7-oracle-cuda"
REMOTE_WORK="/workspace/flame_oracle"
...
$SCP_CMD "$ORACLE_C"       "root@$SSH_HOST:$REMOTE_WORK/tool/flame_phase4d7_gpu_path_oracle.c"
$SCP_CMD "$ORACLE_SH"      "root@$SSH_HOST:$REMOTE_WORK/tool/flame_phase4d7_gpu_path_oracle.sh"
...
$SSH_CMD "cd $REMOTE_WORK && \
    chmod +x tool/flame_phase4d7_gpu_path_oracle.sh ; \
    echo '=== nvidia-smi PRE ===' ; \
    ...
    nohup bash -c 'S=\$(date +%s); timeout ${RUN_BUDGET_SEC} bash tool/flame_phase4d7_gpu_path_oracle.sh --cuda > oracle.out 2>&1; R=\$?; E=\$(date +%s); echo oracle_rc=\$R > oracle_meta.txt; echo wall_seconds=\$((E-S)) >> oracle_meta.txt; touch oracle.done' >/dev/null 2>&1 & disown ; \
    sleep 3 ; \
