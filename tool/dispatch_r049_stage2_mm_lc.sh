#!/bin/bash
# dispatch_r049_stage2_mm_lc.sh — RFC 049 Stage 2 fire-validation.
#
# Fires self/cuda/experiments/r049_stage2_mm_lc.cu on a vast.ai sm_80+
...
#
# Output: state/forge_rfc049_stage2_mmlc_2026_05_19/{result.json,build.log,fire.log}
...
LOCAL_DIR="$REPO/state/forge_rfc049_stage2_mmlc_2026_05_19"
SRC_CU="$REPO/self/cuda/experiments/r049_stage2_mm_lc.cu"
SRC_RT="$REPO/self/cuda/runtime_bf16.c"
REMOTE_WORK="/workspace/rfc049_mmlc"
LABEL="forge-rfc049-stage2-mmlc"
...
echo "=== RFC 049 Stage 2 matmul+layercast fire — $(date -u) ==="
...
$SCP "$SRC_CU" "root@$SSH_HOST:$REMOTE_WORK/experiments/r049_stage2_mm_lc.cu"

# ── 6) build + run ────────────────────────────────────────────────────
echo "[6/8] Build + fire ..."
$SC "cd $REMOTE_WORK/experiments && \
  nvcc -O3 -std=c++17 -arch=native r049_stage2_mm_lc.cu \
    -lcublas -lcudart -lm -o r049_stage2_mm_lc 2>&1 | tee build.log ; \
  BRC=\${PIPESTATUS[0]} ; echo \"BUILD_RC=\$BRC\" ; \
  if [ \"\$BRC\" = '0' ]; then ./r049_stage2_mm_lc 2>&1 | tee fire.log ; \
...
echo "=== RFC 049 Stage 2 matmul+layercast fire DONE — $(date -u) ==="
