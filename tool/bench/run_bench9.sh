...
# epilogue fusion. We attack with cuBLASLt:
#   - GEMM_BACKEND=5            : cublasLtMatmulAlgoGetHeuristic AUTOTUNE (TF32 lane)
#   - GEMM_BACKEND=5 -DLT_BF16 : same, BF16 lane (cast to 16BF, COMPUTE_32F)
...
nvcc $NVCC_FLAGS -DBENCH_PREC=1 -DUSE_TF32 -DGEMM_BACKEND=5            -o /tmp/flame_lt_tf32     "$HERE/flame_bench_step_og.cu" -lcublas -lcublasLt || { log "BUILD FAIL lt-tf32"; exit 1; }
nvcc $NVCC_FLAGS -DBENCH_PREC=1 -DUSE_TF32 -DGEMM_BACKEND=5 -DLT_BF16  -o /tmp/flame_lt_bf16     "$HERE/flame_bench_step_og.cu" -lcublas -lcublasLt || { log "BUILD FAIL lt-bf16"; exit 1; }
log "build OK"
