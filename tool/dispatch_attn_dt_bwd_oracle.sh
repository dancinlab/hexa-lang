...
#   This oracle (tool/cuda_test_attn_dt_bwd.cu, self-contained: kernel
...
#
# Falsifiers (from the .cu): F-MK2C4-ATTNDT-BWD-EXACT |Δ|==0,
#                            F-MK2C4-ATTNDT-BWD-BWD-EXACT |Δ|==0
#
...
PHASE_ID="attn_dt_bwd_oracle_2026_05_19_d"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOCAL_DIR="${REPO_ROOT}/state/${PHASE_ID}"

ORACLE_CU="${REPO_ROOT}/tool/cuda_test_attn_dt_bwd.cu"

PHASE_LABEL="flame-rmsnorm-mh-oracle"
REMOTE_WORK="/workspace/attn_dt_bwd_oracle"
...
echo "[5/8] Upload cuda_test_attn_dt_bwd.cu ..."
$SCP_CMD "$ORACLE_CU" "root@$SSH_HOST:$REMOTE_WORK/cuda_test_attn_dt_bwd.cu"

# ── 6) nvcc build + run (default n=786432 + a d768 shape) ─
echo "[6/8] Build + run oracle on sm_${DEV_CC} ..."
$SSH_CMD "cd $REMOTE_WORK && \
    echo '=== nvcc build ===' ; \
    nvcc -arch=sm_${DEV_CC} -O2 -Xcompiler -ffp-contract=off -o cuda_test_attn_dt_bwd cuda_test_attn_dt_bwd.cu 2>&1 | tee build.log ; \
    BRC=\${PIPESTATUS[0]} ; echo \"BUILD_RC=\$BRC\" ; \
    if [ \"\$BRC\" = '0' ]; then \
        echo '=== RUN default (T=256 nh=12 nkv=4 hd=64) ===' ; \
        timeout ${RUN_BUDGET_SEC} ./cuda_test_attn_dt_bwd 2>&1 | tee oracle_default.out ; \
        echo \"DEFAULT_RC=\${PIPESTATUS[0]}\" ; \
        echo '=== RUN stress (T=512 nh=12 nkv=4 hd=64) ===' ; \
        timeout ${RUN_BUDGET_SEC} ./cuda_test_attn_dt_bwd 512 12 4 64 2>&1 | tee oracle_stress.out ; \
...
for f in oracle_default.out oracle_stress.out; do
    if [ -f "$LOCAL_DIR/$f" ]; then
        ...
        echo "--- verdict ($f) ---"
        grep -E 'F-MK2C4-ATTNDT-BWD|ALL-PASS|FAIL' "$LOCAL_DIR/$f" || echo "  (no verdict line)"
    fi
done
echo ""
