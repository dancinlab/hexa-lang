/* tool/sweep_pred_512_host.c — F-SHARED-512-NUMERIC host harness
 *
 * Validates PR #1396 (RFC 085 Step B/C/D end-to-end [T;N] threading)
 * with N=512 (4096 B shared allocation; vs prior 2048 B hardcoded default).
 *
 * Build (ubu-2 RTX 5070):
 *   gcc tool/sweep_pred_512_host.c -o /tmp/sweep_pred_512_host \
 *       -I/usr/local/cuda/include -L/lib/x86_64-linux-gnu -lcuda
 *   /tmp/sweep_pred_512_host /tmp/sweep_pred_512.hexa.ptx
 *
 * Expected: PASS partial[0] == 131328.0 (= 512 × 513 / 2) with max|Δ|=0.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char** argv){
    const char* ptx_path = (argc>1)?argv[1]:"sweep_pred_512.hexa.ptx";
    CUresult cr;
    ...
    CUmodule mod;
    cr = load_ptx_logged(&mod, ptx, "sweep_pred_512");
    if(cr){ fprintf(stderr,"FAIL JIT: %s\n", cu_err(cr)); free(ptx); return 5; }

    CUfunction kfn;
    cr = cuModuleGetFunction(&kfn, mod, "sweep_pred_512");
    ...
    double expected = (double)N * (double)(N+1) / 2.0;  /* 131328.0 */
    double got = partial[0];
    double delta = got - expected; if(delta<0) delta=-delta;

    fprintf(stderr,"got=%.17g expected=%.17g max|delta|=%.17g\n", got, expected, delta);
    if(got == expected){
        printf("PASS partial[0]=%.3f (max|delta|=0, N=%d, shared alloc=4096 B vs prior 2048 default)\n", got, N);
