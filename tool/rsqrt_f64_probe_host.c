/* tool/rsqrt_f64_probe_host.c — F-RSQRTF64-NUMERIC host harness
 *
 * Validates PR #1335 (RFC 055 §13 f64 rsqrt = sqrt.rn.f64 + rcp.rn.f64).
 * Both sub-instructions are round-to-nearest-even, so composed result
 * is within ~1 ulp of CPU libm 1.0/sqrt(x).
 *
 * Build (ubu-2 RTX 5070):
 *   gcc tool/rsqrt_f64_probe_host.c -o /tmp/rsqrt_f64_host \
 *       -I/usr/local/cuda/include -L/lib/x86_64-linux-gnu -lcuda -lm
 *   /tmp/rsqrt_f64_host /tmp/rsqrt_f64_probe.hexa.ptx
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char** argv){
    const char* ptx_path = (argc>1)?argv[1]:"rsqrt_f64_probe.hexa.ptx";
    CUresult cr;
    ...
    CUmodule mod;
    cr = load_ptx_logged(&mod, ptx, "rsqrt_f64_probe");
    if(cr){ fprintf(stderr,"FAIL JIT: %s\n", cu_err(cr)); free(ptx); return 5; }

    CUfunction kfn;
    cr = cuModuleGetFunction(&kfn, mod, "rsqrt_f64_probe");
    if(cr){ fprintf(stderr,"FAIL cuModuleGetFunction: %s\n", cu_err(cr)); return 6; }

    double a[N], c[N];
    for(int i=0;i<N;i++){
        a[i] = ((double)(i + 1)) * 0.1;   /* [0.1, 25.6] strictly positive */
        c[i] = 0.0;
    }
    ...
    fprintf(stderr, "max_rel_err = %.17g (at i=%d, x=%.6f, got=%.17g, expected=%.17g)\n",
            max_rel_err, max_rel_idx, a[max_rel_idx], c[max_rel_idx], 1.0/sqrt(a[max_rel_idx]));

    int ok = (max_rel_err < 1e-14);
    if(ok){
        printf("PASS max_rel_err=%.3e < 1e-14 (N=%d, x in [0.1, 25.6])\n", max_rel_err, N);
    } else {
        printf("FAIL max_rel_err=%.3e >= 1e-14 (N=%d)\n", max_rel_err, N);
    }
