#!/bin/bash
# dispatch_r052_combined_fire.sh — RFC 052 Stage 2 Hopper combined-kernel fire.
#
# Fires self/cuda/experiments/r052_combined_bf16_dsm.cu on a vast.ai Hopper
# instance (sm_90+ — H100/H200/GH200 ONLY; the kernel uses the cluster API).
# Measures the RFC 052 combined BF16 WMMA + DSM kernel vs the RFC 049 cuBLAS
# GemmEx BF16 chain across INFER/SMALL/MEDIUM/LARGE shapes — 5 falsifiers.
#
# Output: state/forge_rfc052_2026_05_19/{result.json, build.log, fire.log,
#         nvidia_smi.csv}
# Budget: ~$5-20 (one fire, 4 shapes, short).
#
...
LOCAL_DIR="$REPO/state/forge_rfc052_2026_05_19"
SRC="$REPO/self/cuda/experiments/r052_combined_bf16_dsm.cu"
REMOTE_WORK="/workspace/rfc052_combined"
LABEL="forge-rfc052-combined-bf16-dsm"
...
echo "=== RFC 052 combined-kernel fire — $(date -u) ==="

# ── 1) search offer (Hopper sm_90+ ONLY — cluster API requirement) ─────
echo "[1/8] Searching Hopper (H100/H200/GH200) offers ..."
OFFER_JSON=$($VASTAI search offers \
    'gpu_name in [H100_SXM,H100_PCIE,H100_NVL,H100,H200,GH200] num_gpus=1 rentable=true dph_total<25.0 cuda_max_good>=12.4 disk_space>40' \
    -o dph_total --raw 2>&1)
...
[ -z "$OID" ] && { echo "ERROR: no Hopper offer"; exit 1; }
echo "  offer: $OFFER_ID"
...
# ── 4) sanity (must be sm_90+) ────────────────────────────────────────
echo "[4/8] Toolchain sanity ..."
$SC "nvidia-smi --query-gpu=name,memory.total,compute_cap --format=csv,noheader|head -1
  nvcc --version|tail -2
...
# ── 6) build + run ────────────────────────────────────────────────────
# -arch=sm_90 — cluster API + BF16 WMMA both require Hopper.
echo "[6/8] Build + fire ..."
$SC "cd $REMOTE_WORK && \
  nvcc -O3 -std=c++17 -arch=sm_90 r052_combined_bf16_dsm.cu \
    -lcublas -lcudart -lm -o r052_combined_bf16_dsm 2>&1 | tee build.log ; \
  ...
  if [ \"\$BRC\" = '0' ]; then ./r052_combined_bf16_dsm 2>&1 | tee fire.log ; \
