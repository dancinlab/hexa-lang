...
#   This oracle (tool/cuda_test_attn_dt_fwd.cu, self-contained: kernel
...
#   shape (n=786432) — sub-second compute, ~$0.20.
#
# Falsifiers (from the .cu): F-MK2C4-ATTNDT-FWD-EXACT |Δ|==0,
#                            F-MK2C4-ATTNDT-FWD-BWD-EXACT |Δ|==0
#
...
PHASE_ID="attn_dt_fwd_oracle_2026_05_19_c"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOCAL_DIR="${REPO_ROOT}/state/${PHASE_ID}"

ORACLE_CU="${REPO_ROOT}/tool/cuda_test_attn_dt_fwd.cu"

PHASE_LABEL="flame-rmsnorm-mh-oracle"
REMOTE_WORK="/workspace/attn_dt_fwd_oracle"
...
echo "[5/8] Upload cuda_test_attn_dt_fwd.cu ..."
$SCP_CMD "$ORACLE_CU" "root@$SSH_HOST:$REMOTE_WORK/cuda_test_attn_dt_fwd.cu"

# ── 6) nvcc build + run (default n=786432 + a d768 shape) ─
echo "[6/8] Build + run oracle on sm_${DEV_CC} ..."
$SSH_CMD "cd $REMOTE_WORK && \
    echo '=== nvcc build ===' ; \
    nvcc -arch=sm_${DEV_CC} -O2 -Xcompiler -ffp-contract=off -o cuda_test_attn_dt_fwd cuda_test_attn_dt_fwd.cu 2>&1 | tee build.log ; \
    BRC=\${PIPESTATUS[0]} ; echo \"BUILD_RC=\$BRC\" ; \
    if [ \"\$BRC\" = '0' ]; then \
        echo '=== RUN default (T=256 nh=12 nkv=4 hd=64) ===' ; \
        timeout ${RUN_BUDGET_SEC} ./cuda_test_attn_dt_fwd 2>&1 | tee oracle_default.out ; \
        echo \"DEFAULT_RC=\${PIPESTATUS[0]}\" ; \
        echo '=== RUN stress (T=512 nh=12 nkv=4 hd=64) ===' ; \
        timeout ${RUN_BUDGET_SEC} ./cuda_test_attn_dt_fwd 512 12 4 64 2>&1 | tee oracle_stress.out ; \
        echo \"STRESS_RC=\${PIPESTATUS[0]}\" ; \
...
pull_with_retry "$REMOTE_WORK/oracle_stress.out"  "$LOCAL_DIR/oracle_stress.out"  || true
...
for f in oracle_default.out oracle_stress.out; do
    if [ -f "$LOCAL_DIR/$f" ]; then
        echo "--- $f ---"; cat "$LOCAL_DIR/$f"
        echo "--- verdict ($f) ---"
        grep -E 'F-MK2C4-ATTNDT-FWD|ALL-PASS|FAIL' "$LOCAL_DIR/$f" || echo "  (no verdict line)"
    fi
done
echo ""
