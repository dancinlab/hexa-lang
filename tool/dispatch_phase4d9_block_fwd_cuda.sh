...
#   The oracle is a SINGLE translation unit: tool/flame_phase4d7_gpu_
#   path_oracle.c provides the farr-table shim + CUDA bridge shims + the
#   CPU reference + main(); the .sh splices in the REAL tool/flame_
#   phase4d6_matmul_primitives.c (no fork). It needs NO runtime.c, NO
#   17 native/*.c, NO corpus blob — just 4 files:
#     tool/flame_phase4d9_block_fwd_oracle.c
...
#
# VERDICT
#   PASS  F-PHASE4D9-BLOCK-FWD-GPU-PATH-ORACLE  max|Δ| ≤ 3e-11  → GPU-path byte-safe
#   FAIL  ...                       max|Δ| > 3e-11  → GPU-path REGRESSION
#   any NaN/Inf                                     → -nan signature FAIL
#
# Falsifier:
#   F-PHASE4D9-BLOCK-FWD-CUDA  the d768 GPU-path projection
...
PHASE_ID="flame_phase4d9_block_fwd_cuda_2026_05_18"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOCAL_DIR="${REPO_ROOT}/state/${PHASE_ID}"

ORACLE_C="${REPO_ROOT}/tool/flame_phase4d9_block_fwd_oracle.c"
ORACLE_SH="${REPO_ROOT}/tool/flame_phase4d9_block_fwd_oracle.sh"
PRIM_C="${REPO_ROOT}/tool/flame_phase4d6_matmul_primitives.c"
PRIM_FWD="${REPO_ROOT}/tool/flame_phase4d7_block_fwd_primitive.c"
RUNTIME_CUDA_C="${REPO_ROOT}/self/cuda/runtime_cuda.c"

PHASE_LABEL="flame-phase4d9-block-fwd-cuda"
REMOTE_WORK="/workspace/flame_blockfwd"
...
[ -f "$PRIM_C" ]          || { echo "ERROR: $PRIM_C missing"; exit 1; }
[ -f "$PRIM_FWD" ]      || { echo "ERROR: $PRIM_FWD missing"; exit 1; }
...
echo "[5/9] Upload oracle.{c,sh} + matmul_primitives.c + runtime_cuda.c ..."
$SCP_CMD "$ORACLE_C"       "root@$SSH_HOST:$REMOTE_WORK/tool/flame_phase4d9_block_fwd_oracle.c"
$SCP_CMD "$ORACLE_SH"      "root@$SSH_HOST:$REMOTE_WORK/tool/flame_phase4d9_block_fwd_oracle.sh"
$SCP_CMD "$PRIM_C"         "root@$SSH_HOST:$REMOTE_WORK/tool/flame_phase4d6_matmul_primitives.c"
$SCP_CMD "$PRIM_FWD"      "root@$SSH_HOST:$REMOTE_WORK/tool/flame_phase4d7_block_fwd_primitive.c"
...
$SSH_CMD "cd $REMOTE_WORK && \
    chmod +x tool/flame_phase4d9_block_fwd_oracle.sh ; \
    echo '=== nvidia-smi PRE ===' ; \
    ...
    nohup bash -c 'S=\$(date +%s); timeout ${RUN_BUDGET_SEC} bash tool/flame_phase4d9_block_fwd_oracle.sh --cuda > oracle.out 2>&1; R=\$?; E=\$(date +%s); echo oracle_rc=\$R > oracle_meta.txt; echo wall_seconds=\$((E-S)) >> oracle_meta.txt; touch oracle.done' >/dev/null 2>&1 & disown ; \
    sleep 3 ; \
