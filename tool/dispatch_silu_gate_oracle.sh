...
#   This oracle (tool/cuda_test_silu_gate.cu, self-contained: kernel
...
#
# Falsifiers (from the .cu): F-MK2C1B-SILUGATE-EXACT |Δ|==0,
#                            F-MK2C1B-SILUGATE-BWD-EXACT |Δ|==0
#
...
PHASE_ID="silu_gate_oracle_2026_05_19"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOCAL_DIR="${REPO_ROOT}/state/${PHASE_ID}"

ORACLE_CU="${REPO_ROOT}/tool/cuda_test_silu_gate.cu"

PHASE_LABEL="flame-silu-gate-oracle"
REMOTE_WORK="/workspace/silu_gate_oracle"
...
echo "[5/8] Upload cuda_test_silu_gate.cu ..."
$SCP_CMD "$ORACLE_CU" "root@$SSH_HOST:$REMOTE_WORK/cuda_test_silu_gate.cu"

# ── 6) nvcc build + run (default n=786432 + a d768 shape) ─
echo "[6/8] Build + run oracle on sm_${DEV_CC} ..."
$SSH_CMD "cd $REMOTE_WORK && \
    echo '=== nvcc build ===' ; \
    nvcc -arch=sm_${DEV_CC} -O2 -Xcompiler -ffp-contract=off -o cuda_test_silu_gate cuda_test_silu_gate.cu 2>&1 | tee build.log ; \
    BRC=\${PIPESTATUS[0]} ; echo \"BUILD_RC=\$BRC\" ; \
    if [ \"\$BRC\" = '0' ]; then \
        echo '=== RUN default (n=786432) ===' ; \
        timeout ${RUN_BUDGET_SEC} ./cuda_test_silu_gate 2>&1 | tee oracle_default.out ; \
        echo \"DEFAULT_RC=\${PIPESTATUS[0]}\" ; \
        echo '=== RUN d768-class (n=2097152) ===' ; \
        timeout ${RUN_BUDGET_SEC} ./cuda_test_silu_gate 2097152 2>&1 | tee oracle_d768.out ; \
...
for f in oracle_default.out oracle_d768.out; do
    if [ -f "$LOCAL_DIR/$f" ]; then
        ...
        echo "--- verdict ($f) ---"
        grep -E 'F-MK2C1B-SILUGATE|ALL-PASS|FAIL' "$LOCAL_DIR/$f" || echo "  (no verdict line)"
    fi
done
echo ""
