#!/bin/bash
# dispatch_r050_perf_inherit.sh — RFC 050 PERF-INHERITANCE fire script.
#
# Fires self/cuda/experiments/r050_perf_inherit_validate.cu on a vast.ai
# sm_80+ instance (A100 = RFC 049 Stage 2 anchor device; H100 acceptable).
# Measures wall (BF16 dispatch wrapper) vs wall (FP64 cuBLAS Dgemm
# reference) at d768·12L FFN shapes (64·768·3072 + 128·768·3072) and
# validates the four falsifiers:
#   F-RFC050-PERF-INHERIT-FFN-D768-64    speedup >= 5x
#   F-RFC050-PERF-INHERIT-FFN-D768-128   speedup >= 5x  (RFC 049 anchor)
#   F-RFC050-PERF-INHERIT-CORRECT        max|Δ|/max|Y| <= 5e-2
#   F-RFC050-PERF-INHERIT-DISPATCH-OK    dispatcher rc == FORGE_OK
#
# The harness #includes ../runtime_bf16.c AND ../../forge/forge_tier_v1.c
# (which itself #includes forge_tier_v1.h). All four files are uploaded
# preserving the relative layout:
#   $REMOTE_WORK/cuda/runtime_bf16.c
#   $REMOTE_WORK/forge/forge_tier_v1.c
#   $REMOTE_WORK/forge/forge_tier_v1.h
#   $REMOTE_WORK/cuda/experiments/r050_perf_inherit_validate.cu
#
# Output:
#   state/forge_rfc050_perf_inherit_2026_05_19/{result.json,build.log,fire.log}
# Budget: ~$2-4 (one short fire; perf+correct+dispatch, no full sweep).
#
...
LOCAL_DIR="$REPO/state/forge_rfc050_perf_inherit_2026_05_19"
SRC_CU="$REPO/self/cuda/experiments/r050_perf_inherit_validate.cu"
SRC_RT="$REPO/self/cuda/runtime_bf16.c"
SRC_FORGE_C="$REPO/self/forge/forge_tier_v1.c"
SRC_FORGE_H="$REPO/self/forge/forge_tier_v1.h"
REMOTE_WORK="/workspace/rfc050_perf_inherit"
LABEL="forge-rfc050-perf-inherit"
...
[ -f "$SRC_FORGE_C" ] || { echo "ERROR: $SRC_FORGE_C missing"; exit 1; }
[ -f "$SRC_FORGE_H" ] || { echo "ERROR: $SRC_FORGE_H missing"; exit 1; }
mkdir -p "$LOCAL_DIR"; cd "$LOCAL_DIR"
echo "=== RFC 050 PERF-INHERITANCE fire — $(date -u) ==="

# ── 1) search offer (sm_80+ — A100 preferred, RFC 049 anchor device) ──
...
$SCP "$SRC_CU"      "root@$SSH_HOST:$REMOTE_WORK/cuda/experiments/r050_perf_inherit_validate.cu"

# ── 6) build + run ────────────────────────────────────────────────────
echo "[6/8] Build + fire ..."
$SC "cd $REMOTE_WORK/cuda/experiments && \
  nvcc -O3 -std=c++17 -arch=native r050_perf_inherit_validate.cu \
    -lcublas -lcudart -lm -o r050_perf_inherit_validate 2>&1 | tee build.log ; \
  BRC=\${PIPESTATUS[0]} ; echo \"BUILD_RC=\$BRC\" ; \
  if [ \"\$BRC\" = '0' ]; then ./r050_perf_inherit_validate 2>&1 | tee fire.log ; \
...
echo "=== RFC 050 PERF-INHERITANCE fire DONE — $(date -u) ==="
