#!/bin/bash
# dispatch_r050_dispatch_validate.sh — RFC 050 Stage 2 (forge-side)
# dispatch-routing fire-validation.
#
# Fires self/cuda/experiments/r050_dispatch_validate.cu on a vast.ai
# sm_80+ instance (A100 fine — BF16 Tensor Core needs only sm_80+).
# Validates that forge_tier_dispatch_v1 (forge_tier_v1.c) routes
# MATMUL/FFN + PURE_BF16 work through the RFC 049 BF16 substrate
# (runtime_bf16.c) via the RFC 050 ForgeArgs pointer-cast ABI, and that
# unsupported (family, precision, regime, det) combos fall back with a
# code, never crash.
#
...
# with the relative layout preserved:
#   $REMOTE_WORK/runtime_bf16.c
#   $REMOTE_WORK/forge/forge_tier_v1.c
#   $REMOTE_WORK/forge/forge_tier_v1.h
#   $REMOTE_WORK/experiments/r050_dispatch_validate.cu
#
# Output: state/forge_rfc050_stage2_2026_05_19/{result.json,build.log,fire.log}
# Budget: ~$2-4 (one short fire — routing + correctness, no perf sweep).
#
...
LOCAL_DIR="$REPO/state/forge_rfc050_stage2_2026_05_19"
SRC_CU="$REPO/self/cuda/experiments/r050_dispatch_validate.cu"
...
REMOTE_WORK="/workspace/rfc050_stage2"
LABEL="forge-rfc050-stage2-validate"
...
echo "=== RFC 050 Stage 2 dispatch-validate fire — $(date -u) ==="
...
$SC "cd $REMOTE_WORK/cuda/experiments && \
  ...
  if [ \"\$BRC\" = '0' ]; then ./r050_dispatch_validate 2>&1 | tee fire.log ; \
...
echo "=== RFC 050 Stage 2 dispatch-validate fire DONE — $(date -u) ==="
