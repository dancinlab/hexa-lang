#!/usr/bin/env bash
# tool/regen_hxlcl_strdup_native_s.sh — reproducibly regenerate the frozen native
# hxlcl_strdup seeds from the SSOT stdlib/runtime/hxlcl_core.hexa, using the native
# compiler. The WALL-2 free twin of tool/regen_regex_rt_native_s.sh
# (zero-c #29 WALL-2 free-flip prereq-(a) — Fable bmtd0lpxf).
#
#   tool/regen_hxlcl_strdup_native_s.sh [darwin|x86_64|arm64-linux|all]   (default: all)
#
# WHY this seed exists (prereq-(a), root-caused #4489/#4545):
#   The old resolver emitted hxlcl_strdup's Route-C .o LIVE at Stage 0b via
...
#   2) .globl DEMOTION post-pass: keep ONLY hxlcl_strdup (`_hxlcl_strdup` on Mach-O) global,
...
#   4) sanity: cross-assemble + assert EXACTLY 1 defined-global hxlcl_strdup + no libc U.
#
...
# to a 1-symbol (hxlcl_strdup) contract is what makes this seed ar-able next to the
# libc-shim member for the OTHER hxlcl_* (which stay shim-served until their own flip).
#
# Targets:
#   darwin      → self/native/hxlcl_strdup_arm64.s        (arm64-apple-darwin, Mach-O)
#   x86_64      → self/native/hxlcl_strdup_x86_64.s        (x86_64-linux-gnu,   ELF)
#   arm64-linux → self/native/hxlcl_strdup_arm64-linux.s   (arm64-linux-gnu,    ELF)
#
...
[ -x "$APRIME" ] || { echo "[regen_hxlcl_strdup] ERROR: native compiler not at $APRIME (set APRIME=)" >&2; exit 1; }

SRC_MAIN="$HX/stdlib/runtime/hxlcl_core.hexa"
[ -f "$SRC_MAIN" ] || { echo "[regen_hxlcl_strdup] ERROR: SSOT missing: $SRC_MAIN" >&2; exit 1; }

# The single external-contract symbol the resolver ar's into runtime.a on flip.
CONTRACT='hxlcl_strdup'

TMP="$(mktemp -d -t regen_hxlcl_strdup.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
emit_one() {
    ...
    if ! env HEXA_CABI_HXLCL=1 "$APRIME" "$TMP/_drv.hexa" --emit=asm --target="$triple" \
        ...
        echo "[regen_hxlcl_strdup] ERROR: $triple aprime --emit=asm failed:" >&2
        sed 's/^/    /' "$TMP/emit.err" >&2
        exit 1
    fi

    # --- .globl DEMOTION post-pass: keep ONLY hxlcl_strdup .globl, drop the rest ---
    ...
    [ "$kept" -eq 1 ] || { echo "[regen_hxlcl_strdup] ERROR: $triple kept $kept/1 hxlcl_strdup global (raw emit missing the shim?)" >&2; exit 1; }
...

    {
        ...
        printf '//   keeping ONLY the hxlcl_strdup shim global.\n'
        printf '//   Provides hxlcl_strdup (native Route-C body, no libc free) that\n'
    ...
    if $CC $cc_extra -c "$s" -o "$o" 2>/dev/null; then
        local t; t="$( (nm "$o" 2>/dev/null || echo) | grep -cE ' T _?hxlcl_strdup')"
        [ "$t" -eq 1 ] || { echo "[regen_hxlcl_strdup] ERROR: $triple nm shows $t/1 T hxlcl_strdup" >&2; exit 1; }
        ...
        echo "[regen_hxlcl_strdup] $triple → $out (1 globl · $t T hxlcl_strdup · ${libc_u} non-carrier U)"
        [ "${libc_u:-0}" -eq 0 ] || echo "[regen_hxlcl_strdup] WARN $triple: seed has ${libc_u} non-carrier undefined symbols — inspect (floor-reduction assert)" >&2
    else
        echo "[regen_hxlcl_strdup] WARN $triple: cross-assemble check skipped (no matching toolchain)" >&2
        echo "[regen_hxlcl_strdup] $triple → $out (1 globl)"
    fi
}

case "$WHICH" in
    darwin)      emit_one arm64-apple-darwin "$HX/self/native/hxlcl_strdup_arm64.s" "Mach-O, _hxlcl_strdup underscore-prefixed" ;;
    x86_64)      emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_strdup_x86_64.s" "ELF, hxlcl_strdup no underscore" ;;
    arm64-linux) emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_strdup_arm64-linux.s" "ELF aarch64, hxlcl_strdup no underscore" ;;
    all)
        emit_one arm64-apple-darwin "$HX/self/native/hxlcl_strdup_arm64.s" "Mach-O, _hxlcl_strdup underscore-prefixed"
        emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_strdup_x86_64.s" "ELF, hxlcl_strdup no underscore"
        emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_strdup_arm64-linux.s" "ELF aarch64, hxlcl_strdup no underscore"
        ;;
    *) echo "[regen_hxlcl_strdup] usage: $0 [darwin|x86_64|arm64-linux|all]" >&2; exit 1 ;;
esac
echo "[regen_hxlcl_strdup] done."
