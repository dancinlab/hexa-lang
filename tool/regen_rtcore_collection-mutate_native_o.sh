...
# from its emitter SSOT self/native/rtcore_collection-mutate_emit.hexa BEFORE compiling, so a
...
bash "$ROOT/tool/regen_rtcore_collection-mutate_c.sh" "$ROOT" >&2 || true
