#!/usr/bin/env bash
# tool/regen_hxlcl_strtoll_native_s.sh — reproducibly regenerate the frozen native
# hxlcl_strtoll seeds from the SSOT stdlib/runtime/hxlcl_core.hexa, using the native
# compiler. The strtoll analog of tool/regen_hxlcl_strstr_native_s.sh (same
# .globl-demotion 1-symbol-contract shape, integer-C-ABI so no fp-ABI xmm dep).
# RFC061 R1c HEXA_RT_NATIVE_STRTOLL-flip prereq-(a): converts the STRTOLL block
# in stage_resolve_runtime_a from a Stage-0b LIVE emit (impossible in faithful
# CI, root-caused #4489/#4545 for strcmp) to a frozen-seed-consume.
# Contract: hxlcl_strtoll WRAPS on overflow (no clamp/errno) — see the R1c
# resolution state/hexa-own/rfc061-r1c-strtoll-contract-resolution.md.
#
#   tool/regen_hxlcl_strtoll_native_s.sh [darwin|x86_64|arm64-linux|all]   (default: all)
#
# WHY this seed exists (prereq-(a), root-caused #4489/#4545):
#   The old resolver emitted hxlcl_strtoll's Route-C .o LIVE at Stage 0b via
...
#   2) .globl DEMOTION post-pass: keep ONLY hxlcl_strtoll (`_hxlcl_strtoll` on Mach-O) global,
...
#   4) sanity: cross-assemble + assert EXACTLY 1 defined-global hxlcl_strtoll + no libc U.
#
# hxlcl_core.hexa defines the whole hxlcl_* libc-floor family (strtoll/strchr/strtoll/
# calloc/realloc/free/…); emitting the module gives them all as globals, so the demotion
# to a 1-symbol (hxlcl_strtoll) contract is what makes this seed ar-able next to the
# libc-shim member for the OTHER hxlcl_* (which stay shim-served until their own flip).
#
# Targets:
#   darwin      → self/native/hxlcl_strtoll_arm64.s        (arm64-apple-darwin, Mach-O)
#   x86_64      → self/native/hxlcl_strtoll_x86_64.s        (x86_64-linux-gnu,   ELF)
#   arm64-linux → self/native/hxlcl_strtoll_arm64-linux.s   (arm64-linux-gnu,    ELF)
#
...
[ -x "$APRIME" ] || { echo "[regen_hxlcl_strtoll] ERROR: native compiler not at $APRIME (set APRIME=)" >&2; exit 1; }

SRC_MAIN="$HX/stdlib/runtime/hxlcl_core.hexa"
[ -f "$SRC_MAIN" ] || { echo "[regen_hxlcl_strtoll] ERROR: SSOT missing: $SRC_MAIN" >&2; exit 1; }

# The single external-contract symbol the resolver ar's into runtime.a on flip.
CONTRACT='hxlcl_strtoll'

TMP="$(mktemp -d -t regen_hxlcl_strtoll.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
emit_one() {
    ...
    if ! env HEXA_CABI_HXLCL=1 "$APRIME" "$TMP/_drv.hexa" --emit=asm --target="$triple" \
        ...
        echo "[regen_hxlcl_strtoll] ERROR: $triple aprime --emit=asm failed:" >&2
        sed 's/^/    /' "$TMP/emit.err" >&2
        exit 1
    fi

    # --- .globl DEMOTION post-pass: keep ONLY hxlcl_strtoll .globl, drop the rest ---
    ...
    [ "$kept" -eq 1 ] || { echo "[regen_hxlcl_strtoll] ERROR: $triple kept $kept/1 hxlcl_strtoll global (raw emit missing the shim?)" >&2; exit 1; }
    [ "$total" -eq 1 ] || { echo "[regen_hxlcl_strtoll] ERROR: $triple demotion left $total globals (expected 1 — sed pattern drift)" >&2; exit 1; }
    [ "$pext" -eq 0 ] || { echo "[regen_hxlcl_strtoll] ERROR: $triple demotion left $pext .private_extern (Mach-O N_PEXT stays external → ld -r multidef)" >&2; exit 1; }
    ...
    python3 "$(dirname "$0")/isolate_native_seed.py" "$demoted" "$CONTRACT" "$demoted.iso" \
        || { echo "[regen_hxlcl_strtoll] ERROR: $triple isolation slice failed" >&2; exit 1; }
    mv "$demoted.iso" "$demoted"
    [ "$(grep -cE '^[[:space:]]*\.globl[[:space:]]' "$demoted")" -eq 1 ] \
        || { echo "[regen_hxlcl_strtoll] ERROR: $triple isolation left != 1 global" >&2; exit 1; }
    ! grep -qE 'errno_location' "$demoted" \
        || { echo "[regen_hxlcl_strtoll] ERROR: $triple isolation left an errno_location ref" >&2; exit 1; }

    {
        printf '// %s — FROZEN BOOTSTRAP SEED (RT-NATIVE zero-c #29 — WALL-2 hxlcl_strtoll).\n' "$(basename "$out")"
        printf '// GENERATED: tool/regen_hxlcl_strtoll_native_s.sh — aprime_cc _drv.hexa --emit=asm\n'
        printf '//   --target=%s -o %s hxlcl_core.hexa, then a .globl DEMOTION post-pass\n' "$triple" "$(basename "$out")"
        printf '//   keeping ONLY the hxlcl_strtoll shim global.\n'
        printf '//   Provides hxlcl_strtoll (native Route-C body, no libc strtoll) that\n'
        printf '//   stage_resolve_runtime_a ar'"'"'s into runtime.a under HEXA_RT_NATIVE_STRSTR,\n'
        printf '//   replacing the libc-shim strtoll member so `strtoll` drops from the nm-UND floor.\n'
    ...
    if $CC $cc_extra -c "$s" -o "$o" 2>/dev/null; then
        local t; t="$( (nm "$o" 2>/dev/null || echo) | grep -cE ' T _?hxlcl_strtoll')"
        ...
        echo "[regen_hxlcl_strtoll] WARN $triple: cross-assemble check skipped (no matching toolchain)" >&2
        echo "[regen_hxlcl_strtoll] $triple → $out (1 globl)"
    fi
}

case "$WHICH" in
    darwin)      emit_one arm64-apple-darwin "$HX/self/native/hxlcl_strtoll_arm64.s" "Mach-O, _hxlcl_strtoll underscore-prefixed" ;;
    x86_64)      emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_strtoll_x86_64.s" "ELF, hxlcl_strtoll no underscore" ;;
    arm64-linux) emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_strtoll_arm64-linux.s" "ELF aarch64, hxlcl_strtoll no underscore" ;;
    all)
        emit_one arm64-apple-darwin "$HX/self/native/hxlcl_strtoll_arm64.s" "Mach-O, _hxlcl_strtoll underscore-prefixed"
        emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_strtoll_x86_64.s" "ELF, hxlcl_strtoll no underscore"
        emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_strtoll_arm64-linux.s" "ELF aarch64, hxlcl_strtoll no underscore"
        ;;
    *) echo "[regen_hxlcl_strtoll] usage: $0 [darwin|x86_64|arm64-linux|all]" >&2; exit 1 ;;
esac
echo "[regen_hxlcl_strtoll] done."
