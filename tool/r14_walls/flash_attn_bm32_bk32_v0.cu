/* R14: FlashAttention-2 BM=32 BK=32 wedge — closed-form re-audit empirical test.
 *
 * Pre-registered re-audit finding (2026-05-28, GPU.attention.md): R10 oracle
 * ALREADY uses register-resident O + cp.async.cg double-buffer; the only
 * differences for R14 wedge are (BM=64 -> BM=32, KTILE=16 -> 32) tile-size
 * changes. The wedge tests whether smaller tiles + larger BK improve over
 * R10's measured 1.149x @ N=4096 d=64 vs cuBLAS-TC.
 *
 * Closed-form expectation: BM=32 reduces grid count (N/32 vs N/64) and
 * smem/CTA, allowing higher occupancy but reducing per-CTA parallelism.
 * R10 is reg-bound at 5 CTAs/SM (91 reg/thd * 128 thd/CTA); BM=32 with
 * 2 warps (64 thd/CTA) has reg-CTAs ~11 but smem-CTAs ~4 -> bound = 4
 * CTAs/SM. Whether the smaller grid + larger BK (32 keys per softmax round
 * vs 16) wins on wall is the empirical question.
 *
 * Geometry (this kernel): BM=32, BK=32, d=64, 2 warps/CTA, grid=N/32,
 * block=64. K/V tiles are 32 rows x 64 cols half each, double-buffered.
 * V WMMA-API row_major matrix_b (no .trans / no V-pretranspose at this
 * style; risk-d finding applied only to low-level mma.sync.m16n8k16 .trans
 * + ldmatrix.x4 path, NOT to nvcuda::wmma API). Each warp owns 16 query
 * rows (warp w -> rows [w*16..w*16+15]).
 *
 * Build: nvcc -O2 -arch=sm_120 -o fa_bm32_bk32 flash_attn_bm32_bk32_v0.cu
 */
...
#define NWARP 2
#define BM    32          /* 2 warps * 16 query rows */
#define KTILE 32          /* K/V tile = 32 rows (BK=32) */
#define D     64

/* Rescale O accumulator rows by per-row factor; m16n16k16 fp32 acc row map:
 * elems {0,1,4,5}->row=lane/4, {2,3,6,7}->row=lane/4+8 (R8/R10 finding). */
...
/* Cooperatively load one 32x64 half tile (K or V) gmem -> smem via cp.async.
 * 32*64 = 2048 halves = 4096 bytes = 256 x 16-byte vectors. 64 threads ->
 * 4 vectors each. Caller commits + waits. */
__device__ __forceinline__ void load_tile_cpasync(__half* dst, const __half* src, int tid) {
    #pragma unroll
    for (int v = 0; v < 4; ++v) {
        int off = (v * 64 + tid) * 8;   /* contiguous 8-half (16B) chunks */
        cp_async_16(dst + off, src + off);
    }
}

extern "C" __global__ void fa_bm32_bk32(const __half* q, const __half* k,
                                         const __half* v, __half* o,
                                         int N, float scale) {
    /* double-buffered K/V tiles in smem: 32 rows x 64 cols * 2 bytes * 2 buffers */
    __shared__ __half k_sm[2][KTILE*D];   // 2 * 4096 B = 8192
    __shared__ __half v_sm[2][KTILE*D];   // 2 * 4096 B = 8192
    /* per-warp reduction scratch + finalize store scratch */
    __shared__ float  red_max[NWARP][16*4];   // 2 * 64 floats = 512 B
    __shared__ float  red_sum[NWARP][16*4];   // 2 * 64 floats = 512 B
    __shared__ float  o_dbg [NWARP][16*16];   // 2 * 256 floats = 2048 B
    /* total smem ~19.5 KB per CTA */

    int tid  = threadIdx.x;          // 0..63
    int wid  = tid >> 5;             // warp id 0..1
    int lane = tid & 31;

    int qrow_base = blockIdx.x * BM + wid * 16;   // this warp's 16 query rows
    const __half* qb = q + (size_t)qrow_base * D;

    // Q resident in matrix_a fragments across the whole K loop (4 d-tiles of 16)
    ...
    int n_tiles = N >> 5;   /* N / KTILE; KTILE=32 */
    ...
    for (int kt=0; kt<n_tiles; ++kt) {
        ...
        /* prefetch NEXT tile (async) while we wait on + compute the current */
        if (kt+1 < n_tiles) {
        ...
        /* BK=32 = 2 chunks of 16. Loop over both chunks of K for the
         * 16x32 S tile (per warp), iterate softmax+P*V per chunk online. */
        #pragma unroll
        for (int kc = 0; kc < 2; ++kc) {
            ...
            // ---- row max via per-warp SMEM scratch ----
...
int main(int argc,char**argv){
    ...
    /* occupancy query */
    int nblk = -1;
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&nblk, fa_bm32_bk32, block, 0);
    printf("occupancy_max_active_blocks_per_sm=%d ", nblk);

    fa_bm32_bk32<<<grid,block>>>(dq,dk,dv,dO,N,scale);
    ...
    printf("bm32_bk32 N=%d grid=%d block=%d max_abs=%.4g rel_rowscale=%.4g rms_rel=%.4g naninf=%ld numeric=%s",
           N,grid,block,maxa,relrs,rms,nanc,pass?"PASS":"FAIL");
    if(do_time){cudaEvent_t s,n;cudaEventCreate(&s);cudaEventCreate(&n);
        for(int w=0;w<20;++w)fa_bm32_bk32<<<grid,block>>>(dq,dk,dv,dO,N,scale);cudaDeviceSynchronize();
        int reps=200;double *ms=(double*)malloc(reps*8);
        for(int r=0;r<reps;++r){cudaEventRecord(s,0);fa_bm32_bk32<<<grid,block>>>(dq,dk,dv,dO,N,scale);cudaEventRecord(n,0);cudaEventSynchronize(n);float t;cudaEventElapsedTime(&t,s,n);ms[r]=t;}
