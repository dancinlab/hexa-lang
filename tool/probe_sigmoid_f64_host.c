/* tool/probe_sigmoid_f64_host.c — F-SIGMOIDF64-NUMERIC host harness
 *
 * Validates the f64 sigmoid codegen (#1496, 0.5+0.5*tanh(x/2)) on
 * ubu-2 RTX 5070 driver-JIT. Range [-8,+8) at N=256. Tolerance
 * max_abs_err < 1e-7 (sigmoid range bounded to (0,1)). Mirror of
 * probe_tanh_f64_host.c (#1595).
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char** argv){
    const char* ptx_path = (argc>1)?argv[1]:"probe_sigmoid_f64.hexa.ptx";
    CUresult cr;
    ...
    CUmodule mod;
    cr = load_ptx_logged(&mod, ptx, "sigmoid_f64_probe");
    if(cr){ fprintf(stderr,"FAIL JIT: %s\n", cu_err(cr)); free(ptx); return 5; }

    CUfunction kfn;
    cr = cuModuleGetFunction(&kfn, mod, "sigmoid_f64_probe");
    if(cr){ fprintf(stderr,"FAIL cuModuleGetFunction: %s\n", cu_err(cr)); return 6; }

    double a[N], c[N];
    for(int i=0;i<N;i++){
        a[i] = ((double)i / (double)N) * 16.0 - 8.0;
        c[i] = 0.0;
    }
    ...
    int max_abs_idx = -1;
    for(int i=0;i<N;i++){
        double expected = 1.0 / (1.0 + exp(-a[i]));
        double got = c[i];
    ...
    fprintf(stderr, "max_abs_err = %.17g (at i=%d, x=%.6f, got=%.17g, expected=%.17g)\n",
            max_abs_err, max_abs_idx, a[max_abs_idx], c[max_abs_idx],
            1.0/(1.0+exp(-a[max_abs_idx])));
