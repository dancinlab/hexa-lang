#!/usr/bin/env bash
# tool/routec_cos_native_verify.sh — literal-∅ libm-leaf RUNG 4 (hxlcl_cos via the
# fp-ABI xmm Route C codegen path).
#
# Verifies the HEXA_RT_NATIVE_COS wiring — hxlcl_cos LEAVES the libc shim and is
# supplied by the hexa-NATIVE Route C fp-ABI (xmm) .o emitted from
# stdlib/runtime/hxlcl_core.hexa (HEXA_CABI_HXLCL=1):
#   (A) DEFAULT (HEXA_RT_NATIVE_COS OFF) shim.o is BYTE-IDENTICAL to the origin/main
#       baseline shim.o (the new `#ifndef HEXA_RT_NATIVE_COS` guard compiles out →
#       the libc-delegate body is untouched; release-integrity invariant).
#   (B) ON: the Route C codegen emits `hxlcl_cos` (with the __sin/__cos kernels inlined
#       into its body) as a DEFINED (T) external in the native .o, AND the
#       ON shim.o (-DHEXA_RT_NATIVE_COS) NO LONGER defines hxlcl_cos — the symbol
#       MOVES from the C shim to the native object (literal-∅ progress: one member
#       dropped). NO undefined libm `cos`/`sin` ref in the native object (libm-free).
#   (C) VALUE-EXACT: link the native .o against a C harness that CALLS hxlcl_cos as a
#       raw C-ABI `double f(double)` and compares to libc cos over an N-point sweep
...
#       gate asserts max|ULP| ≤ 1 (NOT 0-ULP — glibc cos is a different correctly-
#       rounded algorithm). reference-match: musl src/math/{cos,__sin,__cos,
...
set -uo pipefail
ROOT="${ROOT:-$PWD}"; OUT="${OUT:-/tmp/routec_cos}"; CC="${CC:-clang}"
...
if [ "$(uname -s)" != "Linux" ] || { [ "$(uname -m)" != "x86_64" ] && [ "$(uname -m)" != "amd64" ]; }; then
    echo "[routec-cos] SKIP — fp-ABI xmm Route C is x86_64-linux-only (host=$(uname -sm))"
    exit 0
fi
echo "════ routec_cos_native_verify ($(uname -srm) · $($CC --version|head -1)) ════"

CFLAGS="-c -O2 -std=gnu11 -D_GNU_SOURCE"

# ── [A] DEFAULT shim.o byte-identity vs origin/main ─────────────────────────
echo "[A] DEFAULT (HEXA_RT_NATIVE_COS OFF) shim.o byte-identity vs origin/main…"
...
echo "[D] shim hxlcl_* DEFINITION count: default vs ON (-DHEXA_RT_NATIVE_COS)…"
$CC $CFLAGS "$SHIM" -o "$OUT/shim_def.o" 2>/dev/null
$CC $CFLAGS -DHEXA_RT_NATIVE_COS "$SHIM" -o "$OUT/shim_on.o" 2>"$OUT/shim_on.err"
...
COS_DEF_DEFAULT=$(nm "$OUT/shim_def.o" 2>/dev/null | grep -E ' T _?hxlcl_cos$' | wc -l | tr -d ' ')
COS_DEF_ON=$(nm     "$OUT/shim_on.o"   2>/dev/null | grep -E ' T _?hxlcl_cos$' | wc -l | tr -d ' ')
echo "    hxlcl_cos defined in shim:  default=$COS_DEF_DEFAULT  ON=$COS_DEF_ON  (expect 1 → 0)"
[ "$COS_DEF_DEFAULT" = "1" ] && [ "$COS_DEF_ON" = "0" ] && echo "SHIM_COS_MEMBER_DROPPED=YES" || echo "SHIM_COS_MEMBER_DROPPED=NO"
...
echo "[B] Route C native emit of hxlcl_cos (HEXA_CABI_HXLCL=1) via $BIN…"
...
COS_T_NATIVE=$(nm "$OUT/routec.o" 2>/dev/null | grep -E ' T _?hxlcl_cos$' | wc -l | tr -d ' ')
echo "    hxlcl_cos defined (T) in native routec.o = $COS_T_NATIVE  (expect 1; __sin/__cos kernels inlined)"
# undefined libm sin/cos ref in the native object? (must be ZERO — libm-free)
COS_U_NATIVE=$(nm "$OUT/routec.o" 2>/dev/null | grep -E ' U _?(sin|cos)$' | wc -l | tr -d ' ')
echo "    undefined libm 'sin'/'cos' refs in native routec.o = $COS_U_NATIVE  (expect 0)"
[ "$COS_T_NATIVE" = "1" ] && [ "$COS_U_NATIVE" = "0" ] && echo "NATIVE_COS_SUPPLIED_LIBMFREE=YES" || echo "NATIVE_COS_SUPPLIED_LIBMFREE=NO"

# ── [C] value-exact: native hxlcl_cos vs libc cos, ≤1-ULP sweep (covered domain) ──
echo "[C] value-exact: native hxlcl_cos vs libc cos (≤1-ULP sweep, |x|<2^20*pi/2)…"
cat > "$OUT/acc.c" <<'EOF'
#include <stdio.h>
#include <stdint.h>
#include <math.h>
#include <stdlib.h>
extern double hxlcl_cos(double x);
...
int main(void){
    ...
    for(int i=0;i<nb;i++){
        for(int s=-1;s<=1;s+=2){
            ...
            if(u>1){ fail++; if(fail<=12) printf("  >1ULP cos(%.17g): got %.17g want %.17g ulp=%lld\n", x, r, e, u); }
            n++;
        }
    }
...
EOF
# hxlcl_cos is SELF-CONTAINED (kernels inlined; no external call/reloc). routec.o is
# the WHOLE module emit, so wholesale linking drags siblings' undefined externs (setenv/
# malloc/environ/__errno_location — NONE referenced by cos). Keep only hxlcl_cos so the
# value-exact gate links cos in isolation. Fallback: weak stubs for siblings.
cat > "$OUT/stubs.c" <<'EOF'
#include <stdlib.h>
...
if [ ! -x "$OUT/acc" ]; then
    echo "    [C] whole-module link pulled a sibling extern; isolating hxlcl_cos…"
    objcopy --keep-global-symbol=hxlcl_cos "$OUT/routec.o" "$OUT/cos_only.o" 2>/dev/null
    $CC "$OUT/acc.c" "$OUT/cos_only.o" -o "$OUT/acc" -lm 2>"$OUT/link2.err" \
        || { echo "LINK_FAIL"; cat "$OUT/link.err" "$OUT/link2.err" >&2; exit 1; }
fi
"$OUT/acc"
RC=$?
echo "════ routec_cos_native_verify done (acc RC=$RC) ════"
exit 0
