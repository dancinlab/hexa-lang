#!/bin/sh
# tool/find_local_hexa.sh — POSIX-sh probe for a local hexa driver binary.
#
...
# executable hexa driver binary it finds, in priority order. Callers
...
#     routing marker `hexa-r ubu`; if found, we skip — it's a remote
...
is_local_binary() {
    ...
    # stable across the known wrapper variants (~/.hx/bin/hexa contains
    # `hexa-r ubu`).
...
if [ "${HEXA_BIN:-}" != "" ]; then
    if is_local_binary "$HEXA_BIN"; then
        printf '%s\n' "$HEXA_BIN"
        exit 0
    fi
    printf 'find_local_hexa: HEXA_BIN=%s is not a usable local binary\n' "$HEXA_BIN" >&2
fi
...
cat >&2 <<EOF
find_local_hexa: no local hexa binary found.
...
Fix: set HEXA_BIN=/abs/path to a compiled local hexa driver.
EOF
exit 1
