...
#   This oracle (tool/cuda_test_rmsnorm_mh.cu, self-contained: kernel
...
#
# Falsifiers (from the .cu): F-MK2C2-RMSNORM-EXACT |Δ|==0,
#                            F-MK2C2-RMSNORM-BWD-EXACT |Δ|==0
#
...
PHASE_ID="rmsnorm_mh_oracle_2026_05_19_b"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOCAL_DIR="${REPO_ROOT}/state/${PHASE_ID}"

ORACLE_CU="${REPO_ROOT}/tool/cuda_test_rmsnorm_mh.cu"

PHASE_LABEL="flame-rmsnorm-mh-oracle"
REMOTE_WORK="/workspace/rmsnorm_mh_oracle"
...
echo "[5/8] Upload cuda_test_rmsnorm_mh.cu ..."
$SCP_CMD "$ORACLE_CU" "root@$SSH_HOST:$REMOTE_WORK/cuda_test_rmsnorm_mh.cu"

# ── 6) nvcc build + run (default n=786432 + a d768 shape) ─
echo "[6/8] Build + run oracle on sm_${DEV_CC} ..."
$SSH_CMD "cd $REMOTE_WORK && \
    echo '=== nvcc build ===' ; \
    nvcc -arch=sm_${DEV_CC} -O2 -Xcompiler -ffp-contract=off -o cuda_test_rmsnorm_mh cuda_test_rmsnorm_mh.cu 2>&1 | tee build.log ; \
    BRC=\${PIPESTATUS[0]} ; echo \"BUILD_RC=\$BRC\" ; \
    if [ \"\$BRC\" = '0' ]; then \
        echo '=== RUN default (T=1024 d=768) ===' ; \
        timeout ${RUN_BUDGET_SEC} ./cuda_test_rmsnorm_mh 2>&1 | tee oracle_default.out ; \
        echo \"DEFAULT_RC=\${PIPESTATUS[0]}\" ; \
        echo '=== RUN stress (T=4096 d=768) ===' ; \
        timeout ${RUN_BUDGET_SEC} ./cuda_test_rmsnorm_mh 4096 768 2>&1 | tee oracle_stress.out ; \
...
for f in oracle_default.out oracle_stress.out; do
    if [ -f "$LOCAL_DIR/$f" ]; then
        ...
        echo "--- verdict ($f) ---"
        grep -E 'F-MK2C2-RMSNORM|ALL-PASS|FAIL' "$LOCAL_DIR/$f" || echo "  (no verdict line)"
    fi
done
echo ""
