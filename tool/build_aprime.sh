...
# Cycle 46-49 (RUNTIME.md Phase 1 Tier-A.1 + Tier-A.2 partial): hxlcl_*
# helpers + textual #define override eliminate 20 libc string+numeric+
# memory symbols (137 → 117 externs). The -fno-builtin-{bzero,memcpy}
# flags close clang -Oz libcall-recognition residuals where the optimizer
# converts `memset(p,0,n)` → bzero and `hxlcl_memcpy(*,*,const)` → libc
# memcpy after our #define has already fired. (___memcpy_chk fortified
# variant also dropped automatically once memcpy is unhooked.)
