...
CL_ERR="$(clang -Oz $ARCH_FLAG -std=gnu11 -D_GNU_SOURCE -Wno-trigraphs \
    ...
    -I self -I . "$APPOST" $ZEROC_RT_HI_OBJ $ARRAY_CORE_OBJ $MAP_CORE_OBJ $ALLOC_CORE_OBJ $STR_CORE_OBJ $RTCORE_LEAF_OBJ $RTCORE_STRARR_READ_OBJ $RTCORE_ARITH_OBJ $RTCORE_MATH_OBJ $RTCORE_MATH2_OBJ $RTCORE_VALOP_DISPATCH_OBJ $RTCORE_MAP_QUERY_DISPATCH_OBJ $RTCORE_MAP_QUERY_FOLD_OBJ $RTCORE_COLLECTION_MUTATE_OBJ $RTCORE_ATL_OBJ $RTCORE_FS_RW_OBJ $RTCORE_ACF_OBJ $RTCORE_RUNTIME_MISC_OBJ $RT_A -o "$OUT" -lm 2>&1 | grep -iE 'error:|undefined|referenced from|symbol\(s\) not found|^[[:space:]]+"' | head -20)"
...
# and the old `grep -iE 'error:|undefined'` matched the first and last lines but NOT the
# middle one, so a link failure reached CI as a bare "Undefined symbols for architecture
# arm64:" with the symbol invisible — undiagnosable from the log alone (2026-07-13, the
# arm64 `bl hexa_eq` break). Keep `referenced from` + the indented quoted-symbol lines.
