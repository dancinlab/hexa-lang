...
#   5. PROBE-B "naive SELFEMIT flip"    → the same TU with the BROAD
#        -DHEXA_RT_SELFEMIT master flip. This is the r2/r3 "whole-runtime flip".
#        It is EXPECTED TO FAIL TO COMPILE from a clean tree (the frozen runtime.c
#        blob carries `static hxlcl_*` libc-wrapper decls that conflict with the
#        non-static externs SELFEMIT introduces) — the script captures that wall
#        verbatim instead of papering over it with an ephemeral edit.
#
...
# ── stage 5: PROBE-B — naive SELFEMIT whole-runtime flip (the r2/r3 path) ───
echo "[5/5] PROBE-B: naive -DHEXA_RT_SELFEMIT whole-runtime flip (r2/r3 path)"
...
if [ -n "$B_ERR" ] || [ ! -f "$RTB" ]; then
    ...
    echo "      PROBE-B DOES NOT COMPILE FROM CLEAN TREE (the wall r2/r3 hand-edited):"
    echo "$B_ERR" | sed 's/^/        /'
    echo "FLIP_B_COMPILES=NO  frozen_static_conflicts=$STATIC_WALL"
else
    nm -u "$RTB" 2>/dev/null | awk '{print $NF}' | sort -u > "$OUT_DIR/undef_B.txt"
    echo "      PROBE-B OK — undefined (unique): $(wc -l < "$OUT_DIR/undef_B.txt" | tr -d ' ')"
