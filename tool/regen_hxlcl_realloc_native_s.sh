#!/usr/bin/env bash
# tool/regen_hxlcl_realloc_native_s.sh — reproducibly regenerate the frozen native
# hxlcl_realloc seeds from the SSOT stdlib/runtime/hxlcl_core.hexa, using the native
# compiler. The zero-c #29 realloc F1-part2 twin of tool/regen_hxlcl_calloc_native_s.sh
# (mirror of the merged #4548 free / #4556 calloc seed-consume — same COMPOSITE
# inner-malloc class as calloc).
#
#   tool/regen_hxlcl_realloc_native_s.sh [darwin|x86_64|arm64-linux|all]   (default: all)
#
# WHY this seed exists (prereq-(a), root-caused #4489/#4545 for free, same class for realloc):
#   The old resolver emitted hxlcl_realloc's Route-C .o LIVE at Stage 0b via
...
#   2) .globl DEMOTION post-pass: keep ONLY hxlcl_realloc (`_hxlcl_realloc` on Mach-O)
...
#      objcopy isolate). NOTE: hxlcl_realloc's body issues an inner C-ABI call to
...
#   4) sanity: cross-assemble + assert EXACTLY 1 defined-global hxlcl_realloc + no libc U.
#
...
# to a 1-symbol (hxlcl_realloc) contract is what makes this seed ar-able next to the
# libc-shim member for the OTHER hxlcl_* (which stay shim-served until their own flip).
#
# Targets:
#   darwin      → self/native/hxlcl_realloc_arm64.s        (arm64-apple-darwin, Mach-O)
#   x86_64      → self/native/hxlcl_realloc_x86_64.s        (x86_64-linux-gnu,   ELF)
#   arm64-linux → self/native/hxlcl_realloc_arm64-linux.s   (arm64-linux-gnu,    ELF)
#
...
[ -x "$APRIME" ] || { echo "[regen_hxlcl_realloc] ERROR: native compiler not at $APRIME (set APRIME=)" >&2; exit 1; }

SRC_MAIN="$HX/stdlib/runtime/hxlcl_core.hexa"
[ -f "$SRC_MAIN" ] || { echo "[regen_hxlcl_realloc] ERROR: SSOT missing: $SRC_MAIN" >&2; exit 1; }

# The single external-contract symbol the resolver ar's into runtime.a on flip.
CONTRACT='hxlcl_realloc'

TMP="$(mktemp -d -t regen_hxlcl_realloc.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
emit_one() {
    ...
    if ! env HEXA_CABI_HXLCL=1 HEXA_INLINE_INT_BOX=1 HEXA_INLINE_BOOL_BOX=1 \
        ...
        echo "[regen_hxlcl_realloc] ERROR: $triple aprime --emit=asm failed:" >&2
        sed 's/^/    /' "$TMP/emit.err" >&2
        exit 1
    fi

    # --- .globl DEMOTION post-pass: keep ONLY hxlcl_realloc .globl, drop the rest ---
    ...
    [ "$kept" -eq 1 ] || { echo "[regen_hxlcl_realloc] ERROR: $triple kept $kept/1 hxlcl_realloc global (raw emit missing the shim?)" >&2; exit 1; }
    [ "$total" -eq 1 ] || { echo "[regen_hxlcl_realloc] ERROR: $triple demotion left $total globals (expected 1 — sed pattern drift)" >&2; exit 1; }
    [ "$pext" -eq 0 ] || { echo "[regen_hxlcl_realloc] ERROR: $triple demotion left $pext .private_extern (Mach-O N_PEXT stays external → ld -r multidef)" >&2; exit 1; }

    {
        printf '// %s — FROZEN BOOTSTRAP SEED (RT-NATIVE zero-c #29 — hxlcl_realloc F1-part2).\n' "$(basename "$out")"
        printf '// GENERATED: tool/regen_hxlcl_realloc_native_s.sh — aprime_cc _drv.hexa --emit=asm\n'
        printf '//   --target=%s -o %s hxlcl_core.hexa, then a .globl DEMOTION post-pass\n' "$triple" "$(basename "$out")"
        printf '//   keeping ONLY the hxlcl_realloc shim global.\n'
        printf '//   Provides hxlcl_realloc (native Route-C body: !p→malloc(n); n==0→0; np=hxlcl_malloc(n);\n'
        printf '//   !np→0; old_n=*(size_t*)(p-16); copy min(n,old_n)) that stage_resolve_runtime_a ar'"'"'s\n'
        printf '//   into runtime.a under HEXA_RT_NATIVE_REALLOC, replacing the libc-shim realloc member\n'
        printf '//   so `realloc` drops from the nm-UND floor. The inner hxlcl_malloc callee stays served\n'
        printf '//   by the shim (undefined extern here, resolved within runtime.a — NO co-drop).\n'
    ...
    if $CC $cc_extra -c "$s" -o "$o" 2>/dev/null; then
        local t; t="$( (nm "$o" 2>/dev/null || echo) | grep -cE ' T _?hxlcl_realloc')"
        [ "$t" -eq 1 ] || { echo "[regen_hxlcl_realloc] ERROR: $triple nm shows $t/1 T hxlcl_realloc" >&2; exit 1; }
        ...
        echo "[regen_hxlcl_realloc] $triple → $out (1 globl · $t T hxlcl_realloc · ${libc_u} non-carrier U)"
        [ "${libc_u:-0}" -eq 0 ] || echo "[regen_hxlcl_realloc] WARN $triple: seed has ${libc_u} non-carrier undefined symbols — inspect (floor-reduction assert)" >&2
    else
        echo "[regen_hxlcl_realloc] WARN $triple: cross-assemble check skipped (no matching toolchain)" >&2
        echo "[regen_hxlcl_realloc] $triple → $out (1 globl)"
    fi
}

case "$WHICH" in
    darwin)      emit_one arm64-apple-darwin "$HX/self/native/hxlcl_realloc_arm64.s" "Mach-O, _hxlcl_realloc underscore-prefixed" ;;
    x86_64)      emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_realloc_x86_64.s" "ELF, hxlcl_realloc no underscore" ;;
    arm64-linux) emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_realloc_arm64-linux.s" "ELF aarch64, hxlcl_realloc no underscore" ;;
    all)
        emit_one arm64-apple-darwin "$HX/self/native/hxlcl_realloc_arm64.s" "Mach-O, _hxlcl_realloc underscore-prefixed"
        emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_realloc_x86_64.s" "ELF, hxlcl_realloc no underscore"
        emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_realloc_arm64-linux.s" "ELF aarch64, hxlcl_realloc no underscore"
        ;;
    *) echo "[regen_hxlcl_realloc] usage: $0 [darwin|x86_64|arm64-linux|all]" >&2; exit 1 ;;
esac
echo "[regen_hxlcl_realloc] done."
