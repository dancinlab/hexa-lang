#!/usr/bin/env bash
# tool/regen_hxlcl_calloc_native_s.sh — reproducibly regenerate the frozen native
# hxlcl_calloc seeds from the SSOT stdlib/runtime/hxlcl_core.hexa, using the native
...
#
#   tool/regen_hxlcl_calloc_native_s.sh [darwin|x86_64|arm64-linux|all]   (default: all)
#
# WHY this seed exists (prereq-(a), root-caused #4489/#4545 for free, same class for calloc):
#   The old resolver emitted hxlcl_calloc's Route-C .o LIVE at Stage 0b via
...
#   2) .globl DEMOTION post-pass: keep ONLY hxlcl_calloc (`_hxlcl_calloc` on Mach-O)
...
#      objcopy isolate). NOTE: hxlcl_calloc's body issues an inner C-ABI call to
...
#   4) sanity: cross-assemble + assert EXACTLY 1 defined-global hxlcl_calloc + no libc U.
#
...
# to a 1-symbol (hxlcl_calloc) contract is what makes this seed ar-able next to the
# libc-shim member for the OTHER hxlcl_* (which stay shim-served until their own flip).
#
# Targets:
#   darwin      → self/native/hxlcl_calloc_arm64.s        (arm64-apple-darwin, Mach-O)
#   x86_64      → self/native/hxlcl_calloc_x86_64.s        (x86_64-linux-gnu,   ELF)
#   arm64-linux → self/native/hxlcl_calloc_arm64-linux.s   (arm64-linux-gnu,    ELF)
#
...
[ -x "$APRIME" ] || { echo "[regen_hxlcl_calloc] ERROR: native compiler not at $APRIME (set APRIME=)" >&2; exit 1; }

SRC_MAIN="$HX/stdlib/runtime/hxlcl_core.hexa"
[ -f "$SRC_MAIN" ] || { echo "[regen_hxlcl_calloc] ERROR: SSOT missing: $SRC_MAIN" >&2; exit 1; }

# The single external-contract symbol the resolver ar's into runtime.a on flip.
CONTRACT='hxlcl_calloc'

TMP="$(mktemp -d -t regen_hxlcl_calloc.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
emit_one() {
    ...
    if ! env HEXA_CABI_HXLCL=1 HEXA_INLINE_INT_BOX=1 HEXA_INLINE_BOOL_BOX=1 \
        ...
        echo "[regen_hxlcl_calloc] ERROR: $triple aprime --emit=asm failed:" >&2
...

    {
        ...
        printf '// GENERATED: tool/regen_hxlcl_calloc_native_s.sh — aprime_cc _drv.hexa --emit=asm\n'
        printf '//   --target=%s -o %s hxlcl_core.hexa, then a .globl DEMOTION post-pass\n' "$triple" "$(basename "$out")"
        printf '//   keeping ONLY the hxlcl_calloc shim global.\n'
        printf '//   Provides hxlcl_calloc (native Route-C body: total=nmemb*size; p=hxlcl_malloc(total);\n'
    ...
    if $CC $cc_extra -c "$s" -o "$o" 2>/dev/null; then
        local t; t="$( (nm "$o" 2>/dev/null || echo) | grep -cE ' T _?hxlcl_calloc')"
        [ "$t" -eq 1 ] || { echo "[regen_hxlcl_calloc] ERROR: $triple nm shows $t/1 T hxlcl_calloc" >&2; exit 1; }
        ...
        echo "[regen_hxlcl_calloc] $triple → $out (1 globl · $t T hxlcl_calloc · ${libc_u} non-carrier U)"
        [ "${libc_u:-0}" -eq 0 ] || echo "[regen_hxlcl_calloc] WARN $triple: seed has ${libc_u} non-carrier undefined symbols — inspect (floor-reduction assert)" >&2
    else
        echo "[regen_hxlcl_calloc] WARN $triple: cross-assemble check skipped (no matching toolchain)" >&2
        echo "[regen_hxlcl_calloc] $triple → $out (1 globl)"
    fi
}

case "$WHICH" in
    darwin)      emit_one arm64-apple-darwin "$HX/self/native/hxlcl_calloc_arm64.s" "Mach-O, _hxlcl_calloc underscore-prefixed" ;;
    x86_64)      emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_calloc_x86_64.s" "ELF, hxlcl_calloc no underscore" ;;
    arm64-linux) emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_calloc_arm64-linux.s" "ELF aarch64, hxlcl_calloc no underscore" ;;
    all)
        emit_one arm64-apple-darwin "$HX/self/native/hxlcl_calloc_arm64.s" "Mach-O, _hxlcl_calloc underscore-prefixed"
        emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_calloc_x86_64.s" "ELF, hxlcl_calloc no underscore"
        emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_calloc_arm64-linux.s" "ELF aarch64, hxlcl_calloc no underscore"
        ;;
    *) echo "[regen_hxlcl_calloc] usage: $0 [darwin|x86_64|arm64-linux|all]" >&2; exit 1 ;;
esac
echo "[regen_hxlcl_calloc] done."
