#!/usr/bin/env bash
# tool/regen_hxlcl_free_native_s.sh — reproducibly regenerate the frozen native
...
#
#   tool/regen_hxlcl_free_native_s.sh [darwin|x86_64|arm64-linux|all]   (default: all)
#
...
#
# Targets:
#   darwin      → self/native/hxlcl_free_arm64.s        (arm64-apple-darwin, Mach-O)
#   x86_64      → self/native/hxlcl_free_x86_64.s        (x86_64-linux-gnu,   ELF)
#   arm64-linux → self/native/hxlcl_free_arm64-linux.s   (arm64-linux-gnu,    ELF)
#
...
[ -x "$APRIME" ] || { echo "[regen_hxlcl_free] ERROR: native compiler not at $APRIME (set APRIME=)" >&2; exit 1; }

SRC_MAIN="$HX/stdlib/runtime/hxlcl_core.hexa"
[ -f "$SRC_MAIN" ] || { echo "[regen_hxlcl_free] ERROR: SSOT missing: $SRC_MAIN" >&2; exit 1; }
...
emit_one() {
    ...
    if ! env HEXA_CABI_HXLCL=1 "$APRIME" "$TMP/_drv.hexa" --emit=asm --target="$triple" \
        ...
        echo "[regen_hxlcl_free] ERROR: $triple aprime --emit=asm failed:" >&2
...

    {
        ...
        printf '// GENERATED: tool/regen_hxlcl_free_native_s.sh — aprime_cc _drv.hexa --emit=asm\n'
    ...
    if $CC $cc_extra -c "$s" -o "$o" 2>/dev/null; then
        local t; t="$( (nm "$o" 2>/dev/null || echo) | grep -cE ' T _?hxlcl_free')"
        [ "$t" -eq 1 ] || { echo "[regen_hxlcl_free] ERROR: $triple nm shows $t/1 T hxlcl_free" >&2; exit 1; }
        ...
        echo "[regen_hxlcl_free] $triple → $out (1 globl · $t T hxlcl_free · ${libc_u} non-carrier U)"
        [ "${libc_u:-0}" -eq 0 ] || echo "[regen_hxlcl_free] WARN $triple: seed has ${libc_u} non-carrier undefined symbols — inspect (floor-reduction assert)" >&2
    else
        echo "[regen_hxlcl_free] WARN $triple: cross-assemble check skipped (no matching toolchain)" >&2
        echo "[regen_hxlcl_free] $triple → $out (1 globl)"
    fi
}

case "$WHICH" in
    darwin)      emit_one arm64-apple-darwin "$HX/self/native/hxlcl_free_arm64.s" "Mach-O, _hxlcl_free underscore-prefixed" ;;
    x86_64)      emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_free_x86_64.s" "ELF, hxlcl_free no underscore" ;;
    arm64-linux) emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_free_arm64-linux.s" "ELF aarch64, hxlcl_free no underscore" ;;
    all)
        emit_one arm64-apple-darwin "$HX/self/native/hxlcl_free_arm64.s" "Mach-O, _hxlcl_free underscore-prefixed"
        emit_one x86_64-linux-gnu   "$HX/self/native/hxlcl_free_x86_64.s" "ELF, hxlcl_free no underscore"
        emit_one arm64-linux-gnu    "$HX/self/native/hxlcl_free_arm64-linux.s" "ELF aarch64, hxlcl_free no underscore"
        ;;
    *) echo "[regen_hxlcl_free] usage: $0 [darwin|x86_64|arm64-linux|all]" >&2; exit 1 ;;
esac
echo "[regen_hxlcl_free] done."
