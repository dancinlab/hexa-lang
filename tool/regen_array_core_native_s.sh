...
emit_one() {
    ...
    local n; n="$(grep -cE '^[[:space:]]*\.globl[[:space:]]+_?rt_array_(get|set|len|pop)_native' "$raw" || echo 0)"
    [ "$n" -ge 4 ] || { echo "[regen_array_core] ERROR: $triple emitted only $n/4 rt_array_*_native" >&2; exit 1; }
    {
