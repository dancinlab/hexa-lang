...
# exit42
printf 'fn main() {\n  exit(6 * 7)\n}\n' > "$OUT/exit42.hexa"
"$OUT/aprime_cc_dropon" _drv.hexa --emit=asm --target="$SMOKE_TARGET" -o "$OUT/exit42.s" "$OUT/exit42.hexa" >"$OUT/e42.emit.log" 2>&1
E42_EMIT_RC=$?
...
# hello
printf 'fn main() {\n  println("hello")\n}\n' > "$OUT/hello.hexa"
"$OUT/aprime_cc_dropon" _drv.hexa --emit=asm --target="$SMOKE_TARGET" -o "$OUT/hello.s" "$OUT/hello.hexa" >"$OUT/hello.emit.log" 2>&1
...
"$OUT/aprime_cc_dropon" _drv.hexa --emit=asm --target="$SMOKE_TARGET" -o "$OUT/cc-gen3.s" "$OUT/flat.hexa" >"$OUT/gen3.emit.log" 2>&1
G3_RC=$?
"$OUT/aprime_cc_dropon" _drv.hexa --emit=asm --target="$SMOKE_TARGET" -o "$OUT/cc-gen4.s" "$OUT/flat.hexa" >"$OUT/gen4.emit.log" 2>&1
G4_RC=$?
