...
- [ ] O8 — paper TRIGGER (paper_on_discovery): closed-negative finding 발표. slug=`oeis-prefix-collision-falsifier`. §statement = H "K=10 정수-tuple prefix-match ⟹ OEIS sequence identity" (O1 pre-registered). §method = 374K stripped.gz full sweep, K=10 hash-intersect → K=20 verbatim 2-pass, 20 candidate fn (편향 명시). §verification = `.verdicts/oeis-full-sweep/ledger.json` (1707/336/1334/37) + hits.tsv A000926 row. §finding = H FALSIFIED, 78.1% (1334/1707) first-K artifact → ruled-out axis = short-prefix-as-identity-test. **caveat 必**: candidate-set-relative(20 low-entropy fn) + single K=10→20 pair (else paper_violation). g51 = ≥10p + ≥1 fal.ai fig (funnel + coincidence histogram).
