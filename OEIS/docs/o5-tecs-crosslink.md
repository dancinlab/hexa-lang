...
> repo-root `DOMAINS.tape` 에 cross-domain reuse edge `TECS-L --reuses--> OEIS`
> (@link) 를 등록(commons @D g67)한 closure 기록.
...
검증된 산출물을 재사용하는 격자. (cross-repo STAR hub = g68 은 별개; `DOMAINS.tape`
의 cross-project `@link` 블록이 그 쪽.)
...
- 그래프: repo-root `DOMAINS.tape` (cross-domain `@link TECS-L --reuses--> OEIS`)
