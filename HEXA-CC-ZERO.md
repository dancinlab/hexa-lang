...
- [⊘] P6 — **SUPERSEDED (사용자 결정 2026-05-30 · cold-seed/seed-refresh 제거)** — P6 의 cold-seed CI green / full-CLI seed-refresh claim 은 cold-seed `hexa_cc_seed.c` 제거(option2)로 **무효화**됨. seed-refresh 경로(현 SSOT 재regen + amalgam 으로 `hexa_cc_seed.c` 재생성 + commit)는 사용자가 명시적으로 **retired** — bare-clone cold-boot 능력 포기. 후속 freshness 모델(콜드시드 seed-refresh)은 더 이상 존재하지 않음. CI 는 warm-install bootstrap(`hx install` → `hexa cc --regen` → build, 릴리스 바이너리에서)으로 재구성되어야 하며 그 warm-CI green 은 HEXA-C-ZERO C1 로 이관(현재 release-pipeline + `hexa cc --regen` 두 upstream RED 으로 deferred). 이전 `- [x] 🎉 DOMAIN CLOSED 5/5` 표기는 SUPERSEDED — P1✅·P2✅·P3⊘·P5⊘·P6⊘ (REVERSAL 섹션 참조).
