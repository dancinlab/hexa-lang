...
- [x] P7 — edge-seed chicken-egg 복구 (graduation fallout) · 🟢 **CLOSED 2026-05-31 (PR #2231 squash 3e719d31f)** — main 이 `release`/`nobaseline-gate` 에서 linux-x86_64+darwin-arm64 RED 였던 원인: `edge` 릴리스가 `hexa-linux-arm64.tar.gz` 만 ship → `tool/stage_resolve_runtime_a`/`stage_prebuild_hexat` 의 edge-pull 분기가 x86_64/darwin 에서 씨앗 부재로 `exit 1`. FIX = 두 stage 에 edge-miss fallback 배선 + 신설 `tool/restore_frozen_seeds`(graduation 직전 부모 full-SHA `151c52c82` pin · shallow-checkout 대비 on-demand `git fetch` · 21 seed = `self/runtime.c`+19 #include fragment+`hexa_cc.c`)로 작업트리에만 씨앗 복원(미커밋=.hexa-only 보존)→ 기존 source-build 경로로 `runtime.a` 534928 B·`build/hexat`·`./hexa` 빌드. `grace_consent.yml` `hexa_v2_linux`→`hexat_linux` + `self/runtime.c` 링크를 공유 `build/runtime.a` 로 전환. **nobaseline-gate 3-platform GREEN**(darwin-arm64·linux-x86_64·linux-arm64 · run 26703312462) + grace-consent GREEN. verdict: `.verdicts/hexa-cc-zero/F-HEXA-CC-ZERO-P7-EDGE-SEED.txt` · handoff 726b8b67.
...
## 2026-05-31 C2 — native_gate.c deletion (🟠 DEFERRED · Linux smoke 게이트 미충족 · verdict 첨부)

verdict: `.verdicts/hexa-cc-zero/F-HEXA-CC-ZERO-CFRONT-DELETION.txt` (gate-1 PASS cite + smoke DEFERRED 사유 + git-state)

C1(#2218 gate-1 byte-identity PASS)에 이어 C-front 최종 닫힘 = 커밋된 `native_gate.c` 제거로 self/native 트리를 emit+byte_diff-only 로 일관화. 이번 사이클은 **HALT GATE** 에 막혀 build harness 만 랜딩하고 deletion 은 보류.
