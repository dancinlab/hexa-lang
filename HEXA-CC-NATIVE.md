...
- [ ] N5 — self-host fixpoint (byte-eq): native-built hexac 가 자기 SSOT 를 재컴파일(gen2) → 그 gen2 가 다시 재컴파일(gen3) → gen2≡gen3 실행파일 byte-eq (per-arch deterministic) darwin-arm64 · 🔬 N5a MEASURED(2026-05-31): 진짜 frontier = **native type-checker** (flatten된 compiler/main.hexa 40246L 를 aprime_cc 가 `--emit=obj` 시 codegen 前 **181 diagnostic abort** (분포: HX2001×208 dominant · HX3004×34 · HX3001×34 `named:any↔i64` · HX2003×32 · HX0011×26 · HX4001×26 — `any` 전역에 i64 리터럴 대입류)). runtime.o/hexa_ld 아님. 재정의 → **N5-pre(typecheck 181 mismatch 해소) → N5-sym(emit_runtime_obj 를 full _hexa_* 셋으로 확장; corpus union 이미 8개·runtime_arm64 181 rt_ 가 raw material) → N5-link(hexa_ld 링크→byte-eq fixpoint)**. verdict: `.verdicts/hexa-cc-native/F-HEXA-CC-NATIVE-N5A-SYMSET.txt`
